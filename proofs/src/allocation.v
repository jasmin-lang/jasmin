...
Module M.
  ...
  Definition valid (mvar: Mvar.t Ident.ident) (mid: Ms.t var) :=
    forall x id, Mvar.get mvar x = Some id <-> Ms.get mid id = Some x.
 
  Lemma valid_uniqx mvar mid : valid mvar mid ->
     forall x x' id , Mvar.get mvar x = Some id -> Mvar.get mvar x' = Some id ->
                      x = x'.
  ...
  Record t := mkT { mvar : Mvar.t Ident.ident; mid : Ms.t var; mvalid: valid mvar mid }.
  ...
  Lemma rm_idP m id x : Mvar.get (rm_id m id) x <> Some id.
  Proof.
    rewrite /rm_id. case Heq: ((mid m).[id])%ms => [x'|];last first.
    + by move=> /mvalid;rewrite Heq.   
    by rewrite Mvar.removeP; case: (x' =P x) => // Hne /mvalid;rewrite Heq=> -[] ?;elim Hne.
  Qed. 
  ...
  Lemma valid_set m x id : valid (Mvar.set (rm_id m id) x id) (Ms.set (rm_x m x) id x).
  Proof.
    ...
    case Heq: ((mid m).[id])%ms => [z|];case Heq':(Mvar.get (mvar m) x) => [i|];
    rewrite ?Mvar.removeP ?Ms.removeP;last by apply mvalid.
  ...
  Proof. by move=> x id;rewrite Mvar.get0 Ms.get0. Qed.
  ...
  Proof. by rewrite /get Mvar.get0. Qed.

  Lemma setP_eq m x id: get (set m x id) x = Some id.
  Proof. by rewrite /get /set /=;rewrite Mvar.setP_eq. Qed.

  Lemma setP_neq m x y id id': 
    x != y -> get (set m x id) y = Some id' ->
    get m y = Some id' /\ id <> id'.
  Proof.
    move=> Hne;rewrite /get /set /= Mvar.setP_neq // /rm_id.
    case Heq: ((mid m).[id])%ms => [x'|] //=. 
    + rewrite Mvar.removeP;case:ifP => // /eqP Hne' H;split=>// ?;subst.
...
Definition check_alloc_v (m:M.t) x1 x2 : bool :=
...
Fixpoint check_alloc_e (m:M.t) t1 t2 (e1:pexpr t1) (e2:pexpr t2) : bool := 
  match e1, e2 with
  | Pvar x1  , Pvar x2    => check_alloc_v m x1 x2
  | Pconst n1, Pconst n2  => (n1 =? n2)%num
  | Pbool b1 , Pbool b2   => b1 == b2
  | Papp1 _ _ o1 e1, Papp1 _ _ o2 e2 => 
    eqb_sop1 o1 o2 && check_alloc_e m e1 e2
  | Papp2 _ _ _ o1 e11 e12   , Papp2 _ _ _ o2 e21 e22     =>  
    eqb_sop2 o1 o2 && check_alloc_e m e11 e21 && check_alloc_e m e12 e22
  | Papp3 _ _ _ _ o1 e11 e12 e13, Papp3 _ _ _ _ o2 e21 e22 e23 => 
    eqb_sop3 o1 o2 && check_alloc_e m e11 e21 &&
    check_alloc_e m e12 e22 && check_alloc_e m e13 e23
  | _, _ => false
  end.

Fixpoint check_alloc_rval t1 (x1:rval t1) t2 (x2:rval t2) m :=
  match x1, x2 with
  | Rvar x1, Rvar x2 => 
    if vtype x1 == vtype x2 then Ok unit (M.set m x1 (vname x2))
    else Error tt 
  | Rpair _ _ x11 x12, Rpair _ _ x21 x22 => 
    check_alloc_rval x12 x22 m >>= check_alloc_rval x11 x21
  | _                , _                 => Error tt
  end.
 
Definition check_alloc_bcmd i1 i2 m := 
  match i1, i2 with
  | Assgn _ x1 e1, Assgn _ x2 e2 =>
    if check_alloc_e m e1 e2 then check_alloc_rval x1 x2 m
    else Error tt                                               
  | Load x1 e1   , Load x2 e2    => 
    if check_alloc_e m e1 e2 then check_alloc_rval x1 x2 m
    else Error tt
  | Store e11 e12, Store e21 e22 =>
    if check_alloc_e m e11 e21 && check_alloc_e m e12 e22 then Ok unit m
    else Error tt
  | _            , _             => 
    Error tt
  end.
...
Fixpoint check_alloc_i i1 i2 m := 
  match i1, i2 with
  | Cbcmd i1, Cbcmd i2 => check_alloc_bcmd i1 i2 m
  | Cif e1 c11 c12, Cif e2 c21 c22 =>
    if check_alloc_e m e1 e2 then 
      fold2 tt check_alloc_i c11 c21 m >>= (fun m1 =>
      fold2 tt check_alloc_i c12 c22 m >>= (fun m2 => Ok unit (M.merge m1 m2)))
    else Error tt
  | Cfor i1 (dir1,lo1,hi1) c1, Cfor i2 (dir2,lo2,hi2) c2 =>
    if eqb_dir dir1 dir2 && check_alloc_e m lo1 lo2 && check_alloc_e m hi1 hi2 then
      let check_c m := 
        check_alloc_rval i1 i2 m >>= fold2 tt check_alloc_i c1 c2 in
      loop check_c nb_loop m 
    else Error tt 
  | Cwhile e1 c1, Cwhile e2 c2 =>
    loop (fold2 tt check_alloc_i c1 c2) nb_loop m >>= (fun m =>
      if check_alloc_e m e1 e2 then Ok unit m
      else Error tt)
  | Ccall _ _ x1 fd1 arg1, Ccall _ _ x2 fd2 arg2 => 
    if check_alloc_fd fd1 fd2 && check_alloc_e m arg1 arg2 then 
      check_alloc_rval x1 x2 m
    else Error tt
  | _, _ => Error tt
  end

with check_alloc_fd ta1 tr1 (fd1:fundef ta1 tr1) ta2 tr2 (fd2:fundef ta2 tr2) :=
  match fd1, fd2 with
  | FunDef _ _ p1 c1 r1, FunDef _ _ p2 c2 r2 =>
    isOk (
      check_alloc_rval p1 p2 M.empty >>= (fun m =>
      fold2 tt check_alloc_i c1 c2 m >>= (fun m =>
         if check_alloc_e m (rval2pe r1) (rval2pe r2) then Ok unit tt
         else Error tt)))
  end.
...
Section PROOF.
  ...
  Let Pi (i1:instr) := 
    forall r1 i2 r2, check_alloc_i i1 i2 r1 = Ok unit r2 ->
    forall m1 m2 vm1 vm2, sem_i (Estate m1 vm1) i1 (Estate m2 vm2) ->
    forall vm1', eq_alloc r1 vm1 vm1' ->
    exists vm2', eq_alloc r2 vm2 vm2' /\ sem_i (Estate m1 vm1') i2 (Estate m2 vm2').

  Let Pc (c1:cmd) := 
    forall r1 c2 r2, fold2 tt check_alloc_i c1 c2 r1 = Ok unit r2 ->
    forall m1 m2 vm1 vm2, sem (Estate m1 vm1) c1 (Estate m2 vm2) ->
    forall vm1', eq_alloc r1 vm1 vm1' ->
    exists vm2', eq_alloc r2 vm2 vm2' /\ sem (Estate m1 vm1') c2 (Estate m2 vm2').

  Let Pf ta tr (fd1:fundef ta tr) := 
    forall fd2, check_alloc_fd fd1 fd2 ->
  ...
  Proof.
    ...
    case Hci : check_alloc_i => [ri|] //= Hcc m1 m3 vm1 vm3 H;inversion H;clear H;subst.
  ...
  Lemma check_alloc_rval_eqt t1 t2 (r1 r2:M.t) (rv1:rval t1) (rv2:rval t2):
    check_alloc_rval rv1 rv2 r1 = Ok unit r2 ->
    t1 = t2.
  Proof.
    elim: rv1 t2 rv2 r1 r2 => [x1 | ?? x11 Hx1 x12 Hx2] t2 [x2 | ?? x21 x22] //= r1 r2.
    + by case:ifP => [/eqP|].
    case Heq: check_alloc_rval => [r' /=|//] /Hx1 ->.
    by rewrite (Hx2 _ _ _ _ Heq).
  Qed.
    
  Lemma check_alloc_e_eqt r t1 (e1:pexpr t1) t2 (e2:pexpr t2):
    check_alloc_e r e1 e2 -> t1 = t2.
  Proof.
    elim: e1 t2 e2 =>
      [ x1 | n1 | b1 | ?? o1 e1 He1
    ...
    + by rewrite /check_alloc_v;case:ifP => [/eqP|].
  ...
  Lemma eq_rn_write_aux t1 (rv1:rval t1) t2 (rv2:rval t2) v1 v2 r1 r2 vm vm' : 
     check_alloc_rval rv1 rv2 r1 = Ok unit r2 ->
     eq_alloc r1 vm vm' ->
     JMeq v1 v2 ->
  ...
  Proof.
    ...
    case Heq2: check_alloc_rval => [r1' |] //= Heq1 Hvm Hjm.
    have ?:= check_alloc_rval_eqt Heq1;subst.
    have ?:= check_alloc_rval_eqt Heq2;subst.
    have H:= JMeq_eq Hjm;subst v2 => {H}.
    apply (Hx1 _ _ _ _ _ _ _ _ Heq1)=> //.
    by apply (Hx2 _ _ _ _ _ _ _ _ Heq2).
  Qed.

  Lemma eq_rn_write t (rv1 rv2:rval t) v r1 r2 vm vm' : 
     check_alloc_rval rv1 rv2 r1 = Ok unit r2 ->
     eq_alloc r1 vm vm' ->
     eq_alloc r2 (write_rval vm rv1 v) (write_rval vm' rv2 v).
  Proof.
    move=> Hc Hrn; apply (eq_rn_write_aux Hc Hrn);apply JMeq_refl.
  Qed.

  Lemma eq_rn_sem_aux r vm1 vm1' t1 (e1:pexpr t1) t2 (e2:pexpr t2) : 
    eq_alloc r vm1 vm1' ->
    check_alloc_e r e1 e2 ->
  ...
  Proof.
    move=> Hrn; elim: e1 t2 e2 =>
      ...
      [ [tx2 x2] | n2 | b2 | ?? o2 e2
      | ??? o2 e21  e22 | ???? o2 e21 e22 e23] //=.
    + rewrite /check_alloc_v /=;case:(_ =P _) => Heqt //=.
      by subst; case Hget: M.get=> [id'|] //= /eqP ?;subst;rewrite (Hrn _ _ Hget).
    + by move=> Heq; rewrite (Neqb_ok _ _ Heq).
    + by move=> /eqP ->.
    + move=> /andP[] Ho H1.  
      have ? := check_alloc_e_eqt H1;subst;rewrite (He1 _ _ H1).
      by case:(eqb_sop1P _ Ho) => // ?;subst=> ->.
    + move=> /andP[]/andP[] Ho H1 H2.
      have ? := check_alloc_e_eqt H1;subst;rewrite (He1 _ _ H1).
      have ? := check_alloc_e_eqt H2;subst;rewrite (He2 _ _ H2).
      by case:(eqb_sop2P _ _ Ho) => // ?;subst=> ->.
    move=> /andP[]/andP[]/andP[] Ho H1 H2 H3.
    have ? := check_alloc_e_eqt H1;subst;rewrite (He1 _ _ H1).
    have ? := check_alloc_e_eqt H2;subst;rewrite (He2 _ _ H2).
    have ? := check_alloc_e_eqt H3;subst;rewrite (He3 _ _ H3).
    by case:(eqb_sop3P _ _ _ Ho) => // ?;subst=> ->.
  Qed.

  Lemma eq_rn_sem r vm1 vm1' t (e1 e2:pexpr t): 
    eq_alloc r vm1 vm1' ->
    check_alloc_e r e1 e2->
    sem_pexpr vm1 e1 = sem_pexpr vm1' e2.
  Proof. by move=> Hrn Hc;rewrite (eq_rn_sem_aux Hrn Hc). Qed.
  ...
  Proof.
    ...
    + case Hce: check_alloc_e => //= Hcr.
      have ? := check_alloc_e_eqt Hce;subst.
      move=> m1 m2 vm1 vm2 H;inversion H;clear H;subst=> vm1' Hvm1.
      move:H2=> /=;rewrite (eq_rn_sem Hvm1 Hce).
      case Heq : sem_pexpr => [v|]//= [] <- <-.
      exists (write_rval vm1' rv2 v);split.
      + by apply (eq_rn_write _ Hcr)=> //.
    ...
    rewrite (eq_rn_sem Hvm1 Hce1) (eq_rn_sem Hvm1 Hce2).
  ...
  Lemma eq_alloc_incl r1 r2 vm vm': 
    ...
    M.incl r2 r1 -> 
    eq_alloc r2 vm vm'.
  Proof. move=> H /M.inclP Hi x id /Hi;apply H. Qed.

  (* TODO: move this *)
  Lemma incl_refl r : M.incl r r.
  Proof. by apply/M.inclP. Qed.

  Lemma incl_trans r2 r1 r3  : M.incl r1 r2 -> M.incl r2 r3 -> M.incl r1 r3.
  Proof. by move=> /M.inclP H1 /M.inclP H2;apply/M.inclP=> x id /H1 /H2. Qed.

  Lemma merge_incl_l r1 r2 : M.incl (M.merge r1 r2) r1.
  Proof. by apply/M.inclP=> x id /M.mergeP []. Qed.

  Lemma merge_incl_r r1 r2 : M.incl (M.merge r1 r2) r2.
  Proof. by apply/M.inclP=> x id /M.mergeP []. Qed.

  Lemma eq_alloc_merge r1 r2 vm vm': 
    eq_alloc r1 vm vm' \/ eq_alloc r2 vm vm' ->
    eq_alloc (M.merge r1 r2) vm vm'.
  Proof. by move=> [|]H x id /M.mergeP [Hr1 Hr2];apply H. Qed.
  ...
  Proof.
    ...
    move=> m1 m2 vm1 vm2 H;inversion H;clear H;subst=> vm1' Hvm1.
    move: H5;rewrite (eq_rn_sem Hvm1 Heq).
    case:cond H6=> H6 H5. 
  ...
  Proof.
    ...
    case Heq: check => [r2|]//=.
    case: ifP => Hincl.
    + move => [] ?;subst r1';exists r2;split=>//;apply incl_refl.
    move=> /Hrec [r3 [H1 H2 H3]];exists r3;split=>//.
    apply/(incl_trans H2)/merge_incl_l.
  Qed.
  ...
  Proof.
    ...
    move=> /loopP [r1'] [].
    case Hcr : check_alloc_rval=> [ri|]//= Hcc Hincl Hincl' m1 m2 vm1 vm2 H.
    ...
    + elim: [seq n2w i | i <- _] m1 vm1 H9 {H8 H7}=> [ | w ws Hws] m1 vm1 H10;
      ...
      + by apply (eq_rn_write _ Hcr Hvm1).
      move=> vm3' [Hvm3 Hsc].
      have []:= Hws _ _ H6 vm3';first by apply: eq_alloc_incl Hincl'.
      by move=> vm2' [Hvm2 Hsf];exists vm2';split=>//; apply: EForOne Hsf.
    move=> vm1' Hvm1;have []:= Hfor vm1';first by apply: eq_alloc_incl Hincl.
    move=> vm2' [Hvm2 Hsf];exists vm2';split=>//.
    apply: EFor Hsf;rewrite -?H7 -?H8 /=;symmetry;first by apply (eq_rn_sem Hvm1 Hhi).
    by apply (eq_rn_sem Hvm1 Hlo).
  Qed.
  ...
  Proof.
    ...
    + move: H4 Hcc Hce Hc.
      ...
      + exists vm1';split=>//;constructor=> /=.
        by rewrite -He1;symmetry;apply: (eq_rn_sem Hvm1 Hce).
      case :(Hc _ _ _ Hcc _ _ _ _ Hc1 _ Hvm1) => vm2' [Hvm2 Hsc].
      have []:= Hrec Hcc Hce Hc vm2';first by apply:(eq_alloc_incl Hvm2).
      move=> vm3' /= [Hvm3 Hsw];exists vm3';split=>//.
      apply: EWhileOne Hsw=> //;rewrite -He1 /=;symmetry.
      by apply: (eq_rn_sem Hvm1 Hce).
    move=> vm1' Hvm1;have []:= Hwhile vm1';first by apply: eq_alloc_incl Hincl.
  ...
  Proof.
    ...
    have ? := check_alloc_e_eqt Hca; have ?:= check_alloc_rval_eqt Hcx;subst.
    move=> vm1' Hvm1.
    exists (write_rval vm1' x2 res);split;first by apply (eq_rn_write _ Hcx).
    have Heq := eq_rn_sem Hvm1 Hca; constructor;rewrite -Heq //.
    by apply: Hf H10.
  Qed.
  ...
  Proof.
    ...
    + by apply (eq_rn_write _ Hcx)=> ??;rewrite M.get0.
    move=> vm2' [Hvm2 Hsb];exists vm2';split=>//.
    by have := eq_rn_sem Hvm2 Hcr;rewrite Hvr !sem_rval2pe => -[].
  Qed.

  Lemma check_alloc_fdP ta tr (f1 f2 : fundef ta tr) mem mem' va vr: 
    check_alloc_fd f1 f2 -> 
