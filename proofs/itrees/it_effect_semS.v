...
Require Import expr psem_defs psem_core it_exec it_exec_sem tfam_iso
               eutt_extras rec_facts it_cflow_semS.
...
Definition handle_InstrE :
  ...
  fun _ e =>
    ...
    | OpnE xs tg o es s => isem_Sopn o xs es s
    | SyscallE xs o es s => isem_Syscall xs o es s                              
    | EvalCond e s => isem_Cond e s
    | EvalBounds e1 e2 s => isem_Bound e1 e2 s
    | WriteIndex x z s => isem_WriteIndex x z s
...
(* InstrE interpreter *)
Definition interp_InstrE {A: Type}
...
Definition interp_StE_S {A: Type}
  (t : itree (StE estate +' E) A) : itree E A :=
  interp (ext_handler handle_StE_S) t.

(* StE interpreter with unit *)
Definition interp_StE_U {XS: stateE estate -< E} {A: Type}
  (t : itree (StE unit +' E) A) : itree E A :=
  interp (ext_handler handle_StE_U) t.
...
(*
Let E2 (S: Type) (E: Type -> Type) :=
      @InstrE asm_op syscall_state sip estate fstate S
              +' @FunE asm_op syscall_state sip fstate fundef S +' E.

Definition interp_up2state {S: Type} E
  {XE: ErrEvent -< E} {XS: @StE estate S -< E}
  (HS: forall T : Type,
        StE S T -> itree (callE (funname * fstate) fstate +' (@InstrE asm_op syscall_state sip estate fstate S
              +' @FunE asm_op syscall_state sip fstate fundef S +' E)) T)
  T (t: itree (@callE (funname * fstate) fstate
             +' @InstrE asm_op syscall_state sip estate fstate S
             +' @FunE asm_op syscall_state sip fstate fundef S +' E) T) :
  itree E T := interp_InstrFunE (@interp_recc_AbsI S
             (@InstrE asm_op syscall_state sip estate fstate S
              +' @FunE asm_op syscall_state sip fstate fundef S +' E)
             _ _ HS _ _ t).
                 ...
                 (@interp_recc asm_op syscall_state sip estate fstate fundef
                    (@InstrE asm_op syscall_state sip estate fstate S
                     +' @FunE asm_op syscall_state sip fstate fundef S +' E) S (@handle_InstrE _ _ p S _) _ _ _ t).


(@handle_FunE _ _ p ev S _) T t).
...
Definition isem_interp_up2state E {XE: ErrEvent -< E}
  ...
  (t: itree (@InstrE asm_op syscall_state sip estate fstate
            +' @FunE asm_op syscall_state sip fstate fundef +' E) T) :
  itree E T := interp_FunE p ev (interp_InstrE p t).
...
Definition up2err_interp T
  (t: itree (@callE (funname * fstate) fstate
             ...
             +' ErrEvent +' E) T) (s: estate) :
  itree _ (estate * T) :=
  run_state (up2state_interp t) s.

*)

(*
Definition isem_up2rec_interp T
  (t: itree (@InstrE asm_op syscall_state sip estate fstate
            +' @FunE asm_op syscall_state sip fstate fundef
            +' @stateE estate
