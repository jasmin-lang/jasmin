...
Proof.
  unfold fundef_add_init_fd_rel in Hyp0. 
...
Lemma xxx s1 c s2 :
  lift_vm2 s1 c s2 <-> lift_sem s1 c s2.
  intros.
  unfold lift_vm2.
  unfold lift_sem.
  split.
  unfold with_vm; simpl.
  intros.
  exists (evm s2); eauto.
  reflexivity.
  unfold vm_eq in H0.
  ...
  unfold vm_eq; simpl; intros.
  specialize (H (evm s1)).
  
  
  assert (vm1 = evm s1) as A1.
...
(*******************************************************************)
  ...
  unfold isem_cmd_; simpl.
  

    (isem_foldr isem_i_body p ev
       (List.fold_left
          (λ (a : cmd) (e : Sv.elt),
             match vtype e with
             | sarr n =>
                 ...
                 then
                  MkI ii1
                    (Cassgn {| v_var := e; v_info := var_info_of_ii ii1 |}
                       AT_none (sarr n) (Parr_init n)) :: a
                 else a
             | _ => a
             end) L1
          ...
          [:: MkI ii1
                (Cassgn
                   {|
                     v_var := {| vtype := sarr p0; vname := vname0 |};
                     v_info := var_info_of_ii ii1
                   |} AT_none (sarr p0) (Parr_init p0)); i]
       ) s2)
  
  
  unfold add_init_aux; simpl.
  

Lemma add_init_aux_simpl_lemma
let F := add_init_aux ii a [:: i] in
if is_not_undef_arr_var a
then F = [:: i] else F = MkI ii (Cassgn (Lvar x) AT_none (sarr n) (Parr_init n)) :: c   
                            
 
  
Check add_init_aux.
  
Context (is_not_undef_arr_var : Sv.elt -> bool)


          
Lemma add_init_aux_fold_lemma
let F := fun L: seq Sv.elt =>
           List.fold_left (λ (a0 : cmd) (e : Sv.elt),
               add_init_aux ii1 e a0) L [:: i] in
if (is_not_undef_arr_var a)
then F (a :: L1) = F L1 
else F (a :: L1) =    
    ...
    (List.fold_left (λ (a : cmd) (e : Sv.elt), add_init_aux ii1 e a)
       a :: L1) =
  ...
  destruct vtype0; simpl.
  destruct vtype; simpl.
  
  
  

  
  elim: Sv.elements s1 [:: MkI ii0 i] => [ | x xs ih].
  (* //= s1 l hl hu. *)
...
(* need to derive this from add_initP *)
Lemma instr_add_init_lemma f1 f2 s1 s2 
  (Hyp0 : fundef_add_init_fd_rel f1 f2)
  (Hyp1 : estate_uincl s1 s2) i I :
  undef_except I (evm s1) ->
  xrutt (EE_MR (core_logics.errcutoff (is_error wE)) (D:=recCall))
    (EE_MR core_logics.nocutoff (D:=recCall))
    (HeterogeneousRelations.sum_prerel RecPreRel EPreRel)
    (HeterogeneousRelations.sum_postrel RecPostRel EPostRel) estate_uincl
    (isem_i_body p ev i s1)
    (isem_cmd_ p ev (add_init_i I i).1 s2).
Proof.
  move=> hu. (* rewrite /add_init Sv.fold_spec. *)
...
Print vm_eq.
 ...
 (* set X := (add_init_i I i). *)
  
  unfold undef_except; intro H.
  unfold add_init_i.
  unfold isem_cmd_; simpl.
...
(*  Locate lift_semI.
  ...
  assert (@lift_semI s1 i s2) as AA.
  unfold lift_semI.
  simpl.
  unfold vm_eq.
  unfold with_vm.
...
*)
  ...
  - unfold add_init; simpl.
    unfold Sv.fold, Sv.Raw.fold; simpl.
  ...
  unfold map_prog, map_prog_name; simpl.
  ...
  { clear H fs1 fs2.
    ...
    { intros f1 f2 H0; subst.
      ...
      { intros s1 s2 H2.
        ...
        { unfold fundef_add_init_fd_rel in H0.
          destruct f1, f2; simpl in *.
          ...
          generalize isem_i_body. into ccc.
