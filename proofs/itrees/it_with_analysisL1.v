...
Fixpoint mapR {E2} {A B} (f: A -> itree E2 (list B)) (ls: list A) :
  itree E2 (list B) :=
  match ls with
  | nil => ret nil
  | x :: xs => x' <- f x ;; xs' <- mapR f xs ;; ret (x' ++ xs') end.            

Fixpoint mapC {E2} {A B} (f: A -> itree E2 (list B)) (ls: list A) :
  itree E2 (list B) :=
  match ls with
  | nil => ret nil
  | x :: xs => xs' <- mapC f xs ;; x' <- f x ;; ret (x' ++ xs') end.            
  ...
  { (* Cassgn case *)
    ...
    { intros R1 R2 d1 d2 H.
      ...
      { simpl; intros c1 H.
        ...
        symmetry in H.
        eapply eqit_inv_bind_ret in H.
        destruct H as [c2 [H0 H1]].
        eapply eqit_inv_bind_ret in H1.
        destruct H1 as [c3 [H2 H6]].
