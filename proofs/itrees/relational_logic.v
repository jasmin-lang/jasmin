...
Definition wkequiv_io {I1 I2 O1 O2}
  ...
  XRutt.rutt (errcutoff (is_error wE)) nocutoff RPreInv RPostInv (Q i1 i2) (F1 i1) (F2 i2).
...
Proof. by move=> hPQ i1 i2 hP; apply XRutt.rutt_Ret; apply hPQ. Qed.
...
Proof.
  move=> h h' i1 i2 hP.
  apply XRuttFacts.rutt_bind with (R i1 i2).
  + by apply h.
...
Proof.
  move=> hF hF' s1 s2 hP.
  apply XRuttFacts.rutt_bind with R; eauto.
...
Proof.
  move=> hF hF' s1 s2 hP.
  apply XRuttFacts.rutt_bind with R.
  + by apply (hF _ _ hP).
  by move=> r1 r2 hR; apply hF'.
Qed.
...
Proof. by move=> hbody i1 i2 hI; apply XRuttFacts.rutt_iter with I. Qed.
...
Lemma rutt_iresult (T1 T2:Type) (s1 : estate1) (s2 : estate2) (x1 : exec T1) (x2 : exec T2) (R : T1 -> T2 -> Prop) :
  ...
  XRutt.rutt (errcutoff (is_error wE)) nocutoff RPreInv RPostInv R (iresult s1 x1) (iresult s2 x2).
Proof.
  case: x1 => [ v1 | e1] hok.
  + have [v2 -> /=] := hok _ erefl.
    by apply: XRutt.rutt_Ret.
...
Proof.
  ...
  apply XRuttFacts.rutt_bind with R; first by apply h.
  apply h'.
Qed.
...
Lemma wequiv_while_full I I' ii1 al1 e1 c1 c1' ii2 al2 e2 c2 c2' :
  wequiv I c1 c2 I' ->
  ...
  wequiv I [:: MkI ii1 (Cwhile al1 c1 e1 c1')] [:: MkI ii2 (Cwhile al2 c2 e2 c2')]
...
Lemma wequiv_while I I' ii1 al1 e1 c1 c1' ii2 al2 e2 c2 c2' :
  wrequiv I' (sem_cond (p_globs p1) e1) (sem_cond (p_globs p2) e2) eq ->
  wequiv I c1 c2 I' ->
  wequiv I' c1' c2' I ->
...
Lemma wequiv_while_uincl I I' ii1 al1 e1 c1 c1' ii2 al2 e2 c2 c2' :
...
Lemma wequiv_while_eq I I' ii1 al1 e1 c1 c1' ii2 al2 e2 c2 c2' :
  wrequiv I' (fun (s:estate1) => sem_pexpr true (p_globs p1) s e1)
             (fun (s:estate2) => sem_pexpr true (p_globs p2) s e2) eq ->
  wequiv I c1 c2 I' ->
  wequiv I' c1' c2' I ->
...
Proof.
  ...
  + rewrite /kget_fundef => ??.
    ...
    + by move=> /(_ _ erefl) [fd2 ] -> _ _; apply XRutt.rutt_Ret.
    move=> _ _; apply XRutt.rutt_CutL.
    by rewrite /errcutoff /is_error /subevent /resum /fromErr mid12.
  move=> fd1 fd2 [+ hfd2].
...
Lemma xrutt_weaken_aux post (sem1 : itree (recCall +' E) fstate) (sem2 : itree (recCall +' E) fstate) :
  XRutt.rutt (errcutoff (is_error (FIso_suml recCall (Err:=ErrEvent)))) nocutoff
    (RPreInv (rE0 := relEvent_recCall spec)) (RPostInv (rE0 := relEvent_recCall spec))
   post sem1 sem2 ->
  XRutt.rutt (XRuttFacts.EE_MR (errcutoff (is_error wE)) recCall) (XRuttFacts.EE_MR nocutoff recCall)
