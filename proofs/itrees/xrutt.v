...
(*  Notation cutoff EE e := (@subevent _ _ (RSub EE) _ e). *)
  ...
  Inductive x0ruttF
    (sim : itree E1 R1 -> itree E2 R2 -> Prop) :
    itree' E1 R1 -> itree' E2 R2 -> Prop :=
  | X0EqRet : forall (r1 : R1) (r2 : R2),
      RR r1 r2 ->
      x0ruttF (RetF r1) (RetF r2)
  | X0EqTau : forall (m1 : itree E1 R1)
                   (m2 : itree E2 R2),
      sim m1 m2 ->
      x0ruttF (TauF m1) (TauF m2)
  | X0EqVis : forall (A B : Type) (e1 : E1 A) (e2 : E2 B)
                   (k1 : A -> itree E1 R1)
                   (k2 : B -> itree E2 R2),
      REv e1 e2 ->
      (forall (a : A) (b : B), RAns e1 a e2 b -> sim (k1 a) (k2 b)) ->
      x0ruttF (VisF e1 k1) (VisF e2 k2)
  | X0EqCutL: forall A (e1 : E1 A)
                   (k1: A -> itree E1 R1)
                   (ot2: itree' E2 R2),
      IsCut EE1 e1 ->
      x0ruttF (VisF e1 k1) ot2
  | X0EqCutR: forall A (e2 : E2 A)
                   (k2: A -> itree E2 R2)
                   (ot1: itree' E1 R1),
      IsCut EE2 e2 ->
      x0ruttF ot1 (VisF e2 k2)
  | X0EqTauL : forall (t1 : itree E1 R1)
                    (ot2 : itree' E2 R2),
      x0ruttF (observe t1) ot2 ->
      x0ruttF (TauF t1) ot2
  | X0EqTauR : forall (ot1 : itree' E1 R1)
                    (t2 : itree E2 R2),
      x0ruttF ot1 (observe t2) ->
      x0ruttF ot1 (TauF t2).
  Hint Constructors x0ruttF : itree.

  Definition x0rutt_
    (sim : itree E1 R1 -> itree E2 R2 -> Prop)
    (t1 : itree E1 R1) (t2 : itree E2 R2) :=
    x0ruttF sim (observe t1) (observe t2).
  Hint Unfold x0rutt_ : itree.

  Lemma x0rutt_monot : monotone2 x0rutt_.
  Proof.
    red. intros. red; induction IN; eauto with itree.
  Qed.
  ...
  Definition x0rutt :
    itree E1 R1 -> itree E2 R2 -> Prop :=
    paco2 x0rutt_ bot2.
  Hint Unfold x0rutt : itree.

Lemma x0ruttF_inv_VisF_r {sim} t1 U2 (ee2: E2 U2) (k2: U2 -> _):
  x0ruttF sim t1 (VisF ee2 k2) ->
  (exists U1 (ee1: E1 U1) k1,
    ...
    (exists t1', t1 = TauF t1' /\
                   x0ruttF sim (observe t1') (VisF ee2 k2)).        
  Proof.
    intros.
    remember t1 as t0.
    destruct t0.
...
Lemma x0ruttF_inv_VisF {sim} U1 U2
   (ee1 : E1 U1) (ee2 : E2 U2)
   (k1 : U1 -> itree E1 R1) (k2 : U2 -> itree E2 R2) :
   x0ruttF sim (VisF ee1 k1) (VisF ee2 k2) ->
   (forall v1 v2, RAns ee1 v1 ee2 v2 -> sim (k1 v1) (k2 v2))
   \/
   IsCut EE1 ee1 
   \/
  ...
  Lemma fold_x0ruttF:
    forall (t1: itree E1 R1) (t2: itree E2 R2) ot1 ot2,
    x0ruttF (upaco2 x0rutt_ bot2) ot1 ot2 ->
    ot1 = observe t1 ->
    ot2 = observe t2 ->
    x0rutt t1 t2.
  Proof.
...
Ltac unfold_x0rutt :=
    (match goal with [ |- x0rutt_ _ _ _ _ _ _ _ _ ] => red end) ;
    (repeat match goal with [H: x0rutt_ _ _ _ _ _ _ _ _ |- _ ] =>
                              red in H end).

Tactic Notation "fold_x0ruttF" hyp(H) :=
  try punfold H;
  try red in H;
  match type of H with
  | x0ruttF ?_EE1 ?_EE2 ?_REV ?_RANS ?_RR
      (upaco2 (x0rutt_ ?_EE1 ?_EE2 ?_REV ?_RANS ?_RR) bot2)
      ?_OT1 ?_OT2 =>
      match _OT1 with
      | observe _ => idtac
      | ?_OT1 => rewrite (itree_eta' _OT1) in H
      ...
      eapply fold_x0ruttF in H; [| eauto | eauto]
  end.

#[global] Hint Resolve x0rutt_monot : paco.

Section XRutt_X0Rutt_equiv.
  Context {E1 E2: Type -> Type}.
  Context {R1 R2 : Type}.
...
Lemma xrutt2x0rutt (t1: itree E1 R1) (t2: itree E2 R2) :
  @xrutt E1 E2 R1 R2 EE1 EE2 REv RAns RR t1 t2 ->
  @x0rutt E1 E2 R1 R2 EE1 EE2 REv RAns RR t1 t2.    
Proof.
  revert t1 t2.
  pcofix CIH.  
  intros t1 t2 H.
...
Lemma x0rutt2xrutt (t1: itree E1 R1) (t2: itree E2 R2) :
  @x0rutt E1 E2 R1 R2 EE1 EE2 REv RAns RR t1 t2 ->
  @xrutt E1 E2 R1 R2 (@EE1) (@EE2) (@REv) (@RAns) (@RR) t1 t2.
Proof.
  revert t1 t2.
...
Lemma xrutt_x0rutt_equiv (t1: itree E1 R1) (t2: itree E2 R2) :
  @xrutt E1 E2 R1 R2 EE1 EE2 REv RAns RR t1 t2 <->
  @x0rutt E1 E2 R1 R2 EE1 EE2 REv RAns RR t1 t2.    
Proof.
  split.
