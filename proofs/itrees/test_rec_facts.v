...
(*  
  ...
  (* this *)
  assert (eutt eq (x <- ctx A d;; k x)  (x <- ctx2_cond cond ctx d;; k x)).
  eapply eqit_bind; try reflexivity.
  unfold ctx2_cond, Handler.cat; simpl.
  (* no way *)
*)  

  gfinal; left.
  unfold ctx2_cond, Handler.cat at 2.
  Fail eapply CIH.
  (* hopeless. However, this point corresponds to the second entry of
  invariant. Indeed, compare with 

   euttG eq bot2 bot2 gL' gH' (interp_mrec ctx (x <- t;; k x))
    (interp_mrec (ctx2_cond cond ctx)
       (x <- interp (ctx_cond (cond T1 d1) ctx) t;; k' x))
 *)
  admit.
...
Lemma interp_mrec_loop2_ {D E} (cond : forall T1, D T1 -> forall T2, D T2 -> bool) (ctx : D ~> itree (D +' E)) {R} :
  forall {t u : itree (D +' E) R},
  invariant cond ctx t u -> 
  interp_mrec ctx t ≈ interp_mrec (ctx2_cond cond ctx) u.
Proof with auto.
(*  unfold ctx2_cond, ctx_cond; simpl.
  unfold Handler.cat, Handler.case_; simpl.
  intros.
  setoid_rewrite interp_mrec_as_interp.
  eapply eutt_interp.
  intros T a.
  ...
  ecofix SELF.
  intros t y H.
  induction H  as [t | T1 d1 A t k | A t k k'].
(*  induction 1 as [t | T1 d1 A t k | A t k k']. *) 
- destruct (case_itree t) as [ ? H | u H | A [d|e] k H ]; rewrite <- H;  
