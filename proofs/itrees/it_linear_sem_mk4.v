...
Definition GCntr {E} {XE: ErrEvent -< E}  
  ...
  (* the optional function body *)
    ...
    | Some lc => 
      ...
      | _ => if is_final lc pt then Ret (inr fpt) else throw err end
...
Definition isem_linstr (ir : linstr_r) (l: lcpoint) :
  itree E lcpoint :=
  match ir with
...
Definition isem_lcmd_body (lbl: lcpoint) :
  itree E (lcpoint + lcpoint) := GCntr isem_linstr lbl.
...
(* provides the behaviour wrt the stack abstraction *)
...
(* we interpret stack events in the stack abstraction (alstate) *)
Definition handle_StackA {T} (e: StackE T) : itree E T :=
  match e with    
...
Section HandleStackC.

Context {E: Type -> Type} {XA: @stateE lstate -< E} {XE: ErrEvent -< E}.

Context (store_ra : lcpoint -> lstate -> lstate).
Context (update_ra : lstate -> lstate).
Context (load_ra : lstate -> lcpoint).

(* we interpret stack events directly in the linear state *)
Definition handle_StackC {T} (e: StackE T) : itree E T :=
  match e with    
  | Push l => st <- trigger (@Get lstate) ;;
              trigger (@Put lstate (store_ra l st))
  | Pop => st <- trigger (@Get lstate) ;;
           trigger (@Put lstate (update_ra st)) ;;
           Ret (load_ra st)
  end.
...
(* Abstract alternative: using the stack abstraction, we must express
   its agreeement with the linear state *)
...
Definition handle_LinA {XE: ErrEvent -< E} {T} (e: LinE T) :
...
(* Concrete alternative: we do not use the stack abstraction *)
Context {E: Type -> Type} 
        {XS: @stateE lstate -< E} {XE: ErrEvent -< E}.

(* TODO: incomplete, add missing constructors *)
Definition handle_LinC {XE: ErrEvent -< E} {T} (e: LinE T) :
  itree E T := match e with
  | MatchLabel (fn, lbl)%type =>
      s <- trigger (@Get lstate) ;;
      if (fn == s.(lfn)) && (lbl == lpc s)
...
(* gives the core of the expected denotational semantics *)
Definition interp_Linstr E {XE: ErrEvent -< E}
  {XL: LinE -< E} {XLS: stateE lstate -< E}
  {XST: StackE -< E} {A: Type}
  (t : itree (LinstrE +' E) A) : itree E A :=
  interp (ext_handler (fun T x => @handle_Linstr E XLS XE XL T x)) t.

(* gives the dataflow abstraction semantics *)
Definition interp_LFlow E {XL: LinE -< E} {XST: StackE -< E} {A: Type}
  (t : itree (LFlowE +' E) A) : itree E A :=
  interp (ext_handler (fun T x => @handle_LFlow E XL XST T x)) t.

(* Abstract alternative: stack events (alstate-based) *)
Definition interp_StackA E {XE: ErrEvent -< E}
  {XL: LinE -< E} {XLS: stateE alstate -< E} {A: Type}
  (t : itree (StackE +' E) A) : itree E A :=
  interp (ext_handler (fun T x => @handle_StackA E XLS XE T x)) t.

(* Abtract alternative: alstate-based and lstate-based events *)
Definition interp_LinA E {XE: ErrEvent -< E}
  {XLS: stateE alstate -< E}
  {XS: stateE lstate -< E} {A: Type}
  (t : itree (LinE +' E) A) : itree E A :=
  interp (ext_handler
            (fun T x => @handle_LinA stackAgree E XLS XS XE T x)) t.

(* Concrete alternative: only lstate-based *)
Definition interp_LinC E {XE: ErrEvent -< E}
  {XS: stateE lstate -< E} {A: Type}
  (t : itree (LinE +' E) A) : itree E A :=
  interp (ext_handler
            (fun T x => @handle_LinC E XS XE T x)) t.

Definition interp_up2alstateG E {XE: ErrEvent -< E}
  {XLS: stateE alstate -< E}
  {XS: stateE lstate -< E} {A: Type}
  (t : itree (LinstrE +' LFlowE +' StackE +' LinE +' E) A) : itree E A :=
  interp_LinA (interp_StackA (interp_LFlow (interp_Linstr t))).

Definition interp_up2alstateL E {XE: ErrEvent -< E}
  {XLS: stateE alstate -< E}
  {XS: stateE lstate -< E} {A: Type}
  (t : itree (LinstrE +' StackE +' LinE +' E) A) : itree E A :=
  interp_LinA (interp_StackA (interp_Linstr t)).

(* Concrete alternative: stack events (lstate-based) *)
Definition interp_StackC E {XE: ErrEvent -< E}
  {XL: LinE -< E} {XLS: stateE lstate -< E}
  ...
  {A: Type}
  (t : itree (StackE +' E) A) : itree E A :=
  interp (ext_handler (fun T x => @handle_StackC E XLS sra0 ura0 lra0 T x)) t.

Definition interp_up2lstateG E {XE: ErrEvent -< E}
  {XS: stateE lstate -< E} 
  (sra0 : lcpoint -> lstate -> lstate)
  (ura0 : lstate -> lstate)
  (lra0 : lstate -> lcpoint)
  {A: Type} (t : itree (LinstrE +' LFlowE +' StackE +' LinE +' E) A) 
   : itree E A :=
  interp_LinC (interp_StackC sra0 ura0 lra0 (interp_LFlow (interp_Linstr t))).
...
Definition interp_up2lstateL E {XE: ErrEvent -< E}
  ...
  (ura0 : lstate -> lstate)
  (lra0 : lstate -> lcpoint)
  {A: Type} (t : itree (LinstrE +' StackE +' LinE +' E) A) :
  itree E A :=
  interp_LinC (interp_StackC sra0 ura0 lra0 (interp_Linstr t)).
...
(* if the linear translation of i is straightline code that ends
   ...
   otherwise *)
Definition lsem_LCntr E {XE: ErrEvent -< E} {XI : LinstrE -< E}
  {XL: LinE -< E} {XLS: stateE lstate -< E} {XST: StackE -< E}
  (fn: funname) (n0 n1: nat) : itree E (lcpoint + lcpoint) :=
  LCntr (fun i l => interp_LFlow (isem_linstr i l)) fn n0 n1 (fn, n0).

(* iterate lsem_LCntr *)
Definition lsem_LCntrI E {XE: ErrEvent -< E} {XI : LinstrE -< E}
  {XL: LinE -< E} {XLS: stateE lstate -< E} {XST: StackE -< E}
  (fn: funname) (n0 n1: nat) : itree E lcpoint :=
  LCntrI (fun i l => interp_LFlow (isem_linstr i l)) fn n0 n1 (fn, n0).
...
(* linear semantics of the source code, for the intermediate
   representation. assuming fenv gives the linear code *)
Fixpoint lsem_instr E {XE: ErrEvent -< E}
  ...
  | Copn xs tg o es => LS fn n0 nE                 

  | Csyscall xs o es => LS fn n0 nE    
...
Definition lsem_cmd E {XE: ErrEvent -< E}
  ...
  @lsem_c_seq (callE (lcpoint * funname) lcpoint +' E) _
              loc_instr (lsem_instr LS loc_instr) cc l0 l1.
...
Definition handle_StackAlloc E
  {XE: ErrEvent -< E} {XLS: stateE lstate -< E} {T}
  (e: @StackAllocE T) : itree E T. 
Admitted. 

Definition interp_StackAlloc E {XE: ErrEvent -< E} {XLS: stateE lstate -< E}
  {T} (t: itree (StackAllocE +' E) T) : itree E T :=
  interp (ext_handler (fun T x => @handle_StackAlloc E XE XLS T x)) t.

(* linear semantics of source functions. l1 is the return address *)
Definition lsem_fun E {XE: ErrEvent -< E} {XTSA: StackAllocE -< E}
  (LS: funname -> nat -> nat ->
       itree (callE (lcpoint * funname) lcpoint +' E) lcpoint)
  (loc_instr : instr -> lcpoint -> nat)
  ...
  itree (callE (lcpoint * funname) lcpoint +' E) lcpoint :=
  trigger (Before l1) ;;
  cc <- trigger (GetFunSourceCode fn) ;;
  lsem_cmd LS loc_instr cc (fn, 0) (fn, 0) ;;
  trigger After. 

Definition handle_LRec E {XE: ErrEvent -< E} {XTSA: StackAllocE -< E}
  (LS: funname -> nat -> nat ->
       itree (callE (lcpoint * funname) lcpoint +' E) lcpoint)
  (loc_instr : instr -> lcpoint -> nat) :
 ...
 fun T (rc : callE _ _ T) =>
   match rc with
   | Call (l1, fn) => lsem_fun LS loc_instr l1 fn
   end.
  
Definition interp_LRec E {XE: ErrEvent -< E} {XTSA: StackAllocE -< E}
  (LS: funname -> nat -> nat ->
       itree (callE (lcpoint * funname) lcpoint +' E) lcpoint)  
  (loc_instr : instr -> lcpoint -> nat)
  T (t: itree (callE (lcpoint * funname) lcpoint +' E) T) : itree E T :=
  interp_mrec (handle_LRec LS loc_instr) t.

Definition interp_LRec_cmd E {XE: ErrEvent -< E} {XTSA: StackAllocE -< E}
  (LS: funname -> nat -> nat ->
       itree (callE (lcpoint * funname) lcpoint +' E) lcpoint)  
  (loc_instr : instr -> lcpoint -> nat)
  (l0: lcpoint) (c: cmd) : itree E _ :=
  interp_LRec LS loc_instr (lsem_cmd LS loc_instr c l0 l0).

Definition interp_LRecSA E {XE: ErrEvent -< E} {XLS: stateE lstate -< E}
  (LS: funname -> nat -> nat ->
       itree (callE (lcpoint * funname) lcpoint +' StackAllocE +' E) lcpoint)  
  (loc_instr : instr -> lcpoint -> nat)
  T (t: itree (callE (lcpoint * funname) lcpoint +' StackAllocE +' E) T) :
  itree E T :=
  let hnd := (@handle_LRec (StackAllocE +' E) _ _ LS loc_instr) in
  interp_StackAlloc (interp_mrec hnd t).

Definition interp_LRecSA_cmd E {XE: ErrEvent -< E} {XLS: stateE lstate -< E} 
  (LS: funname -> nat -> nat ->
       itree (callE (lcpoint * funname) lcpoint +' StackAllocE +' E) lcpoint)  
  (loc_instr : instr -> lcpoint -> nat)
  (l0: lcpoint) (c: cmd) : itree E _ :=
  interp_LRecSA LS loc_instr (lsem_cmd LS loc_instr c l0 l0).

Definition interp_LRec_cmd_H0 E {XE: ErrEvent -< E}
  {XI : LinstrE -< E} {XL: LinE -< E} {XLS: stateE lstate -< E}
  {XST: StackE -< E} {XTSA: StackAllocE -< E}
  (loc_instr : instr -> lcpoint -> nat)
  (l0: lcpoint) (c: cmd) : itree E _ :=
  let E' := (callE (lcpoint * funname) lcpoint +' E) in
  let LS := @lsem_LCntrI E' _ _ _ _ _ in
  interp_LRec LS loc_instr (lsem_cmd LS loc_instr c l0 l0).
  
Definition interp_LRecSA_cmd_H0 E {XE: ErrEvent -< E}
  {XI : LinstrE -< E} {XL: LinE -< E} {XLS: stateE lstate -< E}
  {XST: StackE -< E} 
  (loc_instr : instr -> lcpoint -> nat)
  (l0: lcpoint) (c: cmd) : itree E _ :=
  let E' := (callE (lcpoint * funname) lcpoint +' StackAllocE +' E) in
  let LS := @lsem_LCntrI E' _ _ _ _ _ in
  interp_LRecSA LS loc_instr (lsem_cmd LS loc_instr c l0 l0).

(*
Definition interp_LRec_H0 E {XE: ErrEvent -< E}
  {XI : LinstrE -< E} {XL: LinE -< E} {XLS: stateE lstate -< E}
  {XST: StackE -< E} {XTSA: StackAllocE -< E}
  (loc_instr : instr -> lcpoint -> nat)
  T (t: itree (callE (lcpoint * funname) lcpoint +' E) T) :
  itree E T :=
  let E' := (callE (lcpoint * funname) lcpoint +' E) in
  let LS := @lsem_LCntrI E' _ _ _ _ _ in
  interp_LRec LS loc_instr t.

Definition interp_LRecSA_H0 E {XE: ErrEvent -< E}
  {XI : LinstrE -< E} {XL: LinE -< E} {XLS: stateE lstate -< E}
  {XST: StackE -< E} 
  (loc_instr : instr -> lcpoint -> nat)
  T (t: itree (callE (lcpoint * funname) lcpoint +' StackAllocE +' E) T) :
...
Lemma LinearIntermediateRep_cmd_ok E {XE: ErrEvent -< E}
  ...
  (loc_instr : instr -> lcpoint -> nat)
  (l0: lcpoint) (c: cmd) :
  eutt eq (@interp_LRec_cmd_H0 E XE XI XL XLS XST XTSA loc_instr l0 c)
...
Lemma LinearIntermediateRep_cmd_ok1 E {XE: ErrEvent -< E}
  ...
  (loc_instr : instr -> lcpoint -> nat)
  (l0: lcpoint) (c: cmd) :
  eutt eq (@interp_LRecSA_cmd_H0 E XE XI XL XLS XST loc_instr l0 c)
          (interp_LFlow (isem_lcmd l0)). 
Admitted. 
...
Lemma LinearSem_fun_correct (pd : PointerData) (sp: sprog)
  ...
  let E' := (callE (lcpoint * funname) lcpoint +'
         StackAllocE +' LinstrE +' StackE +' LinE +' E2) in
  let LS := @lsem_LCntrI E' _ _ _ _ _ in 
  (forall (fn: funname) (fd: sfundef),
      ...
      fenv fn = Some lc0) -> 
  forall (fn: funname),
    let lden := lsem_fun LS lc_end (fn,0) fn in
    let lin_sem := @interp_up2lstateL E2 XE2 XS2 sra0 ura0 lra0 _ 
                      (interp_LRecSA LS lc_end lden) in
    forall xs es ii,
