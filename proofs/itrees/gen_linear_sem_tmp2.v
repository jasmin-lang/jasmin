...
Definition LACntr_alt (Bd: lpoint -> itree E lpoint)
  (fn: funname) (nS nE: nat) :
  (lpoint + lpoint) -> itree E (lpoint + lpoint) :=
  ACntr Bd
    (* exit condition: whenever either it jumps to another function,
...
Definition LACntr (Bd: lpoint -> itree E lpoint)
  ...
  ACntr Bd
    (* exit condition: whenever either it jumps to another function,
       ...
       as 0 is the entry point in every function. *)
    (fun '(fn0, n0) =>
          if (not_locally_possible fn fn0 nE) then None
          else Some ((nS <= n0) && (n0 < nE) && (0 < n0))).

Definition LACntr_bad (Bd: lpoint -> itree E lpoint) (Cnd : bool)          
  (fn: funname) (nS nE: nat) :
  (lpoint + lpoint) -> itree E (lpoint + lpoint) :=
  ACntr Bd
    (* exit condition: whenever either it jumps to another function,
       ...
       as 0 is the entry point in every function. *)
    (fun '(fn0, n0) =>
          if (not_locally_possible fn fn0 nE || (Cnd == false)) then None
          else Some ((nS <= n0) && (n0 < nE) && (0 < n0))).
...
Lemma LACntrI_cond_error 
  ...
  (A1: (not_locally_possible fn (lp0.1) n1) = true) :
...
Proof.
  ...
  { destruct ((n0 <= n2 < n1) && (0 < n2)) eqn: was_e1; simpl;
      try reflexivity.
...
(*  unfold lsem_imed_recL, isem_lcmd_flow, lsem_imed_recF; simpl. *)
  ...
  { intros; subst Pt; simpl; intros.
    ...
    { destruct (not_locally_possible fn fn1 n0.+1) eqn: was_e2;
        try reflexivity.    
