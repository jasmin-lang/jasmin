...
(* superseded: generic iterator specialized to a semantics of
   instructions. used for Linear Core Semantics *)
Definition SCntr
  (Sem: linstr -> L -> itree E L) (TryFnd: L -> option linstr)
  (NoExit: L -> bool) (l0: L) : itree E (L + L) :=
  ACntr (ALSem Sem TryFnd) NoExit l0.
(* superseded *)
Definition SCntrI
  (Sem: linstr -> L -> itree E L) (TryFnd: L -> option linstr)
  (NoExit: L -> bool) : L -> itree E L :=
                         ...
                         then
                          ITree.iter
                            (λ l2 : L,
                               if NoExit1 l2
                               then
                                ITree.bind (Sem l2) (λ l3 : L, Ret (inl l3))
                               else Ret (inr l2)) l0
                         else
                          if NoExit2 l0
                          then
                          ...
                          else throw err) (λ l2 : L, Ret (inl l2))
                     else Ret (inr l0)) l2)))
           (ITree.iter (λ l0 : L,
                     if NoExit1 l0 || NoExit2 l0
                     then
...
Lemma Cntr_range2_simpl (Sem: L -> itree E L)
  (NoExit1 NoExit2: L -> bool)  
  (DC12: forall x, NoExit1 x -> NoExit2 x -> False) (l1: L) :
  NoExit2 l1 = true ->
  eq_itree eq (ITree.bind
...
Proof.
  ...
  setoid_rewrite unfold_iter at 5.
  rewrite H; simpl.
  assert (NoExit1 l1 = false) as A1.
  { destruct (NoExit1 l1) eqn: e1; auto.
    specialize (DC12 _ e1 H); intuition.
  }
  rewrite A1; simpl.
  setoid_rewrite bind_bind.
  setoid_rewrite bind_ret_l; simpl. reflexivity.
...
(* HERE *)
Lemma Cntr_aux2 (Sem: L -> itree E L)
  (NoExit1 NoExit2: L -> bool)  
  (DC12: forall x, NoExit1 x -> NoExit2 x -> False) (l0: L) :
  let NoExit12 := fun x => NoExit1 x || NoExit2 x in 
  eutt eq (ACntrI Sem NoExit12 l0)
          (ACntrI (fun l1 => if NoExit1 l1
               then ACntrI Sem NoExit1 l1
               else if NoExit2 l1
                    then ACntrI Sem NoExit2 l1
                    else throw err)               
               NoExit12 l0).
Proof.
  assert (forall x, NoExit1 x \/ NoExit2 x 
                    \/ ((NoExit1 x = false) /\ (NoExit2 x = false))) as IV.
  { intros l1.
    destruct (NoExit1 l1); eauto.
    destruct (NoExit2 l1); eauto. }
  unfold ACntrI, ACntr; simpl.
  revert l0.
  ginit. gcofix CIH.
  intro l0.
  setoid_rewrite unfold_iter.
  destruct (NoExit1 l0 || NoExit2 l0) eqn: was_e1; simpl.
  2: { setoid_rewrite bind_ret_l; simpl.
       gstep; red. econstructor; auto. }
  setoid_rewrite bind_bind.
  setoid_rewrite bind_ret_l; simpl.
  destruct (NoExit1 l0) eqn: was_e2; simpl.
  { (* NoExit1 l0 *)
    ...
    {  (* NoExit1 l1 *)
      rewrite Cntr_range1_simpl; eauto.
    ...
    { (* NoExit2 l1 *)
      ...
      assert (NoExit1 l1 = false) as A1.
      { destruct (NoExit1 l1) eqn: e1; auto.
        specialize (DC12 _ e1 IV); intuition.
      }
      rewrite A1; simpl.
      setoid_rewrite bind_ret_l; simpl.
      setoid_rewrite bind_ret_l; simpl.
  ...
  { (* NoExit2 l0 *)
    assert (NoExit2 l0 = true) as A1. 
    { destruct (NoExit2 l0); auto. }
    rewrite A1; simpl.
    setoid_rewrite unfold_iter at 2.
    setoid_rewrite bind_bind.
    rewrite A1; simpl.
    ...
    guclo eqit_clo_bind.
    econstructor 1 with (RU := eq); try reflexivity.
    clear was_e1 was_e2 A1. clear l0.
    ...
    {  (* NoExit1 l1 *)
      setoid_rewrite unfold_iter at 2.
      setoid_rewrite bind_bind.
      assert (NoExit2 l1 = false) as A1.
      { destruct (NoExit2 l1) eqn: e1; auto.
        specialize (DC12 _ IV e1); intuition.
      }
      rewrite A1; simpl.
      setoid_rewrite bind_ret_l; simpl.
      setoid_rewrite bind_ret_l; simpl.
    ...
    {  (* NoExit2 l1 *)
      rewrite Cntr_range2_simpl; eauto.
      gstep; red; econstructor.
      gfinal; left. eapply CIH.
    }
    { setoid_rewrite tau_euttge.
...
Lemma Cntr_aux2_equiv (Sem: L -> itree E L)
  (NoExit1 NoExit2: L -> bool) (l0: L) :
  let NoExit12 := fun x => NoExit1 x || NoExit2 x in 
  eutt eq (ITree.iter (ACntr (fun x =>
                                if NoExit1 x then Sem x
                                else if NoExit2 x then Sem x
                                     else throw err) NoExit12) l0)
         (ITree.iter (ACntr Sem NoExit12) l0). 
Proof.
  unfold ACntr; simpl.
  eapply eutt_iter; intros l1.  
  destruct (NoExit1 l1 || NoExit2 l1) eqn:was_e0; simpl; try reflexivity.
  destruct (NoExit1 l1) eqn:was_e1; simpl; try reflexivity.
  destruct (NoExit2 l1) eqn:was_e2; simpl; try reflexivity.
  intuition.
Qed.  
...
Proof.
  unfold ACntrI, ACntr.
  rewrite Cntr_aux2_equiv.
  eapply Cntr_aux2; auto.
Qed.
...
Definition LACntrI (Sem: lpoint -> itree E lpoint)
  (fn: funname) (nS nE: nat) :
  lpoint -> itree E lpoint := ITree.iter (LACntr Sem fn nS nE).
...
Definition LCntrI (Sem: linstr -> lpoint -> itree E lpoint)
  (fn: funname) (nS nE: nat) :
  lpoint -> itree E lpoint := ITree.iter (LCntr Sem fn nS nE).
...
Definition isem_lcmd_flow_body :
  lpoint -> itree E (lpoint + lpoint) := GCntr isem_li_flow.
...
Definition isem_lfloc_body :
  lpoint -> itree E (lpoint + lpoint) :=
  fun l => let fn := fst l in 
      ACntr (isem_lfun_lfloc fn) halt_pred l.                
...
(*
Notation ACntrI Sem NoExit := (ITree.iter (ACntr Sem NoExit)).
Notation SCntrI Sem TryFnd NoExit := (ITree.iter (SCntr Sem TryFnd NoExit)).
Notation LACntrI Sem fn nS nE := (ITree.iter (LACntr Sem fn nS nE)).
Notation LCntrI Sem fn nS nE := (ITree.iter (LCntr Sem fn nS nE)).
Notation GCntrI Sem := (ITree.iter (GCntr Sem)).
*)
...
(*  if fn == fst lpA then *)
  ...
  | LCallNode _ nb na fn' lc_bef lc_aft li1 li2 =>
      ...
      | false => throw err  
      | true => let n1 := pS + S nb in
                if in_btw pS n1 pA
                then LSC (lc_bef ++ [li1]) lpA 
                else if in_btw n1 (n1 + S na) pA
                     then LSC (li2 :: lc_aft) lpA   
                  (* else Ret (inr plA) *)
                     else Ret lpA             
(*
          CNN (LSC (lc_bef ++ [li1])) 
                  (fun x =>
                     CNN (fun _ => trigger_inl1 (Call fn')) 
                                (LSC (li2 :: lc_aft))
                                n1 (S n1) (S (S n1)) (S (S n1)) 
                                                     ;;
                                  Ret (fn, pS + nb + S (S na)))) lpA 
*)                           
      end 
  end) fn pS pE    
with lsem_cmd_imedA 
...
Lemma LACntrI_match_error {XS: stateE LState -< E}
  ...
  (A1: cond = false) :
  ITree.iter (LACntr (λ (lpA: lpoint),
...
(*  unfold lsem_imed_recL, isem_lcmd_flow, lsem_imed_recF; simpl. *)
  ...
  { intros; subst Pt; simpl; intros.
    ...
    unfold LACntrI in H at 2; simpl in *.
    ...
    set PI := (λ '((_, pA) as lpA),
                  ...
                  then isem_li_aflow lpA
                   ...
                   then ITree.iter (LACntr (lsem_cmd_imedA in_btw Id_cmb
    ...
    destruct lp1 as [fn1 nB] eqn: was_e0; simpl.
...
Lemma instrumented_funlocal2intermediate_funglobal_lcmd {XS: stateE LState -< E}
  (fn: funname) (plS plE: plinfo)
  (lt : LTreeList fn plS plE) (n0: nat) :
  eutt eq (@isem_lfloc E _ _ _ _ code_length (fn, n0))
    (lsem_imed_recF lt (fn, n0)).
Proof.
  set lS := plS.1.
  set lE := plE.1.
  set lpS := (fn, lS).
  set lpE := (fn, lE).
  generalize (fn, n0) as pl0.

  unfold isem_lfloc.
(*  unfold isem_lfloc_body. *)
(*  unfold isem_lfun_lfloc.
  unfold isem_lfun_lfloc_body.
  unfold LCntr, LACntr, ACntr; simpl.
  unfold ALSem; simpl.
  unfold find_linstr_in_fun; simpl.
*)

  (* should match (lfenv l0.1) and lt at pl0; then they are the same
     instruction, and as long as they are not function calls they
  unfold LCntr, LACntr, ACntr; simplbehave the same way.  *)

  intros pl0.
  set fnA := fst pl0.
(*  destruct pl0 as [fnA nA]; simpl. *)
  
  unfold lsem_imed_recF; simpl.
  unfold lsem_cmd_imedF. unfold lsem_cmd_imedAF; simpl.

  (* left: given any fnA, the external loop will return only at the
     fnA halting point, which is code_length (lfenv fnA).
     assuming the internal loop runs on fnA code, it will either
     return at the same point, or jump externally. 
*)

  unfold interp_mrec.
  
  (* here we want to unfold the inner loop on the left, but we cannot
  do it without first unfolding the outer loop. *)
  setoid_rewrite unfold_iter.

Abort.

(*
  destruct (isSome (halt_pred pl0)) eqn: was_e0; simpl.
  { (* (outer loop) it is not a halting point; 
       then we step xinto the inner loop *)
    rewrite bind_bind.
    setoid_rewrite bind_ret_l.
    subst fnA.
    destruct pl0 as [fnA nA]; simpl.
    (* now we are ready to unfold the inner loop on the left *)
    setoid_rewrite unfold_iter at 1.
    setoid_rewrite bind_bind.
    
    (* now the relationship is between 'lfenv (fnA, nA)' on the left
      and lt at () *)
  
  (* ginit. gcofix CIH. *)
  (* CIH : isem_lfloc pl0 ≈ lsem_imed_recF lt pl0 *) 
  ...
  unfold isem_lfun_lfloc.
  unfold isem_lfun_lfloc_body.
  unfold LCntr, LACntr, ACntr.

  unfold interp_mrec.
  (* we unfold the global iter on the left, and the loop on the right *)
  setoid_rewrite unfold_iter.

  destruct (isSome (halt_pred pl0)) eqn: was_e0; simpl.
  { (* (outer loop) it is not a halting point; 
       then we step into the inner loop *)
    rewrite bind_bind.
    setoid_rewrite bind_ret_l.
    destruct pl0; simpl.
    (* we unfold the inner loop on the left *)    
    setoid_rewrite unfold_iter at 1.
    setoid_rewrite bind_bind.
    (* function-level condition true on the left: no external jump *)
    destruct (loc_possible f f (code_length f) && (0 <= n) &&
                (n < code_length f) && (0 < n)) eqn: was_e1. 
    rewrite bind_bind.
    setoid_rewrite bind_ret_l.
    unfold LACntr at 1. unfold ACntr; simpl.
    (* function-level condition true on the right: no external jump *)  
    destruct (loc_possible fn f plE.1 && (plS.1 <= n) && (n < plE.1) &&
                (0 < n)) eqn: was_e2.

    unfold lsem_cmd_imedF.
    unfold lsem_cmd_imedAF. simpl.
    unfold ALSem; simpl.
