...
Require Import psem psem_facts it_sems_core it_sems_core_proof.
Import Utf8.
...
Lemma psem_call :
  ...
  forall fn,
    equiv_f p p ev ev RPreFeq fn fn RPostFeq.
Proof.
...
+ move=> s1' s2' fs1' [hscs hmem hvm]; rewrite /finalize_funcall /RPostFeq.
  t_xrbindP => vs.
  rewrite /get_var_is (mapM_ext (λ (x : var_i) _, get_var_sim hvm x)) hfinal hscs hmem => -> /=.
  move=> ? -> <- /=; eauto.
 set Pi := fun (i:instr) => equiv_rec (wsw1:= nosubword) (wsw2:= withsubword)
                p p ev ev RPreFeq RPostFeq estate_sim [::i] [::i] estate_sim.
 set Pr := fun (i:instr_r) => forall ii, Pi (MkI ii i).
 set Pc := fun (c:cmd) => equiv_rec (wsw1:= nosubword) (wsw2:= withsubword)
                          p p ev ev RPreFeq RPostFeq estate_sim c c estate_sim.
 ...
 + move=> j d lo hi c hc ii; apply equiv_for_eq with estate_sim => //.
 ...
 + by rewrite /RPreFeq /mk_fstate => > [<- <- _] <-.
 + by apply hrec.
 move=> fs1 fs2 fr _ _ <-.
 by apply requiv_sim_upd.
Qed.
...
#[local] Existing Instance SC_nosubword_withsubword.
