...
Definition ext_rhandler {E1 E2} (h: E1 ~> itree (E1 +' E2)) :
  (E1 +' E2) ~> itree (E1 +' E2) :=
  fun T e => match e with
             | inl1 e1 => h _ e1
             | inr1 e2 => trigger e2 end.               

(* mrec (fun d => interp ext_body (body d)) d = mrec body d *)

Lemma xxx {D E: Type -> Type} (h: D ~> itree (D +' E)) T
  (d: D T) :
  let int_x2h := fun V x => interp (ext_rhandler h) (h V x) in    
  eutt eq (mrec int_x2h d) (mrec h d). 
Proof.
  simpl.
  unfold mrec.
  rewrite interp_mrec_as_interp.
...
Proof.
  ...
  mrec (fun (V : Type) (x : D V) => interp (ext_rhandler h) (h V x)) d
  ≈ mrec h d

goal 2 (ID 509) is:
 interp (ext_rhandler h) (h T1 d) ≈ h T1 d
  ...
  Lemma xxx {D E: Type -> Type
    } T (h: D ~> itree (D +' E))
  (d: D T) :
  let xh := ext_rhandler h in 
  let int_x2h := fun V x => interp xh (h V x) in    
  eutt eq (mrec int_x2h d) (mrec h d). 

  
  
  eutt eq
    (mrec (fun x => interp (ext_handler hnd) (hnd x) d)
    (mrec hnd d).             
       (@D1_ext_interp D1 D1 E ctxI _ t))
    (interp_mrec ctxR t).              




Lemma xxx {D E: Type -> Type} T (hnd: D ~> itree (D +' E))
  (t: itree (D +' E) T) :
  eutt eq
    (interp_mrec (fun _ x => interp (ext_handler hnd) (hnd _ x)) t)
    (interp_mrec hnd t).             
       (@D1_ext_interp D1 D1 E ctxI _ t))
    (interp_mrec ctxR t).              
...
Lemma xxx T (t: itree ((D1 +' D1) +' E) T) :
  eutt eq
    (interp_mrec (@D1_ext_interp D1 D1 E ctxI)
       (@D1_ext_interp D1 D1 E ctxI _ t))
    (interp_mrec ctxR t).              
