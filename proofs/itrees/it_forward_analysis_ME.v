...
Definition Iter_state : Iter (Kleisli identity) sum ->
       Iter (Kleisli (stateT estate identity)) sum
  := @Iter_stateTM identity estate Monad_identity.

Lemma xxx : True -> Iter (Kleisli identity) sum.
  intro.
  unfold Kleisli.
...
Context (fff : forall a b : Type, (a -> a + b) -> a -> b).   
  
Definition ggg := Iter_state fff.
Check @ggg.

Lemma xxx1 : True -> Iter (Kleisli (stateT estate identity)) sum.
  intro.
  unfold Kleisli.
  unfold Iter.
Admitted. 
  
Definition hhh : list nat -> list nat :=
  fix rr ls := match ls with
               | nil => nil | x :: xs => 1 :: (rr xs) end.

(* Print (Iter _ (Kleisli identity) sum). *)
Check (Kleisli identity).

Check (Iter_stateTM identity estate unit).

Print Iter_stateTM.

Print Kleisli.
(* Kleisli =
fun (m : Type -> Type) (a b : Type) => a -> m b
     : (Type -> Type) -> Type -> Type -> Type
*)

Check @CategoryOps.iter.
*)

Fixpoint anls_loop {I}
