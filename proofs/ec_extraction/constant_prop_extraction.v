...
Definition full_init len := nseq (Pos.to_nat len) e255.
...
Definition const v :=
  ...
  | Cint z  => Pconst z
  | Cword sz z => wconst z
  | Cfull_init len => PappN (Oarray len) (full_init len)
  end.

Definition is_full_init len e :=
  if e is PappN (Oarray len') es then
    (len == len') && all (eq_expr e255) es
  else false.
...
Definition op2_add_cpm (m:cpm) (e1:pexpr) (e2:pexpr) :=
  if e1 is Pvar {| gv := x; gs := Slocal |} then
    ...
    | PappN (Oarray len) es =>
      if is_full_init len e2 then Some (Mvar.set m x (Cfull_init len))
      else None
    | _ => None
    end
  else None.
  ...
  if rv is Lvar x then
    if cpf rv tag e then
      ...
      | Papp1 (Oword_of_int _) (Pconst z) =>
        ...
        Mvar.set m x w
      | PappN (Oarray len) es =>
          if is_full_init len e then Mvar.set m x (Cfull_init len)
          else m
      | _ => m
      end
    else m
