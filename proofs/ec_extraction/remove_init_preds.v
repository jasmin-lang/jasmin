...
Definition assign_arr_bvar (ii:instr_info) (lv: lval) (t:atype) (e:pexpr)  : cmd :=
  ...
  | aarr ws len =>
    ...
    | Pvar x =>
      let e :=
        if is_glob x then const (Cfull_init (Z.to_pos (type.arr_size ws len)))
        else
          let x := x.(gv) in
...
Definition assign_bvar_syscall (lvs:seq lval) ws (l:positive) (ii:instr_info) : cmd :=
  ...
  | [::Lvar x] =>
    if not_array x.(v_var) then
      ...
      let x := Lvar (var_i_to_bvar x) in
      let e := const (Cfull_init (Z.to_pos (type.arr_size ws l))) in
  ...
  conc_map (fun v =>
    ...
    | Ok sz =>
      if (Sv.mem v args_varsL) then [::]
        ...
        let e:= PappN (Oarray sz) (nseq (Pos.to_nat sz) (word_of_int Unsigned U8 0)) in
