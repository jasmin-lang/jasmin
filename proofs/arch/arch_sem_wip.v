...
Class asm_syscall_sem := {
  get_syscall_ans : syscall_t -> asmmem ->
                    (syscall_state_t * seq u8) ;
  put_syscall_ans : syscall_t -> syscall_state_t ->
                      seq u8 -> asmmem -> exec asmmem; 
    
  (* current memory, answer to rnd, gives us next memory *)
  eval_syscall : syscall_t -> asmmem -> exec asmmem :=
      fun o m => let: (scs', us) := (get_syscall_ans o m) in
        put_syscall_ans o scs' us m ;

  eval_syscall_spec :
    forall o s0 len,
      let: params := take (size (syscall_sig_s o).(scs_tin))
                       call_reg_args in
      let: vargs := [seq Vword (s0.(asm_reg) r) | r <- params ] in
      let: xlen := exec_syscall_arg_s o vargs in
      xlen = ok len ->
      exists (scs': syscall_state_t) (us: seq u8) s1,
        get_syscall_ans o s0 = (scs', us) /\
        size us = Z.to_nat len /\
        exec_syscall_store_s o scs' s0.(asm_mem) vargs us = ok s1 /\
        exists2 s2, 
          put_syscall_ans o scs' us s0 = ok s2 &
          [/\ s2.(asm_scs) = fst (fst s1)
              & s2.(asm_mem) = snd (fst s1)]; 
  ...
  eval_syscall_preserves :
    forall o s1 s2,
      ...
      [/\ forall r, r \in callee_saved -> preserved_register r s1 s2
        ...
        (* TODO this last one is not really needed because 
           we have memory equality with source *)
...
Proof.
  ...
  - t_xrbindP => m hm <-.
    have /= [h1 h2 h3]:= eval_syscall_preserves hm; split => //.
  - by move=> [<-].
  - by move=> _ [<-].
Qed.
...
Definition get_syscall_args (s: asmmem) (o : syscall_t) :
  result error Z := 
  let vargs := map (fun r => Vword (s.(asm_reg) r))
                 (take (size (syscall_sig_s o).(scs_tin))
                    call_reg_args) in
  exec_syscall_arg_s o vargs.

Definition asm_exec_syscall (o : syscall_t) (s : asmmem) :
  itree E asmmem :=
  len <- err_result (fun e => (e, tt)) (get_syscall_args s o) ;;  
  scs_rb <- trigger (@Rnd syscall_state_t (s.(asm_scs)) len) ;;
  err_result (fun e => (e, tt))
          (put_syscall_ans o (fst scs_rb) (snd scs_rb) s).

Definition ifetch_and_eval (s: asm_state) : itree E asm_state :=
  if asm_next_is_syscall s is Some o then 
    m <- asm_exec_syscall o s.(asm_m) ;;
    Ret (AsmState m s.(asm_f) s.(asm_c) s.(asm_ip)) 
    (* fetch arguments, trigger Rnd, call eval_syscall, update ip *)
  else err_result (fun e => (e, tt)) (fetch_and_eval s).


(* TODO catch syscall here? *)
Definition ifetch_and_eval_OLD (s: asm_state) : itree E asm_state :=
...
Proof.
 ...
 (*   case: fetch_and_eval => [s' | ] /=. *)
    destruct (asm_next_is_syscall s) eqn: wasm.
    * rewrite bind_bind; simpl.
      setoid_rewrite bind_ret_l; simpl.
      unfold fetch_and_eval, asm_exec_syscall; simpl.
      rewrite bind_bind; simpl.
      admit.
  + simpl. unfold err_result.
    destruct (fetch_and_eval s); simpl; try reflexivity.
