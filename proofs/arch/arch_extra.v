...
Lemma atype_of_ltypeP env : eval_ltype =1 eval_atype env \o atype_of_ltype.
Proof. by case. Qed.
...
Definition semi_to_atype {tin tout} (semi: sem_prod (map eval_ltype tin) (exec (sem_tuple (map eval_ltype tout)))) :
    forall env, sem_prod (map (eval_atype env) (map atype_of_ltype tin)) (exec (sem_tuple (map (eval_atype env) (map atype_of_ltype tout)))) :=
  fun env =>
  let eq l := computational_eq (etrans (eq_map (atype_of_ltypeP env) _) (map_comp (eval_atype env) atype_of_ltype l)) in
  ecast l (sem_prod l _) (eq tin) (ecast l (sem_prod _ (exec (sem_tuple l))) (eq tout) semi).

Lemma is_not_carr_ltype (tin : seq ltype) env :
  all is_not_carr (map (eval_atype env) (map atype_of_ltype tin)).
Proof.
  elim: tin => [//|ty tys /= ->].
  by case: ty.
Qed.

Definition safe_cond_to_array_length := map_safe_cond ALConst.

Lemma sc_needed_args_map_safe_cond {A B} (f : A -> B) s :
  sc_needed_args (map_safe_cond f s) = sc_needed_args s.
Proof. by case: s. Qed.
...
Proof.
  rewrite size_map.
  elim: safe => [//|s safe ih] /= /andP [hleq /ih{}ih].
  apply /andP; split=> //.
  by rewrite sc_needed_args_map_safe_cond.
Qed.

Lemma semi_to_atype_errty tin tout (semi: sem_prod (map eval_ltype tin) (exec (sem_tuple (map eval_ltype tout)))) env :
  sem_forall (fun r => r <> Error ErrType) (map eval_ltype tin) semi ->
  sem_forall (fun r => r <> Error ErrType) (map (eval_atype env) (map atype_of_ltype tin)) (semi_to_atype semi env).
Proof.
...
Lemma map_safe_cond_to_array_lengthP env vs sc :
  List.Forall (interp_safe_cond vs) (map (eval_safe_cond env) (map safe_cond_to_array_length sc)) ->
