...
Lemma write_i_call xs f als es :
  write_i (Ccall xs f als es) = vrvs xs.
Proof. done. Qed.
...
Lemma read_i_call xs f als es :
  Sv.Equal (read_i (Ccall xs f als es)) (Sv.union (read_rvs xs) (read_es es)).
...
Lemma vars_I_call ii xs fn als args:
  Sv.Equal (vars_I (MkI ii (Ccall xs fn als args))) (Sv.union (vars_lvals xs) (read_es args)).
...
Lemma Hrefl_call: forall xs f als es, Pr (Ccall xs f als es).
Proof. by move=> ????; rewrite /Pr /= !eqxx (all2_refl eq_lval_refl) (all2_refl eq_expr_refl). Qed.
...
Lemma Hsymm_call: forall xs f als es, Pr (Ccall xs f als es).
Proof.
...
Lemma Htrans_call: forall xs f als es, Pr (Ccall xs f als es).
Proof.
  move=> ???? [] //= ???? [] //= ????
...
Lemma Hwrite_call: forall xs f als es, Pr (Ccall xs f als es).
Proof.
