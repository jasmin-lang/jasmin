...
Proof.
  ...
  + move=> h; split => //.
    apply: undef_addr_eq.
    move: h => /(subctype_trans (undef_t_subtype t)) /subtype_undef_tP <-.
...
Lemma vm_truncate_val_subtype_word v ws:
  DB true v ->
  subctype (sword ws) (type_of_val v) ->
...
Proof.
  rewrite /compat_ctype /compat_type_gen; case: ifP => hwsw; last by move=> /type.toto ->; eauto.
  move=> hsub htr.
...
Proof.
  case: (boolP sw_allowed) => /=; last by move=> _ /type.toto ->; apply value_uincl_vm_truncate.
  move=> hsw.
...
Lemma truncatable_subtype (wdb : bool) ty v1 v2 :
  (wdb -> ~sw_allowed -> is_sword ty -> ~is_defined v1 -> subctype ty (type_of_val v2)) ->
...
Proof.
  ...
  + move=> /eqP/eqP ? /negP/negbTE hsw hword hndef; subst wdb.
    apply: subctype_trans (value_uincl_subtype hu).
...
Module Type VM.
  ...
  Parameter initP : forall {wsw:WithSubWord} x,
    get init x = undef_addr (to_ctype (vtype x)).

  Parameter getP : forall {wsw:WithSubWord} vm x,
    compat_val (to_ctype (vtype x)) (get vm x).

  Parameter setP : forall {wsw:WithSubWord} vm x v y,
    get (set vm x v) y = if x == y then vm_truncate_val (to_ctype (vtype x)) v else get vm y.

  Parameter setP_eq : forall {wsw:WithSubWord} vm x v, get (set vm x v) x = vm_truncate_val (to_ctype (vtype x)) v.
...
Module Vm : VM.
  ...
  Definition wf (data: Mvar.t value) :=
    forall x v, Mvar.get data x = Some v -> compat_val (to_ctype (vtype x)) v.
  ...
  Definition get (vm:t) (x:var) := odflt (undef_addr (to_ctype (vtype x))) (Mvar.get vm x).

  Lemma set_prop (vm:t) x v : wf (Mvar.set vm x (vm_truncate_val (to_ctype (vtype x)) v)).
  Proof.
  ...
  Definition set (vm:t) (x:var) v :=
    {| data := Mvar.set vm x (vm_truncate_val (to_ctype (vtype x)) v); prop := @set_prop vm x v |}.

  Lemma initP x : get init x = undef_addr (to_ctype (vtype x)).
  Proof. done. Qed.

  Lemma getP vm x : compat_val (to_ctype (vtype x)) (get vm x).
  ...
  Lemma setP_eq vm x v : get (set vm x v) x = vm_truncate_val (to_ctype (vtype x)) v.
...
Lemma vm_truncate_val_get x vm :
  vm_truncate_val (to_ctype (vtype x)) vm.[x] = vm.[x].
Proof. apply/compat_val_vm_truncate_val/Vm.getP. Qed.

Lemma getP_subtype vm x : subctype (type_of_val vm.[x]) (to_ctype (vtype x)).
Proof. apply/compat_ctype_subctype/Vm.getP. Qed.

Lemma subtype_undef_get vm x :
  subctype (undef_t (to_ctype (vtype x))) (type_of_val vm.[x]).
Proof.
...
Definition set_var wdb vm x v :=
  ...
  Let _ := assert (truncatable wdb (to_ctype (vtype x)) v) ErrType in
  ok vm.[x <- v].
...
Definition vm_initialized_on vm : seq var → Prop :=
  all (λ x, is_ok (get_var true vm x >>= of_val (to_ctype (vtype x)))).

Lemma set_varP wdb vm x v vm' :
  set_var wdb vm x v = ok vm' <-> [/\ DB wdb v, truncatable wdb (to_ctype (vtype x)) v & vm' = vm.[x <- v]].
Proof. by rewrite /set_var; split => [ | [-> -> -> //]]; t_xrbindP. Qed.

Lemma set_var_truncate wdb x v :
  DB wdb v -> truncatable wdb (to_ctype (vtype x)) v ->
  forall vm, set_var wdb vm x v = ok vm.[x <- v].
Proof. by rewrite /set_var => -> ->. Qed.

Lemma set_var_eq_type wdb x v:
  DB wdb v -> type_of_val v = to_ctype (vtype x) ->
  forall vm, set_var wdb vm x v = ok vm.[x <- v].
...
Lemma get_varI vm x v : get_var true vm x = ok v ->
  match v with
  | Vbool _ => to_ctype (vtype x) = sbool
  | Vint _ => to_ctype (vtype x) = sint
  | Varr len _ => to_ctype (vtype x) = sarr len
  | Vword ws _ =>
    exists2 ws', to_ctype (vtype x) = sword ws' &
     if sw_allowed then ((ws <= ws')%CMP:Prop) else ws = ws'
  | Vundef ty' _ => False
  end.
...
Proof.
  by move=> /get_var_compat [] h1 /compat_valEl h2; case:vtype h2 h1 => [ | | [ws len] | ws] // [->|].
Qed.

Lemma type_of_get_var wdb x vm v :
  get_var wdb vm x = ok v ->
  subctype (type_of_val v) (to_ctype x.(vtype)).
Proof.
  by move=> /get_var_compat [] _; rewrite /compat_val /compat_ctype /compat_type_gen; case: ifP => // _ /type.toto <-.
Qed.
...
Lemma type_of_get_var_not_word vm x v :
  ...
  type_of_val v = to_ctype x.(vtype).
Proof.
  move=> h /get_var_compat [] /= hdb; rewrite /compat_val /compat_ctype /compat_type_gen hdb orbF.
  case: ifP => //; last by move=> _ /type.toto.
  by move=> /h; case: vtype => //= [||len] _ /subctypeE.
Qed.

Lemma get_word_uincl_eq vm x ws (w:word ws) :
  value_uincl (Vword w) vm.[x] ->
  subctype (to_ctype (vtype x)) (sword ws) ->
  vm.[x] = Vword w.
Proof.
...
Section REL.
  ...
  Lemma vm_rel_set (P : var -> Prop) vm1 vm2 x v1 v2 :
    (P x -> R (vm_truncate_val (wsw:=wsw1) (to_ctype (vtype x)) v1) (vm_truncate_val (wsw:=wsw2) (to_ctype (vtype x)) v2)) ->
  ...
  Lemma vm_rel_set_r (P : var -> Prop) vm1 vm2 x v2 :
    (P x -> R vm1.[x] (vm_truncate_val (wsw:=wsw2) (to_ctype (vtype x)) v2)) ->
    vm_rel (fun z => x <> z /\ P z) vm1 vm2 ->
    vm_rel P vm1 (vm2.[x <- v2]).
  Proof. move=> h hu y hy; rewrite !Vm.setP; case: eqP => heq; subst; auto. Qed.

  Lemma vm_rel_set_l (P : var -> Prop) vm1 vm2 x v1 :
    (P x -> R (vm_truncate_val (wsw:=wsw1) (to_ctype (vtype x)) v1) vm2.[x]) ->
    vm_rel (fun z => x <> z /\ P z) vm1 vm2 ->
...
Section REL_EQUIV.
  ...
  Lemma uincl_on_set X vm1 vm2 x v1 v2:
    (Sv.In x X -> value_uincl (vm_truncate_val (to_ctype (vtype x)) v1) (vm_truncate_val (to_ctype (vtype x)) v2)) ->
  ...
  Lemma uincl_on_set_l X vm1 vm2 x v :
    (Sv.In x X -> value_uincl (vm_truncate_val (to_ctype (vtype x)) v) vm2.[x]) ->
    vm1 <=[Sv.remove x X] vm2 ->
    vm1.[x <- v] <=[X] vm2.
  Proof. move=> hvu hu; apply vm_rel_set_l => //; apply: vm_relI hu; SvD.fsetdec. Qed.

  Lemma uincl_on_set_r X vm1 vm2 x v :
    (Sv.In x X ->value_uincl vm1.[x] (vm_truncate_val (to_ctype (vtype x)) v)) ->
    vm1 <=[Sv.remove x X] vm2 ->
    vm1 <=[X] vm2.[x <- v].
  ...
  Lemma eq_ex_set s vm1 vm2 x v1 v2 :
    (~Sv.In x s -> vm_truncate_val (to_ctype (vtype x)) v1 = vm_truncate_val (to_ctype (vtype x)) v2) ->
    vm1 =[\Sv.add x s] vm2 ->
    vm1.[x<-v1] =[\ s] vm2.[x<-v2].
  Proof. move=> h1 h2; apply vm_rel_set => // z hz; apply h2; SvD.fsetdec. Qed.

  Lemma eq_ex_set_r s vm1 vm2 x v :
    (~Sv.In x s -> vm1.[x] = vm_truncate_val (to_ctype (vtype x)) v) ->
    vm1 =[\Sv.add x s] vm2 ->
    vm1 =[\ s] vm2.[x<-v].
  Proof. move=> h1 h2; apply vm_rel_set_r => // z hz; apply h2; SvD.fsetdec. Qed.

  Lemma eq_ex_set_l s vm1 vm2 x v :
    (~Sv.In x s -> vm_truncate_val (to_ctype (vtype x)) v = vm2.[x]) ->
    vm1 =[\Sv.add x s] vm2 ->
    vm1.[x<-v] =[\ s] vm2.
  Proof. move=> h1 h2; apply vm_rel_set_l => // z hz; apply h2; SvD.fsetdec. Qed.

  Lemma uincl_ex_set s vm1 vm2 x v1 v2 :
    (~Sv.In x s -> value_uincl (vm_truncate_val (to_ctype (vtype x)) v1) (vm_truncate_val (to_ctype (vtype x)) v2)) ->
    vm1 <=[\Sv.add x s] vm2 ->
    vm1.[x<-v1] <=[\ s] vm2.[x<-v2].
  Proof. move=> h1 h2; apply vm_rel_set => // z hz; apply h2; SvD.fsetdec. Qed.

  Lemma uincl_ex_set_r s vm1 vm2 x v :
    (~Sv.In x s -> value_uincl vm1.[x] (vm_truncate_val (to_ctype (vtype x)) v)) ->
    vm1 <=[\Sv.add x s] vm2 ->
    vm1 <=[\ s] vm2.[x<-v].
  Proof. move=> h1 h2; apply vm_rel_set_r => // z hz; apply h2; SvD.fsetdec. Qed.

  Lemma uincl_ex_set_l s vm1 vm2 x v :
    (~Sv.In x s -> value_uincl (vm_truncate_val (to_ctype (vtype x)) v) vm2.[x]) ->
    vm1 <=[\Sv.add x s] vm2 ->
    vm1.[x<-v] <=[\ s] vm2.
  Proof. move=> h1 h2; apply vm_rel_set_l => // z hz; apply h2; SvD.fsetdec. Qed.
