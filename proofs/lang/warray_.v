...
Require Import xseq.
Require Export utils gen_map word memory_model.
...
#[global]
  ...
  Proof.
    ...
    + move=> /(_ _ _ hfold) ->.
      ...
      rewrite hsize.
      case: len {a hfold hsize} => [|len|len].
      + by move=> /=; case: andb.
      + rewrite Z2Nat.id //.
        by case: andb.
      case: ifPn; rewrite !zify; first by lia.
      case: ifPn; rewrite !zify; first by lia.
      done.
  ...
  Proof.
    ...
    have [hneg|hpos] := Z.le_gt_cases (arr_size ws len) 0.
    + rewrite ziota_neg //=.
      case: ifP => //.
      rewrite !zify.
      by lia.
    elim /natlike_ind: (arr_size ws len) a;
      last by apply: Z.lt_le_incl hpos.
