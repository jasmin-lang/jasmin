...
Section CoreMem.
 ...
 Qed.
  ...
  Lemma read_total_al m al al' p sz :
    read (sm := total) m al p sz = read (sm := total) m al' p sz.
  Proof. by rewrite !readE_total. Qed.
  ...
  Lemma write_read8_total m al p sz (w : word sz) m' p' :
    write (sm := total) m al p w = ok m' ->
    get (sm := total) m' p' =
      if (0 <=? sub p' p) && (sub p' p <? wsize_size sz) && set_dom m p'
      then ok (nth 0%w (LE.encode w) (Z.to_nat (sub p' p)))
      else get (sm := total) m p'.
  Proof.
    rewrite writeE_total -(@in_ziota 0 (wsize_size sz)).
    move: m; apply ziota_ind => /= [m [<-] // | i l hi hrec m].
    t_xrbindP => mi hset hf.
    rewrite (hrec _ hf) (set_dom_set _ hset) (setP_total _ hset) in_cons.
    have hi256 : 0 <= i < wsize_size U256.
    + have : wsize_size sz <= wsize_size U256 by case: (sz).
      by move: hi; lia.
    case hin: (sub p' p \in l) => /=.
    + rewrite orbT andTb; case hd: (set_dom m p') => //.
      case: eqP => [heq | _] //.
      by rewrite heq hd andbF.
    rewrite orbF; case: eqP => [heq | hne].
    + by rewrite -heq sub_add // eqxx.
    by case: eqP => // heq; case: hne; rewrite -heq add_sub.
  Qed.
...
(* On a memory, [set] in the total mode always writes. *)
Lemma setP_total_mem {mem} {CM: coreMem pointer mem} {M: memory CM} m p w p' m' :
  set (sm := total) m p w = ok m' ->
  get (sm := total) m' p' = if p == p' then ok w else get (sm := total) m p'.
Proof. by move=> h; rewrite (setP_total _ h) set_dom_mem andbT. Qed.
