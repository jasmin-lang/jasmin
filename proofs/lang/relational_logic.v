...
Context {wc1 wc2 : WithCatch}.

(* Every semantic function depends on the [WithCatch] instance, so each side of
   a relational judgement has to name its own. *)
Notation sem_pexpr1   := (sem_pexpr (wc:=wc1)).
Notation sem_pexpr2   := (sem_pexpr (wc:=wc2)).
Notation sem_pexprs1  := (sem_pexprs (wc:=wc1)).
Notation sem_pexprs2  := (sem_pexprs (wc:=wc2)).
Notation sem_cond1    := (sem_cond (wc:=wc1)).
Notation sem_cond2    := (sem_cond (wc:=wc2)).
Notation sem_bound1   := (sem_bound (wc:=wc1)).
Notation sem_bound2   := (sem_bound (wc:=wc2)).
Notation write_lval1  := (write_lval (wc:=wc1)).
Notation write_lval2  := (write_lval (wc:=wc2)).
Notation write_lvals1 := (write_lvals (wc:=wc1)).
Notation write_lvals2 := (write_lvals (wc:=wc2)).
Notation upd_estate1  := (upd_estate (wc:=wc1)).
Notation upd_estate2  := (upd_estate (wc:=wc2)).
Notation sem_eassert1 := (sem_eassert (wc:=wc1)).
Notation sem_eassert2 := (sem_eassert (wc:=wc2)).
Notation sem_assert1  := (sem_assert (wc:=wc1) (wa:=wa1)).
Notation sem_assert2  := (sem_assert (wc:=wc2) (wa:=wa2)).
Notation exec_sopn1   := (exec_sopn (wc:=wc1)).
Notation exec_sopn2   := (exec_sopn (wc:=wc2)).
Notation sem_sopn1    := (sem_sopn (wc:=wc1)).
Notation sem_sopn2    := (sem_sopn (wc:=wc2)).
Notation sem_assgn1   := (sem_assgn (wc:=wc1)).
Notation sem_assgn2   := (sem_assgn (wc:=wc2)).
Notation sem_syscall1 := (sem_syscall (wc:=wc1)).
Notation sem_syscall2 := (sem_syscall (wc:=wc2)).
Notation isem_fun1 := (isem_fun (wsw:=wsw1) (dc:=dc1) (ep:=ep) (spp:=spp) (wa:=wa1) (wc:=wc1) (sip:=sip) (pT:=pT1) (scP:= scP1)).
Notation isem_fun2 := (isem_fun (wsw:=wsw2) (dc:=dc2) (ep:=ep) (spp:=spp) (wa:=wa2) (wc:=wc2) (sip:=sip) (pT:=pT2) (scP:= scP2)).
...
Definition wequiv_f_body (P : relPreF) (fn1 fn2 : funname) (Q:relPostF) :=
  wkequiv_io
    (P fn1 fn2)
      (isem_fun_body (wa:=wa1) (wc:=wc1) (dc:=dc1) (sem_F:=sem_F1) p1 ev1 fn1)
      (isem_fun_body (wa:=wa2) (wc:=wc2) (dc:=dc2) (sem_F:=sem_F2) p2 ev2 fn2)
    (Q fn1 fn2).

Definition wequiv (pre:rel_c) (c1 c2 : cmd) (post : rel_c) :=
  wkequiv
    pre
     (isem_cmd_ (wa:=wa1) (wc:=wc1) (dc:=dc1) (sem_F:=sem_F1) p1 ev1 c1)
     (isem_cmd_ (wa:=wa2) (wc:=wc2) (dc:=dc2) (sem_F:=sem_F2) p2 ev2 c2)
    post.

Notation sem_pre1 := (sem_pre (wsw:=wsw1) (wa:=wa1) (wc:=wc1) (dc:=dc1) (pT:=pT1)).
Notation sem_pre2 := (sem_pre (wsw:=wsw2) (wa:=wa2) (wc:=wc2) (dc:=dc2) (pT:=pT2)).
Notation sem_post1 := (sem_post (wsw:=wsw1) (wa:=wa1) (wc:=wc1) (dc:=dc1) (pT:=pT1)).
Notation sem_post2 := (sem_post (wsw:=wsw2) (wa:=wa2) (wc:=wc2) (dc:=dc2) (pT:=pT2)).
...
Lemma wequiv_assgn_core (P Q : rel_c) ii1 x1 tg1 ty1 e1 ii2 x2 tg2 ty2 e2 :
  wrequiv P (sem_assgn1 p1 x1 tg1 ty1 e1) (sem_assgn2 p2 x2 tg2 ty2 e2) Q ->
...
Lemma wequiv_assgn_esem (P Q : rel_c) ii1 x1 tg1 ty e1 c2 :
  wrequiv P (sem_assgn1 p1 x1 tg1 ty e1)
            (esem p2 ev2 c2) Q ->
...
Lemma wequiv_opn (Rve Rvo : rel_vs) P Q ii1 xs1 at1 o1 es1 ii2 xs2 at2 o2 es2 :
  ...
  (forall s1 s2, P s1 s2 -> wrequiv Rve (exec_sopn1 o1) (exec_sopn2 o2) Rvo) ->
  (forall vs1 vs2,
...
(* [exec_sopn] is not the same function on both sides as soon as the [WithCatch]
   instances differ, hence the [SameCatch] requirement (silently inferred
   whenever the two sides share an instance, i.e. in every compiler pass). *)
Lemma wequiv_opn_eq {_ : SameCatch wc1 wc2} P Q ii1 xs1 at1 o es1 ii2 xs2 at2 es2 :
  wrequiv P (fun s => sem_pexprs1 true (p_globs p1) s es1)
...
Proof.
  ...
  + by move=> *; rewrite (same_catch (wc1:=wc1)); apply wrequiv_eq.
  by move=> > <-; apply hx.
Qed.

(* [wrequiv_exec_sopn] only holds at [nocatch] (see [IsNoCatch]). *)
Lemma wequiv_opn_uincl {_ : IsNoCatch wc1} {_ : IsNoCatch wc2} P Q ii1 xs1 at1 o es1 ii2 xs2 at2 es2 :
  wrequiv P (fun s => sem_pexprs1 true (p_globs p1) s es1)
           (fun s => sem_pexprs2 true (p_globs p2) s es2) values_uincl ->
  (forall vs1 vs2,
...
Proof.
  ...
  move=> *; rewrite (is_nocatch (wc:=wc1)) (is_nocatch (wc:=wc2)).
  apply wrequiv_exec_sopn.
Qed.

Lemma wequiv_opn_esem (P Q : rel_c) ii1 xs1 tg1 o1 es1 c2 :
  wrequiv P (fun s => sem_sopn1 (p_globs p1) o1 s xs1 es1)
            (esem p2 ev2 c2) Q ->
...
Lemma wequiv_syscall_esem (P Q : rel_c) ii1 xs1 sc1 es1 c2 :
  wrequiv P (sem_syscall1 p1 xs1 sc1 es1)
            (esem p2 ev2 c2) Q ->
...
Lemma wequiv_assert_esem (P Q : rel_c) ii1 a1 c2 :
  wrequiv P (fun (s:estate1) => Let _ := sem_assert (wsw:=wsw1) (wa:=wa1) (wc:=wc1) (p_globs p1) s a1 in ok s)
            (esem (wa:=wa2) (wc:=wc2) p2 ev2 c2) Q ->
...
Lemma wequiv_assert (P Q : rel_c) ii1 a1 ii2 a2 :
  (assert_allowed (WithAssert:=wa1) ->
   ...
   forall s1 s2,
     P s1 s2 ->
     sem_eassert1 (p_globs p1) s1 a1.2 = ok true ->
     sem_eassert2 (p_globs p2) s2 a2.2 = ok true /\ Q s1 s2) ->
...
Lemma wequiv_assert_eq (P Q : rel_c) ii1 a1 ii2 a2 :
  (assert_allowed (WithAssert:=wa1) ->
     assert_allowed (WithAssert:=wa2) /\
     wrequiv P (fun s => sem_eassert1 (p_globs p1) s a1.2)
               (fun s => sem_eassert2 (p_globs p2) s a2.2) eq) ->
  (assert_allowed (WithAssert:=wa1) ->
   assert_allowed (WithAssert:=wa2) ->
   forall s1 s2,
     P s1 s2 ->
     sem_eassert1 (p_globs p1) s1 a1.2 = ok true ->
     sem_eassert2 (p_globs p2) s2 a2.2 = ok true ->
     Q s1 s2) ->
  wequiv P [:: MkI ii1 (Cassert a1)] [:: MkI ii2 (Cassert a2)] Q.
Proof.
...
Lemma upd_st_rel wdb1 wdb2 gd1 gd2 d d' xs1 xs2 :
  (forall vs1 vs2, Rv vs1 vs2 ->
     wrequiv
       (st_rel d)
       (λ s1, write_lvals1 wdb1 gd1 s1 xs1 vs1) (λ s2, write_lvals2 wdb2 gd2 s2 xs2 vs2)
       (st_rel d')) ->
  (forall fs1 fs2, fs_rel fs1 fs2 ->
     wrequiv
       (st_rel d)
       (upd_estate1 wdb1 gd1 xs1 fs1) (upd_estate2 wdb2 gd2 xs2 fs2)
       (st_rel d')).
...
Lemma upd_st_eq wdb1 wdb2 gd1 gd2 d d' xs1 xs2 :
  (forall vs,
     wrequiv
       (st_rel d)
       (λ s1, write_lvals1 wdb1 gd1 s1 xs1 vs) (λ s2, write_lvals2 wdb2 gd2 s2 xs2 vs)
       (st_rel d')) ->
  (forall fs,
     wrequiv
       (st_rel d)
       (upd_estate1 wdb1 gd1 xs1 fs) (upd_estate2 wdb2 gd2 xs2 fs)
       (st_rel d')).
...
Lemma wrequiv_sem_bound (P : rel_c) lo1 hi1 lo2 hi2 :
  wrequiv P (fun s => sem_pexprs1 true (p_globs p1) s [::lo1; hi1])
            (fun s => sem_pexprs2 true (p_globs p2) s [::lo2; hi2]) values_uincl ->
...
Lemma wequiv_assign_right P Q ii x tg ty e :
  (forall s t, P s t -> exists2 t', sem_assgn2 p2 x tg ty e t = ok t' & Q s t') ->
...
Lemma wequiv_assign_left P Q ii x tg ty e :
  (forall s s' t, P s t -> sem_assgn1 p1 x tg ty e s = ok s' ->  Q s' t) ->
  wequiv P [::MkI ii (Cassgn x tg ty e)] [::] Q.
Proof.
  move=> h; rewrite /wequiv /=.
...
Lemma wequiv_assert_left P Q ii a :
  (assert_allowed (WithAssert:=wa1) ->
   forall s t, P s t -> sem_eassert1 (p_globs p1) s a.2 = ok true -> Q s t) ->
...
Lemma wequiv_asserts_left P Q ii (a_s : list assertion) :
  assert_allowed (WithAssert:=wa1) ->
  (forall s t, P s t ->
     (forall a, List.In a a_s -> sem_eassert1 (p_globs p1) s a.2 = ok true) -> Q s t) ->
  wequiv P [seq MkI ii (Cassert a) | a <- a_s] [::] Q.
Proof.
  rewrite -{1}(app_nil_l a_s) => hwa1 hPQ.
  apply wequiv_weaken with
    (fun s t => P s t /\
       (forall a, List.In a (@nil assertion) -> sem_eassert1 (p_globs p1) s a.2 = ok true))
    Q => //.
  ...
  apply wequiv_cat with
    (fun s t => P s t /\
       (forall a, List.In a (a0::a_s0) -> sem_eassert1 (p_globs p1) s a.2 = ok true)).
  + by apply wequiv_assert_left => _ s t [hP ha_s0] ha0; split => //= a [<- // | hin]; apply ha_s0.
  apply hrec => s t hP h; apply hPQ => // a /in_app_iff /= [hin|[<-|hin]];
...
Class Checker_a_eq :=
 { echeck_aP   :
   forall d a1 a2 d',
     check_a d a1 a2 d' ->
     wrequiv (R d) ((sem_eassert1 (p_globs p1))^~ a1) ((sem_eassert2 (p_globs p2))^~ a2) eq
 }.
...
Lemma wequiv_opn_rel_uincl {_ : IsNoCatch wc1} {_ : IsNoCatch wc2} d de d' ii1 xs1 tg1 o es1  ii2 xs2 tg2 es2 :
...
Lemma wequiv_opn_rel_eq {_ : SameCatch wc1 wc2} d de d' ii1 xs1 tg1 o es1 ii2 xs2 tg2 es2 :
  check_es d es1 es2 de →
  check_lvals de xs1 xs2 d' →
  wequiv (R d) [:: MkI ii1 (Copn xs1 tg1 o es1)] [:: MkI ii2 (Copn xs2 tg2 o es2)] (R d').
Proof using cu.
...
Lemma whoare_wequiv1 (P Q : rel_c) (P1 Q1 : Pred_c (wsw:=wsw1)) c1 c2:
  ...
  hoare (wsw:=wsw1) (wa:=wa1) (wc:=wc1) (dc:=dc1) (iEr := invErrT) p1 ev1 P1 c1 Q1 ->
...
Proof.
  ...
  have /(_ s1_) hw := [elaborate it_writeP (wa:=wa1) (wc:=wc1) (dc:=dc1) p1 ev1 c1 ].
...
Proof.
  ...
  have /(_ s2_) hw := [elaborate it_writeP (wa:=wa2) (wc:=wc2) (dc:=dc2) p2 ev2 c2 ].
...
Notation sem_fun_full1 := (sem_fun_full (wsw:=wsw1) (dc:=dc1) (ep:=ep) (spp:=spp) (wa:=wa1) (wc:=wc1) (sip:=sip) (pT:=pT1) (scP:= scP1)).
Notation sem_fun_full2 := (sem_fun_full (wsw:=wsw2) (dc:=dc2) (ep:=ep) (spp:=spp) (wa:=wa2) (wc:=wc2) (sip:=sip) (pT:=pT2) (scP:= scP2)).
...
Definition wequiv_fun_body_hyp_rec' (RPreF:relPreF) fn1 fn2 (RPostF:relPostF) :=
  ...
  forall fd1, get_fundef (p_funcs p1) fn1 = Some fd1 ->
  exists2 fd2, get_fundef (p_funcs p2) fn2 = Some fd2 &
    sem_pre (wsw:=wsw1) (wa:=wa1) (wc:=wc1) (dc:=dc1) (pT:=pT1) p1 fn1 fs1 = ok tt ->
    sem_pre (wsw:=wsw2) (wa:=wa2) (wc:=wc2) (dc:=dc2) (pT:=pT2) p2 fn2 fs2 = ok tt /\
    forall s11, initialize_funcall (dc:=dc1) p1 ev1 fd1 fs1 = ok s11 ->
    exists2 s21,
      initialize_funcall (dc:=dc2) p2 ev2 fd2 fs2 = ok s21 &
      exists (P Q : rel_c) (tbody epilogue : cmd),
        [/\ P s11 s21
          ...
          , wrequiv Q (finalize_funcall (dc:=dc1) fd1) (finalize_funcall (dc:=dc2) fd2) (RPostF fn1 fn2 fs1 fs2)
          & forall fr1 fr2, RPostF fn1 fn2 fs1 fs2 fr1 fr2 ->
               sem_post (wsw:=wsw1) (wa:=wa1) (wc:=wc1) (dc:=dc1) (pT:=pT1) p1 fn1 fs1.(fvals) fr1 = ok () ->
               sem_post (wsw:=wsw2) (wa:=wa2) (wc:=wc2) (dc:=dc2) (pT:=pT2) p2 fn2 fs2.(fvals) fr2 = ok ()].
...
Notation isem_fun_def1 := (isem_fun_def (wsw:=wsw1) (wa:=wa1) (wc:=wc1) (dc:=dc1) (ep:=ep) (spp:=spp) (sip:=sip) (pT:=pT1) (scP:= scP1) (sem_F:=sem_F1)).

Notation isem_fun_def2 := (isem_fun_def (wsw:=wsw2) (wa:=wa2) (wc:=wc2) (dc:=dc2) (ep:=ep) (spp:=spp) (sip:=sip) (pT:=pT2) (scP:= scP2) (sem_F:=sem_F2)).
...
Lemma wequiv_fun_get fn1 fn2 Pf Qf :
  (forall fd1, get_fundef (p_funcs p1) fn1 = Some fd1 ->
    wiequiv_f (fun fn1 fn2 fs1 fs2 =>
      [/\ Pf fn1 fn2 fs1 fs2
        , sem_pre (dc:=dc1) (wsw:=wsw1) (wa:=wa1) (wc:=wc1) p1 fn1 fs1 = ok tt
...
Proof.
  ...
  + rewrite /= bind_ret_l /isem_pre.
    ...
    + case heq1 : initialize_funcall => [s1 | e].
      + have hpre' :
          [/\ Pf fn1 fn2 fs1 fs2, sem_pre (dc:=dc1) (wsw:=wsw1) (wa:=wa1) (wc:=wc1) p1 fn1 fs1 = ok tt
            & exists s1, initialize_funcall (dc:=dc1) p1 ev1 fd1 fs1 = ok s1].
        + by split => //; exists s1.
...
Arguments Checker_eq {syscall_state} {ep spp} {asm_op} {sip pT1 pT2 wsw1 wsw2 dc1 dc2 wc1 wc2}
  _ _ {D} [R] ce.

Arguments Checker_a_eq {syscall_state} {ep spp} {asm_op} {sip pT1 pT2 wsw1 wsw2 wc1 wc2}
  _ _ {D} [R] ca.

Arguments Checker_uincl {syscall_state} {ep spp} {asm_op} {sip pT1 pT2 wsw1 wsw2 dc1 dc2 wc1 wc2}
  _ _ {D} [R] ce.
