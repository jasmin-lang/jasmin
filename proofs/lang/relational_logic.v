...
Context {sm1 sm2 : SemMode}.

(* Every semantic function depends on the [SemMode] instance, so each side of
   a relational judgement has to name its own. *)
Notation sem_pexpr1   := (sem_pexpr (sm:=sm1)).
Notation sem_pexpr2   := (sem_pexpr (sm:=sm2)).
Notation sem_pexprs1  := (sem_pexprs (sm:=sm1)).
Notation sem_pexprs2  := (sem_pexprs (sm:=sm2)).
Notation sem_cond1    := (sem_cond (sm:=sm1)).
Notation sem_cond2    := (sem_cond (sm:=sm2)).
Notation sem_bound1   := (sem_bound (sm:=sm1)).
Notation sem_bound2   := (sem_bound (sm:=sm2)).
Notation write_lval1  := (write_lval (sm:=sm1)).
Notation write_lval2  := (write_lval (sm:=sm2)).
Notation write_lvals1 := (write_lvals (sm:=sm1)).
Notation write_lvals2 := (write_lvals (sm:=sm2)).
Notation upd_estate1  := (upd_estate (sm:=sm1)).
Notation upd_estate2  := (upd_estate (sm:=sm2)).
Notation sem_eassert1 := (sem_eassert (sm:=sm1)).
Notation sem_eassert2 := (sem_eassert (sm:=sm2)).
Notation sem_assert1  := (sem_assert (sm:=sm1) (wa:=wa1)).
Notation sem_assert2  := (sem_assert (sm:=sm2) (wa:=wa2)).
Notation exec_sopn1   := (exec_sopn (sm:=sm1)).
Notation exec_sopn2   := (exec_sopn (sm:=sm2)).
Notation sem_sopn1    := (sem_sopn (sm:=sm1)).
Notation sem_sopn2    := (sem_sopn (sm:=sm2)).
Notation sem_assgn1   := (sem_assgn (sm:=sm1)).
Notation sem_assgn2   := (sem_assgn (sm:=sm2)).
Notation sem_syscall1 := (sem_syscall (sm:=sm1)).
Notation sem_syscall2 := (sem_syscall (sm:=sm2)).
Notation isem_fun1 := (isem_fun (wsw:=wsw1) (dc:=dc1) (ep:=ep) (spp:=spp) (wa:=wa1) (sm:=sm1) (sip:=sip) (pT:=pT1) (scP:= scP1)).
Notation isem_fun2 := (isem_fun (wsw:=wsw2) (dc:=dc2) (ep:=ep) (spp:=spp) (wa:=wa2) (sm:=sm2) (sip:=sip) (pT:=pT2) (scP:= scP2)).
...
Definition wequiv_f_body (P : relPreF) (fn1 fn2 : funname) (Q:relPostF) :=
  wkequiv_io
    (P fn1 fn2)
      (isem_fun_body (wa:=wa1) (sm:=sm1) (dc:=dc1) (sem_F:=sem_F1) p1 ev1 fn1)
      (isem_fun_body (wa:=wa2) (sm:=sm2) (dc:=dc2) (sem_F:=sem_F2) p2 ev2 fn2)
    (Q fn1 fn2).

Definition wequiv (pre:rel_c) (c1 c2 : cmd) (post : rel_c) :=
  wkequiv
    pre
     (isem_cmd_ (wa:=wa1) (sm:=sm1) (dc:=dc1) (sem_F:=sem_F1) p1 ev1 c1)
     (isem_cmd_ (wa:=wa2) (sm:=sm2) (dc:=dc2) (sem_F:=sem_F2) p2 ev2 c2)
    post.

Notation sem_pre1 := (sem_pre (wsw:=wsw1) (wa:=wa1) (sm:=sm1) (dc:=dc1) (pT:=pT1)).
Notation sem_pre2 := (sem_pre (wsw:=wsw2) (wa:=wa2) (sm:=sm2) (dc:=dc2) (pT:=pT2)).
Notation sem_post1 := (sem_post (wsw:=wsw1) (wa:=wa1) (sm:=sm1) (dc:=dc1) (pT:=pT1)).
Notation sem_post2 := (sem_post (wsw:=wsw2) (wa:=wa2) (sm:=sm2) (dc:=dc2) (pT:=pT2)).
...
(* [exec_sopn] is not the same function on both sides as soon as the [SemMode]
   instances differ, hence the [SameMode] requirement (silently inferred
   whenever the two sides share an instance, i.e. in every compiler pass). *)
Lemma wequiv_opn_eq {_ : SameMode sm1 sm2} P Q ii1 xs1 at1 o es1 ii2 xs2 at2 es2 :
  wrequiv P (fun s => sem_pexprs1 true (p_globs p1) s es1)
...
Proof.
  ...
  + by move=> *; rewrite (same_mode (sm1:=sm1)); apply wrequiv_eq.
  by move=> > <-; apply hx.
Qed.

(* [wrequiv_exec_sopn] only holds in the partial mode (see [IsPartial]). *)
Lemma wequiv_opn_uincl {_ : IsPartial sm1} {_ : IsPartial sm2} P Q ii1 xs1 at1 o es1 ii2 xs2 at2 es2 :
  wrequiv P (fun s => sem_pexprs1 true (p_globs p1) s es1)
...
Proof.
  ...
  move=> *; rewrite (is_partial (sm:=sm1)) (is_partial (sm:=sm2)).
...
Lemma wequiv_assert_esem (P Q : rel_c) ii1 a1 c2 :
  wrequiv P (fun (s:estate1) => Let _ := sem_assert (wsw:=wsw1) (wa:=wa1) (sm:=sm1) (p_globs p1) s a1 in ok s)
            (esem (wa:=wa2) (sm:=sm2) p2 ev2 c2) Q ->
...
Lemma wequiv_opn_rel_uincl {_ : IsPartial sm1} {_ : IsPartial sm2} d de d' ii1 xs1 tg1 o es1  ii2 xs2 tg2 es2 :
...
Lemma wequiv_opn_rel_eq {_ : SameMode sm1 sm2} d de d' ii1 xs1 tg1 o es1 ii2 xs2 tg2 es2 :
  check_es d es1 es2 de →
  check_lvals de xs1 xs2 d' →
  wequiv (R d) [:: MkI ii1 (Copn xs1 tg1 o es1)] [:: MkI ii2 (Copn xs2 tg2 o es2)] (R d').
Proof using cu.
...
Lemma whoare_wequiv1 (P Q : rel_c) (P1 Q1 : Pred_c (wsw:=wsw1)) c1 c2:
  ...
  hoare (wsw:=wsw1) (wa:=wa1) (sm:=sm1) (dc:=dc1) (iEr := invErrT) p1 ev1 P1 c1 Q1 ->
...
Proof.
  ...
  have /(_ s1_) hw := [elaborate it_writeP (wa:=wa1) (sm:=sm1) (dc:=dc1) p1 ev1 c1 ].
...
Proof.
  ...
  have /(_ s2_) hw := [elaborate it_writeP (wa:=wa2) (sm:=sm2) (dc:=dc2) p2 ev2 c2 ].
...
Notation sem_fun_full1 := (sem_fun_full (wsw:=wsw1) (dc:=dc1) (ep:=ep) (spp:=spp) (wa:=wa1) (sm:=sm1) (sip:=sip) (pT:=pT1) (scP:= scP1)).
Notation sem_fun_full2 := (sem_fun_full (wsw:=wsw2) (dc:=dc2) (ep:=ep) (spp:=spp) (wa:=wa2) (sm:=sm2) (sip:=sip) (pT:=pT2) (scP:= scP2)).
...
Definition wequiv_fun_body_hyp_rec' (RPreF:relPreF) fn1 fn2 (RPostF:relPostF) :=
  ...
  forall fd1, get_fundef (p_funcs p1) fn1 = Some fd1 ->
  exists2 fd2, get_fundef (p_funcs p2) fn2 = Some fd2 &
    sem_pre (wsw:=wsw1) (wa:=wa1) (sm:=sm1) (dc:=dc1) (pT:=pT1) p1 fn1 fs1 = ok tt ->
    sem_pre (wsw:=wsw2) (wa:=wa2) (sm:=sm2) (dc:=dc2) (pT:=pT2) p2 fn2 fs2 = ok tt /\
    forall s11, initialize_funcall (dc:=dc1) p1 ev1 fd1 fs1 = ok s11 ->
    exists2 s21,
      initialize_funcall (dc:=dc2) p2 ev2 fd2 fs2 = ok s21 &
      exists (P Q : rel_c) (tbody epilogue : cmd),
        [/\ P s11 s21
          ...
          , wrequiv Q (finalize_funcall (dc:=dc1) fd1) (finalize_funcall (dc:=dc2) fd2) (RPostF fn1 fn2 fs1 fs2)
          & forall fr1 fr2, RPostF fn1 fn2 fs1 fs2 fr1 fr2 ->
               sem_post (wsw:=wsw1) (wa:=wa1) (sm:=sm1) (dc:=dc1) (pT:=pT1) p1 fn1 fs1.(fvals) fr1 = ok () ->
               sem_post (wsw:=wsw2) (wa:=wa2) (sm:=sm2) (dc:=dc2) (pT:=pT2) p2 fn2 fs2.(fvals) fr2 = ok ()].
...
Notation isem_fun_def1 := (isem_fun_def (wsw:=wsw1) (wa:=wa1) (sm:=sm1) (dc:=dc1) (ep:=ep) (spp:=spp) (sip:=sip) (pT:=pT1) (scP:= scP1) (sem_F:=sem_F1)).

Notation isem_fun_def2 := (isem_fun_def (wsw:=wsw2) (wa:=wa2) (sm:=sm2) (dc:=dc2) (ep:=ep) (spp:=spp) (sip:=sip) (pT:=pT2) (scP:= scP2) (sem_F:=sem_F2)).
...
Lemma wequiv_fun_get fn1 fn2 Pf Qf :
  (forall fd1, get_fundef (p_funcs p1) fn1 = Some fd1 ->
    wiequiv_f (fun fn1 fn2 fs1 fs2 =>
      [/\ Pf fn1 fn2 fs1 fs2
        , sem_pre (dc:=dc1) (wsw:=wsw1) (wa:=wa1) (sm:=sm1) p1 fn1 fs1 = ok tt
...
Proof.
  ...
  + rewrite /= bind_ret_l /isem_pre.
    ...
    + case heq1 : initialize_funcall => [s1 | e].
      + have hpre' :
          [/\ Pf fn1 fn2 fs1 fs2, sem_pre (dc:=dc1) (wsw:=wsw1) (wa:=wa1) (sm:=sm1) p1 fn1 fs1 = ok tt
            & exists s1, initialize_funcall (dc:=dc1) p1 ev1 fd1 fs1 = ok s1].
        + by split => //; exists s1.
...
Arguments Checker_eq {syscall_state} {ep spp} {asm_op} {sip pT1 pT2 wsw1 wsw2 dc1 dc2 sm1 sm2}
  _ _ {D} [R] ce.

Arguments Checker_a_eq {syscall_state} {ep spp} {asm_op} {sip pT1 pT2 wsw1 wsw2 sm1 sm2}
  _ _ {D} [R] ca.

Arguments Checker_uincl {syscall_state} {ep spp} {asm_op} {sip pT1 pT2 wsw1 wsw2 dc1 dc2 sm1 sm2}
  _ _ {D} [R] ce.
