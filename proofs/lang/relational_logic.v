...
Lemma wrequiv_exec_sopn o vals :
  wrequiv values_uincl (exec_sopn o vals) (exec_sopn o vals) values_uincl.
...
Definition relPreF := funname -> funname -> seq Z -> seq Z -> fstate -> fstate -> Prop.
Definition relPostF := funname -> funname -> seq Z -> seq Z -> fstate -> fstate -> fstate -> fstate -> Prop.

Definition rel_e := rel pexpr pexpr.
Definition rel_v := rel value value.
Definition rel_vs := rel values values.
Definition rel_c env1 env2 := rel (estate1 env1) (estate2 env2).
...
Definition eq_spec : EquivSpec :=
  {| rpreF_ := fun (fn1 fn2 : funname) (vals1 vals2 : seq Z) (fs1 fs2 : fstate) => fn1 = fn2 /\ vals1 = vals2 /\ fs1 = fs2
   ; rpostF_ := fun (fn1 fn2 : funname) (vals1 vals2 : seq Z) (fs1 fs2 fr1 fr2: fstate) => fr1 = fr2 |}.
...
Lemma wkequiv_iresult_right env1 env2 (P : rel (estate1 env1) (estate2 env2)) (Q : rel (estate1 env1) (estate2 env2)) F2 :
...
Lemma wkequiv_iresult_left env1 env2 (P : rel (estate1 env1) (estate2 env2)) (Q : rel (estate1 env1) (estate2 env2)) F1 :
...
Definition wequiv_f_ii (P : relPreF) ii1 ii2 (fn1 fn2 : funname) (Q:relPostF) :=
  forall vals1 vals2,
  wkequiv_io
    (P fn1 fn2 vals1 vals2)
      (sem_fun1 p1 ev1 ii1 fn1 vals1)
      (sem_fun2 p2 ev2 ii2 fn2 vals2)
    (Q fn1 fn2 vals1 vals2).

Definition wequiv_f_body (P : relPreF) (fn1 fn2 : funname) (Q:relPostF) :=
  forall vals1 vals2,
  wkequiv_io
    (P fn1 fn2 vals1 vals2)
      (isem_fun_body (wa:=wa1) (dc:=dc1) (sem_F:=sem_F1) p1 ev1 fn1 vals1)
      (isem_fun_body (wa:=wa2) (dc:=dc2) (sem_F:=sem_F2) p2 ev2 fn2 vals2)
    (Q fn1 fn2 vals1 vals2).

Definition wequiv env1 env2 (pre:rel_c env1 env2) (c1 c2 : cmd) (post : rel_c env1 env2) :=
  wkequiv
    pre
...
Lemma wequiv_weaken env1 env2 (P1 P2 Q1 Q2 : rel_c env1 env2) c1 c2 :
...
Lemma wequiv_cat env1 env2 (R P Q : rel_c env1 env2) (c1 c1' c2 c2' : cmd) :
  wequiv P c1 c2 R ->
  wequiv R c1' c2' Q ->
...
Lemma wequiv_nil env1 env2 (P Q : rel_c env1 env2) : (forall i1 i2, P i1 i2 -> Q i1 i2) -> wequiv P [::] [::] Q.
Proof. apply wkequiv_ret. Qed.

Lemma wequiv_cons env1 env2 (R P Q : rel_c env1 env2) (i1 i2 : instr) (c1 c2 : cmd) :
  wequiv P [::i1] [::i2] R ->
  wequiv R c1 c2 Q ->
  wequiv P (i1 :: c1) (i2 :: c2) Q.
Proof. rewrite -(cat1s i1 c1) -(cat1s i2 c2); apply wequiv_cat. Qed.

Lemma wequiv_assgn_core env1 env2 (P Q : rel_c env1 env2) ii1 x1 tg1 ty1 e1 ii2 x2 tg2 ty2 e2 :
...
Lemma wequiv_assgn env1 env2 (Rv Rtr: rel_v) (P Q : rel_c env1 env2) ii1 x1 tg1 ty1 e1 ii2 x2 tg2 ty2 e2 :
  wrequiv P (fun s => sem_pexpr true (p_globs p1) s e1)
            (fun s => sem_pexpr true (p_globs p2) s e2) Rv ->
  (forall s1 s2, P s1 s2 -> wrequiv Rv (truncate_val (eval_atype env1 ty1)) (truncate_val (eval_atype env2 ty2)) Rtr) ->
...
Lemma wequiv_opn env1 env2 Rvals (Rve Rvo : rel_vs) (P Q : rel_c env1 env2) ii1 xs1 at1 o1 als1 es1 ii2 xs2 at2 o2 als2 es2 :
  wrequiv P (fun s => sem_pexprs true (p_globs p1) s es1)
            (fun s => sem_pexprs true (p_globs p2) s es2) Rve ->
  Rvals (map (eval env1) als1) (map (eval env2) als2) ->
  (forall s1 s2 vals1 vals2, P s1 s2 -> Rvals vals1 vals2 ->
    wrequiv Rve (exec_sopn o1 vals1) (exec_sopn o2 vals2) Rvo) ->
  (forall vs1 vs2,
    Rvo vs1 vs2 -> wrequiv P (fun s1 => write_lvals true (p_globs p1) s1 xs1 vs1)
                             (fun s2 => write_lvals true (p_globs p2) s2 xs2 vs2) Q) ->
  wequiv P [:: MkI ii1 (Copn xs1 at1 o1 als1 es1)] [:: MkI ii2 (Copn xs2 at2 o2 als2 es2)] Q.
Proof.
  move=> he hvals ho hwr; rewrite /wequiv /isem_cmd_ /=.
  apply wkequiv_bind with Q; last by apply wkequiv_ret.
  ...
  by move=> s1 s2 /ho -/(_ _ _ hvals); apply wrequiv_weaken => // > [-> ->].
Qed.

Lemma wequiv_opn_eq env1 env2 (P Q : rel_c env1 env2) ii1 xs1 at1 o als1 es1 ii2 xs2 at2 als2 es2 :
  wrequiv P (fun s => sem_pexprs true (p_globs p1) s es1)
            (fun s => sem_pexprs true (p_globs p2) s es2) eq ->
  map (eval env1) als1 = map (eval env2) als2 ->
  (forall vs,
    wrequiv P (fun s1 => write_lvals true (p_globs p1) s1 xs1 vs)
              (fun s2 => write_lvals true (p_globs p2) s2 xs2 vs) Q) ->
  wequiv P [:: MkI ii1 (Copn xs1 at1 o als1 es1)] [:: MkI ii2 (Copn xs2 at2 o als2 es2)] Q.
Proof.
  move=> he hvals hx; apply wequiv_opn with eq eq eq => //.
  + move=> s1 s2 vals1 vals2 h <-.
    by apply wrequiv_eq.
  by move=> > <-; apply hx.
Qed.

Lemma wequiv_opn_uincl env (P Q : rel_c env env) ii1 xs1 at1 o als es1 ii2 xs2 at2 es2 :
  wrequiv P (fun s => sem_pexprs true (p_globs p1) s es1)
  ...
  (forall vs1 vs2,
    ...
    wrequiv P (fun s1 => write_lvals true (p_globs p1) s1 xs1 vs1)
             (fun s2 => write_lvals true (p_globs p2) s2 xs2 vs2) Q) ->
  wequiv P [:: MkI ii1 (Copn xs1 at1 o als es1)] [:: MkI ii2 (Copn xs2 at2 o als es2)] Q.
Proof.
  move=> he; apply wequiv_opn with eq values_uincl => //.
  by move=> s1 s2 vals1 vals2 ? <-; apply wrequiv_exec_sopn.
Qed.

Lemma wequiv_opn_esem env1 env2 (P Q : rel_c env1 env2) ii1 xs1 tg1 o1 als1 es1 c2 :
  wrequiv P (fun s => sem_sopn (p_globs p1) o1 s xs1 als1 es1)
            (esem p2 ev2 c2) Q ->
  wequiv P [:: MkI ii1 (Copn xs1 tg1 o1 als1 es1)] c2 Q.
Proof.
  move=> h s t hP /=.
...
Lemma wequiv_syscall env1 env2 Rv Ro (P Q : rel_c env1 env2) ii1 xs1 sc1 es1 ii2 xs2 sc2 es2 :
  wrequiv P (fun s => sem_pexprs true (p_globs p1) s es1)
...
Lemma wequiv_syscall_eq env1 env2 (P Q : rel_c env1 env2) ii1 xs1 sc1 es1 ii2 sc2 xs2 es2 :
...
Lemma wequiv_syscall_esem env1 env2 (P Q : rel_c env1 env2) ii1 xs1 sc1 es1 c2 :
...
Lemma wequiv_assert_esem env1 env2 (P Q : rel_c env1 env2) ii1 a1 c2 :
  wrequiv P (fun (s:estate1 env1) => Let _ := sem_assert (wsw:=wsw1) (wa:=wa1) (p_globs p1) s a1 in ok s)
...
Lemma wequiv_assert env1 env2 (P Q : rel_c env1 env2) ii1 a1 ii2 a2 :
...
Lemma sem_cond_uincl env1 env2 (P : rel_c env1 env2) e1 e2 :
  wrequiv P (fun (s:estate1 env1) => sem_pexpr true (p_globs p1) s e1)
            (fun (s:estate2 env2) => sem_pexpr true (p_globs p2) s e2) value_uincl ->
  wrequiv P (sem_cond (p_globs p1) e1) (sem_cond (p_globs p2) e2) eq.
Proof.
  move=> he; apply: wrequiv_bind wrequiv_to_bool; apply he.
Qed.

Lemma wequiv_assert_eq env1 env2 (P Q : rel_c env1 env2) ii1 a1 ii2 a2 :
  (assert_allowed (WithAssert:=wa1) ->
     assert_allowed (WithAssert:=wa2) /\
...
Lemma st_rel_weaken env1 env2 d d' :
  ...
  forall (s1 : estate1 env1) (s2 : estate2 env2), st_rel d s1 s2 -> st_rel d' s1 s2.
...
Lemma upd_st_rel env1 env2 wdb1 wdb2 gd1 gd2 d d' xs1 xs2 :
...
Lemma wequiv_syscall_uincl env1 env2 (P Q : rel_c env1 env2) ii1 xs1 sc1 es1 ii2 sc2 xs2 es2 :
  (forall s1 s2, P s1 s2 -> escs s1 = escs s2 /\ emem s1 = emem s2) ->
  wrequiv P (fun s => sem_pexprs true (p_globs p1) s es1)
           (fun s => sem_pexprs true (p_globs p2) s es2) values_uincl ->
...
Lemma wequiv_if_full env1 env2 (P Q : rel_c env1 env2) ii1 e1 c1 c1' ii2 e2 c2 c2' :
  wrequiv P (sem_cond (p_globs p1) e1) (sem_cond (p_globs p2) e2) eq ->
  (forall b, wequiv
...
Lemma wequiv_if env1 env2 (P Q : rel_c env1 env2) ii1 e1 c1 c1' ii2 e2 c2 c2' :
  wrequiv P (sem_cond (p_globs p1) e1) (sem_cond (p_globs p2) e2) eq ->
...
Lemma wequiv_if_esem env1 env2 (P P' Q : rel_c env1 env2) ii1 e1 c1 c1' c ii2 e2 c2 c2':
...
Lemma wequiv_if_uincl env1 env2 (P Q : rel_c env1 env2) ii1 e1 c1 c1' ii2 e2 c2 c2' :
  wrequiv P (fun (s:estate1 env1) => sem_pexpr true (p_globs p1) s e1)
            (fun (s:estate2 env2) => sem_pexpr true (p_globs p2) s e2) value_uincl ->
  (forall b, wequiv P (if b then c1 else c1') (if b then c2 else c2') Q) ->
  wequiv P [:: MkI ii1 (Cif e1 c1 c1')] [:: MkI ii2 (Cif e2 c2 c2')] Q.
...
Lemma wequiv_if_eq env1 env2 (P Q : rel_c env1 env2) ii1 e1 c1 c1' ii2 e2 c2 c2' :
  wrequiv P (fun (s:estate1 env1) => sem_pexpr true (p_globs p1) s e1)
            (fun (s:estate2 env2) => sem_pexpr true (p_globs p2) s e2) eq ->
  (forall b, wequiv P (if b then c1 else c1') (if b then c2 else c2') Q) ->
  wequiv P [:: MkI ii1 (Cif e1 c1 c1')] [:: MkI ii2 (Cif e2 c2 c2')] Q.
Proof. by move=> he; apply wequiv_if_uincl; apply: wrequiv_weaken he => // > <-. Qed.

Lemma wequiv_if_rcond env1 env2 (P Q : rel_c env1 env2) ii1 e1 c1 c1' c2 b :
...
Lemma wequiv_for env1 env2 (P0 P Pi : rel_c env1 env2) ii1 i1 d lo1 hi1 c1 ii2 i2 lo2 hi2 c2 :
...
Lemma wrequiv_sem_bound env1 env2 (P : rel_c env1 env2) lo1 hi1 lo2 hi2 :
...
Lemma wequiv_for_uincl env1 env2 (P0 P Pi : rel_c env1 env2) ii1 i1 d lo1 hi1 c1 ii2 i2 lo2 hi2 c2 :
  (forall s1 s2, P0 s1 s2 -> P s1 s2) ->
...
Lemma wequiv_for_eq env1 env2 (P0 P Pi : rel_c env1 env2) ii1 i1 d lo1 hi1 c1 ii2 i2 lo2 hi2 c2 :
  (forall s1 s2, P0 s1 s2 -> P s1 s2) ->
  wrequiv P0 (fun s => sem_pexprs true (p_globs p1) s [::lo1; hi1])
...
Lemma wequiv_while_full env1 env2 (I I' : rel_c env1 env2) ii1 al1 e1 inf1 c1 c1' ii2 al2 e2 inf2 c2 c2' :
  wequiv I c1 c2 I' ->
...
Lemma wequiv_while env1 env2 (I I' : rel_c env1 env2) ii1 al1 e1 inf1 c1 c1' ii2 al2 e2 inf2 c2 c2' :
  wrequiv I' (sem_cond (p_globs p1) e1) (sem_cond (p_globs p2) e2) eq ->
  wequiv I c1 c2 I' ->
  wequiv I' c1' c2' I ->
...
(* Usefull for lowering *)
Lemma wequiv_while_esem env1 env2 (I I1 I' : rel_c env1 env2) ii1 al1 e1 inf1 c1 c1' c ii2 al2 e2 inf2 c2 c2':
  wequiv I c1 c2 I1 ->
...
Proof.
  ...
  apply wkequiv_eutt_r with (F2 :=
    (λ s : estate2 env2,
      s0' <- isem_foldr (isem_i_body (env :=_)) p2 ev2 c2 s;;
      s0 <- isem_foldr (isem_i_body (env := _)) p2 ev2 c s0';;
      b <- isem_cond p2 e2 s0;; (if b then s1 <- isem_foldr (isem_i_body (env := _)) p2 ev2 c2' s0;; ret (inl s1) else ret (inr s0)))).
...
Lemma wequiv_while_uincl env1 env2 (I I' : rel_c env1 env2) ii1 al1 e1 inf1 c1 c1' ii2 al2 e2 inf2 c2 c2' :
  wrequiv I' (fun (s:estate1 env1) => sem_pexpr true (p_globs p1) s e1)
             (fun (s:estate2 env2) => sem_pexpr true (p_globs p2) s e2) value_uincl ->
  wequiv I c1 c2 I' ->
  wequiv I' c1' c2' I ->
  wequiv I [:: MkI ii1 (Cwhile al1 c1 e1 inf1 c1')] [:: MkI ii2 (Cwhile al2 c2 e2 inf2 c2')] I'.
...
Lemma wequiv_while_eq env1 env2 (I I' : rel_c env1 env2) ii1 al1 e1 inf1 c1 c1' ii2 al2 e2 inf2 c2 c2' :
  wrequiv I' (fun (s:estate1 env1) => sem_pexpr true (p_globs p1) s e1)
             (fun (s:estate2 env2) => sem_pexpr true (p_globs p2) s e2) eq ->
  wequiv I c1 c2 I' ->
  wequiv I' c1' c2' I ->
...
Lemma wequiv_while_unroll env1 env2 (P Q : rel_c env1 env2) ii1 al e1 inf1 c1 c1' c2 :
...
Lemma wequiv_call_core_wa env1 env2 (Pf : relPreF) (Qf : relPostF) Rvals Rv (P Q : rel_c env1 env2) ii1 xs1 fn1 als1 es1 ii2 xs2 fn2 als2 es2 :
  wrequiv P (fun s => sem_pexprs (~~ (@direct_call dc1)) (p_globs p1) s es1)
            (fun s => sem_pexprs (~~ (@direct_call dc2)) (p_globs p2) s es2) Rv ->
  Rvals (map (eval env1) als1) (map (eval env2) als2) ->
  (forall vals1 vals2 s1 s2 vs1 vs2, P s1 s2 -> Rvals vals1 vals2 -> Rv vs1 vs2 ->
     sem_pre1 p1 fn1 vals1 (mk_fstate vs1 s1) = ok tt ->
     sem_pre2 p2 fn2 vals2 (mk_fstate vs2 s2) = ok tt) ->
  (forall s1 s2 vals1 vals2 vs1 vs2,
     P s1 s2 -> Rvals vals1 vals2 -> Rv vs1 vs2 -> Pf fn1 fn2 vals1 vals2 (mk_fstate vs1 s1) (mk_fstate vs2 s2)) ->
  wequiv_f_ii Pf ii1 ii2 fn1 fn2 Qf ->
  (forall vals1 vals2 fs1 fs2 fr1 fr2,
     Pf fn1 fn2 vals1 vals2 fs1 fs2 -> Qf fn1 fn2 vals1 vals2 fs1 fs2 fr1 fr2 ->
    sem_post1 p1 fn1 vals1 fs1.(fvals) fr1 = ok () ->
    sem_post2 p2 fn2 vals2 fs2.(fvals) fr2 = ok ()) ->
  (forall vals1 vals2 fs1 fs2 fr1 fr2,
    Pf fn1 fn2 vals1 vals2 fs1 fs2 -> Qf fn1 fn2 vals1 vals2 fs1 fs2 fr1 fr2 ->
    wrequiv
      ...
      Q) ->
  wequiv P [:: MkI ii1 (Ccall xs1 fn1 als1 es1)] [:: MkI ii2 (Ccall xs2 fn2 als2 es2)] Q.
Proof.
  move=> hes hvals hpre hPPf hCall hpost hPQf; rewrite /wequiv /isem_cmd_ /=.
  apply wkequiv_bind with Q; last by apply wkequiv_ret.
  ...
  apply wkequiv_read with (fun u1 u2 => sem_pre1 p1 fn1 [seq eval env1 i | i <- als1] fs1 = ok tt /\ sem_pre2 p2 fn2 [seq eval env2 i | i <- als2] fs2 = ok tt).
  + by apply wkequiv_iresult => _ _ [] [-> ->] he; have ?:= hpre _ _ _ _ _ _ hP hvals hvs he; exists tt.
  move=> _ _ _; apply wkequiv_read with (Qf fn1 fn2 (map (eval env1) als1) (map (eval env2) als2) fs1 fs2).
  + by move=> _ _ [-> ->]; apply/hCall/hPPf.
  move=> fr1 fr2 hQf.
  apply wkequiv_read with (fun u1 u2 => sem_post1 p1 fn1 [seq eval env1 i | i <- als1] fs1.(fvals) fr1 = ok tt /\ sem_post2 p2 fn2 [seq eval env2 i | i <- als2] fs2.(fvals) fr2 = ok tt).
  + apply wkequiv_iresult => _ _ [] _ hpost1.
    have hpost2 := hpost _ _ _ _ _ _ (hPPf _ _ _ _ _ _ hP hvals hvs) hQf hpost1.
    by exists tt.
  move=> _ _ _; apply wkequiv_iresult.
  eapply wrequiv_weaken; last apply (hPQf (map (eval env1) als1) (map (eval env2) als2) fs1 fs2); eauto.
  by move=> ?? [-> ->].
Qed.

Lemma wequiv_call_wa env1 env2 (Pf : relPreF) (Qf : relPostF) Rvals Rv (P Q : rel_c env1 env2) ii1 xs1 fn1 als1 es1 ii2 xs2 fn2 als2 es2 :
  wrequiv P (fun s => sem_pexprs (~~ (@direct_call dc1)) (p_globs p1) s es1)
            (fun s => sem_pexprs (~~ (@direct_call dc2)) (p_globs p2) s es2) Rv ->
  Rvals (map (eval env1) als1) (map (eval env2) als2) ->
  (forall vals1 vals2 s1 s2 vs1 vs2, P s1 s2 -> Rvals vals1 vals2 -> Rv vs1 vs2 ->
  ...
  wequiv P [:: MkI ii1 (Ccall xs1 fn1 als1 es1)] [:: MkI ii2 (Ccall xs2 fn2 als2 es2)] Q.
Proof.
  move=> hes hvals hpre hPPf hpost hCall hPQf.
  apply wequiv_call_core_wa with Pf Qf Rvals Rv => //.
  move=> > hPf hQf; apply wrequiv_weaken with P Q => //.
  + by move=> > [].
  apply: hPQf hPf hQf.
Qed.

Lemma wequiv_call_eq_wa env1 env2 (P Q : rel_c env1 env2) ii1 xs1 fn1 als1 es1 ii2 xs2 fn2 als2 es2 :
  wrequiv P (fun s => sem_pexprs (~~ (@direct_call dc1)) (p_globs p1) s es1)
            (fun s => sem_pexprs (~~ (@direct_call dc2)) (p_globs p2) s es2) eq ->
  map (eval env1) als1 = map (eval env2) als2 ->
  (forall vals s1 s2 vs, P s1 s2 ->
     sem_pre1 p1 fn1 vals (mk_fstate vs s1) = ok tt ->
     sem_pre2 p2 fn2 vals (mk_fstate vs s2) = ok tt) ->
  (forall s1 s2 vals vs,
     P s1 s2 -> rpreF (eS:=eq_spec) fn1 fn2 vals vals (mk_fstate vs s1) (mk_fstate vs s2)) ->
  wequiv_f_ii (rpreF (eS:=eq_spec)) ii1 ii2 fn1 fn2 (rpostF (eS:=eq_spec)) ->
  (forall vals vs fr,
    sem_post1 p1 fn1 vals vs fr = ok () ->
    sem_post2 p2 fn1 vals vs fr = ok ()) ->
  (forall fs,
    wrequiv P (upd_estate (~~ (@direct_call dc1)) (p_globs p1) xs1 fs)
              (upd_estate (~~ (@direct_call dc2)) (p_globs p2) xs2 fs) Q) ->
  wequiv P [:: MkI ii1 (Ccall xs1 fn1 als1 es1)] [:: MkI ii2 (Ccall xs2 fn2 als2 es2)] Q.
Proof.
  move=> he hvals hpre hfs hfn hpost hupd.
  apply wequiv_call_wa with (Pf:=rpreF (eS:=eq_spec)) (Qf:= rpostF (eS:=eq_spec)) (Rvals:=eq) (Rv:=eq) => //.
  + by move=> vals1 vals2 s1 s2 vs1 vs2 hP -> ->; apply hpre.
  + by move=> s1 s2 vals1 _ vs1 _ hP <- <-; apply hfs.
  + by move=> vals1 vals2 fs1 fs2 fr1 fr2 [<- [<- <-]] <-; apply hpost.
  by move=> vals1 vals2 fs1 fs2 ft1 ft2 [<- [<- <-]] /= <-; apply hupd.
Qed.

Definition wequiv_fun_body_hyp' (RPreF:relPreF) fn1 fn2 (RPostF:relPostF) :=
  forall vals1 vals2 fs1 fs2,
  RPreF fn1 fn2 vals1 vals2 fs1 fs2 ->
  forall fd1, get_fundef (p_funcs p1) fn1 = Some fd1 ->
  exists2 fd2, get_fundef (p_funcs p2) fn2 = Some fd2 &
    sem_pre1 p1 fn1 vals1 fs1 = ok tt ->
    sem_pre2 p2 fn2 vals2 fs2 = ok tt /\
    let env1 := create_env fd1.(f_al) vals1 in
    let env2 := create_env fd2.(f_al) vals2 in
    forall s11, initialize_funcall (dc:=dc1) p1 ev1 fd1 env1 fs1 = ok s11 ->
     exists2 s21,
       initialize_funcall (dc:=dc2) p2 ev2 fd2 env2 fs2 = ok s21 &
       exists (P Q : rel_c env1 env2) (tbody epilogue: cmd),
        [/\ P s11 s21
          ...
          , wrequiv Q (finalize_funcall (dc:=dc1) fd1 (env:=env1)) (finalize_funcall (dc:=dc2) fd2 (env:=env2)) (RPostF fn1 fn2 vals1 vals2 fs1 fs2)
          & forall fr1 fr2, RPostF fn1 fn2 vals1 vals2 fs1 fs2 fr1 fr2 ->
                             sem_post1 p1 fn1 vals1 fs1.(fvals) fr1 = ok () -> sem_post2 p2 fn2 vals2 fs2.(fvals) fr2 = ok ()].

Lemma wequiv_fun_body' RPreF fn1 fn2 RPostF :
  wequiv_fun_body_hyp' RPreF fn1 fn2 RPostF ->
  wequiv_f_body RPreF fn1 fn2 RPostF.
Proof.
  move=> hf; rewrite /wequiv_f_body /isem_fun_body => vals1 vals2.
  apply wkequiv_ioP => fs1 fs2 hPf.
  have {}hf:= hf _ _ _ _ hPf.
  ...
  apply wkequiv_read with (fun _ _ => sem_pre1 p1 fn1 vals1 fs1 = ok ()).
  + by apply wkequiv_iresult => ?? [] [-> ->] h; have [-> ?] := hf h; eauto.
  move=> _ _ /hf [hpre2 {}hf].
  move=> _ _ [-> ->].
  set env1 := create_env (f_al fd1) vals1.
  set env2 := create_env (f_al fd2) vals2.
  apply xrutt_bind with (fun s1 s2 =>
     [/\ initialize_funcall (dc:=dc1) p1 ev1 fd1 env1 fs1 = ok s1 &
         initialize_funcall (dc:=dc2) p2 ev2 fd2 env2 fs2 = ok s2]).
...
Proof.
  move=> h.
  apply wequiv_fun_body' => vals1 vals2 fs1 fs2 hpre1 fd1 hget1.
  have [fd2 hget2 {}h] := h vals1 vals2 fs1 fs2 hpre1 fd1 hget1.
  exists fd2 => // {}/h [hpre2 h]; split => // env1 env2 s11 {}/h [s21 hini2 [P] [Q] [hP hbody hfin hpost]].
...
Lemma wequiv_assign_right env1 env2 (P Q : rel_c env1 env2) ii x tg ty e :
...
Lemma wequiv_assign_left env1 env2 (P Q : rel_c env1 env2) ii x tg ty e :
...
Lemma wequiv_assert_left env1 env2 (P Q : rel_c env1 env2) ii a :
  (assert_allowed (WithAssert:=wa1) ->
...
Context {D:Type}.
Context (R : forall env1 env2, D -> estate1 env1 -> estate2 env2 -> Prop).

Class Checker_e :=
 { check_es    : D -> pexprs -> pexprs -> D -> Prop
 ; check_lvals : D -> lvals -> lvals -> D -> Prop
 ; check_esP_rel :
   forall env1 env2 d es1 es2 d', check_es d es1 es2 d' -> forall (s1 : estate1 env1) (s2 : estate2 env2), R d s1 s2 -> R d' s1 s2
 }.

Class Checker_a :=
 { check_a     : D -> eassert -> eassert -> D -> Prop
 ; check_aP_rel :
   forall env1 env2 d a1 a2 d', check_a d a1 a2 d' -> forall (s1 : estate1 env1) (s2 : estate2 env2), R d s1 s2 -> R d' s1 s2
 }.
...
Class Checker_a_eq :=
 { echeck_aP   :
   forall env1 env2 d a1 a2 d',
     check_a d a1 a2 d' ->
     wrequiv (R d) ((sem_eassert (p_globs p1) (env:=env1))^~ a1) ((sem_eassert (p_globs p2) (env:=env2))^~ a2) eq
 }.
...
Lemma wequiv_opn_rel_uincl env d de d' ii1 xs1 tg1 o als es1 ii2 xs2 tg2 es2 :
  check_es d es1 es2 de →
  check_lvals de xs1 xs2 d' →
  wequiv (env1:=env) (env2:=env) (R d) [:: MkI ii1 (Copn xs1 tg1 o als es1)] [:: MkI ii2 (Copn xs2 tg2 o als es2)] (R d').
Proof using cu.
  move=> hes hxs.
...
Lemma wequiv_if_rel_uincl_R env d de d1 d2 d' ii e c1 c2 ii' e' c1' c2' :
  check_es d [::e] [::e'] de ->
  (forall (s1 s2 : estate env), R d1 s1 s2 -> R d' s1 s2) ->
  (forall (s1 s2 : estate env), R d2 s1 s2 -> R d' s1 s2) ->
  wequiv (env1:=env) (env2:=env) (R de) c1 c1' (R d1) ->
  wequiv (env1:=env) (env2:=env) (R de) c2 c2' (R d2) ->
  wequiv (env1:=env) (env2:=env) (R d) [:: MkI ii (Cif e c1 c2)] [:: MkI ii' (Cif e' c1' c2')] (R d').
Proof using cu.
...
Lemma wequiv_for_rel_uincl_R env d0 d dhi di ii i dir lo hi c ii' i' lo' hi' c':
  check_es d0 [::lo; hi] [::lo'; hi'] dhi ->
  (∀ (s1 s2 : estate env), R dhi s1 s2 → R d s1 s2) ->
  check_lvals d [::Lvar i] [::Lvar i'] di ->
  wequiv (env1:=env) (env2:=env) (R di) c c' (R d) ->
  wequiv (env1:=env) (env2:=env) (R d0) [:: MkI ii (Cfor i (dir, lo, hi) c)] [:: MkI ii' (Cfor i' (dir, lo', hi') c')] (R d).
Proof using cu.
  move=> hes hdhi hx hc.
...
Lemma wequiv_while_rel_uincl env d d' de ii1 al1 c1 e1 inf1 c1' ii2 al2 c2 e2 inf2 c2' :
  check_es d' [::e1] [::e2] de ->
  wequiv (env1:=env) (env2:=env) (R d) c1 c2 (R d') →
  wequiv (env1:=env) (env2:=env) (R de) c1' c2' (R d) →
  wequiv (env1:=env) (env2:=env) (R d) [:: MkI ii1 (Cwhile al1 c1 e1 inf1 c1')] [:: MkI ii2 (Cwhile al2 c2 e2 inf2 c2')] (R de).
Proof using cu.
  move=> he hc hc'.
...
Lemma wequiv_syscall_rel_uincl_core_R env d de de' d' ii1 xs1 sc1 es1 ii2 xs2 sc2 es2 :
  (∀ d (s1 s2 : estate env), R d s1 s2 → escs s1 = escs s2 ∧ emem s1 = emem s2) →
  (forall scs mem (s1 s2 : estate env), R de s1 s2 ->
  ...
  wequiv (env1:=env) (env2:=env) (R d) [:: MkI ii1 (Csyscall xs1 sc1 es1)] [:: MkI ii2 (Csyscall xs2 sc2 es2)] (R d').
Proof using cu.
...
Lemma wequiv_call_rel_uincl_R_wa env d de de' d' ii1 xs1 fn1 als1 es1 ii2 xs2 fn2 als2 es2 :
  (∀ d (s1 s2 : estate env), R d s1 s2 → escs s1 = escs s2 ∧ emem s1 = emem s2) →
  (forall scs mem (s1 s2 : estate env), R de s1 s2 ->
     R de' (with_scs (with_mem s1 mem) scs) (with_scs (with_mem s2 mem) scs)) →
  ...
  check_es d es1 es2 de →
  check_lvals de' xs1 xs2 d' →
  (∀ vals (s1 s2 : estate env) vs1 vs2, R d s1 s2 → values_uincl vs1 vs2 →
     sem_pre1 p1 fn1 vals (mk_fstate vs1 s1) = ok () → sem_pre2 p2 fn2 vals (mk_fstate vs2 s2) = ok ()) →
  wequiv_f_ii (fun _ _ vals1 vals2 fs1 fs2 => vals1 = vals2 /\ fs_uincl fs1 fs2) ii1 ii2 fn1 fn2 (fun _ _ _ _ _ _ => fs_uincl) →
  (∀ vals vs1 vs2 fr1 fr2,
    values_uincl vs1 vs2 → fs_uincl fr1 fr2 →
    sem_post1 p1 fn1 vals vs1 fr1 = ok () → sem_post2 p2 fn2 vals vs2 fr2 = ok ()) →
...
Proof using cu.
  ...
  apply wequiv_call_wa with (fun _ _ vals1 vals2 fs1 fs2 => vals1 = vals2 /\ fs_uincl fs1 fs2) (fun _ _ _ _ _ _ => fs_uincl) eq values_uincl.
  + by apply: ucheck_esP hes.
  + by apply hvals.
  + by move=> vals _ s1 s2 vs1 vs2 ? <-; apply hpre.
  + by rewrite /mk_fstate; move=> > /hsm [-> ->] ?.
  + by move=> vals _ fs1 fs2 fr1 fr2 [<- [_ _ ?]]; apply hpost.
  + by apply hf.
  move=> _ _ fs1 fs2 fr1 fr2 _ [hscs hmem hu]; rewrite /upd_estate.
  move=> s1 s2 s1' hR.
  apply ucheck_lvalsP with de' => //.
  rewrite hscs hmem; apply hwith.
  apply: check_esP_rel hes s1 s2 hR.
...
Lemma wequiv_opn_rel_eq env1 env2 d de d' ii1 xs1 tg1 o als1 es1 ii2 xs2 tg2 als2 es2 :
  check_es d es1 es2 de →
  map (eval env1) als1 = map (eval env2) als2 ->
  check_lvals de xs1 xs2 d' →
  wequiv (env1:=env1) (env2:=env2) (R d) [:: MkI ii1 (Copn xs1 tg1 o als1 es1)] [:: MkI ii2 (Copn xs2 tg2 o als2 es2)] (R d').
Proof using cu.
  move=> hes hvals hxs.
  apply wequiv_opn_eq.
...
Lemma wequiv_assert_rel_eq env1 env2 d de ii1 a1 ii2 a2 :
  (assert_allowed (WithAssert:=wa1) → assert_allowed (WithAssert:=wa2)) ->
  check_a d a1.2 a2.2 de ->
  wequiv (env1:=env1) (env2:=env2) (R d) [:: MkI ii1 (Cassert a1)] [:: MkI ii2 (Cassert a2)] (R de).
Proof using caP.
  move=> hassert ha.
...
Lemma wequiv_if_rel_eq_R env1 env2 d de d1 d2 d' ii e c1 c2 ii' e' c1' c2' :
  check_es d [::e] [::e'] de ->
  (forall (s1 : estate env1) (s2 : estate env2), R d1 s1 s2 -> R d' s1 s2) ->
  (forall (s1 : estate env1) (s2 : estate env2), R d2 s1 s2 -> R d' s1 s2) ->
  wequiv (env1:=env1) (env2:=env2) (R de) c1 c1' (R d1) ->
  wequiv (env1:=env1) (env2:=env2) (R de) c2 c2' (R d2) ->
  wequiv (env1:=env1) (env2:=env2) (R d) [:: MkI ii (Cif e c1 c2)] [:: MkI ii' (Cif e' c1' c2')] (R d').
Proof using cu.
  move=> hes hd1 hd2 hc1 hc2.
  apply wequiv_if_eq.
  + by apply: echeck_eP hes.
...
Lemma wequiv_for_rel_eq_R env1 env2 d0 d dhi di ii i dir lo hi c ii' i' lo' hi' c':
  check_es d0 [::lo; hi] [::lo'; hi'] dhi ->
  (∀ (s1 : estate env1) (s2 : estate env2), R dhi s1 s2 → R d s1 s2) ->
  check_lvals d [::Lvar i] [::Lvar i'] di ->
  wequiv (env1:=env1) (env2:=env2) (R di) c c' (R d) ->
  wequiv (env1:=env1) (env2:=env2) (R d0) [:: MkI ii (Cfor i (dir, lo, hi) c)] [:: MkI ii' (Cfor i' (dir, lo', hi') c')] (R d).
Proof using cu.
  move=> hes hdhi hx hc.
...
Lemma wequiv_while_rel_eq env1 env2 d d' de ii1 al1 c1 e1 inf1 c1' ii2 al2 c2 e2 inf2 c2' :
  check_es d' [::e1] [::e2] de ->
  wequiv (env1:=env1) (env2:=env2) (R d) c1 c2 (R d') →
  wequiv (env1:=env1) (env2:=env2) (R de) c1' c2' (R d) →
  wequiv (env1:=env1) (env2:=env2) (R d) [:: MkI ii1 (Cwhile al1 c1 e1 inf1 c1')] [:: MkI ii2 (Cwhile al2 c2 e2 inf2 c2')] (R de).
Proof using cu.
  move=> he hc hc'.
  apply wequiv_weaken with (R d) (R d') => //.
  + by apply: check_esP_rel he.
...
Lemma wequiv_syscall_rel_eq_core_R env1 env2 d de de' d' ii1 xs1 sc1 es1 ii2 xs2 sc2 es2 :
  ...
  wequiv (env1:=env1) (env2:=env2) (R d) [:: MkI ii1 (Csyscall xs1 sc1 es1)] [:: MkI ii2 (Csyscall xs2 sc2 es2)] (R d').
Proof using cu.
  move=> hsm hwith hes hxs hsc.
...
Lemma wequiv_if_rel_uincl env d de d1 d2 d' ii e c1 c2 ii' e' c1' c2' :
  check_es d [::e] [::e'] de ->
  (forall (vm1 : vm1_t env) (vm2 : vm2_t env), R d1 vm1 vm2 -> R d' vm1 vm2) ->
  (forall (vm1 : vm1_t env) (vm2 : vm2_t env), R d2 vm1 vm2 -> R d' vm1 vm2) ->
  wequiv (env1:=env) (env2:=env) (st_rel R de) c1 c1' (st_rel R d1) ->
  wequiv (env1:=env) (env2:=env) (st_rel R de) c2 c2' (st_rel R d2) ->
  wequiv (env1:=env) (env2:=env) (st_rel R d) [:: MkI ii (Cif e c1 c2)] [:: MkI ii' (Cif e' c1' c2')] (st_rel R d').
Proof using cu.
  move=> hes hd1 hd2.
  by apply wequiv_if_rel_uincl_R with ce => //; apply st_rel_weaken.
Qed.

Lemma wequiv_for_rel_uincl env d0 d dhi di ii i dir lo hi c ii' i' lo' hi' c':
  check_es d0 [::lo; hi] [::lo'; hi'] dhi ->
  (∀ (vm1 : vm1_t env) (vm2 : vm2_t env), R dhi vm1 vm2 → R d vm1 vm2) ->
  check_lvals d [::Lvar i] [::Lvar i'] di ->
  wequiv (env1:=env) (env2:=env) (st_rel R di) c c' (st_rel R d) ->
  wequiv (env1:=env) (env2:=env) (st_rel R d0) [:: MkI ii (Cfor i (dir, lo, hi) c)] [:: MkI ii' (Cfor i' (dir, lo', hi') c')] (st_rel R d).
Proof using cu.
  move=> hes hdhi hx hc.
...
Lemma wequiv_syscall_rel_uincl_core env d de d' ii1 xs1 sc1 es1 ii2 xs2 sc2 es2 :
  check_es d es1 es2 de →
  check_lvals de xs1 xs2 d' →
  wrequiv fs_uincl (fexec_syscall (scP:=scP1) sc1) (fexec_syscall (scP:=scP2) sc2) fs_uincl →
  wequiv (env1:=env) (env2:=env) (st_rel R d) [:: MkI ii1 (Csyscall xs1 sc1 es1)] [:: MkI ii2 (Csyscall xs2 sc2 es2)] (st_rel R d').
Proof using cu.
...
Lemma wequiv_call_rel_uincl_wa env d de d' ii1 xs1 fn1 als1 es1 ii2 xs2 fn2 als2 es2 :
  [seq eval env i | i <- als1] = [seq eval env i | i <- als2] ->
  check_es d es1 es2 de →
  check_lvals de xs1 xs2 d' →
  (∀ vals (s1 s2 : estate env) vs1 vs2,
  ...
  (∀ vals vs1 vs2 fr1 fr2,
     values_uincl vs1 vs2 → fs_uincl fr1 fr2 →
     sem_post1 p1 fn1 vals vs1 fr1 = ok () → sem_post2 p2 fn2 vals vs2 fr2 = ok ()) →
  wequiv (env1:=env) (env2:=env) (st_rel R d) [:: MkI ii1 (Ccall xs1 fn1 als1 es1)] [:: MkI ii2 (Ccall xs2 fn2 als2 es2)] (st_rel R d').
Proof using cu.
...
Lemma wequiv_if_rel_eq env1 env2 d de d1 d2 d' ii e c1 c2 ii' e' c1' c2' :
  check_es d [::e] [::e'] de ->
  (forall (vm1 : vm1_t env1) (vm2 : vm2_t env2), R d1 vm1 vm2 -> R d' vm1 vm2) ->
  (forall (vm1 : vm1_t env1) (vm2 : vm2_t env2), R d2 vm1 vm2 -> R d' vm1 vm2) ->
  wequiv (env1:=env1) (env2:=env2) (st_rel R de) c1 c1' (st_rel R d1) ->
  wequiv (env1:=env1) (env2:=env2) (st_rel R de) c2 c2' (st_rel R d2) ->
  wequiv (env1:=env1) (env2:=env2) (st_rel R d) [:: MkI ii (Cif e c1 c2)] [:: MkI ii' (Cif e' c1' c2')] (st_rel R d').
Proof using cu.
  move=> hes hd1 hd2.
  by apply wequiv_if_rel_eq_R with ce => //; apply st_rel_weaken.
Qed.

Lemma wequiv_for_rel_eq env1 env2 d0 d dhi di ii i dir lo hi c ii' i' lo' hi' c':
  check_es d0 [::lo; hi] [::lo'; hi'] dhi ->
  (∀ (vm1 : vm1_t env1) (vm2 : vm2_t env2), R dhi vm1 vm2 → R d vm1 vm2) ->
  check_lvals d [::Lvar i] [::Lvar i'] di ->
  wequiv (env1:=env1) (env2:=env2) (st_rel R di) c c' (st_rel R d) ->
  wequiv (env1:=env1) (env2:=env2) (st_rel R d0) [:: MkI ii (Cfor i (dir, lo, hi) c)] [:: MkI ii' (Cfor i' (dir, lo', hi') c')] (st_rel R d).
Proof using cu.
  move=> hes hdhi hx hc.
...
Lemma wequiv_syscall_rel_eq_core env1 env2 d de d' ii1 xs1 sc1 es1 ii2 xs2 sc2 es2 :
  check_es d es1 es2 de →
  check_lvals de xs1 xs2 d' →
  wrequiv eq (fexec_syscall (scP:=scP1) sc1) (fexec_syscall (scP:=scP2) sc2) eq →
  wequiv (env1:=env1) (env2:=env2) (st_rel R d) [:: MkI ii1 (Csyscall xs1 sc1 es1)] [:: MkI ii2 (Csyscall xs2 sc2 es2)] (st_rel R d').
Proof using cu.
...
Lemma wequiv_call_rel_eq_wa env d de d' ii1 xs1 fn1 als1 es1 ii2 xs2 fn2 als2 es2 :
  [seq eval env i | i <- als1] = [seq eval env i | i <- als2] ->
  check_es d es1 es2 de →
  check_lvals de xs1 xs2 d' →
  (∀ vals (s1 : estate env) (s2 : estate env) vs,
     st_rel R d s1 s2 →
     sem_pre1 p1 fn1 vals (mk_fstate vs s1) = ok () → sem_pre2 p2 fn2 vals (mk_fstate vs s2) = ok ()) →
  wequiv_f_ii (fun _ _ vals1 vals2 fs1 fs2 => vals1 = vals2 /\ fs1 = fs2) ii1 ii2 fn1 fn2 (fun _ _ _ _ _ _ => eq) →
  (∀ vals vs fr,
...
Notation _vm_eq := (fun env1 env2 (_:unit) => vm_eq (wsw1:=wsw1) (wsw2:=wsw2) (env1:=env1) (env2:=env2)).
...
Lemma check_esP_R_st_eq env1 env2 (d : unit) (es1 es2 : pexprs) (d' : unit) :
  check_es_st_eq d es1 es2 d' → ∀ (s1 : estate1 env1) (s2 : estate2 env2), st_rel _vm_eq d s1 s2 → st_rel _vm_eq d' s1 s2.
Proof. done. Qed.
...
Lemma check_aP_st_eq env1 env2 (d : unit) (a1 a2 : eassert) (d' : unit) :
  check_a_st_eq d a1 a2 d' →
  ∀ (s1 : estate1 env1) (s2 : estate2 env2), st_rel _vm_eq d s1 s2 → st_rel _vm_eq d' s1 s2.
Proof. done. Qed.
...
Notation _vm_uincl := (fun env1 env2 (_:unit) => vm_uincl (wsw1:=wsw1) (wsw2:=wsw2) (env1:=env1) (env2:=env2)).
Definition st_uincl := st_rel _vm_uincl.

Lemma check_esP_R_st_uincl env1 env2 (d : unit) (es1 es2 : pexprs) (d' : unit) :
  check_es_st_eq d es1 es2 d' → ∀ (s1 : estate1 env1) (s2 : estate2 env2), st_rel _vm_uincl d s1 s2 → st_rel _vm_uincl d' s1 s2.
...
Lemma whoare_wequiv1 env (P Q : rel_c env env) (P1 Q1 : Pred_c (wsw:=wsw1) env) c1 c2:
...
Lemma whoare_wequiv2 env (P Q : rel_c env env) (P2 Q2 : Pred_c (wsw:=wsw2) env) c1 c2:
...
Lemma wequiv_write1 env (P Q : rel_c env env) c1 c2:
  wequiv p1 p2 ev1 ev2 P c1 c2 Q ->
  (forall s1_,
...
Proof.
  ...
  have /(_ s1_) hw := [elaborate it_writeP (wa:=wa1) (dc:=dc1) (env:=env) p1 ev1 c1 ].
...
Lemma wequiv_write2 env (P Q : rel_c env env) c1 c2:
  wequiv p1 p2 ev1 ev2 P c1 c2 Q ->
  (forall s2_,
    wequiv p1 p2 ev1 ev2
...
Proof.
  ...
  have /(_ s2_) hw := [elaborate it_writeP (wa:=wa2) (dc:=dc2) (env:=env) p2 ev2 c2 ].
...
Definition wequiv_fun_body_hyp_rec' (RPreF:relPreF) fn1 fn2 (RPostF:relPostF) :=
  ...
  forall fd1, get_fundef (p_funcs p1) fn1 = Some fd1 ->
  exists2 fd2, get_fundef (p_funcs p2) fn2 = Some fd2 &
    sem_pre (wsw:=wsw1) (wa:=wa1) (dc:=dc1) (pT:=pT1) p1 fn1 vals1 fs1 = ok tt ->
    sem_pre (wsw:=wsw2) (wa:=wa2) (dc:=dc2) (pT:=pT2) p2 fn2 vals2 fs2 = ok tt /\
    let env1 := create_env fd1.(f_al) vals1 in
    let env2 := create_env fd2.(f_al) vals2 in
    forall s11, initialize_funcall (dc:=dc1) p1 ev1 fd1 env1 fs1 = ok s11 ->
    exists2 s21,
      ...
      exists (P Q : rel_c env1 env2) (tbody epilogue : cmd),
        [/\ P s11 s21
          ...
          , wrequiv Q (finalize_funcall (dc:=dc1) fd1 (env:=env1)) (finalize_funcall (dc:=dc2) fd2 (env:=env2)) (RPostF fn1 fn2 vals1 vals2 fs1 fs2)
          & forall fr1 fr2, RPostF fn1 fn2 vals1 vals2 fs1 fs2 fr1 fr2 ->
               sem_post (wsw:=wsw1) (wa:=wa1) (dc:=dc1) (pT:=pT1) p1 fn1 vals1 fs1.(fvals) fr1 = ok () ->
               sem_post (wsw:=wsw2) (wa:=wa2) (dc:=dc2) (pT:=pT2) p2 fn2 vals2 fs2.(fvals) fr2 = ok ()].
...
Proof.
  ...
  + move=> T1 T2 e1 e2; rewrite /EPreRel sum_prerelP.
    case: e1 e2 => [ [ii1 fn1 vals1 fs1] | e1] [ [ii2 fn2 vals2 fs2] | e2] //=.
    + by case : mfun1. + by case : mfun1.
    by case: mfun1 => // ?; case: mfun1.
  move=> T1 T2 e1 t1 e2 t2; rewrite /EPostRel sum_postrelP.
  case: e1 t1 e2 t2 => [ [ii1 fn1 vals1 fs1] | e1] t1 [ [ii2 fn2 vals2 fs2] | e2] t2 //=.
  by case: mfun1 => // ?; case: mfun1.
Qed.
...
Notation wiequiv_f rpreF fn1 fn2 rpostF :=
   (forall vals1 vals2, wkequiv_io (rpreF fn1 fn2 vals1 vals2) (isem_fun_def1 p1 ev1 fn1 vals1) (isem_fun_def2 p2 ev2 fn2 vals2) (rpostF fn1 fn2 vals1 vals2)).

Lemma wequiv_fun_rec ii1 ii2 fn1 fn2 :
  wequiv_f_rec rpreF ii1 ii2 fn1 fn2 rpostF.
Proof. move=> vals1 vals2 fs1' fs2' hpre'; exact/xrutt_trigger. Qed.
...
Proof.
  move=> hbody fn1 fn2 vals1 vals2 fs1 fs2 hpre.
  apply interp_mrec_xrutt with (RPreInv := (@RPreD spec))
                               (RPostInv := (@RPostD spec)).
  + move=> {hpre fn1 fn2 vals1 vals2 fs1 fs2}.
    move=> _ _ [ii1 fn1 vals1 fs1] [ii2 fn2 vals2 fs2] hpre.
...
Definition wequiv_rec env1 env2 (P : rel_c env1 env2) c1 c2 Q :=
  wequiv (rE0:=relEvent_recCall spec) p1 p2 ev1 ev2 P c1 c2 Q.

Definition wequiv_rec_i env1 env2 (P : rel_c env1 env2) i1 i2 Q := wequiv_rec P [:: i1 ] [:: i2 ] Q.

Definition wequiv_rec_ir env1 env2 (P : rel_c env1 env2) i1 ii1 i2 ii2 Q :=
  wequiv_rec_i P (MkI ii1 i1) (MkI ii2 i2) Q.

Definition wiequiv_f rpreF fn1 fn2 rpostF :=
  forall vals1 vals2,
   (wkequiv_io (rpreF fn1 fn2 vals1 vals2)
      (isem_fun1 p1 ev1 fn1 vals1)
      (isem_fun2 p2 ev2 fn2 vals2)
     (rpostF fn1 fn2 vals1 vals2)).

Definition wiequiv_f' rpreF fn1 fn2 rpostF :=
  (wkequiv_io (rpreF fn1 fn2)
      (fun valsfs1 => isem_fun1 p1 ev1 fn1 valsfs1.1 valsfs1.2)
      (fun valsfs2 => isem_fun2 p2 ev2 fn2 valsfs2.1 valsfs2.2)
     (rpostF fn1 fn2)).

Lemma wiequiv_f_wiequiv_f' rpreF fn1 fn2 rpostF :
  wiequiv_f rpreF fn1 fn2 rpostF <->
  wiequiv_f'
    (fun fn1 fn2 valsfs1 valsfs2 => rpreF fn1 fn2 valsfs1.1 valsfs2.1 valsfs1.2 valsfs2.2)
    fn1 fn2
    (fun fn1 fn2 valsfs1 valsfs2 fr1 fr2 => rpostF fn1 fn2 valsfs1.1 valsfs2.1 valsfs1.2 valsfs2.2 fr1 fr2).
Proof.
  split.
  + by move=> + [vals1 fs1] [vals2 fd2] /=; apply.
  by move=> h vals1 vals2 fs1 fs2; apply (h (vals1, fs1) (vals2, fs2)).
Qed.

Lemma wiequiv_f_weaken rpreF rpreF' fn1 fn2 rpostF rpostF' :
  (forall vals1 vals2 fs1 fs2,
    rpreF' fn1 fn2 vals1 vals2 fs1 fs2 ->
    rpreF fn1 fn2 vals1 vals2 fs1 fs2) ->
  (forall vals1 vals2 fs1 fs2 fr1 fr2,
    rpreF' fn1 fn2 vals1 vals2 fs1 fs2 ->
    rpostF fn1 fn2 vals1 vals2 fs1 fs2 fr1 fr2 ->
    rpostF' fn1 fn2 vals1 vals2 fs1 fs2 fr1 fr2) ->
  wiequiv_f rpreF fn1 fn2 rpostF ->
  wiequiv_f rpreF' fn1 fn2 rpostF'.
Proof.
  move=> hpre hpost.
  rewrite 2!wiequiv_f_wiequiv_f'.
  apply wkequiv_io_weaken => //.
  + by move=> [vals1 fs1] [vals2 fs2] /=; apply hpre.
  by move=> [vals1 fs1] [vals2 fs2] fr1 fr2 /=; apply hpost.
Qed.

Lemma wequiv_fun_get fn1 fn2 Pf Qf :
  (forall fd1, get_fundef (p_funcs p1) fn1 = Some fd1 ->
    wiequiv_f (fun fn1 fn2 vals1 vals2 fs1 fs2 =>
      [/\ Pf fn1 fn2 vals1 vals2 fs1 fs2
        , sem_pre (dc:=dc1) (wsw:=wsw1) (wa:=wa1) p1 fn1 vals1 fs1 = ok tt
        & let env := create_env fd1.(f_al) vals1 in
          exists s1, initialize_funcall (dc:=dc1) p1 ev1 fd1 env fs1 = ok s1])
      fn1 fn2 Qf) ->
  wiequiv_f Pf fn1 fn2 Qf.
Proof.
  move=> hfd vals1 vals2 fs1 fs2 hpre /=.
  rewrite !isem_call_unfold /isem_fun_body /kget_fundef.
  case heq : get_fundef => [fd1 | ].
  + rewrite /= bind_ret_l /isem_pre.
    case heq0 : (sem_pre p1 fn1 vals1 fs1) => [ [] | ].
    + case heq1 : initialize_funcall => [s1 | e].
      + set env := create_env fd1.(f_al) vals1.
        have hpre' :
          [/\ Pf fn1 fn2 vals1 vals2 fs1 fs2, sem_pre (dc:=dc1) (wsw:=wsw1) (wa:=wa1) p1 fn1 vals1 fs1 = ok tt
            & exists s1, initialize_funcall (dc:=dc1) p1 ev1 fd1 env fs1 = ok s1].
        + by split => //; exists s1.
        have /= := hfd _ heq _ _ _ _ hpre'.
        by rewrite !isem_call_unfold /isem_fun_body /kget_fundef heq /= (bind_ret_l fd1) /isem_pre heq0 heq1.
...
Lemma wequiv_noassert env1 env2 ii a c (P Q : rel_c env1 env2) :
...
Lemma wequiv_call env1 env2 (Pf : relPreF) (Qf : relPostF) Rvals Rv (P Q : rel_c env1 env2) ii1 xs1 fn1 als1 es1 ii2 xs2 fn2 als2 es2 :
  wrequiv P (fun s => sem_pexprs (~~ (@direct_call dc1)) (p_globs p1) s es1)
            (fun s => sem_pexprs (~~ (@direct_call dc2)) (p_globs p2) s es2) Rv ->
  Rvals [seq eval env1 i | i <- als1] [seq eval env2 i | i <- als2] ->
  (forall s1 s2 vals1 vals2 vs1 vs2,
     P s1 s2 -> Rvals vals1 vals2 -> Rv vs1 vs2 -> Pf fn1 fn2 vals1 vals2 (mk_fstate vs1 s1) (mk_fstate vs2 s2)) ->
  ...
  (forall vals1 vals2 fs1 fs2 fr1 fr2,
    ...
    wrequiv P (upd_estate (~~ (@direct_call dc1)) (p_globs p1) xs1 fr1)
              (upd_estate (~~ (@direct_call dc2)) (p_globs p2) xs2 fr2) Q) ->
  wequiv p1 p2 ev1 ev2 P [:: MkI ii1 (Ccall xs1 fn1 als1 es1)] [:: MkI ii2 (Ccall xs2 fn2 als2 es2)] Q.
Proof. by move=> hes hvals hPf hf; apply: (wequiv_call_wa hes hvals _ hPf _ hf). Qed.

Lemma wequiv_call_eq env1 env2 (P Q : rel_c env1 env2) ii1 xs1 fn1 als1 es1 ii2 xs2 fn2 als2 es2 :
  wrequiv P (fun s => sem_pexprs (~~ (@direct_call dc1)) (p_globs p1) s es1)
            (fun s => sem_pexprs (~~ (@direct_call dc2)) (p_globs p2) s es2) eq ->
  ...
  (forall fs,
    wrequiv P (upd_estate (~~ (@direct_call dc1)) (p_globs p1) xs1 fs)
              (upd_estate (~~ (@direct_call dc2)) (p_globs p2) xs2 fs) Q) ->
  wequiv p1 p2 ev1 ev2 P [:: MkI ii1 (Ccall xs1 fn1 als1 es1)] [:: MkI ii2 (Ccall xs2 fn2 als2 es2)] Q.
Proof. by move=> hes hvals hPf hf; apply: (wequiv_call_eq_wa hes hvals _ hPf hf). Qed.

Lemma wequiv_call_rel_uincl_R
  {D : Type} [R : forall [env1 env2], D → estate1 env1 → estate2 env2 → Prop] {ce : Checker_e R} {cu : Checker_uincl p1 p2 (R:=R)}
  ...
  check_es d es1 es2 de →
  check_lvals de' xs1 xs2 d' →
  wequiv_f_ii p1 p2 ev1 ev2 (fun _ _ vals1 vals2 fs1 fs2 => vals1 = vals2 /\ fs_uincl fs1 fs2) ii1 ii2 fn1 fn2 (fun _ _ _ _ _ _ => fs_uincl) →
...
Proof.
  ...
  by apply: (wequiv_call_rel_uincl_R_wa (cu:=cu) hscm hde' hvals hes hxs _ hf).
Qed.
...
Lemma wequiv_call_rel_uincl
  {D : Type} [R : forall [env1 env2], D → vm1_t env1 → vm2_t env2 → Prop] {ce : Checker_e (st_rel R)} {cu : Checker_uincl p1 p2 (R:=st_rel R)}
  env d de d' ii1 xs1 fn1 als1 es1 ii2 xs2 fn2 als2 es2 :
  [seq eval env i | i <- als1] = [seq eval env i | i <- als2] ->
  check_es d es1 es2 de →
  check_lvals de xs1 xs2 d' →
  wequiv_f_ii p1 p2 ev1 ev2 (fun _ _ vals1 vals2 fs1 fs2 => vals1 = vals2 /\ fs_uincl fs1 fs2) ii1 ii2 fn1 fn2 (fun _ _ _ _ _ _ => fs_uincl) →
  wequiv (env1:=env) (env2:=env) p1 p2 ev1 ev2 (st_rel R d) [:: MkI ii1 (Ccall xs1 fn1 als1 es1)] [:: MkI ii2 (Ccall xs2 fn2 als2 es2)] (st_rel R d').
Proof.
  move=> hvals hes hxs hf.
  by apply: (wequiv_call_rel_uincl_wa (cu:=cu) hvals hes hxs _ hf).
Qed.

Lemma wequiv_call_rel_eq
  {D : Type} [R : forall [env1 env2], D → vm1_t env1 → vm2_t env2 → Prop] {ce : Checker_e (st_rel R)} {cu : Checker_eq p1 p2 (R:=st_rel R)}
  env d de d' ii1 xs1 fn1 als1 es1 ii2 xs2 fn2 als2 es2 :
  [seq eval env i | i <- als1] = [seq eval env i | i <- als2] ->
  check_es d es1 es2 de →
  check_lvals de xs1 xs2 d' →
...
Proof.
  move=> hvals hes hxs hf.
  by apply: (wequiv_call_rel_eq_wa (cu:=cu) hvals hes hxs _ hf).
Qed.
...
Lemma wequiv_fun_ind :
  (forall fn1 fn2, wequiv_fun_body_hyp_rec rpreF fn1 fn2 rpostF) ->
  forall fn1 fn2 vals1 vals2,
  wkequiv_io
    (rpreF fn1 fn2 vals1 vals2)
    (isem_fun_def (wsw:=wsw1) (dc:=dc1) (pT:=pT1) (scP:=scP1) (sem_F:=sem_F1) p1 ev1 fn1 vals1)
    (isem_fun_def (wsw:=wsw2) (dc:=dc2) (pT:=pT2) (scP:=scP2) (sem_F:=sem_F2) p2 ev2 fn2 vals2)
    (rpostF fn1 fn2 vals1 vals2).
Proof.
  move=> hrec.
  apply wequiv_fun_ind_wa => fn1 fn2 vals1 vals2 fs1 fs2 /(hrec fn1 fn2 vals1 vals2){}h fd1 {}/h [fd2 hfd2 h].
  exists fd2 => // _; split => // env1 env2 s1 {}/h [s2 ? [P] [Q] [*]]; exists s2 => //.
  by exists P, Q.
Qed.
...
Context {D : Type}
        {R : forall env1 env2, D -> Vm.t env1 -> Vm.t env2 -> Prop}
        {ce : Checker_e (st_rel R)}.

Lemma wequiv_syscall_rel_uincl {cu: Checker_uincl p1 p2 (ce:=ce)} env d de d' ii1 xs1 sc es1 ii2 xs2 es2 :
  check_es d es1 es2 de →
  check_lvals de xs1 xs2 d' →
  wequiv (sem_F1:=sem_F1) (sem_F2:=sem_F2) (env1:=env) (env2:=env)
...
Lemma wequiv_syscall_rel_eq {cu: Checker_eq p1 p2 (ce:=ce)} env1 env2 d de d' ii1 xs1 sc es1 ii2 xs2 es2 :
  check_es d es1 es2 de →
  check_lvals de xs1 xs2 d' →
  wequiv (sem_F1:=sem_F1) (sem_F2:=sem_F2) (env1:=env1) (env2:=env2)
    p1 p2 ev1 ev2 (st_rel R d) [:: MkI ii1 (Csyscall xs1 sc es1)] [:: MkI ii2 (Csyscall xs2 sc es2)] (st_rel R d').
Proof.
  move=> hes hxs.
...
Lemma wiequiv_f_trans p1 p2 p3 ev1 ev2 ev3 fn1 fn2 fn3 rpreF12 rpreF23 rpreF13
  ...
  (forall vals1 vals3 fs1 fs3,
      rpreF13 fn1 fn3 vals1 vals3 fs1 fs3 ->
      exists vals2 fs2, rpreF12 fn1 fn2 vals1 vals2 fs1 fs2 /\ rpreF23 fn2 fn3 vals2 vals3 fs2 fs3) ->
  (forall vals1 vals2 vals3 fs1 fs2 fs3 r1 r3,
      rpreF12 fn1 fn2 vals1 vals2 fs1 fs2 ->
      rpreF23 fn2 fn3 vals2 vals3 fs2 fs3 ->
      rcompose (rpostF12 fn1 fn2 vals1 vals2 fs1 fs2) (rpostF23 fn2 fn3 vals2 vals3 fs2 fs3) r1 r3 ->
      rpostF13 fn1 fn3 vals1 vals3 fs1 fs3 r1 r3) ->
...
Proof using rE_trans.
  move=> hpre hpost.
  rewrite 3!wiequiv_f_wiequiv_f'.
  apply: wkequiv_io_trans.
  + move=> [vals1 fs1] [vals3 fs3] /= /hpre [vals2 [fs2 [??]]].
    by exists (vals2, fs2).
  by move=> [vals1 fs1] [vals2 fs2] [vals3 fs3] r1 r3 /=; apply hpost.
Qed.
