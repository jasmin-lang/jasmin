...
Axiom force_word_to_Set : forall (s: wsize) (T: comRingType),
    T = word s -> Set.

Axiom force_array_to_Set : forall (n: positive) (T: Type),
   T = WArray.array n -> Set. 

(* PROBLEM: this cannot be in Set because of WArray.array *)
Definition sem_t (t : stype) : Type :=
  match t with
  | sbool    => bool
  | sint     => Z
  | sarr n   => @force_array_to_Set n (WArray.array n) (@erefl _ _)   
  | sword s  => @force_word_to_Set s (word s) (@erefl _ _)
  end.
