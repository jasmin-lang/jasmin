...
Variable ty_expr : wsize -> pexpr -> exec stype.
...
if (size tys == size es) 
then mapM2 ErrType (check_expr pd) es tys 
else type_error.

End TypeE.

(*------------- Typing rules for expressions ---------------*) 
Fixpoint ty_expr (pd : wsize) (e : pexpr) : exec stype := 
match e with 
| Pconst _ => ok sint
| Pbool _ => ok sbool
| Parr_init len => ok (sarr len)
| Pvar x => ok (x.(gv).(v_var).(vtype))
| Pget aa ws x e => ty_get_set ty_expr pd ws x e
| Psub aa ws len x e => ty_get_set_sub ty_expr pd ws len x e
| Pload ws x e => ty_load_store ty_expr pd ws x e 
| Papp1 op e => let (tin, tout) := type_of_op1 op in 
                Let ce := check_expr ty_expr pd e tin in 
                ok tout
| Papp2 op e1 e2 => let top := type_of_op2 op in 
                    Let ce1 := check_expr ty_expr pd e1 top.1.1 in
                    Let ce2 := check_expr ty_expr pd e2 top.1.2 in 
                    ok top.2
| PappN op es => let top := type_of_opN op in 
                 Let ces := check_pexprs ty_expr pd es top.1 in 
                 ok top.2
| Pif ty b et ef => Let cb := check_expr ty_expr pd b sbool in
                    Let ct := check_expr ty_expr pd et ty in 
                    Let cf := check_expr ty_expr pd ef ty in 
                    ok ty
end.
...
| Lmem ws x e => ty_load_store ty_expr pd ws x e
| Laset aa ws x e => ty_get_set ty_expr pd ws {| gv := x; gs := Slocal |} e
| Lasub aa ws len x e => ty_get_set_sub ty_expr pd ws len {| gv := x; gs := Slocal |} e
end.
