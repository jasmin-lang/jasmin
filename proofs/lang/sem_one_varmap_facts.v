...
Lemma Hcons : sem_Ind_cons p extra_free_registers var_tmp callee_saved Pc Pi.
Proof. move => ki kc x y z i c _ xy _ yz; red; transitivity (emem y); assumption. Qed.

Lemma HmkI : sem_Ind_mkI p extra_free_registers var_tmp callee_saved Pi Pi_r.
Proof. by []. Qed.
...
Lemma Hsyscall : sem_Ind_syscall p callee_saved Pi_r.
Proof. 
  move => ii s1 s2 o xs es scs m ves vs hes /exec_syscallS [ho _] /write_lvals_stack_stable hw.
  by rewrite /Pi_r ho.
Qed.

Lemma Hif_true : sem_Ind_if_true p extra_free_registers var_tmp callee_saved Pc Pi_r.
Proof. by []. Qed.

Lemma Hif_false : sem_Ind_if_false p extra_free_registers var_tmp callee_saved Pc Pi_r.
Proof. by []. Qed.

Lemma Hwhile_true : sem_Ind_while_true p extra_free_registers var_tmp callee_saved Pc Pi Pi_r.
Proof.
...
Lemma Hwhile_false : sem_Ind_while_false p extra_free_registers var_tmp callee_saved Pc Pi_r.
Proof. by []. Qed.

Lemma Hcall : sem_Ind_call p extra_free_registers var_tmp callee_saved Pi_r Pfun.
Proof. by []. Qed.

Lemma Hproc : sem_Ind_proc p extra_free_registers var_tmp callee_saved Pc Pfun.
Proof.
...
Lemma sem_stack_stable k s1 c s2 :
  sem p extra_free_registers var_tmp callee_saved k s1 c s2 → emem s1 ≡ emem s2.
Proof.
  exact:
    (@sem_Ind _ _ _ _ _ _ _ p extra_free_registers var_tmp callee_saved Pc Pi Pi_r Pfun
...
Lemma sem_I_stack_stable k s1 i s2 :
  sem_I p extra_free_registers var_tmp callee_saved k s1 i s2 → emem s1 ≡ emem s2.
Proof.
  exact:
    (@sem_I_Ind _ _ _ _ _ _ _ p extra_free_registers var_tmp callee_saved Pc Pi Pi_r Pfun
...
Lemma sem_i_stack_stable ii k s1 i s2 :
  sem_i p extra_free_registers var_tmp callee_saved ii k s1 i s2 → emem s1 ≡ emem s2.
Proof.
  exact:
    (@sem_i_Ind _ _ _ _ _ _ _ p extra_free_registers var_tmp callee_saved Pc Pi Pi_r Pfun
...
Lemma sem_call_stack_stable ii k s1 fn s2 :
  sem_call p extra_free_registers var_tmp callee_saved ii k s1 fn s2 → emem s1 ≡ emem s2.
Proof.
  exact:
    (@sem_call_Ind _ _ _ _ _ _ _ p extra_free_registers var_tmp callee_saved Pc Pi Pi_r Pfun
...
Lemma sem_call_valid_RSP ii k s1 fn s2 :
  sem_call p extra_free_registers var_tmp callee_saved ii k s1 fn s2 →
...
Lemma Hcons_nw : sem_Ind_cons p extra_free_registers var_tmp callee_saved Pc Pi.
Proof.
  move => ki kc x y z i c _ xy _ yz.
  exact: vmap_eq_exceptTI yz.
Qed.

Lemma HmkI_nw : sem_Ind_mkI p extra_free_registers var_tmp callee_saved Pi Pi_r.
Proof.
...
Lemma Hsyscall_nw : sem_Ind_syscall p callee_saved Pi_r.
Proof.
  ...
  apply: (vmap_eq_exceptI (s1:= syscall_kill callee_saved)); first by SvD.fsetdec.
  by move=> y /= /Sv_memP /negPf; rewrite /vm_after_syscall kill_varsE => ->.
Qed.

Lemma Hif_true_nw : sem_Ind_if_true p extra_free_registers var_tmp callee_saved Pc Pi_r.
Proof. by []. Qed.

Lemma Hif_false_nw : sem_Ind_if_false p extra_free_registers var_tmp callee_saved Pc Pi_r.
Proof. by []. Qed.

Lemma Hwhile_true_nw : sem_Ind_while_true p extra_free_registers var_tmp callee_saved Pc Pi Pi_r.
Proof.
...
Lemma Hwhile_false_nw : sem_Ind_while_false p extra_free_registers var_tmp callee_saved Pc Pi_r.
Proof. by []. Qed.

Lemma Hcall_nw : sem_Ind_call p extra_free_registers var_tmp callee_saved  Pi_r Pfun.
Proof. by []. Qed.

Lemma Hproc_nw : sem_Ind_proc p extra_free_registers var_tmp callee_saved Pc Pfun.
Proof.
  ...
  - move=>
    ...
    clear.
    move => r_not_rax r_not_flag r_not_save_stack.
    rewrite Fv.setP_neq; last by apply/eqP => ?; apply: r_not_rax.
    rewrite kill_varsE; case: ifP => rx.
    + by elim: r_not_flag; apply/Sv_memP.
...
Lemma sem_not_written k s1 c s2 :
  sem p extra_free_registers var_tmp callee_saved k s1 c s2 →
  s1 = s2 [\k].
Proof.
  exact:
    (@sem_Ind _ _ _ _ _ _ _ p extra_free_registers var_tmp callee_saved Pc Pi Pi_r Pfun
...
Lemma sem_I_not_written k s1 i s2 :
  sem_I p extra_free_registers var_tmp callee_saved k s1 i s2 →
  s1 = s2 [\k].
Proof.
  exact:
    (@sem_I_Ind _ _ _ _ _ _ _ p extra_free_registers var_tmp callee_saved Pc Pi Pi_r Pfun
...
Lemma Hcons_pm : sem_Ind_cons p extra_free_registers var_tmp callee_saved Pc Pi.
Proof.
  move => ki kc x y z i c _ xy _.
  by rewrite /Pc disjoint_unionE xy.
Qed.

Lemma HmkI_pm : sem_Ind_mkI p extra_free_registers var_tmp callee_saved Pi Pi_r.
Proof.
...
Lemma Hsyscall_pm : sem_Ind_syscall p callee_saved Pi_r.
Proof. by []. Qed.

Lemma Hif_true_pm : sem_Ind_if_true p extra_free_registers var_tmp callee_saved Pc Pi_r.
Proof. by []. Qed.

Lemma Hif_false_pm : sem_Ind_if_false p extra_free_registers var_tmp callee_saved Pc Pi_r.
Proof. by []. Qed.

Lemma Hwhile_true_pm : sem_Ind_while_true p extra_free_registers var_tmp callee_saved Pc Pi Pi_r.
Proof. by []. Qed.

Lemma Hwhile_false_pm : sem_Ind_while_false p extra_free_registers var_tmp callee_saved Pc Pi_r.
Proof. by []. Qed.

Lemma Hcall_pm : sem_Ind_call p extra_free_registers var_tmp callee_saved Pi_r Pfun.
Proof. by []. Qed.
...
Proof.
  ...
  - have := SvD.F.inter_1 X.
    by move=> /sv_of_listP -/(mapP (T1:=ceqT_eqType)) /= [? _ ->].
...
Lemma Hproc_pm : sem_Ind_proc p extra_free_registers var_tmp callee_saved Pc Pfun.
Proof.
...
Lemma sem_RSP_GD_not_written k s1 c s2 :
  sem p extra_free_registers var_tmp callee_saved k s1 c s2 → disjoint k (magic_variables p).
Proof.
  exact:
    (@sem_Ind _ _ _ _ _ _ _ p extra_free_registers var_tmp callee_saved Pc Pi Pi_r Pfun
...
Lemma sem_I_RSP_GD_not_written k s1 i s2 :
  sem_I p extra_free_registers var_tmp callee_saved k s1 i s2 → disjoint k (magic_variables p).
Proof.
  exact:
    (@sem_I_Ind _ _ _ _ _ _ _ p extra_free_registers var_tmp callee_saved Pc Pi Pi_r Pfun
...
Lemma sem_preserved_RSP_GD k s1 c s2 :
  sem p extra_free_registers var_tmp callee_saved k s1 c s2 → evm s1 =[magic_variables p] evm s2.
Proof.
  move => exec.
...
Lemma sem_I_preserved_RSP_GD k s1 i s2 :
  sem_I p extra_free_registers var_tmp callee_saved k s1 i s2 → evm s1 =[magic_variables p] evm s2.
Proof.
  move => exec.
  apply: eq_except_disjoint_eq_on.
...
Lemma validw_stable_cons : sem_Ind_cons p extra_free_registers var_tmp callee_saved Pc Pi.
Proof. move => ki kc x y z i c _ xy _ yz; red; etransitivity; eassumption. Qed.

Lemma validw_stable_mkI : sem_Ind_mkI p extra_free_registers var_tmp callee_saved Pi Pi_r.
Proof. by []. Qed.
...
Lemma validw_stable_syscall : sem_Ind_syscall p callee_saved Pi_r.
Proof. 
  by move => ii s1 s2 o xs es scs m ves vs _ /exec_syscallS [_ ho] /write_lvals_validw hw => ??; rewrite ho.
Qed.

Lemma validw_stable_if_true : sem_Ind_if_true p extra_free_registers var_tmp callee_saved Pc Pi_r.
Proof. by []. Qed.

Lemma validw_stable_if_false : sem_Ind_if_false p extra_free_registers var_tmp callee_saved Pc Pi_r.
Proof. by []. Qed.

Lemma validw_stable_while_true : sem_Ind_while_true p extra_free_registers var_tmp callee_saved Pc Pi Pi_r.
Proof.
  move => ii k k' krec s1 s2 s3 s4 aa c e c' _ A _ _ B _ C; red.
  etransitivity; first exact: A.
  etransitivity; first exact: B.
...
Lemma validw_stable_while_false : sem_Ind_while_false p extra_free_registers var_tmp callee_saved Pc Pi_r.
Proof. by []. Qed.

Lemma validw_stable_call : sem_Ind_call p extra_free_registers var_tmp callee_saved Pi_r Pfun.
Proof. by []. Qed.

Lemma validw_stable_proc : sem_Ind_proc p extra_free_registers var_tmp callee_saved Pc Pfun.
Proof.
