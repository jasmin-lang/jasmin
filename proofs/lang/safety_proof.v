...
Require Import safety_cond_facts sem_op_typed_facts sopn_semi_facts.
Import Utf8.
...
Lemma sc_to_e_op1 o c e :
  sc_op1_to_e (fun sg ws e => Papp1 (Oint_of_word sg ws) e) o c e = Papp1 o e.
...
Lemma sem_pexpr_sc_to_e gd0 s es vs c v :
  sem_pexprs_t true gd0 s es = ok vs ->
  safety_cond_expr c -> safety_cond_below (size es) c ->
  sem_pexpr_t true gd0 s (sc_to_e (fun sg ws e => Papp1 (Oint_of_word sg ws) e) es c) = ok v ->
  sem_safety_cond vs c = ok v.
Proof.
  move=> hes; elim: c v => //=.
  + by move=> k v _ hk; rewrite (nth_sem_pexprs_nth hk hes) => -[<-].
  + move=> o c ih v htot hk.
    rewrite sc_to_e_op1 /=.
    t_xrbindP => v1 /(ih _ htot hk) hv1.
    rewrite /sem_sop1; t_xrbindP => x hof r hr <-.
    move: hr; rewrite sem_sop1_typed_total => -[<-].
    by rewrite hv1 /= hof.
  move=> o c1 ih1 c2 ih2 v /andP [ht1 ht2] /andP [hk1 hk2].
  t_xrbindP => v1 /(ih1 _ ht1 hk1) hv1 v2 /(ih2 _ ht2 hk2) hv2.
  rewrite /sem_sop2; t_xrbindP => x1 hof1 x2 hof2 r hr <-.
  move: hr; rewrite sem_sop2_typed_total => -[<-].
  by rewrite hv1 /= hv2 /= hof1 /= hof2.
Qed.
...
Proof.
  ...
  case: ifP => [hk | _]; last by move=> [].
  have hPexpr : forall c, safety_cond_below (size es) c ->
      sem_eassert_t gd0 s
        (Pexpr (if safety_cond_expr c
                then sc_to_e (fun sg ws e => Papp1 (Oint_of_word sg ws) e) es c
                else Pbool false)) = ok true ->
      safety_cond_holds vs c.
  + move=> c hkc; case: ifP => [hexp | _]; last by move=> [].
    rewrite /=; t_xrbindP => v hv /to_boolI ?; subst v.
    by rewrite /safety_cond_holds (sem_pexpr_sc_to_e hes hexp hkc hv).
  case: sc hk => [b | z | k | o c | o c1 c2 | o c1 c2 c3] hk;
    [ exact: (hPexpr (IBool b)) | exact: (hPexpr (IConst z))
    | exact: (hPexpr (IVar k)) | exact: (hPexpr (IOp1 o c))
    | exact: (hPexpr (IOp2 o c1 c2)) | ].
  move: hk => /= /and3P [hk1 hk2 hk3].
  case: ifP => [/and3P [he1 he2 he3] | _]; last by move=> [].
  rewrite /= /sem_opN_safety; t_xrbindP => ? ? hv1 ? ? hv2 ? ? hv3 <- <- <- hop.
  by rewrite /safety_cond_holds /= (sem_pexpr_sc_to_e hes he1 hk1 hv1)
    (sem_pexpr_sc_to_e hes he2 hk2 hv2) (sem_pexpr_sc_to_e hes he3 hk3 hv3) /= hop.
Qed.
