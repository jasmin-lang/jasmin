...
Lemma eval_atype_carr ty n : 
  ...
  exists ws len, ty = aarr ws len /\ n = Z.to_pos (arr_size ws len).
...
Lemma sc_in_boundP s ty n (i ilen : sem_t cint) aa sz (e elen : pexpr) :
  ...
  sem_cond_wc gd (eands (sc_in_bound ty aa sz e elen)) s = ok true ->
  (0 <= i * mk_scale aa sz /\ i * mk_scale aa sz + ilen <= n)%Z.
Proof.
  rewrite /sc_in_bound /= /emk_scale /emuli /sem_cond => /eval_atype_carr [ws [len [-> ?]]] he helen; subst n.
  case: aa; rewrite /= helen he /arr_size => /= -[]/andP [/ZleP h1 /ZleP h2]; Lia.lia.
Qed.

Lemma sc_in_boundP_all s ty n (t : sem_t (carr n)) (i: sem_t cint) aa sz e :
  eval_atype ty = carr n -> 
  sem_pexpr_wc true gd s e = ok (to_val i) ->
  sem_cond_wc gd (eands(sc_in_bound ty aa sz e (Pconst(wsize_size sz)))) s = ok true ->
...
Lemma sc_in_sub_boundP s ty n (t : sem_t (carr n)) a e1 e2 (ve1 ve2: Z) :
  ...
  0 <= a < ve2 ->
  sem_cond_wc gd (eands (sc_in_bound' ty e1 e2)) s = ok true ->
...
Lemma sc_arr_initP s ty n (t : WArray.array n) (i : sem_t cint) x aa sz e :
  ...
  sem_cond_wc gd (eands (sc_arr_init ty x aa sz e)) s = ok true ->
  all (fun j => WArray.in_bound t (i * mk_scale aa sz + j)) (ziota 0 (wsize_size sz)) ->
...
Lemma arr_isdef s x len : eval_atype (vtype (gv x)) = carr len -> is_defined (evm s).[gv x].
Proof.
...
(*
Lemma eval_atype_arr n t :  eval_atype t = carr n -> exists ws, exists2 len, t = aarr ws len & n = Z.to_pos (arr_size ws len).
Proof.
*)
...
Proof.
  ...
  + move=> x s v.
    ...
    + move: harr => /is_aarrP[ws [len htx]].
      have /(_ x (Z.to_pos (arr_size ws len))) -> // := arr_isdef s.
      by rewrite htx.
    by t_xrbindP => z /= [] <- /= [] ->.
  ...
  + move=> al aa sz x e he s v /eandsE_cat[/he{}he].
    ...
    move: hbound.
    move=> /(sc_in_boundP_all r htx hewc) hbound.
  ...
  + move=> idx hidx op vi body hbody start hstart len hlen s v.
    ...
    + by have -> := (isdef_errtype (t:=cbool) (sem_pexpr_defined h1)) heq.
    ...
    + by have -> := (isdef_errtype (t:=cbool) (sem_pexpr_defined pok)) heq.
  ...
  + rewrite /sc_fun /=; clear hget hinit hgetsc.
    ...
    + move=> xs tg o es ii.
      ...
      + move=> s1 s2 [] ??; subst s1 s2 => vs _ vf [] <- hewc he.
        ...
        have : List.Forall2 (fun ty vs => type_of_val vs = eval_atype ty) (tin (get_instr_desc o)) vs.
        + move: hwt he => [] {hewc hsco hsce}.
          elim: (tin _) es vs => [ | ty tys ih] [ | e es] //=.
          + by move=> vs _ [<-]; constructor.
          move=> vs' /andP [hty htys].
          t_xrbindP => v he vs hes <-; constructor.
