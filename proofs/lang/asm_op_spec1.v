...
Lemma asm_op_spec1_eq_axiom : Equality.axiom asm_op_spec1_beq.
Proof.
Admitted.

Definition asm_op_spec1_eqMixin := Equality.Mixin asm_op_spec1_eq_axiom.
Canonical  asm_op_spec1_eqType  := EqType asm_op_spec1 asm_op_spec1_eqMixin.
...
Definition asm_op_spec1_prim_constructor (f:asm_op -> asm_op_spec1) (p : prim_constructor asm_op) : prim_constructor asm_op_spec1 :=
  match p with
...
Definition asm_op_spec1_prim_string : seq (string * prim_constructor asm_op_spec1) :=
  [::
...
Global Instance asmOp_spec1 : asmOp asm_op_spec1 :=
  { sopn.asm_op_instr := get_instr_desc;
    sopn.prim_string := asm_op_spec1_prim_string }.
...
#[export]
  Instance spp_of_asm_op_spec1 : SemPexprParams asm_op_spec1 syscall_state :=
    {
      _pd := pd;
      _asmop := asmOp_spec1;
      _fcp := fcp;
