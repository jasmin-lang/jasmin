...
(* ** Values
 * -------------------------------------------------------------------- *)
Definition iabstract s := Cabstract.iabstract s.
...
Definition abstract_uincl (s1 s2 : string) (a1: iabstract s1) (a2: iabstract s2) :=
  ∃ p : s1 = s2, ecast s (iabstract s) p a1 = a2.
...
Proof. by case => ? ? [] ??; subst; exists erefl. Qed.
...
Variant cast_abstract_spec s s' (a: iabstract s') : exec (iabstract s) → Type :=
  | CastAbstractEq (h: s' = s) : cast_abstract_spec (ok (ecast s (iabstract s) h a))
  | CastAbstractLt : (s < s')%CMP -> cast_abstract_spec type_error
  | CastAbstractGt : (s' < s)%CMP → cast_abstract_spec type_error
.
...
Proof.
  ...
  + move=> ws1 w1 [_ [_ [-> _ _]]] [ws2 [-> hcmp2]].
    ...
    by subst; rewrite /truncate_val /= cast_abstract_u /=; eexists.
Qed.
...
Proof.
  ...
  + by move=> /to_wordI [? [? [-> ? <-]]] /=; exact: truncate_word_uincl.
    by move=> /to_abstractI [? [? [-> ? <-]]] /=; exact: cast_abstractP.
Qed.
...
Lemma swap_semu ty (vs vs' : seq value) (v : values):
  ...
  exists2 v' :
    values, @app_sopn_v [::ty; ty] [::ty; ty] (sem_prod_ok [::ty; ty] (@swap_semi ty)) vs' =
              ok v' & List.Forall2 value_uincl v v'.
Proof.
  rewrite /app_sopn_v.
