...
Lemma type_of_valI v t : type_of_val v = t ->
  ...
  | sarr len => exists p a, v = @Varr p a /\ len = AL_const p
...
Definition array_eq n1 n2 (t1 : WArray.array n1) (t2 : WArray.array n2) :=
  WArray.uincl t1 t2 /\ WArray.uincl t2 t1.

Lemma array_eq_len n1 n2 (t1 : WArray.array n1) (t2 : WArray.array n2) :
  array_eq t1 t2 -> n1 = n2.
Proof.
  move=> [[hle12 hincl12] [hle21 hincl21]].
  by lia.
Qed.

Lemma array_eq_trans len1 len2 len3 (a2 : WArray.array len2) (a1 : WArray.array len1)
  (a3 : WArray.array len3) :
  array_eq a1 a2 -> array_eq a2 a3 -> array_eq a1 a3.
Proof.
  move=> [[hle12 hincl12] [hle21 hincl21]] [[hle23 hincl23] [hle32 hincl32]].
  split.
  + split; first by lia.
    eauto.
  split; first by lia.
...
Definition value_uincl (v1 v2:value) :=
  ...
  | Varr n1 t1, Varr n2 t2 => array_eq t1 t2
...
Lemma value_uinclE v1 v2 :
  ...
  match v1 with
  | Varr n1 t1 =>
    exists n2 t2, v2 = @Varr n2 t2 /\ array_eq t1 t2
...
+ case => //= n' t' [] [] hle1 _ [] hle2 _.
  apply /eqP. f_equal. lia.
+ by case => //= s' w' /andP[].
move=> /= v2.
case: ty h => //=.
case: (type_of_val v2) => // ws _ _ _.
...
Proof.
  ...
  + by move=> [? [? [-> /array_eq_trans h]]] /value_uinclE [? [? [-> /h]]].
...
(* FIXME: I leave cast to have the right type, but this is ugly. *)
Definition to_arr len v : exec (WArray.array len) :=
  match v with
  | Varr len' t => if len == len' then WArray.cast len t else type_error
  | _ => type_error
  end.
...
Proof. case: v => //= n' t'.
  case: eqP => // ?; subst n'.
  rewrite WArray.castK; congruence.
Qed.

Lemma to_arr_undef p v : to_arr p v <> undef_error.
Proof. by case: v => //= ??; case: eqP => //; rewrite /WArray.cast; case: ifP. Qed.
...
Definition of_val t : value -> exec (sem_t tmap t) :=
  match t return value -> exec (sem_t tmap t) with
  | sbool => to_bool
  | sint => to_int
  | sarr n => to_arr (pos_of_length tmap n)
  | sword s => to_word s
  end.
...
Lemma of_val_typeE t v v' : of_val t v = ok v' ->
  ...
  | sarr len, vt =>
      v = Varr vt (*
  exists len' (a: WArray.array len'),
    v = Varr a /\ WArray.cast (pos_of_length tmap len) a = ok vt *)
...
Lemma of_valE t v v' : of_val t v = ok v' ->
  ...
  | Varr len a => exists len' (h: t = sarr len') (h2: len = pos_of_length tmap len'),
    eq_rect _ _ v' _ h = eq_rect _ _ a _ h2
  | Vword ws w => exists ws' (h: t = sword ws') w',
    truncate_word ws' w = ok w' /\ eq_rect _ _ v' _ h = w'
  | Vundef t h => False
  end.
Proof.
  case: t v' => [b|n|len a|ws w] /of_val_typeE.
  + by move=> ->; exists erefl.
  + by move=> ->; exists erefl.
  + by move=> ->; exists len, erefl, erefl.
  by move=> [ws' [w' [-> htr]]]; exists ws, erefl, w.
Qed.
(*
Lemma of_val_subtype t v sv : of_val t v = ok sv -> subtype t (type_of_val v).
Proof.
  case: t sv => > /of_val_typeE; try by move=> ->.
  + move=> ->. simpl. case=> ?[?[-> /WArray.cast_len]] /=. simpl in *. ? [? [-> /WArray.cast_len /ZleP]].
...
Lemma of_value_uincl ty v v' vt :
  ...
  | Varr len a => exists len' (h: ty = sarr len') a',
    of_val (sarr len') v' = ok a' /\ array_eq (eq_rect _ _ vt _ h) a'
  | _ => of_val ty v' = ok vt
  end.
Proof.
  case: v => [b|z|len a|ws w|t hnarr] /value_uinclE + /of_valE //.
  + by move=> -> [?]; subst ty => /= ->.
  + by move=> -> [?]; subst ty => /= ->.
  + move=> [n2 [a2 [-> heq]]] [len' [? [?]]]; subst ty len => /= ->.
    have ? := array_eq_len heq; subst n2.
    exists _, erefl.
    rewrite eq_refl WArray.castK /=.
    by exists a2.
  move=> [? [? [-> ?]]].
  move=> [? [? [? [?]]]]; subst ty => /= ->.
...
Lemma of_value_uincl_te ty v v' vt :
  value_uincl v v' -> of_val ty v = ok vt ->
  match ty as ty return sem_t tmap ty -> Prop with
  | sarr n => fun vt =>
...
Definition to_val t : sem_t tmap t -> value :=
  match t return sem_t tmap t -> value with
  | sbool => Vbool
  | sint => Vint
  | sarr n  => @Varr (pos_of_length tmap n)
  | sword s => @Vword s
  end.

Lemma to_val_inj t (v1 v2: sem_t tmap t) : to_val v1 = to_val v2 -> v1 = v2.
Proof. by case: t v1 v2 => /= > => [[]|[]| /Varr_inj1 |[]]. Qed.

Lemma to_valI t (x: sem_t tmap t) v : to_val x = v ->
  match v with
  | Vbool b => exists h: t = sbool, eq_rect _ _ x _ h = b
  | Vint i => exists h: t = sint, eq_rect _ _ x _ h = i
  | Varr len a => exists len' (h: t = sarr len') (h2: len = pos_of_length tmap len'),
    eq_rect _ _ x _ h = eq_rect _ _ a _ h2
  | Vword ws w => exists h: t = sword ws, eq_rect _ _ x _ h = w
  | Vundef _ _ => False
  end.
Proof.
  case: t x => /= > <-; try exists erefl => //.
  by exists _, erefl, erefl.
Qed.
(*
Lemma type_of_to_val t (s: sem_t tmap t) : type_of_val (to_val s) = t.
Proof. case: t s => //. simpl. Qed.*)

Definition oto_val t : sem_ot tmap t -> value :=
  match t return sem_ot tmap t -> value with
  | sbool => fun ob => if ob is Some b then Vbool b else undef_b
  | x => @to_val x
  end.
(*
Lemma type_of_oto_val t (s: sem_ot tmap t) : type_of_val (oto_val s) = t.
Proof. case: t s => //=. move=> -[] //. simpl. Qed. *)

Definition val_uincl (t1 t2:stype) (v1:sem_t tmap t1) (v2:sem_t tmap t2) :=
  value_uincl (to_val v1) (to_val v2).
(*
Lemma val_uincl_alt t1 t2 : @val_uincl t1 t2 =
  match t1, t2 return sem_t tmap t1 -> sem_t tmap t2 -> Prop with
  | sarr _, sarr _ => WArray.uincl
  | sword s1, sword s2 => @word_uincl s1 s2
  | t1', t2' => if boolP (t1' == t2') is AltTrue h
    then eq_rect _ (fun x => sem_t tmap t1' -> sem_t tmap x -> Prop) eq _ (eqP h)
    else fun _ _ => False
  end.
Proof.
  case: t1; case: t2 => >; rewrite /val_uincl //=.
  have: match (2 =P 3) with | ReflectT _ => 3 | ReflectF _ => 4 end = 2.
  destruct eqP.
  case: {2}_/eqP.
  case: (2 =P 3).
  case: boolP.
...
Fixpoint list_ltuple (ts:list stype) : sem_tuple tmap ts -> values :=
  match ts return sem_tuple tmap ts -> values with
  | [::] => fun (u:unit) => [::]
  | t :: ts =>
    let rec := @list_ltuple ts in
    match ts return (sem_tuple tmap ts -> values) -> sem_tuple tmap (t::ts) -> values with
    | [::] => fun _ x => [:: oto_val x]
    | t1 :: ts' =>
      fun rec (p : sem_ot tmap t * sem_tuple tmap (t1 :: ts')) =>
       oto_val p.1 :: rec p.2
    end rec
  end.
(*
Lemma type_of_val_ltuple tout (p : sem_tuple tmap tout) :
  List.map type_of_val (list_ltuple p) = tout.
Proof.
  elim: tout p => //= t1 [|t2 tout] /=.
  + rewrite /sem_tuple /= => _. simpl. x;rewrite type_of_oto_val.
  by move=> hrec [] x xs /=; rewrite type_of_oto_val hrec.
Qed.
*)
(* ----------------------------------------------------------------------- *)

Fixpoint app_sopn T ts : sem_prod tmap ts (exec T) → values → exec T :=
  match ts return sem_prod tmap ts (exec T) → values → exec T with
  | [::] => λ (o : exec T) (vs: values), if vs is [::] then o else type_error
  | t :: ts => λ (o: sem_t tmap t → sem_prod tmap ts (exec T)) (vs: values),
    if vs is v :: vs
...
Definition app_sopn_v tin tout (semi: sem_prod tmap tin (exec (sem_tuple tmap tout))) vs :=
...
Lemma vuincl_app_sopn_v_eq tin tout (semi: sem_prod tmap tin (exec (sem_tuple tmap tout))) :
...
Lemma vuincl_copy_eq ws p :
  ...
  @app_sopn_v [::sarr (const_length sz)] [::sarr (const_length sz)] (@WArray.copy ws p) vs = ok v ->
  @app_sopn_v [::sarr (const_length sz)] [::sarr (const_length sz)] (@WArray.copy ws p) vs' = ok v.
Proof.
  move=> sz _ _ v [// | v1 v2 [_ /value_uinclE hu /List_Forall2_inv_l -> |]];
    rewrite /app_sopn_v /=; t_xrbindP=> // ??.
  move=> /to_arrI ?; subst v1.
  move: hu => [n2 [t2 [? heq]]]. subst v2.
  move=> hcopy <-.
  move=> /=. have ? := array_eq_len heq; subst n2. rewrite eq_refl WArray.castK /=.
  case: heq => hin1 hin2.
  have := WArray.uincl_copy hin1 hcopy. move=> ->. simpl. done.
Qed.

Lemma vuincl_app_sopn_v tin tout (semi: sem_prod tmap tin (exec (sem_tuple tmap tout))) :
  all is_not_sarr tin ->
...
Lemma vuincl_copy ws p :
  ...
  List.Forall2 value_uincl vs vs' ->
  @app_sopn_v [::sarr (const_length sz)] [::sarr (const_length sz)] (@WArray.copy ws p) vs = ok v ->
  exists2 v' : values, @app_sopn_v [::sarr (const_length sz)] [::sarr (const_length sz)] (@WArray.copy ws p) vs' = ok v' & List.Forall2 value_uincl v v'.
Proof.
...
Section FORALL.
  ...
  Fixpoint mk_forall (l:seq stype) : sem_prod tmap l (exec T) -> Prop := 
    match l as l0 return sem_prod tmap l0 (exec T) -> Prop with 
    | [::] => fun o => forall t, o = ok t -> P t
    | t::l => fun o => forall (x:sem_t tmap t), @mk_forall l (o x)
    end.
  ...
  Fixpoint mk_forall_ex (l:seq stype) : sem_prod tmap l (exec T) -> sem_prod tmap l (exec T) -> Prop := 
    match l as l0 return sem_prod tmap l0 (exec T) -> sem_prod tmap l0 (exec T) -> Prop with 
    | [::] => fun o1 o2 => forall t, o1 = ok t -> exists2 t', o2 = ok t' & P2 t t' 
    | t::l => fun o1 o2 => forall (x:sem_t tmap t), @mk_forall_ex l (o1 x) (o2 x)
    end.
  ...
  Fixpoint mk_forall2 (l:seq stype) : sem_prod tmap l (exec T) -> sem_prod tmap l (exec T) -> Prop := 
    match l as l0 return sem_prod tmap l0 (exec T) -> sem_prod tmap l0 (exec T) -> Prop with 
    | [::] => fun o1 o2 => forall t1 t2, o1 = ok t1 -> o2 = ok t2 -> P2 t1 t2
    | t::l => fun o1 o2 => forall (x:sem_t tmap t), @mk_forall2 l (o1 x) (o2 x)
    end.
