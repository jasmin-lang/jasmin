...
Proof.
  ...
  exact: (Eqdep_dec.inj_pair2_eq_dec _ N.eq_dec).
Qed.

Corollary Varr_inj1 n t t' : @Varr n t = @Varr n t' -> t = t'.
Proof.
  by move=> /Varr_inj [en ]; rewrite (Eqdep_dec.UIP_dec N.eq_dec en erefl).
Qed.
...
Definition interp_safe_cond (vs : values) (sc : safe_cond) :=
  ...
  | AllInit ws n k =>
    forall t, to_arr (Z.to_N (arr_size ws n)) (nth undef_b vs k) = ok t ->
    forall i, (0 <= i < Z.of_N n)%Z ->
