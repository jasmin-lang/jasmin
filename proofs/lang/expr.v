...
Section CMD_RECT.
  ...
  Hypothesis Hcall: forall xs f als es, Pr (Ccall xs f als es).
  ...
  with instr_r_Rect (i:instr_r) : Pr i :=
    ...
    | Ccall xs f als es => @Hcall xs f als es
    end.
...
Record _fundef (extra_fun_t: Type) := MkFun {
  ...
  f_al     : seq (Uint63.int * Ident.ident);
  f_tyin   : seq atype;
  f_params : seq var_i;
  f_body   : cmd;
  f_tyout  : seq atype;
...
Fixpoint eq_instr_r (i1 i2:instr_r) :=
  ...
  | Ccall x1 f1 als1 arg1, Ccall x2 f2 als2 arg2 =>
    all2 eq_lval x1 x2 && (f1 == f2) && (als1 == als2) && all2 eq_expr arg1 arg2
  | _, _ => false
  end
with eq_instr i1 i2 :=
  match i1, i2 with
