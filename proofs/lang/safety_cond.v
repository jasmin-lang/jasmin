...
Definition eint_of_word sg sz e := Papp1 (Oint_of_word sg sz) e.
...
Definition sc_lt e1 e2 := LT e1 e2.
Definition sc_le e1 e2 := LE e1 e2.
Definition sc_eq e1 e2 := EQ e1 e2.
Definition sc_not sc := Not sc.
Definition sc_neq e1 e2 := Not (EQ e1 e2).
Definition sc_and sc1 sc2 := And sc1 sc2.
...
Definition sc_divmod sg sz e1 e2 :=
 let sc := signed [::]
                  [:: sc_not (sc_and (sc_eq e1 (emin_signed sz)) (sc_eq e2 (Pconst (-1)))) ] sg in
 [::sc_neq e2 ezero & sc].
...
Definition sc_arr_init (x:gvar) aa sz e :=
  if is_lvar x then
    ...
    let hi := eaddi lo (Pconst (wsize_size sz - 1)) in
    [:: Is_arr_init (gv x) lo hi]
  else [::].
...
Definition sc_is_aligned_if_m al sz e :=
  ...
  else [:: Is_aligned sz (eint_of_word Unsigned Uptr e)].

Fixpoint sc_e (e : pexpr) : seq safe_cond :=
  match e with
  ...
  | Pload al ws x e =>
    ...
    let sce := sc_e e in
    let plo := Papp2 (Oadd (Op_w Uptr)) (Plvar x) e in
    let phi := Papp2 (Oadd (Op_w Uptr)) plo (Papp1 (Oword_of_int Uptr) (Pconst (wsize_size ws - 1))) in
    let sca := sc_is_aligned_if_m al ws plo in
    let sc_load := [:: Is_mem_init plo phi] in
    scx ++ sce ++ sca ++ sc_load
