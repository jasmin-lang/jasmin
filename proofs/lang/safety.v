(* * Generation of the safety conditions of a program.
   ...
   meant to be discharged under the total semantics ([sem_pexpr (sm := total)],
   see [SemMode] in utils.v), which is the one EasyCrypt implements. *)

From mathcomp Require Import ssreflect ssrfun ssrbool eqtype.
From Coq Require Import ZArith.
Require Import expr compiler_util word op_semi.
...
Definition sc_var (x:var_i) : safety_asserts :=
  if is_aarr (vtype x) then [::]
  else [:: Pis_var_init x].

Definition sc_gvar (x:gvar) : safety_asserts :=
...
(* The safety conditions of an operator, and those of an array or memory
   access, are asserted through the generic translation of [safety_cond]s, with
   the word-to-integer coercion of the source language. *)
Definition safety_cond_to_e (vs : pexprs) (c : safety_cond) : eassert :=
  sc_to_eassert (fun sg ws e => Papp1 (Oint_of_word sg ws) e) vs c.

(* ------------------------------------------------------------------------- *)
(* Arrays and memory                                                          *)

Definition assert_false : safety_asserts := [:: Pexpr (Pbool false)].

(* The conditions under which an access succeeds are the ones the semantics
   itself checks ([safety_cond.v]): they are written on the values of the
   arguments of the access and asserted on the argument expressions.  The error
   attached to each condition plays no role here. *)
Definition sc_access (vs : pexprs) (scs : seq (safety_cond * error)) : safety_asserts :=
  map (safety_cond_to_e vs) (unzip1 scs).
...
Definition sc_get_noinit len al aa ws : seq (safety_cond * error) :=
  take 2 (sc_get len al aa ws).

(* The arguments of an array access, in the order of [sc_get]: the array, then
   the index. *)
Definition sc_arr_get (x:gvar) al aa sz e : safety_asserts :=
...
Definition sc_arr_set (x:var_i) al aa sz e : safety_asserts :=
  match vtype x with
  | aarr ws len => sc_access [:: Plvar x; e] (sc_set (arr_size ws len) al aa sz)
  | _ => assert_false
  end.

Definition sc_arr_get_sub (x:gvar) aa sz len e : safety_asserts :=
  match vtype (gv x) with
  | aarr ws lenx => sc_access [:: Pvar x; e] (sc_get_sub (arr_size ws lenx) aa sz len)
  | _ => assert_false
  end.

Definition sc_arr_set_sub (x:var_i) aa sz len e : safety_asserts :=
  match vtype x with
...
Definition sc_mem al sz e : safety_asserts :=
  safety_cond_to_e [:: e] (sc_mem_aligned al sz 0)
...
Definition sc_op1 (o : sop1) (e : pexpr) : safety_asserts :=
  map (safety_cond_to_e [:: e]) (op1_safe o).

Definition sc_op2 (o : sop2) (e1 e2 : pexpr) : safety_asserts :=
  map (safety_cond_to_e [:: e1; e2]) (op2_safe o).
...
Fixpoint sc_pexpr (e : pexpr) : safety_asserts :=
  match e with
...
Fixpoint sc_eassert (a : eassert) : safety_asserts :=
  match a with
  | Pexpr e => sc_pexpr e
...
Definition sc_lval (lv : lval) : safety_asserts :=
  match lv with
  | Lnone _ _ => [::]
  | Lvar x => [::]
  | Lmem al ws x e =>
...
Definition sc_lvals (lvs:lvals) okmem : safety_asserts :=
  let scs := map sc_lval lvs in
  Pexpr (Pbool (check_xs okmem Sv.empty lvs scs))
    :: flatten scs.

(* ------------------------------------------------------------------------- *)
(* Operators of instructions                                                  *)

Definition get_sopn_safe_conds (es: pexprs) (o: sopn) : safety_asserts :=
  map (safety_cond_to_e es) (get_instr_desc o).(i_safe).
...
Fixpoint sc_instr_ir ii (ir : instr_r) : (safety_asserts * instr_r) :=
  match ir with
  | Cassgn lv _ _ e =>
