...
Definition sc_in_bound' ty e1 e2 :=
  match ty with
  | aarr ws len =>
    [:: eand (elei ezero e1) (elei (eaddi e1 e2) (Pconst (arr_size ws len)))]
  | _ => sc_false
  end.
...
Definition type_of_expr (e:pexpr) : atype :=
  ...
  | Psub al ws len x e => aarr ws len
  ...
  | Pis_var_init _ => abool
  | Pis_mem_init _ _ => abool
  end.
...
Definition sc_arr_init ty x aa sz e :=
  ...
  | aarr ws len =>
    if is_lvar x then
      ...
      [:: PappN (Ois_arr_init (Z.to_pos (arr_size ws len))) [:: Pvar x; lo; Pconst (wsize_size sz)]]
    else
      [::]
  | _ => sc_false
  end.
...
Definition isOarray op :=
  if op is Oarray _ then true else false.
...
Definition get_sopn_wt (es: pexprs) (o: sopn) :=
  let instr_descr := get_instr_desc o in
  Pbool (all2 (fun ty e => convertible (type_of_expr e) ty) instr_descr.(tin) es).
