...
Variant ex3_8 (A B C: Type) (P1 P2 P3 P4 P5 P6 P7 P8: A → B → C → Prop) : Prop :=
| Ex3_8 a b c of P1 a b c & P2 a b c & P3 a b c & P4 a b c & P5 a b c & P6 a b c & P7 a b c & P8 a b c.
...
Lemma sem_callE ii s fn s' :
  ...
  ex3_8
    ...
    (λ f _ _, (f.(f_extra).(sf_return_address) == RAnone) || is_align (top_stack s.(emem)) f.(f_extra).(sf_align))
...
Proof.
  case => { ii s fn s' } ii s s' fn f m1 s2' => ok_f ok_ra ok_sp ok_RSP ok_alloc exec_body ok_RSP' /= ->.
  by exists f m1 s2'.
Qed.
...
Section SEM_IND.
  Variables
    ...
    (Pi_r : instr_info → estate → instr_r → estate → Prop)
    (Pfun : instr_info → estate → funname → estate → Prop).

  Definition sem_Ind_nil : Prop :=
    ∀ s : estate, Pc s [::] s.

  Definition sem_Ind_cons : Prop :=
    ∀ (s1 s2 s3 : estate) (i : instr) (c : cmd),
  ...
  Definition sem_Ind_assgn : Prop :=
    ∀ (ii: instr_info) (s1 s2 : estate) (x : lval) (tag : assgn_tag) ty (e : pexpr) v v',
      ...
      truncate_val ty v = ok v' →
      write_lval gd x v' s1 = ok s2 →
      Pi_r ii s1 (Cassgn x tag ty e) s2.

  Definition sem_Ind_opn : Prop :=
    ∀ (ii: instr_info) (s1 s2 : estate) t (o : sopn) (xs : lvals) (es : pexprs),
      sem_sopn gd o s1 xs es = ok s2 →
      Pi_r ii s1 (Copn xs t o es) s2.

  Definition sem_Ind_if_true : Prop :=
    ∀ (ii: instr_info) (s1 s2 : estate) (e : pexpr) (c1 c2 : cmd),
    sem_pexpr gd s1 e = ok (Vbool true) →
    sem s1 c1 s2 → Pc s1 c1 s2 → Pi_r ii s1 (Cif e c1 c2) s2.

  Definition sem_Ind_if_false : Prop :=
    ∀ (ii: instr_info) (s1 s2 : estate) (e : pexpr) (c1 c2 : cmd),
    sem_pexpr gd s1 e = ok (Vbool false) →
    sem s1 c2 s2 → Pc s1 c2 s2 → Pi_r ii s1 (Cif e c1 c2) s2.

  Definition sem_Ind_while_true : Prop :=
    ∀ (ii: instr_info) (s1 s2 s3 s4 : estate) a (c : cmd) (e : pexpr) (c' : cmd),
    sem s1 c s2 → Pc s1 c s2 →
    sem_pexpr gd s2 e = ok (Vbool true) →
    sem s2 c' s3 → Pc s2 c' s3 →
    sem_i ii s3 (Cwhile a c e c') s4 → Pi_r ii s3 (Cwhile a c e c') s4 → Pi_r ii s1 (Cwhile a c e c') s4.

  Definition sem_Ind_while_false : Prop :=
    ∀ (ii: instr_info) (s1 s2 : estate) a (c : cmd) (e : pexpr) (c' : cmd),
    sem s1 c s2 -> Pc s1 c s2 →
    sem_pexpr gd s2 e = ok (Vbool false) →
    Pi_r ii s1 (Cwhile a c e c') s2.
  ...
  Definition sem_Ind_call : Prop :=
    ∀ (ii: instr_info) (s1 s2: estate) ini res fn args xargs xres,
      mapM get_pvar args = ok xargs →
      mapM get_lvar res = ok xres →
      sem_call ii s1 fn s2 →
      Pfun ii s1 fn s2 →
      Pi_r ii s1 (Ccall ini res fn args) s2.
  ...
  Fixpoint sem_Ind (s1 : estate) (c : cmd) (s2 : estate) (s: sem s1 c s2) {struct s} :
    Pc s1 c s2 :=
    match s in sem s1 c s2 return Pc s1 c s2 with
    | Eskip s0 => Hnil s0
    | @Eseq s1 s2 s3 i c s0 s4 =>
        @Hcons s1 s2 s3 i c s0 (@sem_I_Ind s1 i s2 s0) s4 (@sem_Ind s2 c s3 s4)
    end

  with sem_i_Ind (ii: instr_info) (e : estate) (i : instr_r) (e0 : estate) (s : sem_i ii e i e0) {struct s} :
    Pi_r ii e i e0 :=
    match s in sem_i ii s1 i s2 return Pi_r ii s1 i s2 with
    ...
    | @Eif_true ii s1 s2 e1 c1 c2 e2 s0 =>
      @Hif_true ii s1 s2 e1 c1 c2 e2 s0 (@sem_Ind s1 c1 s2 s0)
    | @Eif_false ii s1 s2 e1 c1 c2 e2 s0 =>
      @Hif_false ii s1 s2 e1 c1 c2 e2 s0 (@sem_Ind s1 c2 s2 s0)
    | @Ewhile_true ii s1 s2 s3 s4 a c e1 c' s0 e2 s5 s6 =>
      @Hwhile_true ii s1 s2 s3 s4 a c e1 c' s0 (@sem_Ind s1 c s2 s0) e2 s5 (@sem_Ind s2 c' s3 s5) s6
          (@sem_i_Ind ii s3 (Cwhile a c e1 c') s4 s6)
    | @Ewhile_false ii s1 s2 a c e1 c' s0 e2 =>
      @Hwhile_false ii s1 s2 a c e1 c' s0 (@sem_Ind s1 c s2 s0) e2
    | @Ecall ii s1 s2 ini res fn args xargs xres hargs hres exec =>
      @Hcall ii s1 s2 ini res fn args xargs xres hargs hres exec (@sem_call_Ind ii s1 fn s2 exec)
    end
  ...
  with sem_call_Ind (ii: instr_info) (s1: estate) (fn: funname) (s2: estate) (s: sem_call ii s1 fn s2) {struct s} : Pfun ii s1 fn s2 :=
    match s with
    | @EcallRun ii s1 s2 fn fd m1 s2' ok_fd ok_ra ok_sp ok_rsp ok_m1 exec ok_rsp' ok_s2 =>
      @Hproc ii s1 s2 fn fd m1 s2' ok_fd ok_ra ok_sp ok_rsp ok_m1 exec (@sem_Ind _ _ _ exec) ok_rsp' ok_s2
    end.
