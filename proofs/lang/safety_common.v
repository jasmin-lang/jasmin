...
(* Translation of a condition on the argument values of an operator into an
   expression (resp. an assertion) on its argument expressions. The coercion
   from words to integers is a parameter: the safety pass applies
   [Oint_of_word], [wint_int] takes the identity.

   An out-of-range variable, or a condition the expression language cannot
   express (an operator outside of [safety_cond_expr], or an [IAppN_safety]
   under an operator), gives [false]: it is then asserted false, which is
   sound. *)
...
Definition sc_to_eassert (vs : pexprs) (c : safety_cond) : eassert :=
  match c with
  | IAppN_safety o cs =>
    if all safety_cond_expr cs && safety_cond_vars_below (size vs) c
    then PappN_safety o (map (sc_to_e vs) cs)
    else Pexpr (Pbool false)
  | _ =>
    Pexpr (if safety_cond_expr c && safety_cond_vars_below (size vs) c
...
(* [check_xs] is generic in the language the conditions are written in:
