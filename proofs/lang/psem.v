...
Inductive sem : forall env, estate env -> cmd -> estate env -> Prop :=
| Eskip env (s : estate env) :
    sem s [::] s

| Eseq env (s1 s2 s3 : estate env) i c :
    sem_I s1 i s2 -> sem s2 c s3 -> sem s1 (i::c) s3

with sem_I : forall env, estate env -> instr -> estate env -> Prop :=
| EmkI env ii i (s1 s2 : estate env) :
    sem_i s1 i s2 ->
    sem_I s1 (MkI ii i) s2

with sem_i : forall env, estate env -> instr_r -> estate env -> Prop :=
| Eassgn env (s1 s2 : estate env) (x:lval) tag ty e v v' :
...
| Eopn env (s1 s2 : estate env) t o xs es:
    sem_sopn gd o s1 xs es = ok s2 ->
    sem_i s1 (Copn xs t o es) s2

| Esyscall env (s1 : estate env) scs m s2 o xs es ves vs:
...
| Eif_true env (s1 s2 : estate env) e c1 c2 :
    sem_pexpr true gd s1 e = ok (Vbool true) ->
    sem s1 c1 s2 ->
    sem_i s1 (Cif e c1 c2) s2

| Eif_false env (s1 s2 : estate env) e c1 c2 :
    sem_pexpr true gd s1 e = ok (Vbool false) ->
    sem s1 c2 s2 ->
    sem_i s1 (Cif e c1 c2) s2

| Ewhile_true env (s1 s2 s3 s4 : estate env) a c e ei c' :
    sem s1 c s2 ->
...
| Ewhile_false env (s1 s2 : estate env) a c e ei c' :
    sem s1 c s2 ->
    sem_pexpr true gd s2 e = ok (Vbool false) ->
    sem_i s1 (Cwhile a c e ei c') s2

| Efor env (s1 s2 : estate env) (i:var_i) d lo hi c vlo vhi :
...
| Ecall env (s1 : estate env) scs2 m2 s2 xs f args vargs vs :
...
with sem_for : forall env, var_i -> seq Z -> estate env -> cmd -> estate env -> Prop :=
| EForDone env (s : estate env) i c :
    sem_for i [::] s c s

| EForOne env (s1 s1' s2 s3 : estate env) i w ws c :
...
| EcallRun scs1 m1 scs2 m2 fn f vargs vargs' (s0 s1 s2 : estate empty_env) vres vres' :
    get_fundef (p_funcs P) fn = Some f ->
    mapM2 ErrType dc_truncate_val (map (eval_atype empty_env) f.(f_tyin)) vargs' = ok vargs ->
    init_state f.(f_extra) (p_extra P) ev (Estate scs1 m1 (Vm.init empty_env)) = ok s0 ->
    write_vars (~~direct_call) f.(f_params) vargs s0 = ok s1 ->
    sem s1 f.(f_body) s2 ->
    get_var_is (~~ direct_call) s2.(evm) f.(f_res) = ok vres ->
    mapM2 ErrType dc_truncate_val (map (eval_atype empty_env) f.(f_tyout)) vres = ok vres' ->
    scs2 = s2.(escs) ->
...
Section SEM_IND.
  Variables
    (Pc   : forall env, estate env -> cmd -> estate env -> Prop)
    (Pi_r : forall env, estate env -> instr_r -> estate env -> Prop)
    (Pi   : forall env, estate env -> instr -> estate env -> Prop)
    (Pfor : forall env, var_i -> seq Z -> estate env -> cmd -> estate env -> Prop)
    (Pfun : syscall_state_t -> mem -> funname -> seq value -> syscall_state_t -> mem -> seq value -> Prop).

  Definition sem_Ind_nil : Prop :=
    forall env (s : estate env), Pc s [::] s.

  Definition sem_Ind_cons : Prop :=
    forall env (s1 s2 s3 : estate env) (i : instr) (c : cmd),
  ...
  Definition sem_Ind_mkI : Prop :=
    forall env (ii : instr_info) (i : instr_r) (s1 s2 : estate env),
      sem_i s1 i s2 -> Pi_r s1 i s2 -> Pi s1 (MkI ii i) s2.

  Hypothesis HmkI : sem_Ind_mkI.

  Definition sem_Ind_assgn : Prop :=
    forall env (s1 s2 : estate env) (x : lval) (tag : assgn_tag) ty (e : pexpr) v v',
      sem_pexpr true gd s1 e = ok v →
      truncate_val (eval_atype env ty) v = ok v' →
      write_lval true gd x v' s1 = ok s2 →
  ...
  Definition sem_Ind_if_true : Prop :=
    forall env (s1 s2 : estate env) (e : pexpr) (c1 c2 : cmd),
    sem_pexpr true gd s1 e = ok (Vbool true) ->
  ...
  Definition sem_Ind_while_true : Prop :=
    forall env (s1 s2 s3 s4 : estate env) a (c : cmd) (e : pexpr) (ei: instr_info) (c' : cmd),
  ...
  Definition sem_Ind_while_false : Prop :=
    forall env (s1 s2 : estate env) a (c : cmd) (e : pexpr) (ei: instr_info) (c' : cmd),
    sem s1 c s2 -> Pc s1 c s2 ->
    sem_pexpr true gd s2 e = ok (Vbool false) ->
  ...
  Definition sem_Ind_for : Prop :=
    forall env (s1 s2 : estate env) (i : var_i) (d : dir)
  ...
  Definition sem_Ind_for_nil : Prop :=
    forall env (s : estate env) (i : var_i) (c : cmd), Pfor i [::] s c s.

  Definition sem_Ind_for_cons : Prop :=
    forall env (s1 s1' s2 s3 : estate env) (i : var_i)
           (w : Z) (ws : seq Z) (c : cmd),
  ...
  Definition sem_Ind_call : Prop :=
    forall env (s1 : estate env) (scs2 : syscall_state_t) (m2 : mem) (s2 : estate env)
  ...
  Definition sem_Ind_proc : Prop :=
    forall (scs1 : syscall_state_t) (m1 : mem) (scs2 : syscall_state_t) (m2 : mem)
           ...
           (s0 s1 s2: estate empty_env) (vres vres': seq value),
      get_fundef (p_funcs P) fn = Some f ->
      mapM2 ErrType dc_truncate_val (map (eval_atype empty_env) f.(f_tyin)) vargs' = ok vargs ->
      init_state f.(f_extra) (p_extra P) ev (Estate scs1 m1 (Vm.init empty_env)) = ok s0 ->
  ...
  Fixpoint sem_Ind env (e : estate env) (l : cmd) (e0 : estate env) (s : sem e l e0) {struct s} :
    Pc e l e0 :=
    match s in sem e1 l0 e2 return Pc e1 l0 e2 with
    | @Eskip _ s0 => Hnil s0
    | @Eseq _ s1 s2 s3 i c s0 s4 =>
        @Hcons _ s1 s2 s3 i c s0 (@sem_I_Ind _ s1 i s2 s0) s4 (@sem_Ind _ s2 c s3 s4)
    end

  with sem_i_Ind env (e : estate env) (i : instr_r) (e0 : estate env) (s : sem_i e i e0) {struct s} :
    Pi_r e i e0 :=
    match s in sem_i e1 i0 e2 return Pi_r e1 i0 e2 with
    | @Eassgn _ s1 s2 x tag ty e1 v v' h1 h2 h3 =>
      @Hasgn _ s1 s2 x tag ty e1 v v' h1 h2 h3
    | @Eopn _ s1 s2 t o xs es e1 =>
      @Hopn _ s1 s2 t o xs es e1
    | @Esyscall _ s1 scs m s2 o xs es ves vs h1 h2 h3 =>
      @Hsyscall _ s1 scs m s2 o xs es ves vs h1 h2 h3
    | @Eif_true _ s1 s2 e1 c1 c2 e2 s0 =>
      @Hif_true _ s1 s2 e1 c1 c2 e2 s0 (@sem_Ind _ s1 c1 s2 s0)
    | @Eif_false _ s1 s2 e1 c1 c2 e2 s0 =>
      @Hif_false _ s1 s2 e1 c1 c2 e2 s0 (@sem_Ind _ s1 c2 s2 s0)
    | @Ewhile_true _ s1 s2 s3 s4 a c e1 ei c' s0 e2 s5 s6 =>
      @Hwhile_true _ s1 s2 s3 s4 a c e1 ei c' s0 (@sem_Ind _ s1 c s2 s0) e2 s5 (@sem_Ind _ s2 c' s3 s5) s6
          (@sem_i_Ind _ s3 (Cwhile a c e1 ei c') s4 s6)
    | @Ewhile_false _ s1 s2 a c e1 ei c' s0 e2 =>
      @Hwhile_false _ s1 s2 a c e1 ei c' s0 (@sem_Ind _ s1 c s2 s0) e2
    | @Efor _ s1 s2 i0 d lo hi c vlo vhi e1 e2 s0 =>
      @Hfor _ s1 s2 i0 d lo hi c vlo vhi e1 e2 s0
        (@sem_for_Ind _ i0 (wrange d vlo vhi) s1 c s2 s0)
    | @Ecall _ s1 scs2 m2 s2 xs f13 args vargs vs e2 s0 e3 =>
      @Hcall _ s1 scs2 m2 s2 xs f13 args vargs vs e2 s0
        (@sem_call_Ind (escs s1) (emem s1) f13 vargs scs2 m2 vs s0) e3
    end

  with sem_I_Ind env (e : estate env) (i : instr) (e0 : estate env) (s : sem_I e i e0) {struct s} :
    Pi e i e0 :=
    match s in sem_I e1 i0 e2 return Pi e1 i0 e2 with
    | @EmkI _ ii i0 s1 s2 s0 => @HmkI _ ii i0 s1 s2 s0 (@sem_i_Ind _ s1 i0 s2 s0)
    end

  with sem_for_Ind env (v : var_i) (l : seq Z) (e : estate env) (l0 : cmd) (e0 : estate env)
         (s : sem_for v l e l0 e0) {struct s} : Pfor v l e l0 e0 :=
    match s in sem_for v0 l1 e1 l2 e2 return Pfor v0 l1 e1 l2 e2 with
    | EForDone _ s0 i c => Hfor_nil s0 i c
    | @EForOne _ s1 s1' s2 s3 i w ws c e1 s0 s4 =>
      @Hfor_cons _ s1 s1' s2 s3 i w ws c e1 s0 (@sem_Ind _ s1' c s2 s0)
         s4 (@sem_for_Ind _ i ws s2 c s3 s4)
    end
...
Lemma semE env (s : estate env) c s' :
  sem s c s' ->
  match c with
  | [::] => s' = s
...
Lemma sem_IE env (s : estate env) i s' :
  sem_I s i s' ->
  let 'MkI ii r := i in
  sem_i s r s'.
Proof. by case. Qed.
...
Lemma sem_iE env (s:estate env) i s' :
  ...
  | Cassgn lv _ ty e =>
    ∃ v v',
    [/\ sem_pexpr true gd s e = ok v, truncate_val (eval_atype env ty) v = ok v' & write_lval true gd lv v' s = ok s' ]
...
Proof.
  case => {env s i s'} //.
  - by move => env s s' x _ ty e v v' hv hv' hw; exists v, v'.
  - by move => env s scs m s' xs o es ves vs h1 h2 h3; exists scs, m, ves, vs.
  - by move => env s s' e th el he hth; exists true.
  - by move => env s s' e th el he hel; exists false.
  - by move => env s si sj s' c e c' ei hc he hc' hrec; exists si, true; constructor => //; exists sj.
  - by move => env s s' c e c' ei hc he; exists s', false.
  - by move => env s s' i d lo hi c vlo vhi hlo hhi hc; exists vlo, vhi.
  by move=> env s scs m s' xs f es vs rs hvs h hrs; exists vs, scs, m, rs.
Qed.

Lemma sem_forE env i ws (s : estate env) c s' :
  sem_for i ws s c s' →
  if ws is w :: ws then
    exists s1 s2,
      [/\
...
Proof.
  case => { env i ws s c s' } // env s s1 s2 s' i w ws c ok_s1 exec_c ih.
  by exists s1, s2.
Qed.
...
Lemma sem_app env l1 l2 (s1 s2 s3 : estate env) :
...
Lemma sem_seq1 env i (s1 s2 : estate env) :
  sem_I s1 i s2 -> sem s1 [::i] s2.
Proof.
  move=> Hi; apply (Eseq Hi);constructor.
Qed.

Lemma sem_seq1_iff env (i : instr) (s1 s2 : estate env) :
  sem_I s1 i s2 <-> sem s1 [:: i] s2.
Proof.
...
Lemma sem_seq_ir env ii ir (s0 s1 : estate env) :
  sem_i s0 ir s1
  -> sem s0 [:: MkI ii ir ] s1.
Proof. by move=> /(EmkI ii) /sem_seq1. Qed.

Lemma esem_sem env c (s s' : estate env) : esem P ev c s = ok s' -> sem s c s'.
Proof.
  set Pi := fun i => forall (s s' : estate env), esem_i P ev i s = ok s' -> sem_I s i s'.
  set Pi_r := fun i => forall ii (s s' : estate env), esem_i P ev (MkI ii i) s = ok s' -> sem_i s i s'.
  set Pc := fun c => forall (s s' : estate env), esem P ev c s = ok s' -> sem s c s'.
  move: s s'.
...
Lemma esem_i_sem env i (s s' : estate env) : esem_i P ev i s = ok s' -> sem_I s i s'.
Proof.
...
Lemma writeP env c (s1 s2 : estate env) :
...
Proof.
  apply:
    (sem_Ind
       ...
       (Pfun := Pfun))
    => {env c s1 s2} //.
  + move=> env s1 s2 s3 i c _ Hi _ Hc z;rewrite write_c_cons => Hnin.
    by rewrite Hi ?Hc //;SvD.fsetdec.
  + move=> env s1 s2 x tag ty e v v' ? hty Hw z.
    by rewrite write_i_assgn;apply (vrvP Hw).
  + move=> env s1 s2 t o xs es; rewrite /sem_sopn.
    case: (Let _ := sem_pexprs _ _ _ _ in _) => //= vs Hw z.
    by rewrite write_i_opn;apply (vrvsP Hw).
  + move=> env s1 scs m s2 x o es ves vs hes hscs hw.
    by rewrite /Pi_r write_i_syscall; apply (vrvsP hw).
  + by move=> env s1 s2 e c1 c2 _ _ Hrec z;rewrite write_i_if => Hnin;apply Hrec;SvD.fsetdec.
  + by move=> env s1 s2 e c1 c2 _ _ Hrec z;rewrite write_i_if => Hnin;apply Hrec;SvD.fsetdec.
  + by move=> env s1 s2 s3 s4 a c e ei c' _ Hc _ _ Hc' _ Hw z Hnin; rewrite Hc ?Hc' ?Hw //;
     move: Hnin; rewrite write_i_while; SvD.fsetdec.
  + move=> env s1 s2 a c e ei c' _ Hc _ z Hnin; rewrite Hc //.
    by move: Hnin; rewrite write_i_while; SvD.fsetdec.
  + by move=> env s1 s2 i d lo hi c vlo vhi _ _ _ Hrec z;rewrite write_i_for;apply Hrec.
  + move=> env s1 s1' s2 s3 i w ws c Hw _ Hc _ Hf z Hnin.
    by rewrite (vrvP_var Hw) ?Hc ?Hf //;SvD.fsetdec.
  move=> env s1 scs2 m2 s2 xs fn args vargs vs _ _ _ Hw z.
  rewrite write_i_call. apply (vrvsP Hw).
Qed.

Lemma write_IP env i (s1 s2 : estate env) :
...
Proof.
  ...
  by move=> ????; apply wequiv_call_rel_eq with (checker_st_eq env) tt.
Qed.
...
Let Pfor env (i:var_i) zs (s1 : estate env) c s2 :=
  forall vm1 X,
    Sv.Subset (read_c c) X ->
      evm s1 =[X] vm1 ->
...
Lemma read_cP env X (s1 : estate env) c s2 vm1 :
  sem p ev s1 c s2 ->
  Sv.Subset (read_c c) X ->
  evm s1 =[X] vm1 ->
  exists2 vm2, sem p ev (with_vm s1 vm1) c (with_vm s2 vm2) & evm s2 =[X] vm2.
Proof.
  ...
  apply : (sem_Ind (Pc := Pc) (Pi := Pi) (Pi_r := Pi_r) (Pfor := Pfor) (Pfun := Pfun)) => {env s1 c s2}.
  + by move=> env s vm1 X hsub heq; exists vm1 => //;constructor.
  + move=> env s1 s2 s3 i c _ ihi _ ihc vm1 X; rewrite read_c_cons => hsub heq1.
  ...
  + move=> env ii i s1 s2 _ ih vm1 X; rewrite read_Ii => hsub heq1.
    by case: (ih vm1 X hsub heq1) => vm2 ??;exists vm2.
  + move=> env s1 s2 x t ty e v v' he htr hw vm1 X.
  ...
  + move=> env s1 scs m s2 o xs es ves vs hes ho hw vm1 X.
    ...
    by move=> z;rewrite read_esE => hz;apply heq1; SvD.fsetdec.
  + move=> env s1 s2 e c1 c2 he _ ih vm1 X.
  ...
  + move=> env s1 s2 e c1 c2 he _ ih vm1 X.
    ...
    rewrite -read_e_eq_on_empty //.
    by rewrite read_eE; apply: eq_onI heq1; SvD.fsetdec.
  + move=> env s1 s2 s3 s4 a c1 e ei c2 _ ih1 he _ ih2 _ ihw vm1 X.
  ...
  + move=> env s1 s2 a c1 e ei c2 _ ih1 he vm1 X.
    rewrite read_i_while => hsub heq1.
    have [|vm2 hs1 heq2]:= ih1 vm1 X _ heq1; first SvD.fsetdec.
    exists vm2 => //; apply: Ewhile_false; eauto.
    rewrite -read_e_eq_on_empty //.
    by rewrite read_eE; apply: eq_onI heq2; SvD.fsetdec.
  + move=> env s1 s2 i d lo hi c vlo vhi hlo hhi _ ih vm1 X.
  ...
  + by move=> env s1 i c vm1 X hsub heq1; exists vm1 => //;constructor.
  + move=> env s1 s2 s3 s4 i z zs c hwi _ ihc _ ihf vm1 X hsub heq1.
  ...
  + move=> env s1 scs2 m2 s2 xs fn args vargs vs hargs hcall _ hw vm1 X.
...
Lemma sem_vm_eq env (s1 : estate env) c s2 vm1:
  sem p ev s1 c s2 ->
  (evm s1 =1 vm1)%vm ->
...
Proof. by move=> env s vm1 Hvm1;exists vm1;split=> //;constructor. Qed.

Local Lemma Hcons : sem_Ind_cons p ev Pc Pi.
Proof.
  move=> env s1 s2 s3 i c _ Hi _ Hc vm1 /Hi [vm2 []] Hsi /Hc [vm3 []] Hsc ?.
  by exists vm3;split=>//;econstructor;eauto.
Qed.

Local Lemma HmkI : sem_Ind_mkI p ev Pi_r Pi.
Proof. by move=> env ii i s1 s2 _ Hi vm1 /Hi [vm2 []] Hsi ?;exists vm2. Qed.

Local Lemma Hasgn : sem_Ind_assgn p Pi_r.
Proof.
  move=> env s1 s2 x tag ty e v v' hsem hty hwr vm1 Hvm1.
...
Proof.
  move=> env s1 s2 t o xs es H vm1 Hvm1; apply: rbindP H => rs;apply: rbindP => vs.
...
Proof.
  move=> env s1 scs m s2 xs o es ves vs hes hex hw vm1 hvm1.
...
Proof.
  move=> env s1 s2 e c1 c2 H _ Hc vm1 Hvm1.
...
Proof.
  move=> env s1 s2 s3 s4 a c e ei c' _ Hc H _ Hc' _ Hw vm1 Hvm1.
...
Proof.
  move=> env s1 s2 a c e ei c' _ Hc H vm1 Hvm1.
  have [vm2 [Hs2 Hvm2]] := Hc _ Hvm1.
  have [v' H1 /value_uinclE ?]:= sem_pexpr_uincl Hvm2 H;subst.
...
Proof.
  move=> env s1 s2 i d lo hi c vlo vhi H H' _ Hfor vm1 Hvm1.
...
Proof. by move=> env s i c vm1 Hvm1;exists vm1;split=> //;constructor. Qed.

Local Lemma Hfor_cons : sem_Ind_for_cons p ev Pc Pfor.
Proof.
  move=> env s1 s1' s2 s3 i w ws c Hi _ Hc _ Hf vm1 Hvm1.
...
Proof.
  move=> env s1 scs2 m2 s2 xs fn args vargs vs Hargs Hcall Hfd Hxs vm1 Hvm1.
...
Lemma sem_i_uincl env (s1 : estate env) i s2 vm1 :
  vm_uincl (evm s1) vm1 ->
  sem_i p ev s1 i s2 ->
  exists vm2,
    sem_i p ev (with_vm s1 vm1) i (with_vm s2 vm2) /\
...
Lemma sem_uincl env (s1 : estate env) c s2 vm1 :
  vm_uincl (evm s1) vm1 ->
  sem p ev s1 c s2 ->
  exists vm2,
    sem p ev (with_vm s1 vm1) c (with_vm s2 vm2) /\
...
Lemma read_es_st_uincl d gd wdb es :
  wrequiv (st_uincl (env:=env) d) ((sem_pexprs wdb gd)^~ es) ((sem_pexprs wdb gd)^~ es) (List.Forall2 value_uincl).
...
Proof. by move=> env s [|//] _; apply Eskip. Qed.

Local Lemma Hcmd_eq_cons : sem_Ind_cons p ev Pc Pi.
Proof.
  move=> env s1 s2 s3 i c _ hi _ hc [//|i' c'] /= /andP [/hi{}hi /hc{}hc].
  by apply Eseq with s2.
Qed.

Local Lemma Hcmd_eq_mkI : sem_Ind_mkI p ev Pi_r Pi.
Proof. by move=> env ii i s1 s2 _ hi [??] /= /hi{}hi; apply EmkI. Qed.

Local Lemma Hcmd_eq_assgn : sem_Ind_assgn p Pi_r.
Proof.
  move=> env s1 s2 x t ty e v v' he htr hw [] //= x' t' ty' e'
...
Proof.
  move=> env s1 s2 t o xs es hopn [] //= t' o' xs' es'
...
Proof.
  move=> env s1 s2 e c1 c2 He Hs Hc [] //= e' c1' c2' /andP[] /andP[] heqe /Hc{}Hc _.
  apply Eif_true => //.
  by rewrite -(eq_exprP _ _ _ heqe).
Qed.
...
Proof.
  move=> env s1 s2 e c1 c2 He Hs Hc [] //= e' c1' c2' /andP[] /andP[] heqe _ /Hc{}Hc.
  apply Eif_false => //.
  by rewrite -(eq_exprP _ _ _ heqe).
Qed.
...
Proof.
  move=> env s1 s2 s3 s4 a c1 e ei c2 sem_s1_s2 H_s1_s2.
...
Proof.
  move=> env s1 s2 a c1 e ei c2 He Hc eq_s_e [] //= a' c1' e' ei' c2'
...
Proof.
  move=> env s1 x c x' c' _ _.
  by apply EForDone.
Qed.

Local Lemma Hcmd_eq_for_cons : sem_Ind_for_cons p ev Pc Pfor.
Proof.
  move=> env s1 s2 s3 s4 [x ?] w ws c eq_s2 sem_s2_s3 H_s2_s3 H_s3_s4 Pfor_s3_s4.
  move=> [x' ?] c' /= <- heqc.
...
Proof.
  move=> env s1 s2 x d lo hi c vlo vhi cpl_lo cpl_hi cpl_for sem_s1_s2.
...
Proof.
  move=> env s1 scs m s2 lv fn args vargs vres hvargs hcall _ hs2.
...
Proof.
  move=> env s1 scs m s2 o xs es ves vs hes ho hw [] //= xs' o' es'
...
Lemma eq_instr_rP env (s1 : estate env) i i' s2 :
  eq_instr_r i i' ->
  sem_i p ev s1 i s2 ->
  sem_i p ev s1 i' s2.
Proof.
...
Lemma eq_instrP env (s1 : estate env) i i' s2 :
  eq_instr i i' ->
  sem_I p ev s1 i s2 ->
  sem_I p ev s1 i' s2.
Proof.
...
Lemma eq_cmdP env (s1 : estate env) c c' s2 :
  eq_cmd c c' ->
  sem p ev s1 c s2 ->
  sem p ev s1 c' s2.
Proof.
