...
At this stage, the only pre- and post-conditions we consider are
permissions associated with zeroization. Basically, we only
distinguish between locations that are only writable and those that
are also readable. Here permissions are meant to abstractly represent
semantic invariants (alternatively, we would need at least fat
pointers). Zeroization is treated as a hybrid operation: on one side,
a data operation defined in terms of 'set'; on the other hand, a
memory operation insofar as it changes a permission.

External calls can change the permissions on their context: in
particular, a call may be passed a write-only location and zeroize
it. This change is meant to be global (as it would be natural for a
return value location), and so we choose to keep a global record of
permissions (what we call the permission map). Notice that of course,
the actual capabilities of the executing function are determined by
the permission map as well as by the limitation to its own frame and
context instance.

Currently for each export function we specify the minimum permissions
for the context instance, and the minimum permissions the instance can
have when the function returns, as signature pre- and post-conditions
(where the order is simply: bottom < write only < top). *)
...
Definition is_WOnly (p: Permission) : bool :=
  match p with
  | WOnly => true
  | _ => false end.                            
...
Definition is_Bot (p: Permission) : bool :=
  match p with
  | Bot => true
  | _ => false end.                            
...
Notation CPerms := (seq Permission) (only parsing).  

Notation CBounds := (PMap -> CSeq -> CPerms -> Prop) (only parsing).

Context (frame_size : Sz) (perm_bounded : CBounds)
        (restrict_pmap : PMap -> Ctx -> PMap).               
...
Class freshSpec : Type := FreshSpec {
      fresh_loc (in_stk: bool) : CSeq -> Sz -> pointer
}.
...
Class fixedMem (FS: freshSpec) (mem: Type) : Type :=
  FixedMem {
...
(* default value *)
Definition empty_cstack (ps: CPerms) (p: pointer) : cStack :=
  CStack nil id (CStackParams ps 0 p).
(* Definition empty_cstack (p: pointer) : cStack :=
  CStack nil id (CStackParams nil 0 p). *)

(* mutable global memory with teh current stack chunk (cstack),
   continuation and global permission map *)
Class globMem (FS: freshSpec) (mem: Type) (FM: fixedMem FS mem) : Type :=
  GlobMem {         
     gcstack : mem -> cStack
   ; gcontinuation : mem -> seq cStack
   ; gpermissions : Ctx -> mem -> PMap
   ...
   ; gstack_top ctx m : pointer := fst (gstack_head m)
                                   
   ; gstack_topP ctx m : in_chunk (fstack_chunk m) (gstack_top m ctx)
                                    
   ; gstack_current_size ctx m : Z :=
       p2Z (gstack_root m) - p2Z (gstack_top m ctx)
}.
...
Class absMem (mem: Type) : Type := AbsMem {
    ...
    ; zeroize_pt : @eMem mem -> pointer -> exec (@eMem mem)
}.
...
Class absMemP (FS: freshSpec) (mem: Type) (FM: fixedMem FS mem)
  ...
  (AM: @absMem mem) : Type := AbsMemP {  
   ...
   (* fresh always succeeds, but then alloc may fail of the chunk is
      out of bounds *)                               
    fresh_loc_stackP m (sz: Sz) :
       let p := fresh_loc true (gsupport m) sz in                              
       chunk_bpred_incl (p, sz) (pt_invalid m) 
 ...
 ; zeroize_ptP m (p: pointer) :
      forall m',  
        (zeroize_pt m p = ok m') ->
        (set m p u8_zero = ok m') /\
        (pt_validWC m p) /\ (pt_validRWC m' p) 

  ; weak_invalid_permP m (p: pointer) (sz: Sz) :
       weak_invalid m p sz = 
           chunk_bpred_incl (p, sz)  
             (fun p0 => is_Bot (gpermissions (base_ctx m) (base_mem m) p0))
             
  ; validZ_permP m (p: pointer) (sz: Sz) :
       validZC m p sz = 
           chunk_bpred_incl (p, sz)  
             (fun p0 => is_WOnly (gpermissions (base_ctx m) (base_mem m) p0))
             
  ; validRW_permP m (p: pointer) (sz: Sz) :
       validRWC m p sz = 
           chunk_bpred_incl (p, sz)  
             (fun p0 => is_Top (gpermissions (base_ctx m) (base_mem m) p0))
}.

(* specification of external calls (pre and post conditions) *)
Class xCallSpec (FS: freshSpec) (mem: Type) (FM: fixedMem FS mem)
  (GM: globMem FM) (CM: @coreMem mem) (CP: @coreMemP mem CM)
  ...
  : Type := XCallSpec {
   ...
   means the program returns *)           
     continuation_head m : cStack :=
      head (empty_cstack (out_ctx_perms (main_pars m)) (stack_root m))
           (gcontinuation m) 
                
  (* before allocation: clr = caller, cle = callee, ctx is the whole
     program context, cleP is determined by the program signature
     (given the function name) *)
   ; initialize_external_call (ctx : Ctx)
         (clr cle: mem) (cleP: expParams): Prop :=
      let cle_size := stack_size cleP in
      let cle_in_perms := in_ctx_perms cleP in
      let cle_out_perms := out_ctx_perms cleP in
      let cle_params :=
        CStackParams cle_out_perms cle_size
          (fresh_loc true (gsupport clr) cle_size) in
      let clr_ctx := gstack_ctx clr in
      let cle_ctx := gstack_ctx cle in
      let clr_pmap := gpermissions (clr_ctx ctx) clr in
      let cle_pmap := gpermissions (cle_ctx ctx) cle in
      
      cle_pmap = restrict_pmap clr_pmap (cle_ctx ctx) /\
      (* input context permission check *)  
      perm_bounded cle_pmap (cle_ctx ctx) cle_in_perms /\

      gpermissions (cle_ctx ctx) clr = gpermissions (clr_ctx ctx) cle /\ 
      gcstack cle = CStack nil cle_ctx cle_params /\
      gcontinuation cle = gcstack clr :: gcontinuation clr

  (* after deallocation *)
   ; finalize_external_call (ctx: Ctx) (cle clr: mem) : Prop :=
      let clr_ctx := gstack_ctx clr in
      let cle_ctx := gstack_ctx cle in
      let cle_pmap := gpermissions (cle_ctx ctx) cle in
      
      gstack cle = nil /\
      (* output context permission check *)          
      perm_bounded cle_pmap (cle_ctx ctx) (cstack_perms (gparams cle)) /\
        
      gpermissions (clr_ctx ctx) clr = gpermissions (cle_ctx ctx) cle /\ 
      gcstack clr = continuation_head cle /\  
      gcontinuation clr = List.tail (gcontinuation cle)     
}.

(* global memory packed with context instance *)
Record cMem (FS: freshSpec) (mem: Type) (FM: fixedMem FS mem)
...
(* all together *)
Class fullMem (FS: freshSpec) (prog mem: Type) (FM: fixedMem FS mem)
  (GM: globMem FM) (CM: @coreMem mem) (CP: @coreMemP mem CM)
...
Class fixedMem (mem: Type) : Type :=
  FixedMem {
    ...
    ; pre_perms : mem -> CPerms
    ; post_perms : mem -> CPerms                       
    ; code_blocks : mem -> seq (pointer * Sz)
    ; stack_size m := p2Z (stack_root m) - p2Z (stack_limit m)
    ; stack_sizeP m : 0 <= (stack_size m)  
    ; stack_chunk m : (pointer * Sz) := (stack_root m, stack_size m) 
}.      
*)
...
(*      let pm0 := concrete_context_head_pmap m0 *)
      ...
      (* the args need to match something in concr_ctx, and the
          ...
          args are stored in the frame and the context is only the
          return value location *)
          check_input_permissions args pm0 msk /\         
          stack_root m1 = fresh_loc m0 true sz0 
                    (* stack_top m0 + 1 *) /\
          stack_size m1 = sz0 }.  
                    (* stack_limit m1 = stack_limit m0 *) /\
      (* still need to check the consistency of m1 *)  
...
(* global memory with concrete stack, permission map, context and code memory.
   If we have just one module, gcontext is empty and the context in
   gstack remains abstract. *)
Class gMem (mem: Type) : Type := GMem {         
      gstack_spec : stackSpec
    ; gstack : mem -> concStack mem
    ; gabsmodspec : absmodSpec                                                
    ; gcontext : seq (pointer * Sz) -> mem -> seq (concStack mem)
    ; gpermissions : mem -> PMap
    ; gcode_mem : mem -> seq (pointer * Sz)
}.
...
(* spec of a memory module: stack size, input and out permissions of
   the context *)
Record absmodSpec : Type := AbsModSpec {
    absmod_size : Sz
  ; absmod_ctx_perms : seq (Permission * Permission)           
}.      

(* program with local functions, export functions and oracle for the
   export functions *)
Class absmodProg (prog: Type): Type := AbsModProg {
    prog_local (pr: prog) : funname -> bool
  ; prog_export (pr: prog) : funname -> bool
  ; prog_oracle (pr: prog) (fn: funname) : prog_export pr fn -> absmodSpec
}.

(* abstract module with a context instance (after call) *)
Record modSpec : Type := ModSpec {
     abs_modspec : absmodSpec  
   ; modspec_context : seq (pointer * Sz)  
}.      

(* runtime module with a stack instance *)
Class stackMod (mem: Type) : Type :=
  StackModMem {
     modspec : modSpec  
   ; stackmod_mem : mem -> seq (pointer * Sz)  
}.      

(* abbreviation for size and limits *)
Record stackSpec : Type :=
  StSpec {
      stspec_root : pointer
    ; stspec_limit :  pointer  
    ; stspec_size := p2Z stspec_root - p2Z stspec_limit
    ; stspec_sizeP : 0 <= stspec_size  
    ; stspec_chunk : (pointer * Sz) := (stspec_root, stspec_size) 
}.      

(* global memory with stack, permission map, context and code memory.
   If we have just one module, gcontext is empty and the context in
   gstack remains abstract. *)
Class gMem (mem: Type) : Type := GMem {         
      gstack_spec : stackSpec           
    ; gstack : mem -> stackMod mem
    ; gpermissions : mem -> PMap
    ; gcontext : mem -> seq (stackMod mem)
    ; gcode_mem : mem -> seq (pointer * Sz)
}.
...
Class permMem (mem: Type) (BM: baseMem mem) (FM: finMem BM)
  ...
  ; validZ_permP (m: mem) (p: pointer) (sz: Sz) :
       validZ m p sz = 
           chunk_bpred_incl (p, sz)  
             (fun p0 => is_WOnly (permissions m p0))
...
Class capMem (mem: Type) (BM: baseMem mem) (FM: finMem BM)
  ...
  ; validWP' (m: mem) (p: pointer) (sz: Sz) (fn: FunName) :
       ...
       validW m p sz <-> 
           chunk_bpred_incl (p, sz)  
             (fun p0 => is_WOnly (capability m p0 fn))
...
Variant Permission1 : Type :=
  Bot1 | FO1 | WO1 | RO1 | WOnly1 | FRO1 | WRO1 | Top1. 

Definition can_write (p: Permission1) : bool :=
  match p with
  | WO1 | WOnly1 | WRO1 | Top1 => true
  | _ => false end.                            
