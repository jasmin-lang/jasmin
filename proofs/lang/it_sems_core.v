...
Proof.
  ...
  + move=> > /=.
    ...
    first by apply interp_iresult.
    move=> len; rewrite interp_bind; apply eqit_bind.
    + setoid_rewrite interp_trigger; rewrite /=; reflexivity.
    move=> [scs' bytes].
    rewrite interp_bind; apply eqit_bind; first by apply interp_iresult.
...
Proof.
  ...
  + move=> ii1 fn1 fs1.
    ...
    + by move=> > ? > ? > ; apply interp_cond_iresult.
    + move=> > _ >. rewrite interp_bind /isem_cond interp_cond_iresult.
      apply eutt_eq_bind => ? /=.
      rewrite /fexec_syscall /=.
      rewrite interp_bind.
      apply eutt_eq_bind'; last first.
      + move=> >; rewrite  interp_cond_iresult; reflexivity.
      rewrite interp_bind interp_cond_iresult.
      apply eutt_eq_bind => ? /=.
      rewrite interp_bind /=. setoid_rewrite interp_trigger.
      apply eutt_eq_bind => ? /=.
