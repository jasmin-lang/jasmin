...
Definition compat_type (sw:bool) :=
  fun st1 st2 => if sw then subtype st1 st2 else eq_op st1 st2.
...
Class Tabstract := {iabstract : string -> Type}.

Definition sem_t {A : Tabstract} (t : stype) : Type :=
  match t with
  | sbool    => bool
  | sint     => Z
  | sarr n   => WArray.array n
  | sword s  => word s
  | sabstract s => iabstract s
  end.

Definition sem_prod {A : Tabstract} ts tr := lprod (map sem_t ts) tr.

Definition sem_ot {A : Tabstract} (t:stype) : Type :=
  if t is sbool then option bool
  else sem_t t.

Definition sem_tuple {A : Tabstract} ts := ltuple (map sem_ot ts).

Fixpoint sem_prod_ok {A : Tabstract} {T: Type} (tin : seq stype) :
  sem_prod tin T -> sem_prod tin (exec T) :=
  match tin return sem_prod tin T -> sem_prod tin (exec T) with
  | [::] => fun o => ok o
  | t :: ts => fun o v => @sem_prod_ok A T ts (o v)
  end.
Arguments sem_prod_ok {A}%type_scope {T}%type_scope tin%seq_scope _.

Fixpoint sem_forall {A : Tabstract} {T: Type} (P: T -> Prop) (tin : seq stype) :
  sem_prod tin T -> Prop :=
  match tin return sem_prod tin T -> Prop with
  | [::] => P
  | t :: ts => fun o => forall v, @sem_forall A T P ts (o v)
  end.
Arguments sem_forall {A}%type_scope {T}%type_scope P%function_scope tin%seq_scope _.

Lemma sem_prod_ok_ok {A : Tabstract} {T: Type} (tin : seq stype) (o : sem_prod tin T) :
  sem_forall (fun et => exists t, et = ok t) tin (sem_prod_ok tin o).
Proof. elim: tin o => /= [o | a l hrec o v]; eauto. Qed.

Lemma sem_prod_ok_error {A : Tabstract} {T: Type} (tin : seq stype) (o : sem_prod tin T) e :
...
Context {A : Tabstract} (T : Type) (of_T : forall t : stype, T -> exec (sem_t t)).
