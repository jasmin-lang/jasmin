...
(* Switch between the two semantics of unsafe operations:
   ...
   - when [true] ([withcatch]), the semantics is the *total* one: an invalid
     array or memory access reads default bytes and drops the bytes it cannot
     write, an undefined variable reads as the default value of its type, and
     an operator or an instruction computes its total version. The only
     remaining error is [ErrType]: it denotes an ill-typed program, not the
...
(* [withcatch] does not preserve [value_uincl]: where [nocatch] fails, the total
   semantics returns a value, which is unrelated to anything.  So the relational rules whose
...
(* Stronger still: the monotonicity theory ([value_uincl]) is simply false under
   [withcatch] — a failing operation returns the result of its total version,
   which is unrelated to the value the more defined side computes.  The rules that rely on it are
