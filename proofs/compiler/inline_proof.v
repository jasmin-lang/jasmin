...
Section SUBSET.
  ...
  Proof.
    by apply (@cmd_rect _ _ Pr Pi Pc Smk Snil Scons Sasgn Sopn Ssyscall Sassert Sif Sfor Swhile Scall).
  Qed.

  Lemma inline_i_subset i : Pr i.
  Proof.
    by apply (@instr_r_Rect _ _ Pr Pi Pc Smk Snil Scons Sasgn Sopn Ssyscall Sassert Sif Sfor Swhile Scall).
  Qed.

  Lemma inline_i'_subset i : Pi i.
  Proof.
    by apply (@instr_Rect _ _ Pr Pi Pc Smk Snil Scons Sasgn Sopn Ssyscall Sassert Sif Sfor Swhile Scall).
  Qed.
...
Proof.
  ...
  + move=> z ? <- [<-] /=.
    rewrite -Hves=> {Hse Hves};case:s => scs sm svm /=.
...
Section PROOF.
  ...
  Proof.
    ...
    have h : svm1 <=[read_e e] vm1 by apply: vmap_uincl_onI Hvm;SvD.fsetdec.
  ...
  Proof.
    ...
    have h : svm1 <=[read_es es] vm1 by apply: vmap_uincl_onI Hvm;SvD.fsetdec.
  ...
  Proof.
    ...
    rewrite read_i_syscall => Hwf Hvm.
    have h : svm1 <=[read_es es] vm1 by apply: vmap_uincl_onI Hvm;SvD.fsetdec.
    have [v2 Hv2 Huv2 {h}] := sem_pexprs_uincl_on' h Hse.
  ...
  Proof.
    ...
    have [||/= vm1' Wvm1' Uvm1']:= @writes_uincl _ _ _ gd _ _ vm1 _ vargs0 vargs0 _ _ Hwv.
  ...
  Proof.
    ...
    move: Hres; have /= <-:= @sem_pexprs_get_var _ _ _ gd svm2 => Hres.
    ...
    by rewrite -(@sem_pexprs_get_var _ _ _ gd {| escs := escs2; emem := emem2; evm := vm2' |}).
  Qed.
  ...
  Proof.
    ...
    apply (@sem_call_Ind _ _ _ _ _ _ p ev Pc Pi_r Pi Pfor Pfun Hskip Hcons HmkI Hassgn Hopn Hsyscall
             ...
             Hfor_cons Hcall Hproc scs mem f va scs' mem' vr Hsem _ Hall).
  Qed.
