...
Proof using lt_0_stk_max halign le_ws_ws_align hstack hsmall hbody rsp_nin.
  ...
  + apply: (lsem_n_eval_lin (n:=5) hbody) => //=.
    ...
    + rewrite /exec_sopn /= truncate_word_u /sopn_sem /sopn_sem_ /= /semi_to_atype_t computational_eq_refl.
      rewrite /x86_MOV.
...
Proof using lt_0_stk_max halign le_ws_ws_align hstack hlarge hbody rsp_nin.
  ...
  + apply: (lsem_n_eval_lin (n:=6) hbody) => //=.
    ...
    rewrite /exec_sopn /= truncate_word_u /= /sopn_sem /sopn_sem_ /= /semi_to_atype_t computational_eq_refl /x86_VMOVDQ.
...
Local Opaque wsize_size Z.of_nat.
  ...
  + apply: (lsem_n_eval_lin1 (n:= 4+n) hbody) => //=.
    ...
    rewrite /exec_sopn /= (@truncate_word_u ws) /= /sopn_sem /sopn_sem_ /= /semi_to_atype_t computational_eq_refl /x86_VMOVDQ.
    rewrite wsize_nle_u64_size_128_256 /=; last by apply /negbTE /negP.
    rewrite /get_var /= hsr.(sr_rsp) /sem_sop2 /= !truncate_word_u /= truncate_word_u /=.
    rewrite hm' /=.
    by rewrite /of_estate /= /lnext_pc /= -addnS.
