...
Section CHECK_SLHO.
  ...
  Definition check_lv_msf ii lv :=
    ...
    Let _ :=
      assert
        (if ox is Some x then vtype x == aword msf_size else true)
  ...
  Fixpoint init_fun_env env (xs:seq var_i) (ttys: seq atype) (tys: seq slh_t) :=
    ...
    | x::xs, t::ttys, ty::tys =>
        Let env :=
          if ty is Slh_msf then
            Let _ := assert ((vtype (v_var x) == aword msf_size) && (t == aword msf_size))
  ...
  Fixpoint check_res env (xs:seq var_i) (ttys: seq atype) (tys: seq slh_t) :=
    ...
    | x::xs, t::ttys, ty::tys =>
      ...
      Let _ := assert (if ty is Slh_msf then t == aword msf_size else true)
