...
Let Pi i :=
  ...
  wequiv_rec (env1:=env) (env2:=env) p p' ev ev uincl_spec (st_uincl tt) [::i] im (st_uincl tt).

Let Pi_r i := forall ii, Pi (MkI ii i).

Let Pc c :=
  let cm := remove_init_c is_reg_array c in
  wequiv_rec (env1:=env) (env2:=env) p p' ev ev uincl_spec (st_uincl tt) c cm (st_uincl tt).

Lemma it_remove_init_fdP_rec c : Pc c.
Proof.
  apply (cmd_rect (Pr := Pi_r) (Pi:=Pi) (Pc:=Pc)) => // {c}; rewrite /Pi_r /Pi /Pc /=.
 + by apply wequiv_nil.
 + by move=> > hi hc /=; rewrite -cat1s; apply wequiv_cat with (st_uincl tt).
 + move=> x tg ty e ii.
   have h : wequiv_rec (env1:=env) (env2:=env) p p' ev ev uincl_spec
...
Definition undef_vm_eq env1 env2 (I : Sv.t) (vm1 : Vm.t env1) (vm2 : Vm.t env2) :=
...
Definition cmpl_inv env (I : Sv.t) := st_rel undef_vm_eq (env1:=env) (env2:=env) I.
...
Let Pi i :=
  forall I,
    ...
    wequiv_rec (env1:=env) (env2:=env) p p' ev ev eq_spec (cmpl_inv I) [::i] im.1 (cmpl_inv im.2).
...
Let Pc c :=
  forall I,
    ...
    wequiv_rec (env1:=env) (env2:=env) p p' ev ev eq_spec (cmpl_inv I) c cm.1 (cmpl_inv cm.2).

Lemma add_init_auxP ii c c' I I' X :
 disjoint I X ->
 wequiv_rec (env1:=env) (env2:=env) p p' ev ev eq_spec (cmpl_inv I) c c' (cmpl_inv I') ->
 wequiv_rec (env1:=env) (env2:=env) p p' ev ev eq_spec (cmpl_inv I) c (Sv.fold (add_init_aux ii) X c') (cmpl_inv I').
Proof.
...
Lemma it_aux I ii1 ii i :
  wequiv_rec (env1:=env) (env2:=env) p p' ev ev eq_spec (cmpl_inv I)
     [:: MkI ii i]
...
Lemma it_add_init_callP_rec c : Pc c.
Proof.
