...
Definition sem_fopn_args env (p : seq lexpr * riscv_op * seq rexpr) (s : estate env) :=
  let: (xs,o,es) := p in
...
Proof.
  ...
  rewrite -(@get_var_neq _ _ _ tmp _ _ (Vword (wrepr U32 imm))) // in hgety.
  rewrite
    (op_sem_fopn_args _ (with_vm _ _) _ _ _ _ (wrepr reg_size imm) hc2 hgety) /with_vm /=;
