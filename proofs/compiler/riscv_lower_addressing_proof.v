...
Lemma compute_addrP env ii (tmp : var_i) e prelude ep (s1 : estate env) we :
...
Lemma Hopn_aux env (s1 s2 : estate env) (t : assgn_tag) (o : sopn) (xs : lvals) (es : pexprs) (ii : instr_info) (tmp : var_i) (vm1 : Vm.t env) X:
...
Proof.
  ...
  have [vm2 hsem eq_vm2] :
     exists2 vm2 : Vm.t env, sem_sopn (p_globs p) o (with_vm s1 vm1) xs es = ok (with_vm s2 vm2) & evm s2 =[X] vm2.
  ...
  + move: hxs => /is_one_LmemP ?; subst xs.
    ...
    have /(_ _ (with_vm s1 vm1) we) := compute_addrP ii _ tmp_ty hcompute.
    rewrite ok_ve /= ok_we.
...
Proof.
  move=> env s X tmp vm1 _ _ _ eq_vm1; exists vm1 => //.
  by apply: Eskip.
Qed.

Local Lemma Hcons : sem_Ind_cons p ev Pc Pi.
Proof.
  move=> env s1 s2 s3 i c _ Hu _ Hc X tmp vm1 tmp_ty tmp_nin; rewrite read_writeE => hsub eq_vm1.
...
Proof.
  move=> env s1 s2 x tag ty e v v' He htr Hw ii X tmp vm1 _ _; rewrite !read_writeE => hsub eq_vm1 /=.
...
Proof.
  ...
  move=> env s1 s2 t o xs es hopn ii X tmp vm1 htytmp hnin hsub heq.
...
Proof.
  move=> env s1 scs mem s2 o xs es ves vs hes ho hw ii X tmp vm1 tmp_ty tmp_nin hsub eq_vm1.
...
Proof.
  move=> env s1 s2 e c1 c2 He _ Hc1 ii X tmp vm1 tmp_ty tmp_nin; rewrite !read_writeE => hsub eq_vm1 /=.
...
Proof.
  move=> env s1 s2 e c1 c2 He _ Hc2 ii X tmp vm1 tmp_ty tmp_nin; rewrite !read_writeE => hsub eq_vm1 /=.
...
Proof.
  move=> env s1 s2 s3 s4 a c e info c' _ Hc He _ Hc' Hw1 Hw ii X tmp vm1 tmp_ty tmp_nin hsub eq_vm1 /=.
...
Proof.
  move=> env s1 s2 a c e info c' _ Hc He ii X tmp vm1 tmp_ty tmp_nin; rewrite !read_writeE => hsub eq_vm1.
  have [|vm2 hsem2 eq_vm2] := Hc X tmp vm1 tmp_ty tmp_nin _ eq_vm1; first SvD.fsetdec.
  exists vm2 => //=.
...
Proof.
  move=> env s1 s2 i d lo hi c vlo vhi Hlo Hhi _ Hfor ii X tmp vm1 tmp_ty tmp_nin; rewrite !read_writeE => hsub eq_vm1.
...
Proof.
  by move=> env s i c X tmp vm1 tmp_ty tmp_nin hsub eq_vm1; exists vm1 => //; constructor.
Qed.

Local Lemma Hfor_cons : sem_Ind_for_cons p ev Pc Pfor.
Proof.
  move => env s1 s1' s2 s3 i w ws c Hw _ Hc _ Hf X tmp vm1 tmp_ty tmp_nin hsub eq_vm1.
...
Proof.
  move=> env s1 scs2 m2 s2 xs fn args vargs vs Hargs Hcall Hfun Hvs
...
Proof.
  ...
  set Pi := fun i =>
    ...
    wequiv_rec (sip:=sip) (env:=env) p p' ev ev eq_spec (st_eq_on X) [::i] (lower_addressing_i tmp i) (st_eq_on X).
  set Pi_r := fun i => forall ii, Pi (MkI ii i).
  set Pc := fun c =>
    Sv.Subset (read_c c) X ->
    wequiv_rec (sip:=sip) (env:=env) p p' ev ev eq_spec (st_eq_on X) c (lower_addressing_c tmp c) (st_eq_on X).
  ...
  + move=> x tg ty e ii; rewrite !read_writeE => hsub.
    apply (wequiv_assgn_rel_eq (sip:=sip)) with (checker_st_eq_on env) X => //.
