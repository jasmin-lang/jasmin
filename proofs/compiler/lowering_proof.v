 ...
 Lemma push_cast_szP sz e s v le:  
  ...
  Lemma mulr_ok l sz w1 w2 (z1 : word w1) (z2:word w2) e1 e2 le1 le2 o e' lte s s' lv: 
    ...
    [ /\ Sv.Subset (read_es e') (read_e (Papp2 (Omul (Op_w sz )) e1 e2)), 
      ...
      LSub (unzip2 vs) = leak_E stk lte (LSub [:: le1; le2]) &
      write_lvals gd s
  ...
  Lemma lower_cassgn_classifyP e l s s' v le ty v' lv (Hs: sem_pexpr gd s e = ok (v, le))
    ...
    | (LowerCopn o a, lte) => 
      ∃ vs, [ /\ sem_pexprs gd s a = ok vs, exec_sopn o (unzip1 vs) = ok ([:: v'], LEmpty) & 
                 LSub (unzip2 vs) = leak_E stk lte le]
    | (LowerInc o a, lte) =>
      ∃ b1 b2 b3 b4 vs, 
             ...
             exists s1' v'' lv',
              [/\ 
               (let v0 : word sz :=
                 ...
                 (LSub lv' = leak_E stk lte lv /\ le = LSub [:: LSub[:: la; lb]; v''.2])]])]),
    ...
    | (LowerConcat hi lo, lte) =>
      ∃ vs,
        [/\ sem_pexprs gd s [:: hi ; lo ] = ok vs,
         exec_sopn Oconcat128 (unzip1 vs) = ok ([:: v' ], LEmpty)
         & LSub (unzip2 vs) = leak_E stk lte le]
  ...
  Proof.
   ...
   + case: o => //.
          ...
          + have hlnot : forall e, match is_lnot e with
            ...
            + move=> hsz1''. case: ifP=> //=.
              + move=> hsz'' /=. rewrite /leak_sop1 /= /truncate_word /= hsz /= /leak_sop1_typed /= in hlo.
                ...
                by case: hlo'=> <-.
              move=> hsz''. by case: (sz) hsz''=> //=.
            move=> hsz1''. by rewrite hsz1 in hsz1''.
    ...
    + case: op => // - [] // - [] //.
      ...
      rewrite /leak_sop1 /=.
      t_xrbindP => zhi ok_zhi [<-].
      move => <- ???? -> <-.
      move => ?? /to_wordI[] szlo [] wlo [] szlo_ge /= -> -> <- _ zlo ok_zlo [<-] <- <- <- ?? /(@ok_inj _ _ _ _) <- <- ?? ??.
      subst => /=.
      case: ok_v' => <-{Hw v'}.
      ...
      rewrite szhi_ge /=.
      congr (ok ([:: (Vword (wrepr _ (word.wcat_r _))) ], _)).
  ...
  Proof.
    ...
    exists s'. exists l2. exists l6. split => //.
    repeat econstructor. rewrite /sem_sopn /= hx /= hr /=.
    by rewrite h1 /= h2 /= h3' /= h4 /= h5 /= h6 -hs'' /=.
   Qed.
  ...
  Proof.
    ...
    + move => sz [d b sc o] /= [hsz] [Hsc] [hrl] [w [? Hslea]]; subst v'; set f := Lnone_b _.
      ...
      case: use_lea.
      (* true *) (* LT_ilea *)
      + rewrite /=. split. constructor. exists s2'. rewrite /= in hes.
      ...
      + subst d; case: eqP => [ ? | _].
        ...
        + (* b is wconst 0 *) (* not done *)
          ...
          split; last by eauto using eq_exc_freshT.
          move: H; rewrite hl.
        ...
        move: hes. rewrite Hvb /= Hvo /= /sem_sop2 /= /leak_sop2 /= Hwo /= Hwb /= /truncate_word /= hsz2 /= /truncate_word /= cmp_le_refl /=.
        rewrite /truncate_word /= cmp_le_refl /=. move=> [] hrs; subst rs. rewrite /= in Hlea'. subst ob oo.
        have Hvb' := (sem_pexpr_var_empty Hvb). have Hvo' := (sem_pexpr_var_empty Hvo). 
        case: b elea hrl Hslea Hvb Hvb' Hlea'=> //=. 
        case: o Hvo Hvo'=> //=.
    ...
    + set vi := var_info_of_lval _.
      ...
      exact Hdisjl.
      move=> s2'' [lcf] [lr] [hls] H Heq /=.
      split; first by constructor.
      exists s2''; split; last by have := eq_exc_freshT Heq Hs2'.
      move: (sem _ _ _) H => P.
      by rewrite /leak_EI -map_comp /comp /= hl lt_composeE.
    (* LowerEq *) (* done *)
    ...
    + move=> d u w s p0 p1 /= [] [va] [vb] [wa] [la] [lb] [hva] hwa hdiv hty' hle1 hle2; subst ty.
      ...
      have [s1'1 [/sem_seq1_iff hsem1 hget heq1]]: exists s1'1,
        [/\ sem p'.1 s1' [:: (MkI ii i1)] [:: Lopn (leak_E stk lt (LSub [:: le ; lw ])) ] s1'1,
      ...
      + rewrite /i1 /va0 /lt; case: (u); eexists; split.
        ...
        + apply: sem_seq1; apply: EmkI;  apply: Eopn; rewrite /sem_sopn /exec_sopn /sopn_sem /= truncate_word_u /=
          ...
          by rewrite /leak_sopn /= /truncate_word /= cmp_le_refl.
      ...
      split. constructor.
      move: (Hex'); rewrite /exec_sopn; t_xrbindP => yt happ lo ok_lo ?; subst v'''.
      exists s1''';split.
      + econstructor; first exact: hsem1.
        apply: sem_seq1; apply: EmkI; apply: Eopn.
        rewrite /sem_sopn /= hget /= Hp0' /= Hp1' /= Hex' /= Hws' /=.
        by rewrite Hle' lt_composeE /= Hlv'.
    ...
    + move => hi lo [] vs [] ok_vs ok_v' ok_lte.
      ...
      apply: sem_seq1; apply: EmkI; apply: Eopn.
      by rewrite /sem_sopn  ok_vs /= ok_v' /= Hw' ok_lte lt_composeE /=.
    (* LowerAssgn *)
  ...
  Lemma lower_addcarry_correct ii si so si' sub sz xs t es x v le lws :
    ...
    ∃ so',
      ...
      (leak_EI stk (lower_addcarry fv sz sub xs t es).2 (LSub [:: LSub (unzip2 x) ; le ; LSub lws]))
       so' ∧
      eq_exc_fresh so' so.
    Proof.
  ...
  Proof.
    ...
    move=> vs Hx [v lv] Hv [s1'' l1''] Hw /= hs1'' hlo ii Hdisj s1' Hs1'.
    ...
    + case: (lower_addcarry_correct ii t (sub:= true) Hs1' Hdisjl Hdisje Hx' Hv Hw').
      ...
      + rewrite /exec_sopn in Hv. move: Hv.
        t_xrbindP=> /=. case: (unzip1 vs)=> //=.
      by rewrite -hs1'';intuition eauto using eq_exc_freshT.
    all: rewrite -hlo; split; [ constructor | apply default ].
  Qed.
