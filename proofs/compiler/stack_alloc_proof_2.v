...
Proof using hmap. by move=> /init_mapP [_ _ [? _ _ _]]. Qed.

Lemma init_map_bounded x1 ofs1 ws1 :
  Mvar.get mglob x1 = Some (ofs1, ws1) ->
  exists2 len1, size_slot x1 = ok len1 & 0 <= ofs1 /\ ofs1 + len1 <= glob_size.
Proof using hmap. by move=> /init_mapP [len1 hlen1 [_ ? _ _]]; exists len1. Qed.
...
Proof using hlayout.
  ...
  + move=> [h1 h2]; split => //; first by clear -hle h1; lia.
    by move=> x1 ofs1 ws1 /h2 [len1 hlen1 [?? h ??]]; exists len1 => //; split=> //; clear -hle h; lia.
  move: hfold.
  ...
  + move=> <- [<- <-].
    ...
    by move=> /hbase2 [{}len2 -> [_ _ h _ _]] _ [<-]; right; clear -h1 h; lia.
  move=> /hbase2 /= [len1 hlen1 [?? h ??]]; exists len1 => //; split=> //.
...
Proof using hlayout. by have [_ _ h] := init_stack_layoutP => /h [_ _ [? _ _ _ _]]. Qed.
...
Proof using hlayout. by have [_ _ h] := init_stack_layoutP => /h [len1 hlen1 _]; exists len1. Qed.

Lemma init_stack_layout_bounded_ioff x1 ofs1 ws1 len1 :
  Mvar.get stack x1 = Some (ofs1, ws1) ->
  size_slot x1 = ok len1 ->
  sao.(sao_ioff) <= ofs1 /\ ofs1 + len1 <= sao.(sao_size).
Proof using hlayout. by have [_ _ h] := init_stack_layoutP => /h [{}len1 -> [_ _ ? _ _]] [<-]. Qed.

Lemma init_stack_layout_bounded x1 ofs1 ws1 len1 :
  Mvar.get stack x1 = Some (ofs1, ws1) ->
  size_slot x1 = ok len1 ->
  0 <= ofs1 /\ ofs1 + len1 <= sao.(sao_size).
Proof using hlayout. by have [h1 _ h] := init_stack_layoutP => /h [{}len1 -> [_ _ h2 _ _]] [<-]; clear -h1 h2; lia. Qed.
