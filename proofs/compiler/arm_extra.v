...
Definition large_load_instr_cc (ws : wsize) (off : Z) : instruction_desc :=
  ...
  let f x b old_x := ok (woff, if b then zero_extend reg_size x else old_x) in
  {|
    ...
    semu :=
      @values.vuincl_app_sopn_v
        [:: sword ws; sbool; sreg ] [:: sreg; sreg ] f refl_equal;
    i_safe := [::];
  |}.
...
Definition large_store_instr_cc (ws : wsize) (off : Z) : instruction_desc :=
  let woff := wrepr reg_size off in
  let f x b old_x := ok (woff, if b then zero_extend ws x else old_x) in
  {|
    str := pp_sz "large_store_cc" ws;
    tin := [:: sword ws; sbool; sword ws ];
    i_in := [:: E 0; Ea 3; E 2 ];
    tout := [:: sreg; sword ws ];
    i_out := [:: E 1; E 2 ];
    conflicts := [:: (APout 0, APin 0) ];
    ...
    semu :=
      @values.vuincl_app_sopn_v
        [:: sword ws; sbool; sword ws ] [:: sreg; sword ws ] f refl_equal;
    i_safe := [::];
  |}.
