...
have h : lowering_options. constructor. constructor. constructor.
have h' : instr_info → warning_msg → instr_info. auto.
have h'' : var_i -> bool. constructor. 
have /= := (mk_leaP (p:= (Build_prog gd [::])) h h' h'' (s:=s) _ hle hle heq). 
have hf : forall f, fvars_correct f [::].
move=> [] /= v1 v2 v3 v4 v5 v6 //=. admit.
(*move=> Hlea. move: (Hlea hf). rewrite /=.  
move=> {Hlea} Hlea. move: (Hlea s  (LSub [:: LEmpty; lo]) (z + z')%R). 
...
Lemma compile_lval ii gd msb_flag max_imm loargs ad ty (vt:sem_ot ty) m m' le s lv1 e1:
  lom_eqv m s ->
  check_arg_dest ad ty ->
  write_lval gd lv1 (oto_val vt) m = ok (m', le) ->
  pexpr_of_lval ii lv1 = ok e1 ->
  check_sopn_dest ii max_imm loargs e1 (ad, ty) ->
  exists s', mem_write_val msb_flag loargs (ad, ty) (oto_val vt) s = ok (s', leak_e_asm le) /\ lom_eqv m' s'.
Proof.
  ...
  + case: lv1 => //=; last by move=> ???? [<-].
    move=> x hw [] <- /= /eqP heq.
    move: hw; rewrite /write_var heq; t_xrbindP => s' vm hvm <- <- <- /= {heq}.
  ...
  + move=> [x xii] /= hw [<-]; rewrite /arg_of_pexpr.
    ...
    rewrite /write_var; t_xrbindP => s' vm hset <- <- <-.
  ...
  move=> wp vp hget hp [wofs l] he vofs /= hofs w hw m1 hm1 <- <- <-.
  ...
  + case: e he hadr=> //=.
    ...
    + move=> op1 e. t_xrbindP=> -[v1 l1] he vo hop hv /= <- h.
      Search _ to_pointer. admit.
...
Lemma compile_lvals ii id_max_imm gd m lvs m' le s loargs 
  ...
  write_lvals gd m lvs (list_ltuple vt) = ok (m', le) ->
  lom_eqv m s ->
  check_sopn_dests ii id_max_imm loargs lvs (zip id_out id_tout) ->
  utils.all2 check_arg_dest id_out id_tout ->
  exists s', 
    mem_write_vals msb_flag s loargs id_out id_tout (list_ltuple vt) = ok (s', flatten (map leak_e_asm le)) ∧ lom_eqv m' s'.
Proof.
  elim : id_out id_tout lvs vt m s m' le.
  (* empty case *)
  + by move=> [] //= []//= _ m s m' le _ [<- <-] hleq _; exists s.
  ...
  move=> m' le. rewrite /write_lvals /=.
  case: lvs => //= lv1 lvs; t_xrbindP => he -[m1 l1] hw1 -[m2 l2] /= hwn <- <- hlo /=.
  ...
  rewrite /mem_write_vals /=. rewrite /mem_write_vals in hrec.
  rewrite /write_lvals in hrec. rewrite /check_sopn_dests in hrec.
  have [s1 [-> /= h2]]:= compile_lval msb_flag hlo hca hw1 he1 hce1.
  case: he=> he.
  move: (hrec tys lvs vtn m1 s1 m2 l2 he hwn h2). rewrite hes /=.
  move=> {hrec} hrec. move: (hrec hces hcall). move=> {hrec} [] m3 [] hrec hlo'.
  exists m3. rewrite hrec /=;split=> //.
Qed.

Lemma compile_x86_opn ii (loargs : seq asm_arg) gd op m s args lvs m' le:
let id := instr_desc op in
...
Proof.
  ...
  + subst op lvs args; t_xrbindP. 
    ...
    + move=> r' v'; rewrite /get_var /on_vu /= /RegMap.set ffunE.
      ...
      move: mk_leaP.
      have hl : lowering_options. + constructor. constructor. constructor.
      have hf : (instr_info → warning_msg → instr_info). + auto.
      move=> Hlp. move: (Hlp {| p_globs := gd; p_funcs := [::] |} hl hf).
      move=> {Hlp} Hlp.
      have hfr : ∀ fv : fresh_vars,
          (var_i → bool)
          → fvars_correct fv (p_funcs {| p_globs := gd; p_funcs := [::] |}).
      + move=> fv /= h. admit.
      admit.
    + move=> r' v'; rewrite /get_var /on_vu /=.
      by rewrite Fv.setP_neq //; apply h3.
...
Proof.
 case: op => //=.
  + move=> sz; rewrite /sem_sopn /exec_sopn /sopn_sem /=.
    ...
    move : compile_lvals. move=> Hc.
    move: (Hc ii id.(id_max_imm) gd m lvs m1 l1 s [:: a0; XMM r; XMM r]
               id.(id_out) id.(id_tout)
               (0%R: word sz)
               MSB_CLEAR (refl_equal _) hw hlo hcd id.(id_check_dest)).
     move=> [] m2 [] hm hlo'. exists m2. split=> //. rewrite /= in hm. by rewrite hm /= cats0.
  + case: lvs => // -[] // x [] //.
    ...
    move => <- u /= htwa [<-] <-; t_xrbindP => [m1 lm] m2 hwx <- [m3 lm'] /= [] <- <- <- /= <- <-.
    ...
    case: a1 hidc hca1=> //=.
    (* Imm *)
    + move=> sz ws h h' [] <- <- /= ht; eexists; split=> //.
      case: hlo=> h1 h2 h3 h4.
      ...
      + move=> r' v''; rewrite /get_var /on_vu /= /RegMap.set ffunE.
        ...
        + rewrite Fv.setP_eq word_extend_reg_id // zero_extend_u => -[<-].
          move: ht; rewrite /truncate_word /=. by move=> [] <-.
          rewrite Fv.setP_neq. 
          + apply h2. 
          by apply /eqP=> H; apply hne; apply var_of_register_inj.
      + move=> r' v''; rewrite /get_var /on_vu /=.
        by rewrite Fv.setP_neq //; apply h3.
      move=> f v''; rewrite /get_var /on_vu /=.
      by rewrite Fv.setP_neq //; apply h4. 
    (* Glob *)
    + move=> g ht h. t_xrbindP=> y hg <- <- //= ht'. rewrite /eval_args_in /=.
      rewrite hg /= ht' /=; eexists; split=> //.
      case: hlo=> h1 h2 h3 h4.
      move: hwx; rewrite /write_var /set_var /=.
      rewrite -xr => -[<-]{m'}.
      constructor => //=.
      + move=> r' v''; rewrite /get_var /on_vu /= /RegMap.set ffunE.
        case: eqP=> [-> | hne].
        + rewrite Fv.setP_eq word_extend_reg_id // zero_extend_u => -[<-].
          by move: ht; rewrite /truncate_word /=.
          rewrite Fv.setP_neq. 
          + apply h2. 
          by apply /eqP=> H; apply hne; apply var_of_register_inj.
      + move=> r' v''; rewrite /get_var /on_vu /=.
      ...
      by rewrite Fv.setP_neq //; apply h4. 
    (* Reg *)
    + move=> r1 ht ha [] <- <- /= ht'; eexists; split=> //.
      case: hlo=> h1 h2 h3 h4.
      move: hwx; rewrite /write_var /set_var /=.
      rewrite -xr => -[<-]{m'}.
      constructor => //=.
      ...
      by rewrite Fv.setP_neq //; apply h4. 
    (* Adr *)
    + move=> adr ht ha /=. t_xrbindP; rewrite /eval_args_in /=.
      move=> y -> /= <- <- /= ht'; eexists; split=> //.
      case: hlo=> h1 h2 h3 h4.
      move: hwx; rewrite /write_var /set_var /=.
      rewrite -xr => -[<-]{m'}.
      constructor => //=.
