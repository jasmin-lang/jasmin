...
Require Import sem_op_typed_facts wint_int safety_common_proof.
Import Utf8.
...
Definition wf_m :=
 forall x,
    ...
    | None => true
    | Some (s, xi) =>
      [/\ is_aword (vtype x), vtype xi = aint &
...
Proof. by case: t1 t2 => [||ws1 l1| [[]|] sz1] [||ws2 l2|[[]|] sz2]. Qed.
...
Lemma to_atypeK sg t : to_atype (to_etype sg t) = t.
Proof. by case: t. Qed.

Lemma get_var_type_of vm (x : var) (v : value) :
  get_var true vm x = ok v → type_of_val v = eval_atype (to_atype (etype_of_var m x)).
Proof.
  rewrite /get_var /etype_of_var; t_xrbindP => /= hdef <-.
  have := Vm.getP vm x; rewrite /compat_val hdef /= => /eqP ->.
  by rewrite to_atypeK.
Qed.

Lemma get_gvar_type_of vm (x : gvar) (v : value) :
  get_gvar true gd vm x = ok v → type_of_val v = eval_atype (to_atype (etype_of_gvar m x)).
Proof.
  rewrite /get_gvar /etype_of_gvar /sign_of_gvar; case: ifP => _.
  + by apply get_var_type_of.
  by move=> /type_of_get_global ->; rewrite to_atypeK.
Qed.

Lemma sem_pexpr_type_of s e v :
  sem_pexpr true gd s e = ok v ->
  type_of_val v = eval_atype (to_atype (etype_of_expr m e)).
Proof.
  case: e => //=; t_xrbindP.
  1-3: by move=> > <-.
  ...
  + by rewrite /sem_opN; t_xrbindP => *; subst; rewrite to_atypeK type_of_to_val.
  + move=> > _ _ > _ htr1 > _ htr2 <-.
    rewrite to_atypeK; case: ifP => _; eauto using truncate_val_has_type.
  all: by move=> > *; subst.
Qed.
...
Proof using FV ep hwf_m m syscall_state.
...
Let P e :=
  ...
  forall v si s, eqst tt si s ->
    sem_eassert gd si (aands ei.1) = ok true ->
...
Let Q es :=
  ...
  forall vs si s, eqst tt si s ->
    sem_eassert gd si (aands eis.1) = ok true ->
...
Lemma wi2i_variP x xi v si s :
  ...
  exists2 v', get_var true (evm s) x = ok v'
            & v = val_to_int (sign_of_var m x) v'.
Proof using FV ep hwf_m m syscall_state. by move => heqs; rewrite /wi2i_vari; t_xrbindP => + <-; apply wi2i_varP. Qed.
...
Proof using FV asm_op ep hwf_m m p sip syscall_state.
...
Lemma wi2i_var_type x :
  in_FV_var FV x → vtype (wi2i_var m x) ≠ aint → vtype x = vtype (wi2i_var m x).
Proof using FV hwf_m m.
  by rewrite /wi2i_var; have := hwf_m x; case: (m x) => // -[sg x'] [_ ->].
Qed.

Lemma wi2i_vari_type x xi:
  wi2i_vari m FV x = ok xi → vtype xi ≠ aint → vtype x = vtype xi.
Proof using FV hwf_m m. by rewrite /wi2i_vari; t_xrbindP => hin <- /=; apply wi2i_var_type. Qed.

Lemma wi2i_gvar_type x xi:
  wi2i_gvar m FV x = ok xi -> vtype (gv xi) <> aint -> vtype (gv x) = vtype (gv xi).
Proof using FV hwf_m m.
...
Lemma esubtype_of_val et t v1 v1':
  ...
  of_val (eval_atype (wi2i_type (sign_of_etype et) t)) (val_to_int (sign_of_etype et) v1) = ok v1' ->
  type_of_val v1 = eval_atype (to_atype et) ->
  exists2 v, of_val (eval_atype t) v1 = ok v & to_val v1' = val_to_int (sign_of_etype et) (to_val v).
Proof.
  move=> hsub htr htyof.
  ...
  have [ws [sg [heq1 ?]]] : exists ws sg, t = aword ws /\ et = ETword _ (Some sg) ws.
...
Lemma esubtype_truncate_val et t v1 v1':
  esubtype (to_etype (sign_of_etype et) t) et ->
  truncate_val (eval_atype (wi2i_type (sign_of_etype et) t)) (val_to_int (sign_of_etype et) v1) = ok v1' ->
  type_of_val v1 = eval_atype (to_atype et) ->
  exists2 v, truncate_val (eval_atype t) v1 = ok v & v1' = val_to_int (sign_of_etype et) v.
Proof.
  rewrite /truncate_val; t_xrbindP => hsub v1_ hof <- htyof.
  have [v -> /= ->] := esubtype_of_val hsub hof htyof; eauto.
Qed.

Lemma to_etype_to_atype et : to_etype (sign_of_etype et) (to_atype et) = et.
Proof. by case: et => // -[]. Qed.

Lemma sc_divmodP si sg sz e1 e2 (w1 w2 : word sz) :
  sem_pexpr true gd si e1 = ok (Vint (int_of_word sg w1)) ->
  sem_pexpr true gd si e2 = ok (Vint (int_of_word sg w2)) ->
  sem_eassert gd si
    (aands (map (sc_to_eassert (fun _ _ e => e) [:: e1; e2]) (sc_divmod sg sz 0 1))) = ok true ->
  [/\ all (safety_cond_holds [:: Vword w1; Vword w2]) (sc_divmod sg sz 0 1),
      int_of_word sg w2 <> 0%Z &
      sg = Signed -> ~(wsigned w1 = wmin_signed sz /\ wsigned w2 = (-1)%Z)].
Proof.
case: sg => /= he1 he2; rewrite ?he1 he2 /= => -[];
  rewrite /safety_cond_holds /= !truncate_word_u /= !andbT.
+ move=> /andP [hn0 hn1].
  have := int_of_word_eqb Unsigned w2 0; have := int_of_word_eqb Signed w2 0.
  rewrite /= wsigned0 wunsigned0 => hs hu.
  rewrite hu -hs hn0 hn1; split => //.
  + by move=> h; move: hn0; rewrite h.
  by move=> _ [h1 h2]; move: hn1; rewrite h1 h2 !Z.eqb_refl.
by move=> hn0; split => // h; move: hn0; rewrite h.
Qed.
...
Lemma wi2i_op2P si s t1 t2 o e1 e2 v1 v2 v :
  ...
  match eval_atype (to_atype t1) with
  | cbool => ∃ b : bool, v1 = Vbool b
  | cint => ∃ i0 : Z, v1 = Vint i0
  | carr len => ∃ a : WArray.array len, v1 = Varr a
  | cword ws => ∃ w : word ws, v1 = Vword w
  end →
  match eval_atype (to_atype t2) with
  | cbool => ∃ b : bool, v2 = Vbool b
  | cint => ∃ i0 : Z, v2 = Vint i0
  | carr len => ∃ a : WArray.array len, v2 = Varr a
  | cword ws => ∃ w : word ws, v2 = Vword w
  end →
  sem_pexpr true gd si e1 = ok (val_to_int (sign_of_etype t1) v1) →
  sem_pexpr true gd si e2 = ok (val_to_int (sign_of_etype t2) v2) →
  sem_eassert gd si (aands (sc_op2 o e1 e2)) = ok true →
...
Opaque esubtype.
  ...
  1-3:
    move=> hsc; rewrite /sem_sop2 /= !truncate_word_u /= => -[<-];
    case: (sg) he1 he2 hsc => /= he1 he2; rewrite he1 he2 /= => -[hr];
    (rewrite (sem_sop2_typed_safeE (v1 := Vword w1) (v2 := Vword w2)) /= ?truncate_word_u //;
      last by rewrite /safety_cond_holds /= !truncate_word_u /= hr);
    (eexists; first reflexivity);
    rewrite /=; (rewrite -(int_of_word_wrepr (sg:=Signed) hr) ||
                 rewrite -(int_of_word_wrepr (sg:=Unsigned) hr));
    rewrite ?wrepr_sub ?wrepr_add ?wrepr_mul ?wrepr_signed ?wrepr_unsigned ?sub_wordE;
    reflexivity.
  + move=> hsc; rewrite /sem_sop2 /= !truncate_word_u /= => -[<-].
    have [hall hw2 hnand] := sc_divmodP he1 he2 hsc.
    rewrite (sem_sop2_typed_safeE (o := Owi2 sg sz WIdiv) (v1 := Vword w1) (v2 := Vword w2) _ _ hall) /= ?truncate_word_u //.
    eexists; first reflexivity.
    rewrite /=; case: sg hw2 hnand {he1 he2 hsc hall} => /= hw2 hnand.
    + rewrite /wdivi wsigned_repr //.
      apply: (Z_quot_bound (half_modulus_pos sz) (wsigned_range w1) (wsigned_range w2)) => //.
      by apply: hnand.
    rewrite /wdiv wunsigned_repr_small //.
    have ? := wunsigned_range w1; have ? := wunsigned_range w2.
    split; first by apply Z.div_pos; Lia.lia.
    apply Z.div_lt_upper_bound; Lia.nia.
  + move=> hsc; rewrite /sem_sop2 /= !truncate_word_u /= => -[<-].
    have [hall hw2 _] := sc_divmodP he1 he2 hsc.
    rewrite (sem_sop2_typed_safeE (o := Owi2 sg sz WImod) (v1 := Vword w1) (v2 := Vword w2) _ _ hall) /= ?truncate_word_u //.
    eexists; first reflexivity.
    rewrite /=; case: sg hw2 {he1 he2 hsc hall} => /= hw2.
    + rewrite /wmodi wsigned_repr //.
      have /(Z_rem_bound (wsigned w1)) := hw2.
      move: (wsigned_range w1) (wsigned_range w2).
      by rewrite /wmin_signed /wmax_signed; Lia.lia.
    rewrite /wmod wunsigned_repr_small //.
    move: (wunsigned_range w1) (wunsigned_range w2) (Z.mod_pos_bound (wunsigned w1) (wunsigned w2)).
    Lia.lia.
  + move=> hsc; rewrite /sem_sop2 /= !truncate_word_u /= => -[<-].
    have [hn _] := wunsigned_range w2.
    have h0 : (0 <=? wunsigned w2)%Z = true by apply/Z.leb_le.
    case: (sg) he1 he2 hsc => /= he1 he2; rewrite he1 he2 /= => -[hr];
    (rewrite (sem_sop2_typed_safeE (v1 := Vword w1) (v2 := Vword w2)) /= ?truncate_word_u //;
      last by rewrite /safety_cond_holds /= !truncate_word_u /= hr);
    (eexists; first reflexivity);
    (rewrite -(int_of_word_wrepr (sg:=Signed) hr) ||
     rewrite -(int_of_word_wrepr (sg:=Unsigned) hr));
    rewrite /= /zlsl h0 /sem_shl /sem_shift (wshl_sem _ hn) GRing.mulrC wrepr_mul
      ?wrepr_signed ?wrepr_unsigned; reflexivity.
  + move=> _; rewrite /sem_sop2 /= !truncate_word_u /= => -[<-].
    by eexists; first reflexivity; rewrite /= int_of_word_shr.
  all: by move=> _; rewrite /sem_sop2 /= !truncate_word_u /= => -[<-]; eexists.
Qed.

Lemma wi2i_eP_ : (forall e, P e) /\ (forall es, Q es).
Proof using FV P Q asm_op ep hwf_m m p sip spp syscall_state.
  ...
  + move=> e he es hes ?; rewrite /wi2i_es /=; t_xrbindP.
    ...
    move=> /aandsE_cat [hei1 heis1]; t_xrbindP => ? hei2 ? heis2 <-.
  ...
  + move=> al aa sz x e he _ /eqP htye xi hxi ei /he{}he <- v si s heqs /= hsc.
    ...
    + rewrite (wi2i_gvar_type hxi).
      + by case: vtype htyx.
      by move=> h; rewrite h in htyx.
  ...
  + move=> o e hrec _ hte ei /hrec{}hrec <- v si s heqs /= /aandsE_cat [hei1 hsco] hei2.
    ...
    + move=> hsc hei2.
      ...
      move: hei2; rewrite he /= /sign_of_expr /= hse val_to_int_None.
      case: etype_of_expr hte (sem_pexpr_type_of he)=> //= _.
      move=> /(sem_pexpr_tovI he) [i ?]; subst v' => /= hei.
      have hr : signed in_uint_range in_sint_range sg sz i.
      + by move: hsc; case: (sg) => /=; rewrite hei /= => -[].
      rewrite /sem_sop1 /= (sem_sop1_typed_safeE (v1 := Vint i)) //=; last first.
      + by case: (sg) hr => /= hr; rewrite /safety_cond_holds /= andbT.
      by eexists; first reflexivity; rewrite /= int_of_word_wrepr.
    ...
    + move=> _; rewrite /sem_sop1 /=; t_xrbindP.
      ...
      move=> _ [<-] w'; rewrite /sem_sop1_typed mk_sem_op1E; t_xrbindP.
      move=> _ <- <-; rewrite wrepr_int_of_word truncate_word_u /=.
    ...
    + rewrite hei2 /= => -[h].
      rewrite (sem_sop1_typed_safeE (v1 := Vword w)) /= ?truncate_word_u //;
        last by rewrite /safety_cond_holds /= truncate_word_u /= h.
      eexists; first reflexivity.
      rewrite /= wsigned_opp //.
      by move=> heq; move: h; rewrite heq Z.eqb_refl.
    rewrite hei2 /= => -[/Z.eqb_eq h].
    rewrite (sem_sop1_typed_safeE (v1 := Vword w)) /= ?truncate_word_u //;
      last by rewrite /safety_cond_holds /= truncate_word_u /= h.
    eexists; first reflexivity.
    have -> : w = 0%R by rewrite -(wrepr_unsigned w) h wrepr0.
    by rewrite /= opp_wordE GRing.oppr0 wunsigned0.

  + move=> o e1 hrec1 e2 hrec2 ei /andP [hte1 hte2].
    move=> ei1 hei1 ei2 hei2 <- /= v si i heqs.
    rewrite !aandsE_cat => -[hei11] [hei21] hsc; t_xrbindP.
  ...
  + move=> t e he e1 he1 e2 he2 ei_ /andP[] hs1 hs2.
    ...
    rewrite !aandsE_cat => -[hei1] [hei11 hei21]; t_xrbindP.
...
Lemma wi2i_eP e : P e.
Proof using FV P Q asm_op ep hwf_m m p sip spp syscall_state. by case wi2i_eP_. Qed.
...
(* Counterpart of [wi2i_condP] for the assertion language. *)
Lemma wi2i_eassertP b a ai :
  wi2i_eassert m FV a = ok ai ->
  forall si s, eqst tt si s ->
    sem_eassert gd si (aands ai.1) = ok true ->
    sem_eassert gd si ai.2 = ok b ->
    sem_eassert gd s a = ok b.
Proof using FV asm_op ep hwf_m m p sip spp syscall_state.
  elim: a ai b => /=.
  + move=> e ai b; t_xrbindP => ce hce <- si s heqs hsc /= hb.
    by apply: (wi2i_condP hce heqs hsc hb).
  + move=> o es ai b; t_xrbindP => hall esi hesi <- si s heqs hsc /=; t_xrbindP.
    move=> vs hsem hvs.
    have [vs'] := wi2i_esP hesi heqs hsc hsem; rewrite /sem_pexprs => {}hsem.
    have hsz := size_mapM hsem; rewrite hsem => hvs' /=.
    suff -> : vs' = vs by [].
    rewrite hvs' => {hvs hvs' hsem hesi}; elim: es vs' hall hsz => [ | e es hrec] [ | v' vs'] //=.
    by move=> /andP[]/eqP -> /hrec h [] /h <-; rewrite val_to_int_None.
  + move=> x ai b; t_xrbindP => xi hxi <- si s heqs _ /=.
    move: hxi; rewrite /wi2i_vari; t_xrbindP => hin <- /=.
    rewrite !Vm.is_var_initE; case: heqs => _ _ /(_ _ hin) ->.
    by rewrite is_defined_val_to_int => ->.
  + move=> e1 e2 ai b; t_xrbindP => /andP[/eqP hte1 /eqP hte2] ce1 hce1 ce2 hce2 <- si s heqs /=.
    rewrite aandsE_cat => -[hsc1 hsc2] /=; t_xrbindP.
    move=> w vi1 hsem1 hto1 i vi2 hsem2 hto2 <-.
    have [v1] := wi2i_eP hce1 heqs hsc1 hsem1.
    have [v2] := wi2i_eP hce2 heqs hsc2 hsem2.
    rewrite /sign_of_expr hte1 hte2 !val_to_int_None => -> /= ? -> /= ?; subst vi1 vi2.
    rewrite hto1 hto2 /=.
    by case: heqs => _ <-.
  move=> a1 hrec1 a2 hrec2 ai b; t_xrbindP => ai1 hai1 ai2 hai2 <- si s heqs /=.
  rewrite aandsE_cat => -[hsc1 hsc2] /=; t_xrbindP => b1 hb1 b2 hb2 <-.
  by rewrite (hrec1 _ _ hai1 _ _ heqs hsc1 hb1) (hrec2 _ _ hai2 _ _ heqs hsc2 hb2).
Qed.

Lemma sign_to_etype_type_of ty sg sg' v :
  type_of_val v = eval_atype ty ->
  sign_of_etype (to_etype sg ty) = Some sg' ->
  sg = Some sg' /\
  exists ws, v = undef_w \/ exists (w:word ws), v = Vword w.
Proof.
  move=> hty; have := (@type_of_valI v _ erefl); rewrite hty => {hty}.
  case: ty => //=.
  move=> ws h; case: sg => // ? [->]; eauto.
Qed.

Lemma is_awordP ty : is_aword ty -> exists ws, ty = aword ws.
Proof. case: ty => //; eauto. Qed.

Lemma eval_atypeI ty :
  match eval_atype ty with
  | cbool => ty = abool
  | cint  => ty = aint
  | carr len => exists ws len', ty = aarr ws len' /\ len = arr_size ws len'
  | cword ws => ty = aword ws
  end.
Proof. by case: ty => // ws len'; exists ws, len'. Qed.

Lemma wi2i_lvarP x xi ty ety si si' s v :
  eqst tt si s ->
  type_of_val v = eval_atype ty ->
  wi2i_lvar m FV (to_etype (sign_of_etype ety) ty) x = ok xi ->
  write_var true xi (val_to_int (sign_of_etype (to_etype (sign_of_etype ety) ty)) v) si = ok si' ->
  exists2 s' : estate,
      write_var true x v s = ok s' &
      eqst tt si' s'.
Proof using FV ep hwf_m m syscall_state.
  rewrite /wi2i_lvar /wi2i_vari; t_xrbindP => heqs heval hsub hin ?; subst xi.
  move: hsub; rewrite /wi2i_vari /wi2i_var /etype_of_var /sign_of_var.
  case heqm: m (hwf_m x) => [[sg xi]| ] /=.
  + move=> [/is_awordP [sw hxty] htxi hdiff] hsub.
    have := (esubtype_sign_of hsub); rewrite hxty /=.
    move=> hsig; rewrite hsig.
    move=> /write_varP [-> hdb htr].
    have [heq [ws [? | [w ?]]]] := sign_to_etype_type_of heval hsig; subst.
    + by move: hdb; rewrite /DB.
    move: hsub; rewrite /esubtype hxty heq /=.
    have := eval_atypeI ty; rewrite -heval /= => -> /andP[_ /eqP?]; subst sw.
    rewrite /val_to_int /=.
...
Lemma wi2i_lvP ty ety lv lvi si si' s v:
  eqst tt si s ->
  type_of_val v = eval_atype ty ->
  let ety := to_etype (sign_of_etype ety) ty in
  wi2i_lv m FV ety lv = ok lvi ->
  sem_eassert gd si (aands lvi.1) = ok true ->
  write_lval true gd lvi.2 (val_to_int (sign_of_etype ety) v) si = ok si' ->
  exists2 s', write_lval true gd lv v s = ok s' & eqst tt si' s'.
Proof using FV asm_op ep hwf_m m p sip spp syscall_state.
  move=> heqs heval; case: lv => /=.
  + move=> i ty'; t_xrbindP.
    move=> hsub <- /= hsc /write_noneP [-> htr hdb]; exists s => //=.
    rewrite /write_none.
    case heq: sign_of_etype htr hdb => [sg | ] /=.
    + have [_ [ws [ ? | [w ?]]]] := sign_to_etype_type_of heval heq; subst v.
      + by rewrite /DB.
      rewrite /= => _ _.
      rewrite heq in hsub.
      move: heq hsub => /=.
      have := eval_atypeI ty; rewrite -heval /= => -> /=.
      case: sign_of_etype => // _ [->].
  ...
  + t_xrbindP => a aa ws x e /and3P[hinx /eqP hse /eqP hsety].
    ...
    have hmx : m x = None.
    + have := hwf_m x; case: m => // -[sg ?] [].
      by have := eval_atypeI (vtype x); rewrite htx => -[?] [?] [->].
...
Lemma wi2i_lvsP msg ety lvs lvis si0 si si' s vs okmem:
  ...
  sem_eassert gd si0 (aands lvis.1) = ok true ->
  (evm si0 = evm si) ->
  (okmem -> emem si0 = emem si) ->
  eqst tt si s ->
  write_lvals true gd si lvis.2 (map2 (fun ety v => val_to_int (sign_of_etype ety) v) ety vs) = ok si' ->
  List.Forall2 (fun ety v => exists2 ty, type_of_val v = eval_atype ty &
                             ety = (to_etype (sign_of_etype ety) ty)) ety vs ->
  exists2 s',
    write_lvals true gd s lvs vs = ok s' &
    eqst tt si' s'.
Proof using E E0 FV asm_op ep hwf_m m p rE sip spp syscall_state wE.
  ...
  rewrite aandsE_cat => -[hsc hscs] hvm hokm heqs si1 hw hws /List_Forall2_inv [ [ty heval heq] hall] /=.
  rewrite heq in hlvi, hw.
  rewrite (check_scaP gd hmemok hdisj hvm hokm) in hsc.
  have [s1 -> /= heqs2 ]:= wi2i_lvP heqs heval hlvi hsc hw.
...
Lemma wi2i_esP_none es sce scs :
  ...
  wrequiv (λ si s : estate, eqst tt si s ∧ sem_eassert gd si (aands (sce.1 ++ scs)) = ok true)
    ((sem_pexprs true gd)^~ sce.2) ((sem_pexprs true gd)^~ es) eq.
Proof using FV asm_op ep hwf_m m p sip spp syscall_state.
  move=> hnone hsce si s vsi [heqs /aandsE_cat[hsce1 hscx1]] hsce2.
...
Lemma wi2i_lvsP_none msg vs tys xs scx scs :
  map type_of_val vs = map eval_atype tys →
  wi2i_lvs m FV msg true [seq to_etype None i | i <- tys] xs = ok scx →
  wrequiv (λ si s : estate, eqst tt si s ∧ sem_eassert gd si (aands (scs ++ scx.1)) = ok true)
    (λ s1 : estate, write_lvals true gd s1 scx.2 vs) (λ s2 : estate, write_lvals true gd s2 xs vs)
    (eqst tt).
Proof using E E0 FV asm_op ep hwf_m m p rE sip spp syscall_state wE.
  move=> heq hscx si s si' [heqs /aandsE_cat [hsce1 hscx1]] hw.
  ...
  + have -> //: map2 (λ ety v, val_to_int (sign_of_etype ety) v) [seq to_etype None i | i <- tys] vs = vs.
    ...
    + by have := f_equal size heq; rewrite !size_map.
    elim : (vs) (tys) => [ | ?? hrec] [|??] //= [/hrec ->].
    by rewrite sign_of_to_etype_None val_to_int_None.
  elim : (vs) (tys) heq => [ | ?? hrec] [| ??] //= [{}heq hres]; constructor.
  + by eexists; eauto; rewrite sign_of_to_etype_None.
  by apply hrec.
Qed.

Context (p_funcsi : ufun_decls)
        (sigs : funname → option (seq (extended_type Z) * seq (extended_type Z)))
...
Let Pi_r ir :=
  ...
  wequiv_rec pi p ev ev wi2i_spec
    (fun si s => eqst tt si s /\ sem_eassert (p_globs pi) si (aands sci.1) = ok true)
...
Lemma esubtype_truncate_vals xs tys vs vis' :
  ...
  size xs = size tys →
  List.Forall2 (fun ety v => exists2 ty, type_of_val v = eval_atype ty &
                               esubtype ety (to_etype (sign_of_etype ety) ty)) etys vs →
  mapM2 ErrType truncate_val
      (map (λ ety, eval_atype (wi2i_type (sign_of_etype ety) (to_atype ety))) etys)
  ...
  exists2 vs',
    ...
    map eval_atype tys = map type_of_val vs'.
Proof.
  rewrite /=.
  elim: tys vs xs vis' => [|ty tys hrec] [|v vs] [| x xs] //= .
  + by move=> ? _ _ [<-]; eauto.
  + by move=> ? _ /List_Forall2_inv.
  move=> vis'_ [hsize] /List_Forall2_inv [] [ty1 hty1 hsub] hsubs; t_xrbindP => vi' htr vis' htrs <-.
  have [hsign htoe1 hto2]:
    [/\ sign_of_etype (to_etype (sign_of_var m x) ty) =
        sign_of_etype (to_etype (sign_of_var m x) ty1)
      , to_etype (sign_of_etype (to_etype (sign_of_var m x) ty1)) ty = to_etype (sign_of_var m x) ty
      & to_etype (sign_of_etype (to_etype (sign_of_var m x) ty)) ty1 =
          to_etype (sign_of_var m x) ty1].
  + case: (sign_of_var m x) hsub => [sg | ]; last first.
    + by rewrite !sign_of_to_etype_None.
    by case: (ty) (ty1) => [||ws1 len1|ws1] [||ws2 len2|ws2].
  have := esubtype_truncate_val (et := to_etype (sign_of_var m x) ty1) (t:=ty) (v1:= v) (v1':= vi').
  rewrite !to_atypeK in htr |- *.
  rewrite htoe1 -hsign; rewrite hto2 in hsub.
  move=> /(_ hsub htr hty1) [v' h1 ->] /=; rewrite h1.
  have <- := truncate_val_has_type h1.
  have [vs' -> [-> /= ->] ] := hrec _ _ _ hsize hsubs htrs.
  by eexists; first reflexivity.
Qed.

Lemma wi2i_write_vars msg xs ixs ixsi tys vs :
  let etys := map2 (λ (x : var_i) ty, to_etype (sign_of_var m x) ty) xs tys in
...
Proof using FV ep hwf_m m syscall_state.
  rewrite /=.
  elim: vs xs tys ixs ixsi => [|v vs hrec] //= [| x xs] //= [|ty tys] //= [|ix ixs] ixsi_ //=.
  + by move=> _ _ [<-] si s si' heqs [<-]; exists s.
  move=> [hsize]; t_xrbindP.
  move=> [heqty heqtys] ixi hixi ixis hixis <- si s si' heqs /=; t_xrbindP.
  move=> si1 hw hws.
  have heq : forall ty,
  ...
  have [s1 -> /= heqs1]:= wi2i_lvarP heqs heqty hixi hw.
  have [s' -> ?] := hrec _ _ _ _ hsize heqtys hixis _ _ _ heqs1 hws.
  by exists s'.
Qed.
...
Proof using FV ep hwf_m m syscall_state.
  ...
  by move=> [] /is_awordP [ws ->] -> _ /=; apply undef_x_vundef.
Qed.
...
Proof using FV asm_op ep hwf_m m p sip spp syscall_state.
  ...
  move=> z he <- _ b hsem hb _ /=.
  move: hsem; have -> : b = true by case: b hb.
  rewrite -cats1 => /aandsE_cat [hsc ha2].
  by rewrite (wi2i_eassertP he heqs hsc ha2).
Qed.
...
Proof using FV asm_op ep hp' hsig hwf_m m p p_funcsi pi sigs sip spp syscall_state.
  ...
  rewrite -map_comp in hvpari.
  have [vs' -> [? htys]] := esubtype_truncate_vals hsizei hsub hvpari => /=; subst vparitr.
  have [s' -> heqs /=]:= wi2i_write_vars hsizei (esym htys) hfipar (eqst_init hscs hmem) hw.
  move: hassert; rewrite /= mapM_cat; t_xrbindP => ? hassert _ _ _.
  apply: wi2i_a_andP heqs hfpre hassert.
Qed.

Lemma eq_to_etype_eval s1 s2 ty1 ty2 :
  to_etype s1 ty1 = to_etype s2 ty2 -> eval_atype ty1 = eval_atype ty2.
Proof.
  by case: ty1 ty2 => [||w1 len1|w1] [||w2 len2|w2] //= [] h1 h2; rewrite h2 ?h1.
Qed.
...
Proof using FV asm_op ep hp' hsig hwf_m m p p_funcsi pi sigs sip spp syscall_state.
  ...
  rewrite hval1 -map_comp in hvpari.
  have [vs' -> [? htys]] := esubtype_truncate_vals hsizei hsub1 hvpari => /=; subst vparitr.
  have [s' -> heqs' /=]:= wi2i_write_vars hsizei (esym htys) hfipar (eqst_init hscs hmem) hw1.
  have [hftyo hsize] : map type_of_val (fvals fr) = map eval_atype ftyo /\
                       size fres = size [seq type_of_val i | i <- fvals fr].
  + elim : (fres) (ftyo) (fvals fr) hsizeo heq2 => [|x xs hrec] [|ty tys] //= [|vr vrs] //=.
    1,2: by move=> _ /List_Forall2_inv.
    move=> [/hrec{}hrec] /List_Forall2_inv [] [ty0 h1 h2] /hrec [] -> ->.
    by rewrite -(to_atypeK (sign_of_var m x) ty) h1 to_atypeK (eq_to_etype_eval h2).
  rewrite hval2 /= in hw2.
  rewrite hftyo size_map in hsize.
  have [s -> heqs /=] := wi2i_write_vars hsize hftyo hfires heqs' hw2.
  move: hassert; rewrite /= mapM_cat; t_xrbindP => ? hassert _ _ _.
...
Lemma wi2i_get_var_is msg xs tys xis vis vis' si s :
  let etys := map2 (λ (x : var_i) ty, to_etype (sign_of_var m x) ty) xs tys in
  let f_tyout := map (λ ety, wi2i_type (sign_of_etype ety) (to_atype ety)) etys in
  eqst tt si s →
  size xs = size tys →
  ...
  mapM2 ErrType dc_truncate_val (map eval_atype f_tyout) vis = ok vis' →
  exists vs vs',
    [/\ get_var_is (~~ direct_call) (evm s) xs = ok vs
      , mapM2 ErrType dc_truncate_val (map eval_atype tys) vs = ok vs'
      , vis' =  map2 (λ ety v, val_to_int (sign_of_etype ety) v) etys vs'
      &  List.Forall2 (fun ety v => exists2 ty, type_of_val v = eval_atype ty &
                                      ety = to_etype (sign_of_etype ety) ty) etys vs'].
Proof using FV ep hwf_m m syscall_state.
  ...
  rewrite to_atypeK -(esubtype_sign_of hsub) sign_of_etype_var in htr.
  ...
  constructor => //.
  rewrite (truncate_val_has_type htr') -(esubtype_sign_of hsub) sign_of_etype_var.
  by exists ty.
Qed.

Lemma is_polymorphic_opP o :
  match is_polymorphic_op o with
  | IsSpill k tys => o = Opseudo_op (Ospill k tys)
  | IsSwap ty => o = Opseudo_op (Oswap ty)
  | IsDeclassify ty => o = Opseudo_op (Odeclassify ty)
  | IsOther => true
  end.
Proof. by case: o => // -[]. Qed.

Lemma oto_val_sem_prod_id t (w : sem_t t) : oto_val (sem_prod_id w) = to_val w.
Proof. by case: t w. Qed.

Lemma wi2i_callP_aux fn : wiequiv_f pi p ev ev (rpreF (eS:= wi2i_spec)) fn fn (rpostF (eS:=wi2i_spec)).
Proof using E E0 FV Pc Pi Pi_r asm_op ep ev hp' hsig hwf_m m p p_funcsi pi rE sigs sip spp syscall_state wE.
  apply wequiv_fun_ind_wa => {}fn _ fsi fs [<- [fsig hfsig hrel]] fdi hgeti.
  ...
  set fd := {| f_contract := fc |}. set fdi:= {|f_contract := fc'|} => /=.
  move: hinit; rewrite /initialize_funcall /=; t_xrbindP => vis htr hw.
  rewrite hval1 -map_comp in htr.
  have [vs' -> [? htys]] := esubtype_truncate_vals hsizei hsub1 htr; subst vis.
  have [s /= -> heqs /=] := wi2i_write_vars hsizei (esym htys) hipar (eqst_init hscs hmem) hw.
  ...
  + move=> si1 s1 fr1 heqs1; rewrite /finalize_funcall.
    ...
    eexists; first reflexivity.
    by case: heqs1 => <- <- _.
  clear s heqs htys fd fdi hires hipar hsizeo hfsig hval1 hsub1 hfc' hsizei htr hw vs' fc hmem hscs ires
  ...
  + move=> i ii hi ci_; t_xrbindP => sci /add_iinfoP /(hi ii) {}hi <-.
    ...
    apply wequiv_cat with (λ si s : estate, eqst tt si s ∧ sem_eassert (p_globs pi) si (aands sci.1) = ok true).
  ...
  + t_xrbindP => x tg ty e ii sci hsub scx hscx sce hsce <- /=.
    ...
    move=> si s si' [heqs /aandsE_cat [hsce1 hscx1]]; rewrite /sem_assgn; t_xrbindP.
    ...
    apply: (wi2i_lvP (ety := etype_of_expr m e) (v:=v') heqs _ _ hscx1).
    + by rewrite (truncate_val_has_type htr').
    + by apply hscx.
    by rewrite -(esubtype_sign_of hsub).
  + t_xrbindP => xs tg o es ii sci sce hsce [otys o'].
    case: is_polymorphic_op (is_polymorphic_opP o).
    + move=> k tys -> /=; t_xrbindP => /eqP hsze hsub <- <- /= scx hscx <-.
      apply wequiv_opn with
       (fun vsi vs =>
          map type_of_val vs = map (fun e => eval_atype (to_atype (etype_of_expr m e))) es /\
          vsi = map2 (λ (e : pexpr) (v : value), val_to_int (sign_of_expr m e) v) es vs)
       (fun vsi vs => vsi = vs /\
          map type_of_val vsi = map eval_atype (sopn_tout (Opseudo_op (Ospill k tys)))).
      + move=> si s vsi [heq /aandsE_cat [] hconde hcondx] hsei.
        have [vs hse ->]:= wi2i_esP hsce heq hconde hsei.
        exists vs => //; split => //.
        elim: (es) vs hse => [|e' es' hrec'] /= vs.
        + by move=> -[<-].
        t_xrbindP => v' hv' vs' hvs' <- /=.
        by rewrite (sem_pexpr_type_of hv') (hrec' _ hvs').
      + move=> si s [heq /aandsE_cat [] hconde hcondx] ? vs vis [heval ->].
        rewrite /exec_sopn /= /sopn_sem_ /= /sopn_tout => {hsce}; t_xrbindP.
        move=> [] h <-.
        exists [::] => //.
        move: h; rewrite /semi /= /mk_semi.
        generalize (@nil value) at 1; intros acc1.
        generalize (@nil value) at 2; intros acc2.
        elim: tys es vs heval hsze hsub acc1 acc2 => [ | ty tys hrect] [| e es] => //=.
        + by case.
        case => // v vs [] htof htofs [] hsze /andP [hsub1 hsub] acc1 acc2.
        t_xrbindP => ov hov h.
        have [? -> _ /=]:= esubtype_of_val hsub1 hov htof.
        by apply: hrect htofs hsze hsub _ _ h.
      move=> [|??] _ [<- ] //= _ si s si'.
      by case: xs hscx => //= -[?]; subst scx => /= -[??] [<-]; exists s.
    + move=> ty ->; t_xrbindP => hsub <- <-.
      set (sig := [:: to_etype (sign_of_expr _ _) _; _]).
      move=> rscx hscx <- /=.
      apply wequiv_opn with
       (fun vsi vs =>
          map type_of_val vs = map (fun e => eval_atype (to_atype (etype_of_expr m e))) es /\
      ...
      + move=> si s vsi [heq /aandsE_cat [] hconde hcondx] hsei.
        ...
        by rewrite (sem_pexpr_type_of hv') (hrec' _ hvs').
      + move=> si s [heq /aandsE_cat [] hconde hcondx] ? vs vis [heval ->].
        rewrite /exec_sopn /= /sopn_sem_ /= /sopn_tout /sig => {hsce}; t_xrbindP => {hscx sig}.
        case: es hsub heval => // e1 [] //=.
        + by case: vs => // v1 [] //; t_xrbindP.
        move=> e2 [] //=; last first.
        + by case: vs => // v1 [] // v2 [] //; t_xrbindP.
        move=> /and3P [] hsub1 hsub2 _.
        case: vs => // v1 [] // v2 [] //; t_xrbindP.
        move=> [htyv1 htyv2] [u1 u2] w1 hw1 w2 hw2 [???]; subst u1 u2 vis.
        have [w1' -> /= htow1'] := esubtype_of_val hsub1 hw1 htyv1.
        rewrite /safety_cond_holds /= !filter_ot_true.
        set u := oto_val (sem_prod_id w1).
        have hs: sign_of_expr m e1 = sign_of_expr m e2.
        + by rewrite /sign_of_expr (esubtype_sign_of hsub1) (esubtype_sign_of hsub2).
        move: w2 hw2 htyv2 hsub2; rewrite hs => w2 hw2 htyv2 hsub2; rewrite /u => {u}.
        have [w2' -> /= htow2'] := esubtype_of_val hsub2 hw2 htyv2.
        rewrite /safety_cond_holds /= !filter_ot_true.
        eexists; eauto; split.
        + constructor.
          + exists ty; first by rewrite type_of_oto_val.
            by rewrite -(esubtype_sign_of hsub2).
          constructor; last by constructor.
          exists ty; first by rewrite type_of_oto_val.
          by rewrite -(esubtype_sign_of hsub2).
        rewrite /= -(esubtype_sign_of hsub2) !oto_val_sem_prod_id.
        by rewrite htow1' htow2' -/(sign_of_expr m e1) hs.
      move=> vs1 vs2 [hall ->] si1 s1 si2 [heqst /aandsE_cat [_ hsc]] hw.
      by apply: wi2i_lvsP hscx hsc erefl (fun _ => erefl) heqst hw hall.
   + move=> ty ->; t_xrbindP => hsub <- <- rscx hscx <- /=.
      apply wequiv_opn with
       (fun vsi vs =>
          map type_of_val vs = map (fun e => eval_atype (to_atype (etype_of_expr m e))) es /\
          vsi = map2 (λ (e : pexpr) (v : value), val_to_int (sign_of_expr m e) v) es vs)
      ...
      + move=> si s vsi [heq /aandsE_cat [] hconde hcondx] hsei.
        ...
        by rewrite (sem_pexpr_type_of hv') (hrec' _ hvs').
      + move=> si s [heq /aandsE_cat [] hconde hcondx] ? vs vis [heval ->].
        rewrite /exec_sopn /= /sopn_sem_ /= /sopn_tout => {hsce hscx}.
        case: es hsub heval => // e1 [] //=; last first.
        + by move=> ??; rewrite andbF.
        case: vs => // v1 [] //.
        rewrite andbT; t_xrbindP => hsub1 [htyv1] [] w1 hw1 _ ?; subst vis.
        exists [::] => //.
        by have [w1' -> /= htow1'] := esubtype_of_val hsub1 hw1 htyv1.
      move=> vs1 vs2 [hall ->] si1 s1 si2 [heqst /aandsE_cat [_ hsc]] hw.
      by apply: wi2i_lvsP hscx hsc erefl (fun _ => erefl) heqst hw hall.
    move=> _; t_xrbindP => hnone <- <- scx hscx <- /=.
    apply wequiv_opn with eq (fun vs1 vs2 => vs1 = vs2 /\ map type_of_val vs1 = map eval_atype (sopn_tout o)).
  ...
  + t_xrbindP => xs o es ii sci hnone sce hsce scx hscx <- /=.
    ...
    apply wequiv_syscall with
     eq (fun fs1 fs2 => [/\ fs1 = fs2, emem si0 = fmem fs1
                          & map type_of_val fs1.(fvals) = map eval_atype (scs_tout (syscall_sig_u o))]).
    + apply wrequiv_weaken with
        (fun si s => eqst tt si s ∧ sem_eassert gd si (aands (sce.1 ++ scx.1)) = ok true) eq => //.
      + by move=> ?? [].
      by apply wi2i_esP_none.
    ...
    by case: hpre => -[_ ? ? hsc]; split => //; rewrite (eq_on_sem_eassert _ _ (s' := si)).
  + move=> a ii sci; rewrite /wi2i_a_and; t_xrbindP.
    move=> ? [sc ai] ha <- <- /=.
    apply wequiv_assert => //.
    move=> _; split => //.
    move=> si s [heqs _].
    rewrite /= -cats1 => /aandsE_cat [hsc hai].
    by split => //; apply: (wi2i_eassertP ha heqs hsc hai).
  ...
  + move=> x dir lo hi c hc ii sci_; t_xrbindP.
    ...
    + by move=> > [].
    + move=> si s vis [heqs /aandsE_cat [hsclo1 hschi1] /=]; t_xrbindP.
  ...
  + move=> al c e ii' c' hc hc' ii sci_; t_xrbindP.
    ...
    apply wequiv_weaken with (eqst tt) (λ si s, eqst tt si s ∧ sem_eassert (p_globs pi) si (aands sce.1) = ok true).
    1-2: by move=> > [].
    apply wequiv_while.
    + by move=> ??? [heqs hsc1] hsc; have := (wi2i_condP hsce heqs hsc1 hsc); eauto.
    + rewrite -(cats0 c); apply wequiv_cat with (eqst tt) => //.
      by apply safe_assertP.
  ...
  + move=> si s vis [heqs /aandsE_cat [hsce1 _]] hsce2.
    ...
    have [hall ->] := hrec1 _ _ hsubs hvs.
    rewrite -(esubtype_sign_of hsub); split => //; constructor => //.
    rewrite (sem_pexpr_type_of hv).
    exists (to_atype (etype_of_expr m e)) => //.
    by rewrite -(esubtype_sign_of hsub) to_etype_to_atype.
  ...
  move=> si s si' [[_ _ hvm] /aandsE_cat [_ hscx1]]; rewrite /upd_estate hfvals in hall |- * => hw.
...
Proof using E E0 ep rE spp syscall_state wE.
  ...
  + elim => [| x xs hrec] xs' FV1 FV2 m1 m2 hsub hin hor hget hget' hwf /=.
    ...
    case hw: is_word_type => [ws|] //=; t_xrbindP => /andP[htxi /Sv_memP hxi].
    ...
    + rewrite htx; split => //.
      + have := convertible_subatype htxi.
        by case: vtype.
      move=> y hxy hiny.
...
Proof.
  ...
  have hsigs : ∀ fn fd,
    ...
    get_fundef [seq build_sig info i | i <- p_funcs p] fn =
       Some
          (map2 (λ (x : var_i) (ty : atype), to_etype (sign_of_var M x) ty) (f_params fd) (f_tyin fd),
           map2 (λ (x : var_i) (ty : atype), to_etype (sign_of_var M x) ty) (f_res fd) (f_tyout fd)).
  + move=> fn' fd' hfn'.
    ...
    have heq : forall xs ty,
      ...
      map2 (λ (x : var_i) (ty : atype), to_etype (sign_of_var info x) ty) xs ty =
      map2 (λ (x : var_i) (ty : atype), to_etype (sign_of_var M x) ty) xs ty.
