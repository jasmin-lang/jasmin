...
Lemma is_spillP o :
  ...
  | Some (k, tys) => o = Opseudo_op (pseudo_operator.Ospill k tys)
  | _ => true
  end.
Proof. by case: o => // -[]. Qed.
...
Proof.
  ...
  + t_xrbindP => xs tg o es ii sci sce hsce scx hscx o'.
    case: is_spill (is_spillP o) hscx.
    + move=> [k tys] -> /= hscx; t_xrbindP => /eqP hsze hsub <- <- /=.
      apply wequiv_opn with 
       ...
       (fun vsi vs => vsi = vs /\ 
          map type_of_val vsi = map eval_atype (sopn_tout (Opseudo_op (pseudo_operator.Ospill k tys)))).
