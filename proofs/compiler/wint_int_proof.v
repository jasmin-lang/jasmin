...
Lemma sem_op2_type_of o v v1 v2 :
  ...
  type_of_val v = eval_atype ((type_of_op2 o).2).
Proof.
...
Lemma esubtype_to_word sg sz ty v w :
  ...
  type_of_val v = eval_atype (to_atype ty) ->
  to_word sz v = ok w ->
  v = Vword w.
Proof.
...
Lemma wi2i_type_of_op2 o :
  ...
  [/\ t.1.1 = wi2i_type (sign_of_etype et.1.1) (to_atype et.1.1)
    , t.1.2 = wi2i_type (sign_of_etype et.1.2) (to_atype et.1.2)
    & t.2 = wi2i_type (sign_of_etype et.2) (to_atype et.2)].
Proof.
  have l1 : ∀ o, type_of_opk o = wi2i_type (sign_of_etype (etype_of_opk o)) (to_atype (etype_of_opk o)).
  + by case.
  case: o => //= s w [] //=.
Qed.
...
Lemma e_type_of_op2' o :
  ...
  [/\ t.1.1 = to_atype et.1.1
    , t.1.2 = to_atype et.1.2
    & t.2   = to_atype et.2].
...
Lemma wi2i_eassertP b a ai :
  ...
  forall si s, eqst tt si s ->
    sem_cond gd (eands ai.1) si = ok true ->
...
Proof using FV asm_op ep hwf_m m p sip spp syscall_state.
  ...
  + move=> e1 e2 ai b; t_xrbindP => /andP[/eqP hte1 /eqP hte2] ce1 hce1 ce2 hce2 <- si s heqs /=.
    rewrite eandsE_cat => -[hsc1 hsc2] /=; t_xrbindP.
  ...
  rewrite eandsE_cat => -[hsc1 hsc2] /=; t_xrbindP => b1 hb1 b2 hb2 <-.
...
Proof using E E0 FV asm_op ep hwf_m m p rE sip spp syscall_state wE.
  ...
  rewrite eandsE_cat => -[hsc hscs] hvm hokm heqs si1 hw hws /List_Forall2_inv [ [ty heval heq] hall] /=.
  rewrite heq in hlvi, hw.
  rewrite (check_scP gd hmemok hdisj hvm hokm) in hsc.
...
Proof using FV asm_op ep hwf_m m p sip spp syscall_state.
  ...
  rewrite -cats1 => /aandsE_cat [] /aands_map_PexprE hsc; rewrite aandsE_1 => ha2.
...
Proof using E E0 FV Pc Pi Pi_r asm_op ep ev hp' hsig hwf_m m p p_funcsi pi rE sigs sip spp syscall_state wE.
  ...
  + move=> i ii hi ci_; t_xrbindP => sci /add_iinfoP /(hi ii) {}hi <-.
    ...
    + apply wequiv_weaken with (P2 := eqst tt)
        (Q2 := fun si s => eqst tt si s /\
                 sem_eassert (p_globs pi) si (aands [seq Pexpr i | i <- sci.1]) = ok true).
      * done.
      * by move=> t1 t2 [h1 h2]; split => //; apply/aands_map_PexprE.
  ...
  + t_xrbindP => xs tg o es ii sci sce hsce [otys o'].
    ...
    + move=> k tys -> /=; t_xrbindP => /eqP hsze hsub <- <- /= scx hscx <-.
      ...
      + move=> si s vsi [heq /eandsE_cat [] hconde hcondx] hsei.
      ...
      + move=> si s [heq /eandsE_cat [] hconde hcondx] ? vs vis [heval ->].
        rewrite /exec_sopn /= /sopn_sem_ /= /sopn_tout => {hsce}; t_xrbindP.
        move=> [] h <-.
        exists [::] => //.
        elim: tys es vs heval hsze hsub h => [ | ty tys hrect] [| e es] => //=.
        + by case.
        case => // v vs [] htof htofs []hsze /andP [hsub1 hsub].
        t_xrbindP => ov hov h.
        have {hrect h} := hrect _ _ htofs hsze hsub h.
        t_xrbindP => -[] -> _.
        by have [? -> _]:= esubtype_of_val hsub1 hov htof.
      move=> [|??] _ [<- ] //= _ si s si'.
    ...
    + move=> ty ->; t_xrbindP => hsub <- <-.
      ...
      move=> vs1 vs2 [hall ->] si1 s1 si2 [heqst /eandsE_cat [_ hsc]] hw.
  ...
  + move=> a ii sci; rewrite /wi2i_a_and; t_xrbindP.
    ...
    move=> _; split => //.
    move=> si s [heqs _].
    rewrite /= -cats1 => /aandsE_cat [] /aands_map_PexprE hsc; rewrite aandsE_1 => hai.
  ...
  + move=> al c e ii' c' hc hc' ii sci_; t_xrbindP.
    ...
    + rewrite -(cats0 c); apply wequiv_cat with (eqst tt) => //.
      apply wequiv_weaken with (P2 := eqst tt)
        (Q2 := fun si s => eqst tt si s /\
                 sem_eassert (p_globs pi) si (aands [seq Pexpr i | i <- sce.1]) = ok true) => //.
      * by move=> t1 t2 [h1 h2]; split => //; apply/aands_map_PexprE.
      by apply safe_assertP.
