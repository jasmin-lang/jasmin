...
Definition assemble_i rip (i: linstr) : cexec asm :=
  ...
  | LstoreLabel x lbl =>
    ...
    | Lvar x => if register_of_var x is Some r then ok (Reg r) else Error (fail ii "bad var")
...
Definition assemble_c rip (lc: lcmd) : cexec (seq asm) :=
...
Definition assemble_fd sp rip (fd: lfundef) :=
  ...
  ok (XFundef (lfd_align fd) sp fd' (lfd_export fd)).
...
Definition assemble_prog (p: lprog) : cexec xprog :=
