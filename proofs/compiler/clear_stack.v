           ...
           (E.error
             (pp_box
               [:: pp_s "The clear step ("; pp_s (string_of_wsize ws);
                   ...
                   pp_s (string_of_wsize ws_align); pp_s ")"]))
  in
  Let cmd := clear_stack_cmd css lbl ws_align ws used_stk in
  Let _ :=
...
Definition clear_stack_lfd fn (lfd : lfundef) : cexec lfundef :=
  let ws_align := lfd_align lfd in
  let used_stk := lfd_used_stack lfd in
  let max_stk := lfd_total_stack lfd in
  if css_of_fn fn is Some (css, ws)
  then
    if lfd_export lfd && (0 <? lfd_used_stack lfd)%Z
    then
      Let _ :=
        assert (used_stk + wsize_size ws_align - 1 <=? max_stk)%Z
               (E.error
                 (pp_box
                   [:: pp_s "The used stack size ("; pp_z used_stk;
                       pp_s ") and the total stack size ("; pp_z max_stk;
                       pp_s ") are not coherent"]))
      in
      clear_stack_lfd_body lfd css ws
    else ok lfd
  else
    ok lfd.
