...
Fixpoint wi2i_e (e0:pexpr) : cexec pexpr :=
  ...
  | Pif ty e1 e2 e3 =>
   ...
   | Pbig ei o v e es el =>
     let ty := etype_of_op2 o in
     Let _ := assert [&& esubtype ty.1.1 (etype_of_expr ei)
                         esubtype ty.2 (etype_of_expr ei) &
...
Definition wi2i_ci ci :=
  ...
  in
  ok (MkContra ci.(f_iparams) ci.(f_ires) ci_pre ci_post).
