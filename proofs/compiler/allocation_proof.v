...
Section PROOF.
  ...
  Proof.
    ...
    case: s1 => scs1 sm1 svm1 tr1 He Htr Hw r1 [] //= x2 tag2 ty2 e2 r2 vm1 Hvm1.
  ...
  Proof.
    move=> s t p e b.
    case: s => scs sm svm tr Hve r1 [] //= t' p' e' r2 vm1 Hvm1.
  ...
  Proof.
    move => s1 s2 e c1 c2.
    case: s1 => scs1 sm1 svm1 tr1 Hve _ Hc1 r1 [] //= e' c1' c2' r2 vm1 Hvm1.
  ...
  Proof.
    ...
    case: s2 => scs2 sm2 svm2 tr2 _ Hc Hse _ Hc' _ Hw r1 [] //= a2 c2 e2 c2' r2 vm1 Hvm1.
    rewrite /check_i -/check_I.
  ...
  Proof.
    move => s1 s2 a c e c'.
    case: s2 => scs2 sm2 svm2 tr2 _ Hc Hse r1 [] //= a2 c2 e2 c2' r2 vm1 Hvm1.
    rewrite /check_i -/check_I.
  ...
  Proof.
    ...
    case: s1 => scs1 sm1 svm1 tr1 Hlo Hhi Hc Hfor r1 [] //= i2 [[d2 lo2] hi2] c2 r2 vm1 Hvm1.
    rewrite /check_i -/check_I.
    case: eqP => //= ?;subst d2.
  ...
  Proof.
    move=> s1 scs2 m2 s2 s3 xs fn args vargs vs vpr vpo tr Hes hpr Hsc Hfun Hw hpo -> r1
...
Lemma alloc_call_uprogP ev gd ep1 p1 ep2 p2
  (H: check_prog init_alloc_uprog check_f_extra_u ep1 p1 ep2 p2 = ok tt) f scs mem scs' mem' va vr tr:
    sem_call {|p_globs := gd; p_funcs := p1; p_extra := ep1; |} ev scs mem f va scs' mem' vr tr ->
    exists vr',
     sem_call {|p_globs := gd; p_funcs := p2; p_extra := ep2; |} ev scs mem f va scs' mem' vr' tr /\
                List.Forall2 value_uincl vr vr'.
...
Lemma alloc_fun_uprogP_eq p ev fn f f' scs1 m1 scs2 m2 vargs vargs' vres vres' vpre vpost tr s0 s1 s2:
...
Lemma alloc_call_sprogP ev gd ep1 p1 ep2 p2
  (H: check_prog init_alloc_sprog check_f_extra_s ep1 p1 ep2 p2 = ok tt) f scs mem scs' mem' va vr tr:
    sem_call {|p_globs := gd; p_funcs := p1; p_extra := ep1; |} ev scs mem f va scs' mem' vr tr ->
    exists vr',
     sem_call {|p_globs := gd; p_funcs := p2; p_extra := ep2; |} ev scs mem f va scs' mem' vr' tr /\
                List.Forall2 value_uincl vr vr'.
