...
(* FIXME: this is only used in the next lemma; should it be inlined? *)
Local Lemma is_spill_op_is_Odeclassify o a :
  is_Odeclassify o = Some a -> is_spill_op o = None.
Proof. by case: o => [[]||]. Qed.

Lemma it_lower_spill_fdP fn :
  wiequiv_f p p' ev ev (rpreF (eS:= eq_spec)) fn fn (rpostF (eS:=eq_spec)).
Proof using spill_prog_ok DEind.
  ...
  + move=> x tg ty e ii env env' c' [<- <-]; rewrite vars_I_assgn /vars_lval => hsub.
    apply wequiv_assgn_rel_eq_R with (checker_st_ve S) env => //.
