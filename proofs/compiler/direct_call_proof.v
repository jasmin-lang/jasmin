...
Proof.
  constructor.
  + move=> env1 env2 wdb1 wdb2 d es1 es2 d' + <-; case => -[-> ->].
    + by apply read_es_st_uincl.
    by move=> ??? h /(sem_pexprs_weak false); apply: read_es_st_uincl h.
  move=> env1 env2 wdb1 wdb2 d xs1 xs2 d' + <-; case => -[-> ->].
...
Let Pi i :=
  wequiv_rec (dc1:=indirect_c) (dc2:=direct_c) (env1:=env) (env2:=env) p p ev ev uincl_spec (st_uincl tt) [::i] [::i] (st_uincl tt).

Let Pi_r i := forall ii, Pi (MkI ii i).

Let Pc c :=
  wequiv_rec (dc1:=indirect_c) (dc2:=direct_c) (env1:=env) (env2:=env) p p ev ev uincl_spec (st_uincl tt) c c (st_uincl tt).

Lemma it_indirect_to_direct_rec c : Pc c.
Proof.
...
Proof.
  rewrite /wiequiv_f.
  apply wequiv_fun_ind => {}fn _ vals _ fs1 fs2 [<-] [<-] [hscs hmem hu] fd hget.
  exists fd => //= s.
  ...
  + move=> s t fr1 [hscs hmem _ hle2]; rewrite /finalize_funcall; t_xrbindP.
  ...
  exact: it_indirect_to_direct_rec.
Qed.
