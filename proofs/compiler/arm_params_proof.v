...
Notation zeroized_on := (zeroized_on arm_h_rz_zeroized).
Notation zeroized_on_vars := (zeroized_on_vars arm_h_rz_zeroized).
...
Lemma arm_zeroize_flagsP lp scs vm m fn ii err_flags xname P Q args :
  let: x := {| vname := xname; vtype := sword reg_size; |} in
  arm_zeroize_flags err_flags (Some x) = ok args
  -> zeroized_on vm x
...
Lemma arm_h_rz_cmd_args
  ...
  rz_cmd_args arm_rzparams rzm xs err_flags err_register = ok args
...
Proof.
  ...
  have -> : vxregs = [::].
  - subst vxregs. clear. by case: rzm_xregisters.
  ...
  have -> :
    seq_diff (vregs ++ vflags) (Sv.elements callee_saved ++ xs)
    = (x :: vars) ++ map to_var rflags.
  - rewrite /seq_diff.
    rewrite filter_cat.
    rewrite hvars.
    f_equal.
    ...
    + move=> f.
      ...
      rewrite (flags_notin_callee_saved (asm_e := arm_extra)) /=.
      apply/negP.
      by move=> /hxs.
