...
clone import Pmac_verify_hmac_ct.T with theory LeakageModel <-  LeakageModelTVCL.
...
equiv l_final : M.verify_hmac_jazz ~ M.verify_hmac_jazz :
  ={M.leakages, pmac, maxpad} /\ (out + len){1} = (out + len){2} /\
     to_uint pmac{2} + 64 <= W64.modulus /\ 64 %| to_uint pmac{2} /\ 
...
+ wp; skip=> |> &1 &2 h1 h2 hjm hmax hpmac dpmac hlen1 hlen2; rewrite ultE => hj.
  split; 1: by rewrite /leak_mem /leak_mem_CL !offset_div // 1,2:/#.
