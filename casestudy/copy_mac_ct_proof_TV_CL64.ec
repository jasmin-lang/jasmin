...
qed.
lemma and_invw64 (x:W64.t) : 
  0 <= to_uint x < 64 =>
  0 <= to_uint (x `&` invw (W64.of_int 64)) < 64.
proof.
  move=> h.
  pose x' := (x `&` invw (W64.of_int 64)).
  rewrite (divz_eq (to_uint x') 64); have -> /# : to_uint x' %/ 64 = 0.
  have -> : 64 = 2 ^ 6 by done.
  rewrite -W64.to_uint_shr 1:// -W64.to_uint0; congr.
  apply W64.wordP => i hi.
  rewrite /x' W64.shrwE hi /=.
  have hi5 : 0 <= i + 6 by smt().
  rewrite -(b2i_eq1 x.[i + 6])  -(b2i_eq1 (W64.of_int 64).[i + 6]) !W64.b2i_get 1,2://.
  rewrite to_uint_small 1://.
  case (i = 0) => [-> // | hi0].
  rewrite divz_small 2://. 
  have /= /#: 2 ^ 6 <= 2 ^ (i + 6).
...
lemma offset_div_64 (p offset : W64.t) :
...
lemma offset_div_and_64 (p offset : W64.t) :
  to_uint p + 64 <= W64.modulus =>
  64 %| to_uint p =>
  0 <= to_uint offset < 64 =>
  to_uint (p + (offset `&` invw (W64.of_int 64))) %/ 64  = to_uint p %/ 64.
proof. by move=> h1 h2 h3; rewrite offset_div_64 // and_invw64. qed.
...
lemma offset_div_or_64 (p offset : W64.t) :
  to_uint p + 64 <= W64.modulus =>
  64 %| to_uint p =>
  0 <= to_uint offset < 64 =>
  to_uint (p + (offset `|` (W64.of_int 64))) %/ 64  = to_uint p %/ 64 + 1.
proof. 
  move=> /= h1 h2 h3. 
  rewrite or_add_and (W64.WRingA.addrC ( offset `&` _)) W64.WRingA.addrA.
  have heq : to_uint (p + W64.of_int 64) = to_uint p + 64.
  + rewrite W64.to_uintD_small /=. 1:/#.
  rewrite offset_div_and_64 ?heq //=. /#.
qed.
...
proof.
  ...
  have h64 : 64 %| to_uint rotated_mac{2} by smt().
  split=> //=.
  + split=> //=.
    + 
