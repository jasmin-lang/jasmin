...
(* The safety conditions of an operator are [Safety_cond.safety_cond] terms,
   i.e. boolean expressions on its arguments. The shapes below are the ones
   built by the [sc_*] functions of lang/safety_cond.v: recognising them keeps
   the dedicated abstract conditions ([NotEqual], [InRange], [Initai]), which
   are more precise than the generic [GeneralCond]. If those definitions
   change, the patterns below must change too. *)
let safe_opn pd asmOp safe opn es =
  ...
  (* Translation of a condition into an expression on the arguments; used for
     the shapes that have no dedicated abstract condition. *)
  let rec to_e c =
    match c with
    | Safety_cond.IBool b -> Pbool b
    | Safety_cond.IConst z -> Pconst (Conv.z_of_cz z)
    | Safety_cond.IVar k ->
       (match List.nth_opt es (Conv.int_of_nat k) with
        | Some e -> e
        | None -> Pbool false)
    | Safety_cond.IOp1 (o, c) -> Papp1 (o, to_e c)
    | Safety_cond.IOp2 (o, c1, c2) -> Papp2 (o, to_e c1, to_e c2)
    | Safety_cond.IOpN_safety (_, _, _, _) -> Pbool false
  in
  let is_zero z = Z.equal (Conv.z_of_cz z) Z.zero in
  let rec conds c =
      match c with
      (* [sc_x86_division sz sg] *)
      | Safety_cond.IOp2 (Oand,
            Safety_cond.IOp2 (Oneq Op_int,
              Safety_cond.IOp1 (Oint_of_word (sg, sz), Safety_cond.IVar k),
              Safety_cond.IConst z),
            Safety_cond.IOp1 (Onot, _))
      ...
      (* [sc_not_zero ws k] *)
      | Safety_cond.IOp2 (Oneq Op_int,
            Safety_cond.IOp1 (Oint_of_word (Unsigned, ws), Safety_cond.IVar k),
            Safety_cond.IConst z)
        when is_zero z ->
        [ notZero(ws, arg k) ]

      (* [sc_is_zero ws k]: n ∈ [0; 0] *)
      | Safety_cond.IOp2 (Oeq Op_int,
            Safety_cond.IOp1 (Oint_of_word (Unsigned, ws), Safety_cond.IVar k),
            Safety_cond.IConst z)
        when is_zero z ->
      ...
      | Safety_cond.IOp2 (Oand,
            Safety_cond.IOp2 (Ole Cmp_int, Safety_cond.IConst lo, e1),
            Safety_cond.IOp2 (Ole Cmp_int, e2, Safety_cond.IConst hi))
        when e1 = e2 ->
        [ InRange(Pconst (Conv.z_of_cz lo), Pconst (Conv.z_of_cz hi), to_e e1) ]

      (* [sc_ult ws k z]: n ∈ [0; z-1] *)
      | Safety_cond.IOp2 (Olt Cmp_int,
            Safety_cond.IOp1 (Oint_of_word (Unsigned, ws), Safety_cond.IVar k),
            Safety_cond.IConst z) ->
        [ InRange(Pconst Z.zero, Pconst (Z.pred (Conv.z_of_cz z)), toint Unsigned ws (arg k)) ]

      (* [sc_uge ws z k]: z ∈ [0; n] *)
      | Safety_cond.IOp2 (Ole Cmp_int, Safety_cond.IConst z,
            Safety_cond.IOp1 (Oint_of_word (Unsigned, ws), Safety_cond.IVar k)) ->
        [ InRange(Pconst Z.zero, toint Unsigned ws (arg k), Pconst (Conv.z_of_cz z)) ]

      (* [sc_uadd_le ws k1 k2 z]: n1 + n2 ∈ [0; z] *)
      | Safety_cond.IOp2 (Ole Cmp_int,
            (Safety_cond.IOp2 (Oadd Op_int,
               Safety_cond.IOp1 (Oint_of_word (Unsigned, _), Safety_cond.IVar _),
               Safety_cond.IOp1 (Oint_of_word (Unsigned, _), Safety_cond.IVar _)) as e),
            Safety_cond.IConst z) ->
        [ InRange(Pconst Z.zero, Pconst (Conv.z_of_cz z), to_e e) ]

      (* [sc_all_init ws len k]: every byte of the array is initialised *)
      | Safety_cond.IOpN_safety (Ois_arr_init _,
            Safety_cond.IVar k, Safety_cond.IConst lo, Safety_cond.IConst len) ->
