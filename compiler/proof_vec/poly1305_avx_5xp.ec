...
module M = {
  ...
  proc mulmod_u128_prefetch (global_mem : global_mem_t, in1:W64.t,
                                                        x:(int,W128.t)map,
                                                        y:(int,W128.t)map,
                                                        yx5:(int,W128.t)map) : 
  global_mem_t * (int,W128.t)map* (int,W128.t)map = {
    ...
    x.[3] <- x86_VPADD_2u64 t.[3] z.[4];
    x.[4] <- t.[4];
    return (global_mem, x, xy0);
  }
  
  proc mulmod_add_u128_prefetch (global_mem : global_mem_t, in1:W64.t,
                                                            u:(int,W128.t)map,
                                                            x:(int,W128.t)map,
                                                            y:(int,W128.t)map,
                                                            yx5:(int,W128.t)map) : 
  ...
  global_mem_t * (int,W64.t)map = {
    ...
    if ((inlen `<` (W64.of_int 8))) {
      ...
      while ((c `<` inlen)) {
        t <- (zeroext_8_64 (loadW8 global_mem (in1 + c)));
        t <- (t `<<` (zeroext_64_8 n));
        m.[0] <- (m.[0] `|` t);
        n <- (n + (W64.of_int 8));
        c <- (c + (W64.of_int 1));
      ...
      m.[0] <- (loadW64 global_mem (in1 + (W64.of_int 0)));
      inlen <- (inlen - (W64.of_int 8));
      in1 <- (in1 + (W64.of_int 8));
      c <- (W64.of_int 0);
      n <- (W64.of_int 0);
  ...
  global_mem_t * (int,W128.t)map* W64.t = {
    var xy0:(int,W128.t)map;
    var xy1:(int,W128.t)map;
    (global_mem, hxy, xy0) <@ mulmod_u128_prefetch (global_mem, in1, hxy,
    s_r4r4, s_r4r4x5);
    in1 <- (in1 + (W64.of_int 32));
    (global_mem, hxy, xy1) <@ mulmod_add_u128_prefetch (global_mem, in1, hxy,
    xy0, s_r2r2, s_r2r2x5);
    in1 <- (in1 + (W64.of_int 32));
    hxy <@ add_u128 (hxy, xy1);
    hxy <@ carry_reduce_u128 (hxy);
