...
module LivenessDomain : BackwardAnalyser.Logic with type domain = Sv.t = struct
  ...
  let opn
    ...
    match op with
    | Opseudo_op (Ocopy _) ->
      (*We do not kill copied variables in that ccase since copy is substituted with a loop, and thus not a real assignment*)
      let lv_exprs = Prog.lv_exprs lvs in
      Annotation (Sv.union domain (Sv.union (Prog.vars_es lv_exprs) (Prog.vars_es exprs)))
