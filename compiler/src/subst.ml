...
let rec gsubst_e (flen: ?loc:L.t -> 'len1 -> 'len2) (f: 'len1 ggvar -> 'len2 gexpr) (e: 'len1 gexpr) : 'len2 gexpr =
  ...
  | Parr_init_elem (e,n) -> Parr_init_elem (gsubst_e flen f e,flen n)
  | Pvar v -> f v
  ...
  | Pis_barr_init (v,e1,e2) -> Pis_barr_init (gsubst_vdest f v, gsubst_e flen f e1,gsubst_e flen f e2)
...
let rec gsubst_i (flen: ?loc:L.t -> 'len1 -> 'len2) f i =
  let i_desc =
    ...
    | Cassert(t, p, e) -> Cassert (t, p, (gsubst_e flen f e))
...
let rec int_of_expr ?loc e =
  ...
  | Pbool _ | Parr_init _ | Parr_init_elem _ | Pvar _ | Pis_var_init _ 
...
let remove_params (prog : ('info, 'asm) pprog):('info, 'asm) prog =
  ...
  let mk_word ws e =
    ...
    let c = const_prop_e (fun _ -> assert false) false (Some get_glob) Var0.Mvar.empty e in
