...
module X86AsmTranslate (AsmSyntax: X86AsmSyntax) : AsmTarget.AsmTarget
...
= struct
  ...
  let pp_function_header fn fd =
    let name = fn.fn_name in
    let export = fd.asm_fd_export in
    if export then
    ...
    let pp_function_tail _ (fd:(reg, regx, xreg, rflag, cond, asm_op) asm_fundef) = 
...
module ATTPrinter = AsmTarget.AsmTargetBuilder.Make(TranslateATT)
module InterPrinter = AsmTarget.AsmTargetBuilder.Make(TranslateIntel)
