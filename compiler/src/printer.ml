...
let pp_ge ~debug (pp_len: 'len pp) (pp_var: 'len gvar pp) : 'len gexpr pp =
  ...
  | Papp1(o, e) ->
     ...
     optparent fmt prio side p "(";
    F.fprintf fmt "%s %a" (string_of_op1 ~debug o) (pp_expr Right p) e;
     optparent fmt prio side p ")"
  | Papp2(op,e1,e2) ->
     let p = priority_of_op2 op in
     optparent fmt prio side p "(";
    F.fprintf fmt "%a %s %a" (pp_expr Left p) e1 (string_of_op2 op) (pp_expr Right p) e2;
     optparent fmt prio side p ")"
  ...
  | Pif(_, e,e1,e2) ->
     ...
     optparent fmt prio side p "(";
    F.fprintf fmt "%a ? %a : %a" (pp_expr Left p) e (pp_expr NoAssoc p) e1 (pp_expr Right p) e2;
     optparent fmt prio side p ")"
  in
