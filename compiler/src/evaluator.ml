...
let return s = 
  ...
  | Scall(ii,f,xs,vm1,c,stk) ->
    ...
    let vres = 
      exn_exec ii (mapM (fun (x:var_i) -> get_var vm2 x.v_var) f.f_res) in
    let vres' = exn_exec ii (mapM2 ErrType truncate_val f.f_tyout vres) in
    let s1, _lk = exn_exec ii (write_lvals gd {emem = m2; evm = vm1 } xs vres') in
    { s with 
      s_cmd = c;
      s_estate = s1;
      s_stk = stk }
...
let small_step1 s = 
  ...
  | i :: c ->
    ...
    | Cassgn(x,_,ty,e) ->
      let v, _lkv  = exn_exec ii (sem_pexpr gd s1 e) in
      let v' = exn_exec ii (truncate_val ty v) in
      let s2, _lk = exn_exec ii (write_lval gd x v' s1) in
      { s with s_cmd = c; s_estate = s2 }

    | Copn(xs,_,op,es) ->
      let s2, _lk = exn_exec ii (sem_sopn gd op s1 xs es) in
      { s with s_cmd = c; s_estate = s2 }

    | Cif(e,c1,c2) ->
       let v, _lk = exn_exec ii (sem_pexpr gd s1 e) in
      let b = of_val_b ii v in
      let c = (if b then c1 else c2) @ c in
      { s with s_cmd = c }

    | Cfor (i,((d,lo),hi), body) ->
       let zlo, _lklo = exn_exec ii (sem_pexpr gd s1 lo) in
      let vlo = of_val_z ii zlo in
      let zhi, _lkhi = exn_exec ii (sem_pexpr gd s1 hi) in
