...
let return ep spp s =
  ...
  | Sempty(ii, f) ->
    ...
    let vres' = exn_exec ii (mapM2 ErrType truncate_val (List.map to_ctype f.f_tyout) vres) in
    raise (Final(m2, vres'))
...
let small_step1 ep spp sip s =
  ...
  | i :: c ->
    ...
    | Cassgn(x,_,ty,e) ->
      ...
      let v' = exn_exec ii (truncate_val (to_ctype ty) v) in
    ...
    | Ccall(xs,fn,es) ->
      ...
      let vargs = exn_exec ii (mapM2 ErrType truncate_val (List.map to_ctype f.f_tyin) vargs') in
...
let init_state ep scs0 p ii fn args m =
  ...
  let vargs = exn_exec ii (mapM2 ErrType truncate_val (List.map to_ctype f.f_tyin) args) in
...
(* ----------------------------------------------------------- *)
let pp_undef fmt ty = 
  Format.fprintf fmt "undef<%a>" PrintCommon.pp_ty (Conv.ty_of_cty (Type.to_atype ty))
