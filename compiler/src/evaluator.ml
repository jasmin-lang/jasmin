...
let return ep spp env s =
  ...
  | Sempty(ii, f) ->
    ...
    let vres = 
      exn_exec ii (mapM (fun (x:var_i) -> get_var nosubword env true vm2 x.v_var) f.f_res) in
  ...
  | Scall(ii,f,xs,vm1,c,stk) ->
    ...
    let vres = 
      exn_exec ii (mapM (fun (x:var_i) -> get_var nosubword env true vm2 x.v_var) f.f_res) in
    let vres' = exn_exec ii (mapM2 ErrType truncate_val (List.map (Type.eval_atype env) f.f_tyout) vres) in
    let s1 = exn_exec ii (write_lvals nosubword ep spp env true gd {escs = scs2; emem = m2; evm = vm1 } xs vres') in
    { s with 
      s_cmd = c;
      s_estate = s1;
      s_stk = stk }
  ...
  | Sfor(ii,i,ws,body,c,stk) ->
    ...
    | w::ws ->
      let s1 = exn_exec ii (write_var nosubword ep env true i (Vint w) s.s_estate) in
      { s with s_cmd = body;
               s_estate = s1;
               s_stk = Sfor(ii, i, ws, body, c, stk) }

let small_step1 ep spp sip env s =
  match s.s_cmd with
  | [] -> return ep spp env s
  | i :: c ->
    let MkI(ii,ir) = i in
    ...
    | Cassgn(x,_,ty,e) ->
      let v  = exn_exec ii (sem_pexpr nosubword ep spp env true gd s1 e) in
      let v' = exn_exec ii (truncate_val (eval_atype env ty) v) in
      let s2 = exn_exec ii (write_lval nosubword ep spp env true gd x v' s1) in
    ...
    | Cif(e,c1,c2) ->
      let b = of_val_b ii (exn_exec ii (sem_pexpr nosubword ep spp env true gd s1 e)) in
      let c = (if b then c1 else c2) @ c in
      { s with s_cmd = c }

    | Cfor (i,((d,lo),hi), body) ->
      let vlo = of_val_z ii (exn_exec ii (sem_pexpr nosubword ep spp env true gd s1 lo)) in
      let vhi = of_val_z ii (exn_exec ii (sem_pexpr nosubword ep spp env true gd s1 hi)) in
    ...
    | Ccall(xs,fn,es) ->
      ...
      let sf = 
        exn_exec ii (write_vars nosubword ep env true f.f_params vargs {escs; emem = m1; evm = Vm.init nosubword env}) in
...
let init_state ep env scs0 p ii fn args m =
  let f = BatOption.get (get_fundef p.p_funcs fn) in
  let vargs = exn_exec ii (mapM2 ErrType truncate_val (List.map (eval_atype env) f.f_tyin) args) in
...
let exec ep spp sip env scs0 p ii fn args m =
  let s = init_state ep env scs0 p ii fn args m in
...
let run (type reg regx xreg rflag cond asm_op extra_op)
  ...
  exec ep spp sip env scs0 p ii fn args m
