...
let return ep spp s =
  ...
  | Scall(ii,f,xs,vm1,c,stk) ->
    ...
    let vres = 
      exn_exec ii (mapM (fun (x:var_i) -> get_var nosubword true vm2 x.v_var) f.f_res) in
    let vres' = exn_exec ii (mapM2 ErrType truncate_val (List.map Type.eval_atype f.f_tyout) vres) in
    let s1 = exn_exec ii (write_lvals withCatch nosubword ep spp withAssert true gd {escs = scs2; emem = m2; evm = vm1 } xs vres') in
    { s with 
      s_cmd = c;
      s_estate = s1;
      s_stk = stk }
...
let small_step1 ep spp sip s =
  ...
  | i :: c ->
    ...
    | Cassgn(x,_,ty,e) ->
      let v  = exn_exec ii (sem_pexpr withCatch nosubword ep spp withAssert true gd s1 e) in
      let v' = exn_exec ii (truncate_val (eval_atype ty) v) in
      let s2 = exn_exec ii (write_lval withCatch nosubword ep spp withAssert true gd x v' s1) in
      { s with s_cmd = c; s_estate = s2 }

    | Copn(xs,_,op,es) ->
      let s2 = exn_exec ii (sem_sopn withCatch nosubword ep spp withAssert sip._asmop gd  op s1 xs es) in
      { s with s_cmd = c; s_estate = s2 }

    | Csyscall(xs,o, es) ->
      let ves = exn_exec ii (sem_pexprs withCatch nosubword ep spp withAssert true gd s1 es) in
      let ((scs, m), vs) =
        exn_exec ii (syscall_sem__ sip._sc_sem ep._pd s1.escs s1.emem o ves) in
      let s2 = exn_exec ii (write_lvals withCatch nosubword ep spp withAssert true gd {escs = scs; emem = m; evm = s1.evm} xs vs) in
      { s with s_cmd = c; s_estate = s2 }

    | Cassert (p,a) ->
      let v = exn_exec ii (sem_pexpr withCatch nosubword ep spp withAssert true gd s1 a) in
      let b = of_val_b ii v in
      if not b then raise (Eval_error(ii, ErrAssert p));
      { s with s_cmd = c }

    | Cif(e,c1,c2) ->
      let b = of_val_b ii (exn_exec ii (sem_pexpr withCatch nosubword ep spp withAssert true gd s1 e)) in
      let c = (if b then c1 else c2) @ c in
      { s with s_cmd = c }

    | Cfor (i,((d,lo),hi), body) ->
      let vlo = of_val_z ii (exn_exec ii (sem_pexpr withCatch nosubword ep spp withAssert true gd s1 lo)) in
      let vhi = of_val_z ii (exn_exec ii (sem_pexpr withCatch nosubword ep spp withAssert true gd s1 hi)) in
    ...
    | Ccall(xs,fn,es) ->
      let vargs' = exn_exec ii (sem_pexprs withCatch nosubword ep spp withAssert true gd s1 es) in
      let f = 
