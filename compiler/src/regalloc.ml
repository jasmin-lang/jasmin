...
let pp_liveness vars liveness_per_callsite liveness_table conflicts a =
  ...
  in
  let m_word, m_vector, m_flag = ref 0, ref 0, ref 0 in
  let reset_max () =
    m_word := 0; m_vector := 0; m_flag := 0
  in
  ...
  let pp_liveset fmt s =
    let subset k = Sv.elements (Sv.filter (fun x -> k (kind_of_type Arch.reg_size x.v_ty)) s) in
    ...
    let l =
      (List.filter (fun (_, m) -> List.length m > 0)
         [ Word, words; Vector, vectors; Flag, flags]) in
  ...
  let pp_recap fmt fn (i_w, i_v, i_f) (e_w, e_v, e_f) =
    let pp fmt (k, i, e) =
    ...
    fprintf fmt "@[<v>/* Maximal register usage for %s:@ %a@ */@]@.@."
      ...
      (List.filter (fun (_, i , e) -> i + e > 0)
         [ Word, i_w, e_w; Vector, i_v, e_v; Flag, i_f, e_f])
  in
  ...
  liveness_table |> Hf.iter (fun fn fd ->
    ...
    let intern = !m_word, !m_vector, !m_flag in
    reset_max();
    printf "%a@." pp_callsites fn;
    let extern = !m_word, !m_vector, !m_flag in
