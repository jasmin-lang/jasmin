...
module AbsExpr (AbsDom : AbsNumBoolType) = struct
  ...
  let abs_sub_arr_range_at abs x acc ws len ei =
    match aeval_cst_int abs ei with
  ...
  (*   Gword of Wsize.wsize * Ssralg.GRing.ComRing.sort
   * | Garr of BinNums.positive * Warray_.WArray.array *)
