...
type safe_cond =
  ...
  | Valid of mem_access (* allocated memory region *)
...
module AbsInterpreter (PW : ProgWrap) : sig
  ...
  let mk_offsets m =
    ...
    let ma = unwrap m in
    let o = Mtexpr.var (MmemAccess m) in 
  ...
  let check_overlap abs m m' =
    ...
    | Some base, Some base' ->
      ...
      then true, false
        ...
        let l,  r  = mk_offsets m
        and l', r' = mk_offsets m' in
        (* [l; r[ ∩ [l'; r'[ ≠ ∅ 
           if and only if
           (l ≤ l' ∧ l' < r) ∨ (l' ≤ l ∧ l < r')
           if and only if
      ...
      begin match AbsDom.var_points_to abs (mvar_of_var x) with
        | Ptrs pts ->
          if List.length pts = 1 then
            ...
            ( state, violations)
          else (state, pv :: violations)
        | TopPtr -> (state, pv :: violations) end
...
(*-------------------------------------------------------------------------*)      
  ...
  let mem_ranges_printer state f_decl fmt () =
    ...
    (* Projection on a specific valie*)
    let rel_param_list = 
