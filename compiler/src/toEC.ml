...
let rec leaks_e_rec pd leaks e =
  ...
  | Pload (_,e) -> leaks_e_rec pd (int_of_word pd e :: leaks) e
...
let leaks_lval pd = function
  ...
  | Lmem (_, e) -> leaks_e_rec pd [int_of_word pd e] e
...
let pp_lval1 pd env pp_e fmt (lv, (ety, e)) =
  ...
  | Lmem(ws, e1) ->
    Format.fprintf fmt "@[Glob.mem <-@ storeW%a Glob.mem (W%d.to_uint %a) (%a);@]" pp_size ws
      (int_of_ws pd)
      (pp_wcast pd env) (add_ptr pd e1) pp_e e
  | Lvar x  ->
...
module Leak = struct
  ...
  let rec safe_e_rec pd env safe = function
    ...
    | Pload (ws, e) ->
      (Valid (ws, e) :: safe_e_rec pd env safe e)
  ...
  let safe_lval pd env = function
    | Lnone _ | Lvar _ -> []
    | Lmem(ws, e) ->
      Valid (ws, e) :: safe_e_rec pd env [] e
    | Laset(aa, ws, x,e) ->
      assert (aa = Warray_.AAscale); (* NOT IMPLEMENTED *)
