    ...
    let pp fmt (x,e) = 
      ...
      if ws = xws && aa = Warray_.AAscale then
        ...
        Format.fprintf fmt "@[(get%i%s@ %a@ %a)@]" 
          (int_of_ws ws) 
          (pp_access aa)
          (pp_initi env (pp_var env)) (x, n, xws) (pp_expr env) e in
    let option = 
...
and pp_wcast env fmt (ty, e) = 
  pp_cast env (pp_expr env) fmt (ty, ty_expr e, e)
  ...
  | Laset (aa, ws, x, e1) -> 
    ...
    if ws = xws && aa = Warray_.AAscale then
      ...
      Format.fprintf fmt 
        ...
        (pp_access aa)
        (pp_initi env (pp_var env)) (x, n, xws) (pp_expr env) e1 pp_e e
...
let ty_sopn op = 
  ...
  | Sopn.Ocopy(ws, p) ->  
    ...
    l, l
  | _ ->
      List.map Conv.ty_of_cty (Sopn.sopn_tout (Arch_extra.asm_opI X86_extra.x86_extra) (Arch_decl.arch_pd X86_decl.x86_decl) op), 
      List.map Conv.ty_of_cty (Sopn.sopn_tin (Arch_extra.asm_opI X86_extra.x86_extra) (Arch_decl.arch_pd X86_decl.x86_decl) op)
...
module Normal = struct  
  ...
  let rec init_aux_i env i = 
    ...
    | Copn (lvs, _, op, _) -> 
      if List.length lvs = 1 then env 
        ...
        let tys  = List.map Conv.ty_of_cty (Sopn.sopn_tout (Arch_extra.asm_opI X86_extra.x86_extra) (Arch_decl.arch_pd X86_decl.x86_decl) op) in
  ...
  let pp_assgn_i env fmt lv ((etyo, etyi), aux) =
    ...
    Format.fprintf fmt "@ %a" (pp_lval1 env (pp_cast env pp_e)) (lv, (etyo,aux))
...
module Leak = struct 
  ...
  let safe_opn env safe opn es = 
    let id = Sopn.get_instr_desc (Arch_extra.asm_opI X86_extra.x86_extra) (Arch_decl.arch_pd X86_decl.x86_decl) opn in
    List.pmap (fun c ->
        match c with
        | Wsize.NotZero(sz, i) ->
