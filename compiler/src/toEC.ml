...
module Normal (EA: EcArray):Model = struct
  ...
  let toec_for env c e1 e2 init cond i_upd =
...
struct
  ...
  let pp_valid_post env f =
    ...
    let wp_step = if wp_step > 0 then (-1 * wp_step) else -1 in
    let wp_step = string_of_int wp_step in

    let post = contrat env1 post in

    let acc = List.map (fun x -> [(L.unloc x).v_name]) f.f_ret in
    let eq a b =
      match a, b with
  ...
  let fun_init env f =
    ...
    let old = List.rev old in
    let old = List.map (fun (_,o) -> o) old in
    let v_old = old in

    let vars = Env.vars env in
    let old_v = List.map (fun x -> Mv.find x vars) v_old in
    Env.add_old env old_v f.f_name;
    let old = List.map (var2ec_var env) v_old in
    ...
    let assume = List.map
        (fun (prover,exp) -> ec_trace env fn Assume exp)
        assume
    in
    let assume = List.flatten assume in
    let es = List.map (fun x -> Pvar (gkvar (L.mk_loc L._dummy x))) v_old in
    ...
    let assert_ = List.map
        (fun (prover,exp) -> ec_trace env fn Assert exp)
        assert_
    in
  ...
  let fun_ret env func rets =
    let proofv = Env.get_proofv env func in
    [Etuple (Etuple (rets) :: [Eident [proofv]])]

  let fun_rty env ty =
    [Tuple (Tuple ty :: [Base "trace"])]
  ...
  let toec_call env lvs f es =
    ...
    let i = List.fold (fun acc pre -> ec_trace env f Assert pre @ acc ) [] pre in

    let i = i @
            [EScall ([LvIdent [ttmpt] ; LvIdent ["tmp__trace"]],
                     [Env.get_funname env f], args)]
    in
    ...
    let i =
      List.fold_left (fun acc post -> acc @ ec_trace env f Assume post ) i post
    in
    List.fold_left2
      (fun acc lv e ->
  ...
  let global_vars env =
    ...
    let tmp = ("tmp__trace", Base "trace") :: tmp in
...
struct
  ...
  let toec_ty = M.toec_ty

  let toec_expr_assign env lv e =
    let stmt = M.toec_expr_assign env lv e in
    let leak =
      match e with
      | Parr_init _ -> (ec_leaks_es env [e])
      | _ -> (ec_leaks_es env [e]) @ (ec_leaks_lvs env [lv])
    in
    leak @ stmt
  ...
  let toec_copn env lvs asmOp op es =
    let stmt = M.toec_copn env lvs asmOp op es in
    (ec_leaks_opn env es) @ (ec_leaks_lvs env lvs) @ stmt

  let toec_call env lvs f es =
    let stmt = M.toec_call env lvs f es in
    (ec_leaks_es env es) @
  ...
  let toec_syscall env lvs o es =
    let stmt = M.toec_syscall env lvs o es in
    (ec_leaks_es env es) @
    (ec_leaks_lvs env lvs) @
    stmt

  let toec_assert _ _ _ = assert false
...
struct
  ...
  let rec toec_cmd f asmOp env c = List.flatten (List.map (toec_instr f asmOp env) c)

  and toec_instr f asmOp env i =
      match i.i_desc with
      | Cassgn (lv, _, _, e) ->
      ...
      | Cassert (k,_,e) ->
          M.toec_assert env f k e
      | Cif (e, c1, c2) ->
          let c1 env = toec_cmd f asmOp env c1 in
          let c2 env = toec_cmd f asmOp env c2 in
      ...
      | Cfor (i, (d,e1,e2), c) ->
          ...
          let init, ec_e2 =
            ...
            | Pconst _ -> ([], EC.toec_expr env e2)
            | _ ->
              let aux = Env.create_aux env "inc" (Base "int") in
              let init = ESasgn ([LvIdent [aux]], EC.toec_expr env e2) in
              let ec_e2 = ec_ident aux in
              [init], ec_e2 in
          let ec_i = [ec_vars env (L.unloc i)] in
          let lv_i = [LvIdent ec_i] in
          let init  = init @ [ESasgn (lv_i, EC.toec_expr env e1)] in
          let ec_i1, ec_i2 =
          ...
          let c env = toec_cmd f asmOp env c in

          M.toec_for env c e1 e2 init cond i_upd
  ...
  let toec_fun asmOp env f =
      ...
      let stmts = init @ (toec_cmd f.f_name asmOp env f.f_body) @ final in
