...
let parse () =
  ...
  match !infiles with
  | [] ->
      if !help_intrinsics || !safety_makeconfigdoc <> None || !help_version then
        ""
      else error ()
  | [ infile ] ->
      check_options ();
      ...
      infile
  | infile :: s :: _ ->
      raise CLI_errors.(CLIerror (RedundantInputFile (infile, s)))
  ...
  let () =
    if SafetyConfig.sc_print_program () then
      ...
      Format.eprintf "@[<v>At compilation pass: %s@;%s@;@;%a@;@]@." s1 s2
        (Printer.pp_prog ~debug:true pd asmOp)
        p
  in
...
let main () =
  try
    ...
    let (module P : ArchCoreWithAnalyze) =
      ...
      | X86_64 ->
          (module struct
            module C =
              (val CoreArchFactory.core_arch_x86 ~use_lea:!lea ~use_set0:!set0
                     !call_conv)
    ...
    let prog = if !slice <> [] then Slicing.slice !slice prog else prog in
    ...
    *)
    let visit_prog_after_pass ~debug s p =
      if s = SafetyConfig.sc_comp_pass () && !check_safety then
        check_safety_p Arch.reg_size Arch.asmOp
    ...
    if !ec_list <> [] || !ecfile <> "" then (
      ...
      (try
         BatPervasives.finally
           (fun () -> close ())
           (fun () ->
             ToEC.extract Arch.reg_size Arch.asmOp fmt !model prog fnames)
           ()
       with e ->
       ...
       let exec { L.pl_loc = loc; L.pl_desc = f, m } =
         ...
         try
           let pp_range fmt (ptr, sz) =
             Format.fprintf fmt "%a:%a" Z.pp_print ptr Z.pp_print sz
           in
    ...
    | Utils0.Ok asm ->
        ...
        else if List.mem Compiler.Assembly !print_list then
  ...
  | CLIerror e ->
      Format.eprintf "%a: %s.\n"
        (pp_print_bold_red Format.pp_print_string)
        "Error" (pp_cli_error e);
      exit 1
