...
let rec pp_fexpr side prio fmt = function
  ...
  | Fapp1 (op, e) ->
      ...
      Format.fprintf fmt "%s %a"
        (string_of_op1 ~debug:false op)
        (pp_fexpr NoAssoc p) e;
      optparent fmt prio side p ")"
  | Fapp2 (op, e1, e2) ->
      let p = priority_of_op2 op in
      optparent fmt prio side p "(";
      Format.fprintf fmt "%a %s %a" (pp_fexpr Left p) e1 (string_of_op2 op)
        (pp_fexpr Right p) e2;
      optparent fmt prio side p ")"
  | Fif (c, e1, e2) ->
      let p = priority_ternary in
      optparent fmt prio side p "(";
      Format.fprintf fmt "%a ? %a : %a" (pp_fexpr Left p) c (pp_fexpr NoAssoc p)
        e1 (pp_fexpr Right p) e2;
      optparent fmt prio side p ")"
