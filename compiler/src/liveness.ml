...
and live_d weak d (s_o: Sv.t) =
  ...
  | Csyscall(xs,o,al,es) ->
    let s_i = Sv.union (vars_es es) (dep_lvs s_o xs) in
    s_i, (if weak then weak_dep_lvs s_o xs else s_o), Csyscall(xs,o,al,es)
...
let iter_call_sites (cbf: L.i_loc -> funname -> lvals -> Sv.t * Sv.t -> unit)
                    (cbs: L.i_loc -> Wsize.wsize Syscall_t.syscall_t -> lvals -> Sv.t * Sv.t -> unit)
                    (f: (Sv.t * Sv.t, 'asm) func) : unit =
  iter_instr (fun i ->
      match i.i_desc with
      | Ccall (xs, fn, _, _) -> cbf i.i_loc fn xs i.i_info
      | Csyscall (xs, op, _, _) -> cbs i.i_loc op xs i.i_info
