...
(*  let mk_fun *)
  ...
  let push_ground_modparam (st: 'asm Env.store) arg mparam =
    ...
    | Mprog.MaFun f, Fun f' ->
      ...
      let new_name = match BatString.split_on_string ~by:"::" f'.name.fn_name with
        | [] -> f'.name.fn_name
        | names -> List.hd (List.rev names)
      in
      let f_name = P.F.mk new_name in
      Env.Funs.push st {f with f_name} { fs_tin ; fs_tout } |> fst
    |_, _ ->
...
let rec add_instance_module (arch_info:('a, 'b, 'c, 'd, 'e, 'f, 'g) Pretyping.arch_info) (menv: 'asm MEnv.menv) full_name modfunc modfuncname margs modinfo =
    match has_instance modinfo.MEnv.mi_instances margs with
        ...
        | None ->
          ...
          let menv' = List.fold_left (mt_item arch_info) menv' modinfo.mi_ast in
...
and mt_item (arch_info:('a, 'b, 'c, 'd, 'e, 'f, 'g) Pretyping.arch_info) (menv: 'asm MEnv.menv) mitem : 'asm MEnv.menv =
  ...
  | S.PParam pp -> 
    MEnv.upd_storedecls (tt_param arch_info.pd pp) menv
  | S.PGlobal pg ->
    MEnv.upd_storedecls (tt_global arch_info.pd pg) menv
  | S.PFundef pf ->
...
let add_new_items (new_vars:(string,P.pvar) Utils.Map.t) new_funcs module_name instance_name (store:'asm global_bindings) =
  let new_vars = List.fold_left (fun new_vars (name,(v,_,_)) -> 
      ...
      match Map.find_opt vi_name new_vars with
      | Some v' ->  Map.add name_var_instance v' new_vars
   ...
   let new_funcs = List.fold_left (fun new_funcs (name,(pfs,_)) ->
      ...
      match Map.find_opt fi_name new_funcs with
      | Some f ->  Map.add name_func_instance f new_funcs
      | None -> rs_mjazzerror ~loc:pfs.f_loc (MJazzStringError ("Function " ^ fi_name ^ " not found"))
    ) new_funcs (Map.bindings store.gb_funs)
  in
  new_vars, new_funcs
  
let rec init_instance menv inst_subst new_vars new_funcs args mname =
    ...
    let inst_param new_vars new_funcs mp arg = 
      ...
      | Mprog.Param v, Mprog.MaParam e -> 
        let new_name_mod = replace_with_instance inst_subst v.v_name in
     ...
     | Mprog.Fun fs, Mprog.MaFun fn -> 
        let name = replace_with_instance inst_subst (fs.name.fn_name) in
...
and init_modapp menv new_vars new_funcs inst_subst ma_name ma_func =
  ...
  | Some (args, _, iname, _),_ ->
    ...
    menv, new_vars, new_funcs, items

(* 
Transforms a mprog to pprog by removing all the module constructs.
- module without params - remove the module wrapper
- module with params - do nothing (we have no value for the params)
- module instantiation - substitute the params with the args and generate corresponding instance
                         One instance per different set of args. 
                         If the instance has already been generated, do not generate it again.
*)
