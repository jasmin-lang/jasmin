...
let live_ranges_stmt (alias: Alias.alias) (ptr_classes: Sv.t) =
...
let preprocess_liveset (s: Sv.t) : Sv.t * Sv.t =
  Sv.fold (fun x (s, all) ->
      ...
      then
        ...
        then let p = get_stack_pointer x in Sv.add p s, Sv.add p all
        else s, all
      else
...
let check_class ptr_classes args x s =
  let s = Sv.add x s in
  if not (Sv.disjoint args s)
  then
    if not (Sv.mem x ptr_classes)
    then hierror "cannot put a reg ptr argument into the local stack"
...
let alloc_stack_fd get_fun fd =
  ...
  let _, (ranges, all_stack_vars) = live_ranges_stmt alias ptr_classes (0, (Mint.empty, Sv.empty)) fd.f_body in
  ...
  let pp_color fmt (x, i) =
    Format.fprintf fmt "%a: %d" (Printer.pp_var ~debug:true) x i
  in
  let pp_coloring fmt (sz, c) =
    Format.fprintf fmt "%d ↦ %a" sz (pp_list "@ " pp_color) (Mv.bindings c);
  in
  let coloring = Mint.map G.solve ranges in
