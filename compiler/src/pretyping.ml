...
let assign_from_decl loc (v, e) =
  let d = L.mk_loc loc (S.PLVar v) in
  (None, [d]), `Raw, e, None
...
let rec tt_instr arch_info (env : 'asm Env.env) ((annot,pi) : S.pinstr) : 'asm Env.env * (unit, 'asm) P.pinstr list  =
  ...
  in
  let tt_annot_decl env loc vd (aty: A.annotations * S.pstotype) =
    ...
    let env_rhs = env in
    let var = tt_vardecl (fun _ -> true) arch_info.pd env (aty, fst vd) in
    let env = Env.Vars.push_local env_rhs (L.unloc var) in
    let (ls, eq, op, ocp) = assign_from_decl loc vd in
  ...
  | S.PIdeclinit (ty, vd) -> tt_annot_decl env (L.loc vd) (L.unloc vd) (annot, ty)
