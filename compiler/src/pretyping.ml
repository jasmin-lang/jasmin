...
module Env : sig
  ...
  module Vars : sig
    val push_global   : 'asm env -> (P.pvar * (P.E.sop1, P.E.sop2, P.pexpr_) P.ggexpr) -> 'asm env
...
and tt_mem_access pd ?(mode=`AllVar) (env : 'asm Env.env)
  ...
  let e =
    match e with
    | None -> P.Papp1 (E.Oword_of_int pd, P.Pconst (Z.zero))
    | Some(k, e) ->
...
let cassgn_for (x: P.plval) (tg: E.assgn_tag) (ty: P.pty) (e: P.pexpr) :
  (P.E.sop1, P.E.sop2, P.pexpr_, unit, 'asm) P.ginstr_r =
  Cassgn (x, tg, ty, e)
...
let tt_global pd (env : 'asm Env.env) _loc (gd: S.pglobal) : 'asm Env.env =
  ...
  let mk_pe ws (pe,ety) =
    ...
    | Bty Int -> Papp1 (E.Oword_of_int ws, L.unloc pe)
