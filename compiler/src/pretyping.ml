...
type tyerror =
  ...
  | InvalidTypeAlias    of A.symbol * P.epty
...
let max_ty ty1 ty2 =
  ...
  | ETword(Some Unsigned, w1), ETword(Some Unsigned, w2) ->
      Some(ETword(Some Unsigned, Utils0.cmp_min W.wsize_cmp w1 w2))
  | _, _ -> None
...
let rec tt_expr pd ?(mode=`AllVar) (env : 'asm Env.env) pe =
  ...
  | S.PEpack ((nb, sg, es), args) ->
    ...
    let args = List.map (fun (a, ty) -> cast_int loc None a ty) args in
...
and tt_type pd (env : 'asm Env.env) (pty : S.ptype) : P.epty =
  ...
  | S.TArray (ws, e) ->
     let ws = match ws with
       | TypeWsize ws -> P.ws_of_ety (tt_swsize ws)
       | TypeSizeAlias id ->
