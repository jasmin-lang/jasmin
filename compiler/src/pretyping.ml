...
let rec tt_expr pd ?(mode=`AllVar) (env : 'asm Env.env) pe =
  ...
  | S.PEFetch me ->
    let ct, e = tt_mem_access ~mode pd env me in
    P.Pload (ct, e), P.Bty (P.U ct)
...
let tt_lvalue pd (env : 'asm Env.env) { L.pl_desc = pl; L.pl_loc = loc; } =
  ...
  | S.PLMem me ->
    let ct, e = tt_mem_access ~mode:`AllVar pd env me in
    loc, (fun _ -> P.Lmem (ct, e)), Some (P.Bty (P.U ct))
...
let pexpr_of_plvalue exn l =
  ...
  | S.PLMem(ty,e) -> L.mk_loc (L.loc l) (S.PEFetch(ty,e))
