...
type architecture = X86_64 | ARM_M4

let target_arch = ref X86_64

let set_target_arch a =
  let a' =
    match a with "x86-64" -> X86_64 | "arm-m4" -> ARM_M4 | _ -> assert false
  in
  target_arch := a'
...
let set_printing p () = print_list := p :: !print_list
let set_stop_after p () = stop_after := Some p
let set_all_print () = print_list := Compiler.compiler_step_list
let set_ec f = ec_list := f :: !ec_list
let set_ec_array_path p = ec_array_path := p
let set_slice f = slice := f :: !slice
...
let set_cc cc =
  let cc =
    match cc with "windows" -> Windows | "linux" -> Linux | _ -> assert false
  in
  call_conv := cc
...
let print_strings = function
  ...
  | Compiler.RemovePhiNodes ->
      ("rmphi", "remove phi nodes introduced by splitting")
  | Compiler.DeadCode_Renaming ->
      ("renamingd", "dead code after variable renaming to remove copies")
  ...
  | Compiler.MakeRefArguments ->
      ( "makeref",
        "add assignments before and after call to ensure that arguments and \
         results are ref ptr" )
  | Compiler.LowerInstruction -> ("lowering", "lowering of instructions")
  | Compiler.PropagateInline -> ("propagate", "propagate inline variables")
  | Compiler.SLHLowering ->
      ("slhlowering", "lowering of selective load hardening instructions")
  ...
  | Compiler.DeadCode_RegAllocation ->
      ("rallocd", "dead code after register allocation")
...
let symbol2pass =
  ...
  List.iter
    (fun s -> Hashtbl.add tbl (fst (print_strings s)) s)
    Compiler.compiler_step_list;
...
let options =
  [
    ...
    ( "-latex",
      Arg.Set_string latexfile,
      "[filename] Generate the corresponding LATEX file (deprecated)" );
    ...
    ( "-pliveness",
      ...
      " Print liveness information during register allocation" );
  ]
  @ List.map print_option Compiler.compiler_step_list
  @ List.map stop_after_option Compiler.compiler_step_list
