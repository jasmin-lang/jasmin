...
module ArmTarget : AsmTarget.AsmTarget with 
...
= struct 
  ...
  let pp_data_segment_header globs name =   
  ...
  let pp_function_tail fn fd =
    (* TODO_ARM: Review. *)
    if fd.asm_fd_export then
      pp_instr_r fn (POPPC)
    else []

  (* Print the instruction with the given name and arguments. *)


  let pp_instr_op name op args instr =
    let id = instr_desc arm_decl arm_op_decl (None, op) in
    let pp = id.id_pp_asm args in
    (* We need to perform the check even if we don't use the suffix, for
       instance for [LDR] or [STR]. *)
...
module ArmBuilder = AsmTarget.AsmTargetBuilder.Make(ArmTarget)
