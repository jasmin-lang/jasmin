...
let pp_symbol : type a. a option -> Format.formatter -> a I.symbol -> unit =
  fun x fmt s ->
    ...
    | I.N nt ->
        ...
        | Some x -> Format.fprintf fmt "call printer there"
        end
    | I.T t ->
...
module Recovery = struct
  ...
  let is_production_for_start_symbol = function
    | I.X (I.N I.N_module_), _, _, _ -> true (* never happens *)
    | _ -> false
...
let parse_program ?(name = "") (inc : IO.input) =
  ...
  let extra_tokens, ast = ERParser.parse lexbuf in
