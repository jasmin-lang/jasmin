...
(* Attempt of convention: n stands for normal (non-speculative), s for
   speculative *)
...
module Env : sig
  ...
  *)
  ...
  let gget env venv x =
    ...
    (* TODO: change this accordingly (or remove it) when understood *)
    (* previously set it to transient *)
    else Direct (public env, secret env)
  ...
  let set_ty env venv x nxty =
    ...
    try
      if kind env x = Flexible then
        ...
        | Direct lx, Direct nlx -> VlPairs.add_le nlx lx
        | Indirect (lp, lx), Indirect(nlp, nlx) -> VlPairs.add_le nlp lp; VlPairs.add_le nlx lx
        | _, _ -> assert false
        end;
        venv)
    with Lvl.Unsat _unsat ->
  ...
  let set_init_msf env venv ms =
    ...
    set_ty env venv ms dpublic
    (* currently, this sets every transient register back to public. Why? *)
  ...
  let ensure_le loc venv1 venv2 =
    let doit _ oty1 oty2 =
      ...
      | Indirect(lp1, le1), Indirect(lp2, le2) ->  VlPairs.add_le lp1 lp2; VlPairs.add_le le1 le2
      | _, _ -> assert false
      end; None in
    try ignore (Mv.merge doit venv1.vtype venv2.vtype)
    with Lvl.Unsat _unsat ->
      Pt.rs_tyerror ~loc
      (* TODO: error message is too specific (function applies to if constructs as well *)
...
let ty_get_gen env pp_e loc e ety =
  ...
  | Direct _ -> ety
  | Indirect (lp, lx) ->
    try VlPairs.add_le lp (Env.public2 env); Direct lx
...
let rec ty_expr env venv loc (e:expr) : vty =
  ...
  | Pget (_, _, x, i) ->
    ...
    ignore (check_ty_expr ~indirect_allowed:false env venv loc i (Env.public2 env));
    ty_get env x.gv xty

  | Psub (_, _, _, x, i) ->
    let xty = Env.gget env venv x in
    ignore (check_ty_expr ~indirect_allowed:true env venv loc i (Env.public2 env));
    xty
