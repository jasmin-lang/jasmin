...
let rec ty_expr env venv loc (e : expr) : vty =
  ...
  | Pget (_, aa, ws, x, i) ->
      ...
      let ty = Env.fresh2 env and xty = Env.gget venv x in
...
and ensure_public env venv loc e =
  ensure_smaller env venv loc e (Env.public2 env)
...
and ensure_public_address env venv loc x =
  ...
  | Indirect (le, _) -> (
      ...
      with Lvl.Unsat unsat ->
        error_unsat loc unsat pp_var_i x ety (Direct (Env.public2 env)))
...
let ensure_public_address_expr env venv loc e =
  ...
  | Indirect (le, _) -> (
      try VlPairs.add_le le (Env.public2 env)
      with Lvl.Unsat unsat ->
        error_unsat loc unsat pp_expr e ety (Direct (Env.public2 env)))
...
let rec ty_instr is_ct_asm fenv env ((msf, venv) as msf_e : msf_e) i =
  ...
  | Cassgn (mso, _, _, (Pvar x as msi)) when MSF.is_msf_exact msf x.gv ->
      ...
      let _ =
        if not (MSF.is_msf_exact msf msi) then
          Format.printf "Checking Cassgn(%a, _, _, Pvar %a) at %a@." pp_var
            (L.unloc mso) pp_var (L.unloc msi) L.pp_iloc i.i_loc
      in
  ...
  | Copn (xs, _, o, es) -> (
      ...
      | Mov_msf, [ mso ], [ msi ] ->
          ...
          let _ =
            if not (MSF.is_msf_exact msf msi) then
              Format.printf "Checking Mov_msf(%a, Pvar %a) at %a@." pp_var
                (L.unloc mso) pp_var (L.unloc msi) L.pp_iloc i.i_loc
          in
          MSF.check_msf_exact msf msi;
          (MSF.add mso msf, Env.set_ty env venv mso (Env.dpublic env))
      ...
      | Protect, [ x ], [ e; ms ] ->
          ...
          and ms = reg_expr ~direct:true loc ms in
          let _ =
            if not (MSF.is_msf_exact msf ms) then
              Format.printf "Checking Protect(%a, %a, Pvar %a) at %a@." pp_lval
                x pp_expr e pp_var_i ms L.pp_iloc i.i_loc
          in
  ...
  | Ccall (xs, f, es) ->
      ...
      let input_ty e vfty =
        ...
        | IsMsf ->
            ...
            let _ =
              if not (MSF.is_msf_exact msf ms) then
                Format.printf "Checking IsMsf at %a@." L.pp_iloc i.i_loc
            in
            MSF.check_msf_exact msf ms
        | IsNormal ety' -> (
            let ety = ty_expr env venv loc e in
            try
              ...
              | Indirect (lp, le), Indirect (lp', le') ->
                  ...
                  VlPairs.add_le le le'
            with Lvl.Unsat unsat -> error_unsat loc unsat pp_expr e ety ety')
      in
  ...
  let infer_fun f =
    ...
    let msfin =
      infer_msf_c ~withcheck:false fenv (Hashtbl.create 13) f.f_body
        (List.fold_left2
           (fun s x msf -> if msf then Sv.add (L.unloc x) s else s)
           Sv.empty f.f_ret tyout)
    in
