...
module M = {
  ...
  proc ssl3_cbc_copy_mac_jasmin (out:W64.t, rec:W64.t, orig_len:W32.t,
    ...
    div_spoiler <- aux_0;
    leakages <- LeakAddr([]) :: leakages;
    aux_0 <- (div_spoiler `>>` (W8.of_int 1));
    div_spoiler <- aux_0;
    leakages <- LeakAddr([]) :: leakages;
    aux_0 <- (div_spoiler `<<` (W8.of_int 24));
    div_spoiler <- aux_0;
    leakages <- LeakAddr([]) :: leakages;
    aux_0 <- mac_start;
    rotate_offset <- aux_0;
    leakages <- LeakAddr([]) :: leakages;
    aux_0 <- (rotate_offset + div_spoiler);
    rotate_offset <- aux_0;
    leakages <- LeakAddr([]) :: leakages;
    aux_0 <- (rotate_offset - scan_start);
    rotate_offset <- aux_0;
    leakages <- LeakAddr([(zlog2 (to_uint (rotate_offset)))]) :: leakages;
    aux_0 <- (rotate_offset \umod md_size);
    rotate_offset <- aux_0;
    leakages <- LeakAddr([]) :: leakages;
    aux <- (W64.of_int 0);
    temp <- aux;
...
equiv l2 : M.ssl3_cbc_copy_mac_jasmin ~ M.ssl3_cbc_copy_mac_jasmin :
={M.leakages, md_size, orig_len, out}
==> ={M.leakages}.
proof.
  proc.
  wp.
